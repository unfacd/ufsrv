...
RecyclerPoolHandle *RecyclerInitTypePool (const char *typename, size_t blocksz, size_t expansion_threshold, RecyclerPoolOps *pool_ops_ptr);
...
#if (__UF_TESTING || __UF_FULLDEBUG)
...
#else
void RecyclerTypeReferenced (unsigned type, RecyclerClientData *client_data, int) __attribute__((always_inline));
void RecyclerTypeUnReferenced (unsigned type, RecyclerClientData *client_data, int)	__attribute__((always_inline));
size_t RecyclerTypeGetReferenceCount (unsigned type, RecyclerClientData *client_data) __attribute__((always_inline));
#endif
