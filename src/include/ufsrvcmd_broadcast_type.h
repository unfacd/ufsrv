...
#define UFSRVCMDID_INTER_INTRA_CONVERSION_FACTOR 	9
...
enum MessageQueueVerificationCallFlags {
	MSGQUEFLAG_ALLOW_SELF_PUBLISH		=	 (0x1<<1),
	MSGQUEFLAG_ALLOW_OTHER_TARGET		=	 (0x1<<2),
	MSGQUEFLAG_CHECK_RECEPIENT_UID	=	 (0x1<<3),
	MSGQUEFLAG_CHECK_TARGETING_ONLY	=	 (0x1<<4),	//only check if the broadcast intended for the right target ie no self assignments
};


//this is a temporary holder until all inter messages have moved to native protobuf from json should be replaced with
//MessageQueueMsgPayload
typedef struct TempUfsrvInterMessageDescriptor {
	int 									server_id_origin;
	unsigned long 				session_id_target;
	ContextData *ctx_ptr; //json or MessageQueueMsgPayload
}TempUfsrvInterMessageDescriptor;

typedef UFSRVResult * (*broacast_handler)(MessageQueueMsgPayload *, MessageQueueMessage *, UFSRVResult *, unsigned long);

typedef struct UfsrvCommandBroadcastOps {
	//UFSRVResult * (*inetr_broacast_handler)(TempUfsrvInterMessageDescriptor *, UFSRVResult *, unsigned long);
	unsigned long verfier_callflags;
	int (*broacast_handler)(MessageQueueMsgPayload *, MessageQueueMessage *, UFSRVResult *, unsigned long);
	enum MessageQueueVerificationCodes  (*broacast_verifier)(MessageQueueMsgPayload *, MessageQueueMessage 	**mqm_ptr_out, unsigned long);
