...
//O:origin or server id, t:target session s:status
//#define MSGQ_STATUS_UPDATE "{\"o\":%d, \"t\":%lu, \"s\":%u}"
...
/*enum REDIS_UID_FIELDS {
REDIS_KEY_USER_CID=0,REDIS_KEY_USER_SID, REDIS_KEY_USER_UID, REDIS_KEY_USER_WHEN, REDIS_KEY_USER_HADDRESS,
REDIS_KEY_USER_HPORT, REDIS_KEY_USER_STATUS, REDIS_KEY_USER_WHEN_SERVICED, REDIS_KEY_USER_WHEN_SUSPENDED,
REDIS_KEY_USER_WHEN_RESUMED, REDIS_KEY_USER_USER_NAME, REDIS_KEY_USER_COOKIE
};*/

#define REDIS_CMD_USER_SESSION_RECORD_SET_ALL	"HMSET UID:%lu cid %lu sid %d uid %b profile_key %b when %lu haddress %s baseloc %s status 1 when_serviced %lu when_suspended %lu when_resumed %lu uname %s cookie %s geogroup %d events_counter %lu nickname %s geofid_current %lu geofid_past %lu location_byu %s location_bys %s home_baseloc %s avatar %s"

//#define REDIS_CMD_FULL_USER_SESSION_RECORD_GET	"HVALS UID:%lu"
#define REDIS_CMD_USER_SESSION_RECORD_GET_ALL	"HMGET UID:%lu sid cid status uid uname profile_key haddress baseloc when when_serviced when_suspended when_resumed cookie geogroup events_counter nickname geofid_current geofid_past location_byu location_bys home_baseloc avatar"
...
int SuspendThisSession (Session *sesn_ptr, Session *sesn_ptr_target, unsigned recycle_flag);
//UFSRVResult *InstateMigratedSession (Session *sesn_ptr_migrated, Session *new_sesn_ptr, unsigned call_flags, UFSRVResult *);
...
UfsrvUid *GetUfsrvUid(Session *sesn_ptr, unsigned long user_id, UfsrvUid *uid_ptr_out, bool flag_by_ref,
                      bool *flag_is_local);
InstanceHolderForSession *LocallyLocateSessionByUserId(unsigned long user_id);
Session *LocallyLocateSessionByUfsrvUid (const UfsrvUid *ui_ptr);
...
int AddFenceToSession (Session *sesn_ptr, const Fence *f_ptr);
void DestructSocketMessage (SocketMessage *sock_msg_ptr);
/*int SessionLockRD (Session *sesn_ptr, int);
int SessionLockRW (Session *sesn_ptr, int);
int SessionUnLock (Session *sesn_ptr);
int MessageQueueLock (Session *sesn_ptr, MessageQueue *, int try_flag);
int MessageQueueUnLock (Session *sesn_ptr, MessageQueue *);*/
...
UFSRVResult *CacheBackendGetRawSessionRecordByCookie(Session *sesn_ptr_this, const char *session_cookie,
                                                     unsigned long call_flags, UFSRVResult *res_ptr_in);
Session *SessionInstantiateFromBackendByCookie (Session *sesn_ptr_this, const char *session_cookie, unsigned call_flags);
InstanceHolderForSession *CacheBackendInstantiateRawSessionRecord(Session *sesn_ptr_this, redisReply *redis_ptr,
                                                                  unsigned long sesn_call_flags,
                                                                  InstanceHolderForSession *instance_sesn_ptr_out);
...
UFSRVResult *CacheBackendGetRawSessionRecord(Session *sesn_ptr_this, unsigned long user_id, unsigned long call_flags,
...
UFSRVResult *RegisterSessionEvent (Session *sesn_ptr, EnumEventType event_type, unsigned event_instance_type, void *event_payload, UfsrvEvent *event_ptr_out);
...
{
if (unlikely(IS_EMPTY(sesn_ptr_target)))
{
	syslog(LOG_ERR, "%s {pid:'%lu'}: ERROR COULD NOT FIND TARGET TO ACCESS CONTEXT TO", __func__, pthread_self());
	return;
}
...
SessionLockRWCtx (ThreadContext *thread_ctx_ptr, Session *sesn_ptr, int try_flag, const char *func)
{
  ...
  if (try_flag) {
    ...
    if (lock_state == 0) {
      if (!strcmp("CheckSessionIdleTime", __func__) == 0)
...
if (try_flag)
{
	...
	if (lock_state==0) syslog(LOG_DEBUG, "MessageQueueLock (pid='%lu' cid='%lu'  o='%p' lock::1 ): SUCCESS: MUTEX TRY-LOCK for MessageQueue events acquired...",
			pthread_self(), sesn_ptr->session_id, sesn_ptr);
	else
	{
		char *err_str=thread_error(errno);
		syslog(LOG_DEBUG, "MessageQueueLock (pid='%lu' cid='%lu' o='%p'): ERROR: COULD NOT acquire MUTEX lock for MessageQueue (errno='%d'):  '%s'",
				pthread_self(), sesn_ptr->session_id, sesn_ptr, errno, err_str); free(err_str);
	}
}
else
{
	lock_state = pthread_mutex_lock(&(msg_que_ptr->mutex));
	if (lock_state==0) syslog(LOG_DEBUG, "MessageQueueLock (pid='%lu' cid='%lu' o='%p' lock::1 ): SUCCESS:  MUTEX lock for MessageQueue acquired...",
			pthread_self(), sesn_ptr->session_id, sesn_ptr);
	else
	{
		char *err_str=thread_error(errno);
...
static inline int MessageQueueUnLock (Session *sesn_ptr, MessageQueue *msg_que_ptr)
{
	...
	if (lock_state==0) syslog(LOG_DEBUG, "MessageQueueUnLock: (pid='%lu' cid='%lu' o='%p' lock::-1 ): SUCCESS: RELEASED MUTEX lock for MessageQueue...",
			pthread_self(), sesn_ptr->session_id, sesn_ptr);
	else
	{
		char *err_str=thread_error(errno);
		syslog(LOG_DEBUG, "MessageQueueUnLock: (pid='%lu' cid='%lu' o='%p'): ERROR: COULD NOT RELEASE MUTEX lock for MessageQueue (errno='%d'):  '%s'",
				pthread_self(), sesn_ptr->session_id, sesn_ptr, errno, err_str); free(err_str);
	}
