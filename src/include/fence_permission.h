...
#define REDIS_CMD_FENCE_PERMISSION_SET_ADD 			"SADD %s:%lu %lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_DEL 			"SREM %s:%lu %lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_MEMBERS	"SMEMBERS %s:%lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_ISMEMBER	"SISMEMBER %s:%lu %lu"
...
UFSRVResult *RemoveUserFromFencePermissions (Session *sesn_ptr, Fence *f_ptr, FencePermission *ht_ptr_permission, unsigned long fence_call_flags, FenceEvent *fence_event_ptr_out);
UFSRVResult *AddUserToFencePermissions (Session *sesn_ptr, Fence *f_ptr, FencePermission *ht_ptr_permission, unsigned long fence_call_flags, FenceEvent *fence_event_ptr_out);
UFSRVResult *CacheBackendPermissionMembersSetAdd (Session *sesn_ptr_carrier, Fence *f_ptr, FencePermission *permission_ptr, unsigned long userid_target);
...
IsUserOnFencePermissionList (Session *sesn_ptr, Fence *f_ptr, FencePermission *permission_ptr)
{
  ...
  unsigned long userid = UfsrvUidGetSequenceId(&SESSION_UFSRVUIDSTORE(sesn_ptr));
	...
	return (hopscotch_lookup(FENCE_PERMISSION_PERMITTED_USERS_PTR(permission_ptr), NULL, (uint8_t *)&userid, CONFIG_FENCE_PERMISSIONS_KEYOFFSET(Session, sservice.user.user_details.user_id))!=NULL);
}
