...
static int ConfigFileExists(char *file, void(^on_non_existent)(const char *path));
static double ParsePercentage(const char *str);

/**
 *
 * @param file[inout] full, absolute path and file names for config file. Storage allocated by user. Will be returned
 * with full path and file names constructed.
 * @param on_non_existent lambda to execute if file doesn't exits, with fully constructed path and file name passed as an arg.
 * @return -1 if file names are too long
 *         -2 if file does not exist
 *         0 on success
 */
static int
ConfigFileExists(char *file_path_and_name, void(^on_non_existent)(const char *))
{
	extern ufsrv *const masterptr;

  if (strlen(masterptr->config_dir) + strlen(masterptr->config_file) > _POSIX_PATH_MAX -1) {
    on_non_existent(file_path_and_name);
    return -1;
  }

 	sprintf(file_path_and_name, "%s/%s", masterptr->config_dir, masterptr->config_file);
  if (!IsFileWithContentPossibly(file_path_and_name)) {
    on_non_existent(file_path_and_name);
    return -2;
  }
...
int
ProcessConfigfile(const char *conf)
{
  char lua_config_file[_POSIX_PATH_MAX] = {0};
	unsigned long size;
	FILE *fp;
	extern ufsrv *const masterptr;
	...
	int result = ConfigFileExists(lua_config_file, ^(const char *path){
                  syslog(LOG_INFO, "%s: ERROR: Configuration file is missing or invalid (%s) - %s", __func__, path, strerror(errno));
                  fprintf(stderr, "%s: ERROR: Configuration file is missing or invalid (%s) - %s.\n", __func__, path, strerror(errno));
                  _exit(-1);
              });
      
	if (luaL_loadfile(masterptr->lua_ptr, lua_config_file) || lua_pcall(masterptr->lua_ptr, 0, 0, 0)) {
		fprintf(stderr, "%s: ERROR: COULD NOT processes configuration file '%s/%s': error: '%s'", __func__, lua_config_file, conf, lua_tostring(masterptr->lua_ptr, -1));

		_exit(-2);
	}

	//IMPORTANT for individual parameters ALWAYS ADD NEW ENTRIES BELOW THIS COMMENT LINE  AND INCREMENT THE STACK ACCORDINGLY
//	masterptr->ufsrvmedia_upload_uri
  lua_getglobal(masterptr->lua_ptr, "listener_workers_thread_pool");
	...
	int setsize = 0;
	if (!lua_isnumber(masterptr->lua_ptr, -13)) {
	  syslog(LOG_ERR, "%s: ERROR: UNRECOGNISED VALUE SET FOR 'listener_workers_thread_pool': using default '%d'", __func__, _CONFIGDEFAULT_MAX_LISTENER_WORKERS);
	} else	{
	  setsize = (int)lua_tonumber(masterptr->lua_ptr, -13);
	}
	if (setsize < 0) setsize = _CONFIGDEFAULT_MAX_LISTENER_WORKERS;
	SetListenerWorkersThreadPoolSetsize(setsize);
	...
	if (!lua_isnumber(masterptr->lua_ptr, -10)) {
	  ...
	  sessions_delegator_ptr->setsize = (int)lua_tonumber(masterptr->lua_ptr, -10);

	}
	if (sessions_delegator_ptr->setsize < 1) sessions_delegator_ptr->setsize = _CONFIGDEFAULT_MAX_SESSION_WORKERS;
	SetWorkersThreadPoolSetsize(sessions_delegator_ptr->setsize);
  ...
  lua_getglobal(LUA_CTX, "ufsrv_sys_runtime");
  if (!lua_istable(LUA_CTX, -1)) {
    syslog(LOG_INFO, ">>> %s: Server will run as current user: not processing privileges drop", __func__);
  } else {
    strncpy(masterptr->runtime_user.run_as_user, LUA_GetFieldToString("run_as_user"), MINIBUF - 1);
    if (*masterptr->runtime_user.run_as_user) {
      strncpy(masterptr->runtime_user.chroot_pathname, LUA_GetFieldToString("chroot"), _POSIX_PATH_MAX - 1);
      if (!*masterptr->runtime_user.chroot_pathname) {
        *masterptr->runtime_user.chroot_pathname = '/';
      }
      syslog(LOG_INFO, ">>> %s: Server will attempt to run as user: '%s' with chroot: '%s'", __func__, masterptr->runtime_user.run_as_user, masterptr->runtime_user.chroot_pathname);
    }
  }
	...
	if (!lua_istable(LUA_CTX, -1)) {
		...
		strncpy(masterptr->db_backend.address, CONFIG_DEFAULT_LOOPBACKADDRESS, strlen(CONFIG_DEFAULT_LOOPBACKADDRESS) + 1);
		masterptr->db_backend.port = 3306;
		strncpy(masterptr->db_backend.username, "ufsrv", strlen("ufsrv") + 1);
	...
	if (!lua_istable(LUA_CTX, -1)) {
		...
		strncpy(masterptr->stats_backend.address, CONFIG_DEFAULT_LOOPBACKADDRESS, strlen(CONFIG_DEFAULT_LOOPBACKADDRESS) + 1);
	...
	if (!lua_istable(LUA_CTX, -1)) {
		...
		strncpy(masterptr->geoip_backend.address, CONFIG_DEFAULT_LOOPBACKADDRESS, strlen(CONFIG_DEFAULT_LOOPBACKADDRESS) + 1);
	...
	if (!lua_istable(LUA_CTX, -1)) {
	  ...
	  strncpy(masterptr->persistance_backend_address, CONFIG_DEFAULT_LOOPBACKADDRESS, strlen(CONFIG_DEFAULT_LOOPBACKADDRESS) + 1);
	...
	if (!lua_istable(LUA_CTX, -1)) {
		...
		strncpy(masterptr->cache_backend_address_usrmsg, _CONFIGDEFAULT_BACKENDCACHE_HOST_USRMSG, strlen(_CONFIGDEFAULT_BACKENDCACHE_HOST_USRMSG) + 1);
	...
	if (!lua_istable(LUA_CTX, -1)) {
		...
		strncpy(masterptr->cache_backend_address_fence, _CONFIGDEFAULT_BACKENDCACHE_HOST_FENCE, strlen(_CONFIGDEFAULT_BACKENDCACHE_HOST_FENCE) + 1);
