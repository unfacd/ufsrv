...
static UFSRVResult *_ParseUfsrvCommandMessage (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned frame_offset, size_t len) __attribute__((always_inline));
static UFSRVResult *_UfsrvCommandInvokeCommandCallback (Session *sesn_ptr, WebSocketMessage *wsm_ptr, size_t cmdidx) __attribute__((always_inline));
 ...
 */
UFSRVResult *
UfsrvCommandInvokeCommand (Session *sesn_ptr, Session *sesn_ptr_target, WebSocketMessage *wsm_ptr, struct json_object *jobj_in, void *payload, unsigned req_cmd_idx)
{
	if (sesn_ptr && req_cmd_idx <= ufsrvcmd_user_maxidx) {
#ifdef __UF_TESTING
		syslog (LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ISSUING A UFSRVCMD TO USER USING CMDIDX: '%u'", __func__, pthread_self(), sesn_ptr->session_id, req_cmd_idx);
#endif
    ...
    if (IS_PRESENT(p)) {
      ...
      //>>>>>>>>>>>>>>>>>>>>
      UFSRVResult *r_ptr = (*p->callback)(sesn_ptr, sesn_ptr_target, wsm_ptr, jobj, payload);
      //>>>>>>>>>>>>>>>>>>>>>>>

      //we created it , we destroy it
      if (jobj_in == NULL) json_object_put(jobj);

      return r_ptr;
    } else {
      syslog(LOG_ERR, "%s (pid:'%lu' cid:'%lu'): pointer into UfsrvCommand array invalid: terminating...", __func__, pthread_self(), sesn_ptr->session_id);
 ...
 */
int
UfsrvCommandMarshalTransmission (Session *sesn_ptr_this, Session *sesn_ptr_target, TransmissionMessage *tmsg_ptr, unsigned long call_flags)
{
	bool 										lock_already_owned	= false;
	HttpRequestContext 			*http_ptr		=	NULL;
	Session 								*sesn_ptr		=	NULL;

	assert (sesn_ptr_this!=NULL);

	sesn_ptr=sesn_ptr_this;

	lock_target_session:
	if (IS_PRESENT(sesn_ptr_target))
	{
		if (!(call_flags&CALL_FLAG_DONT_LOCK_SESSION))//todo: this flag is currently not use
		{
//			if ((SessionLockRW(sesn_ptr_target, 1))!=0)	return 0;//NOTE TRY FLAG ON
			SessionLockRWCtx(THREAD_CONTEXT_PTR, sesn_ptr_target, _LOCK_TRY_FLAG_TRUE, __func__);
		...
		sesn_ptr=sesn_ptr_target;//locked herein if CALL_FLAG_DONT_LOCK_SESSION wasnt set
	}
	else
	{
		sesn_ptr=sesn_ptr_this;//locked in calling environment
	}

	if (SESNSTATUS_IS_SET(sesn_ptr_this->stat, SESNSTATUS_EPHEMERAL))
	{
		http_ptr=GetHttpRequestContextUfsrvWorker(sesn_ptr_this);
		if (IS_PRESENT(sesn_ptr_target))		SessionLoadEphemeralMode(sesn_ptr_target);
	}
	else
	{
		if (IS_PRESENT(sesn_ptr_target))	SessionTransferAccessContext (sesn_ptr_this, sesn_ptr_target, 0);
		http_ptr=GetHttpRequestContext(sesn_ptr_this);
	}
	...
	{//block: setup request envelope
	 ...
	 if (tmsg_ptr->type==TRANSMSG_TEXT)
	 {
	 ...
	 if ((tmsg_ptr->eid>0) && _PROTOCOL_CTL_CLOUDMSG_ON_IOERROR(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr)))))
		 StoreStagedMessageCacheRecordForUser (sesn_ptr, tmsg_ptr, IS_PRESENT(sesn_ptr_target)?SESSION_USERID(sesn_ptr_target):SESSION_USERID(sesn_ptr));

		if	((SESSION_SOCKETFD(sesn_ptr)<=0)||(SendToSocket(sesn_ptr, tmsg_ptr, 0)<0))//0 means partial writeso it is not error
		{
			if ((tmsg_ptr->eid>0) && _PROTOCOL_CTL_CLOUDMSG_ON_IOERROR(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr)))))
		...
		{
			if ((tmsg_ptr->eid>0) && _PROTOCOL_CTL_CLOUDMSG_ON_IOERROR(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr)))))
				DeleteStagedMessageCacheRecordForUser (sesn_ptr, tmsg_ptr, IS_PRESENT(sesn_ptr_target)?SESSION_USERID(sesn_ptr_target):SESSION_USERID(sesn_ptr));
		}

		return_restore:
		if (sesn_ptr_target)
		{
			if (SESNSTATUS_IS_SET(sesn_ptr_this->stat, SESNSTATUS_EPHEMERAL))	SessionUnLoadEphemeralMode(sesn_ptr_target);
//			if (!(call_flags&CALL_FLAG_DONT_LOCK_SESSION))	SessionUnLock (sesn_ptr_target);
			if (!(call_flags&CALL_FLAG_DONT_LOCK_SESSION))	if (!lock_already_owned)	SessionUnLockCtx (THREAD_CONTEXT_PTR, sesn_ptr_target, __func__);
		}
...
UFSRVResult *
proto_websocket_hanshake_callback (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned callflags, int **comeback)
{
	...
	switch (res_ptr->result_type)
	{
		case RESULT_TYPE_ERR:
			if (res_ptr->result_code == RESCODE_IO_WOULDBLOCK) {
				...
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'):  COULD NOT HANDSHAKE: connection closed", __func__, SESSION_ID(sesn_ptr), sesn_ptr->session_id);
			} else if (res_ptr->result_code == RESCODE_PROTOCOL_WSHANDSHAKE) {
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'):  COULD NOT HANDSHAKE: parsing error", __func__, SESSION_ID(sesn_ptr), sesn_ptr->session_id);
			}
 ...
 */
UFSRVResult *
proto_websocket_post_hanshake_callback (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned callflags)
{
	...
	UfsrvCommandInvokeCommand (sesn_ptr, NULL, NULL, NULL, NULL, uSTATESYNC_V1_IDX);
...
_UfsrvCommandInvokeCommandCallback (Session *sesn_ptr, WebSocketMessage *wsm_ptr, size_t cmdidx)
{
	...
	if (cmdidx <= ufsrvcmd_maxidx) {
		...
		if ((p = (ufsrvcmd_server_bound_callbacks_ptr + cmdidx))) {
			...
			//>>>>>>>>>>>>>>>>>>>>>>>>>>>>
			 ufsrv_res = (*p->callback)(sesn_ptr, wsm_ptr, jobj);
			//>>>>>>>>>>>>>>>>>>>>>>>>>>>

			 json_object_put(jobj);
			} else {
				ufsrv_res = (*p->callback)(sesn_ptr, wsm_ptr, NULL);
			}
...
UFSRVResult *
proto_websocket_msg_callback (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned frame_offset, size_t len)
{
	ssize_t read_result;

	read_result = ReadFromSocket (sesn_ptr, sock_msg_ptr, frame_offset);//frame_offset is 'callflags' passed by the calling environment
	...
	//start framed_decoded_msg
	{
		...
		while ((loop_counter--)) {
			syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): PARSE ITERATION: '%d': PAYLOAD length: '%lu' READING OFFSET '%d'...", __func__, pthread_self(), SESSION_ID(sesn_ptr), loop_counter+1, len, frame_offset);

			ufcmd_result = _ParseUfsrvCommandMessage (sesn_ptr, sock_msg_ptr, frame_offset, len);
...
UFSRVResult *
proto_websocket_decode_msg_callback (Session *sesn_ptr, SocketMessage *sm_ptr, unsigned frame_offset)
{
	...
	if (opcode == 8) {
#ifdef __UF_FULLDEBUG
		syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', cid:'%lu'}: client sent orderly close frame", __func__, pthread_self(), THREAD_CONTEXT_PTR, SESSION_ID(sesn_ptr));
#endif
	...
	if (sm_ptr->processed_msg_size < 0) {
		decoding_error:
		syslog(LOG_ERR, "%s (pid:'%lu' cid:'%lu'): ERROR: COULD NOT DECODE MESSAGE. SocketMessage will be destructed. Connection  will be suspended", __func__, pthread_self(), SESSION_ID(sesn_ptr));
	...
	if (sm_ptr->missing_msg_size) {
		...
		syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): PARTIAL FRAME from client: amount left to be read: '%ld'. raw_msg_cur_pos='%ld'. holding_buffer_msg_size: '%ld' (This amount will be copied into holding_buffer). Raw buffer will be freed.",
				__func__, pthread_self(), SESSION_ID(sesn_ptr), sm_ptr->missing_msg_size, sm_ptr->raw_msg_cur_pos, sm_ptr->holding_buffer_msg_size);
...
UFSRVResult *
proto_websocket_encode_msg_callback (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned frame_offset)
{
...
/**
 * @brief: read processed buffer based on given offset. This should be a complete, correctly decoded Websocket frame of size len.
 *
 ...
 */
static inline UFSRVResult *
_ParseUfsrvCommandMessage (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned frame_offset, size_t len)
{
  ...
  #ifdef __UF_FULLDEBUG
        syslog(LOG_DEBUG, "%s (pid:'%lu', o:'%p', uname:'%s', command:'%s', id:'%lu'): Received WebSocket Request Command...", __func__,
          pthread_self(), sesn_ptr, SESSION_USERNAME(sesn_ptr), command, wsm_ptr->request->id);
  #endif
        //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  #ifdef __UF_FULLDEBUG
        syslog(LOG_DEBUG, "%s: (pid:'%lu' cid:'%lu'): >> WEB_SOCKET_MESSAGE__TYPE__RESPONSE MESSAGE '%s'", __func__, pthread_self(), SESSION_ID(sesn_ptr), wsm_ptr->response->message);
  #endif
        break;
...
UFSRVResult *
proto_websocket_reset_callback (Session *sesn_ptr, unsigned callflags)
{
	WebSocketSession *ws_ptr;
	ws_ptr = (WebSocketSession *)SESSION_PROTOCOLSESSION(sesn_ptr);

	if (callflags == 0) {
			if (ws_ptr) {
				syslog (LOG_DEBUG, "%s (cid: %lu): WEBSOCKET SESSION: DESTRUCTING OBJECT INSTANCE...", __func__, SESSION_ID(sesn_ptr));
			}
		} else {
			if (ws_ptr) {
				syslog (LOG_DEBUG, "%s (cid: %lu): WEBSOCKET SESSION: PUSHING TO RECYCLER...", __func__, SESSION_ID(sesn_ptr));
			}
		}
 ...
 */
UFSRVResult *
proto_websocket_service_timeout_callback (Session *sesn_ptr, time_t now, unsigned long call_flags)
{
	bool recycle_flag = false;
	bool suspended_flag = false;
	InstanceHolderForSession *instance_sesn_ptr = InstanceHolderFromClientContext(sesn_ptr);
...
UFSRVResult *
proto_websocket_error_callback (Session *sesn_ptr, unsigned call_flags)
{
...
UFSRVResult *
proto_websocket_recycler_error_callback (Session *sesn_ptr, unsigned call_flags)
{
...
UFSRVResult *
proto_websocket_close_callback (Session *sesn_ptr)
{
