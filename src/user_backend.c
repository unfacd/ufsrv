...
static UFSRVResult *_DbAccountDataAttributeGetText (Session *sesn_ptr, unsigned long, const char *attribute_name);
...
static int _DbAccountUpdateData (Session *sesn_ptr, const char *data_path, const char *value, unsigned long userid);
...
BackEndGenerateNonce (Session *sesn_ptr_carrier, time_t expiry_in, const char *prefix, const char *value)
{
	...
	if (IS_PRESENT(value)) {
		redis_ptr=(*pers_ptr->send_command)(sesn_ptr_carrier, "SET %s:%s %s EX %lu"/*EDIS_CMD_REGONONCE_SET*/, prefix, nonce_hashed, value, expiry_in<=0?5L:expiry_in);
	} else {
		redis_ptr=(*pers_ptr->send_command)(sesn_ptr_carrier, "SET %s:%s %lu EX %lu"/*EDIS_CMD_REGONONCE_SET*/, prefix, nonce_hashed, SESSION_ID(sesn_ptr_carrier), expiry_in<=0?5L:expiry_in);
	}
...
#define SQL_QUERY_ATTRIBUTE_VALUE_LENGTH	((struct _h_type_blob *)result.data[0][0].t_data)->length
  ...
  UFSRVResult *res_ptr  = THREAD_CONTEXT_UFSRV_RESULT(ufsrv_thread_context);
	...
	int sql_result = h_query_select(THREAD_CONTEXT_DB_BACKEND(ufsrv_thread_context), sql_query_str, &result);
 ...
 */
static UFSRVResult *
_DbAccountDataAttributeGetText (Session *sesn_ptr, unsigned long userid, const char *attribute_name)
{
  //IMPORTANT: USING JSON_UNQUOTE turns the value from json string,  ie. "value", to my sql string, i.e. value, but the library then returns blob type,
  //as opposed to string type. Without UNQUOTE we get string type, but we have to remove the opening and closing " manually
...
static int
_DbAccountUpdateData (Session *sesn_ptr, const char *data_path, const char *value, unsigned long userid)
{
...
/**
 * 	@brief: at this stage, passowrd is saved in pending-account as clear text, so we must hash it with salt before storing it securely.
...
UpgradePendingAccountByJson(Session *sesn_ptr, PendingAccount *pacct_ptr, struct json_object *jobj_by_user, bool flag_nuke_old)
{
  ...
  if (IS_PRESENT(uid_ptr)) {
    ...
    profile_key_encoded = (char *) DbBackendGetProfileKey(sesn_ptr, uid_ptr, false);
    if (IS_EMPTY(profile_key_encoded)) profile_key_encoded = CONFIG_DEFAULT_PREFS_STRING_VALUE;
...
//	json_object_object_add (jobj_device, ACCOUNT_JSONATTR_E164NUMBER, json_object_new_string(IS_PRESENT(e164number)?e164number:CONFIG_DEFAULT_PREFS_STRING_VALUE)); //allow e164 per device
	...
	if (strlen(profile_key_encoded)>1) 	free (profile_key_encoded); //only gets allocated if != "*"
  ...
  json_object_get(jobj_device);
    ...
    if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_BACKEND_DATA)) {
      ...
      char e164number_updated[CONFIG_E164_NUMBER_SZ_MAX + 1] = {0};
    	...
    	SESSION_RESULT_TYPE(sesn_ptr) = RESULT_TYPE_ERR; //this is necessary as DbAccountGetUserId()returns success if empty set and we 'r onlychecking for RESCODE_BACKEND_DATA above
	...
	//send redis commands
	{
		...
		for (i=0; i<contacts_count; i++) {
			...
			if ((contact_token=json_object_get_string(jobj_contact))) {
				syslog(LOG_DEBUG, "%s (cid: '%lu'): Processing token: '%s'", __func__, SESSION_ID(sesn_ptr), contact_token);
 ...
 * @return profile key encoded in b64
 * TODO: refactor to use ProfileKeyStore
 */
const char *
DbBackendGetProfileKey(Session *sesn_ptr, const UfsrvUid *uid_ptr, bool is_decoded)
{
  _DbAccountDataAttributeGetText (sesn_ptr, UfsrvUidGetSequenceId(uid_ptr), ACCOUNT_JSONATTR_PROFILE_KEY);
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
    ...
    if (!is_decoded) {
    	return (char *) SESSION_RESULT_USERDATA(sesn_ptr);
			...
			int decoded_sz = 0;
			unsigned char *profilekey_decoded=base64_decode((const unsigned char *) SESSION_RESULT_USERDATA(sesn_ptr), CONFIG_USER_PROFILEKEY_MAX_SIZE, &decoded_sz);
			free (SESSION_RESULT_USERDATA(sesn_ptr));

			return (const char *)profilekey_decoded;
    }
  }
...
DbBackendSetProfileKey (Session *sesn_ptr, const char *profile_key_encoded)
{
  if (likely(strlen(profile_key_encoded)<CONFIG_USER_PROFILEKEY_MAX_SIZE_ENCODED)) {
    if ((_DbAccountUpdateData(sesn_ptr, ACCOUNT_JSONATTR_PROFILE_KEY, profile_key_encoded, SESSION_USERID(sesn_ptr)))==0) {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_BACKEND_DATA);
    } else {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_BACKEND_DATA);
    }
...
DbBackendSetProfileKeyIfNecessary (Session *sesn_ptr, const char *profile_key_encoded, bool is_force_update)
{
  ...
  const char *profile_key_encoded_stored = DbBackendGetProfileKey (sesn_ptr, &(SESSION_UFSRVUIDSTORE(sesn_ptr)), false);
  if (IS_PRESENT(profile_key_encoded_stored)) {
    if (strcmp(profile_key_encoded_stored, profile_key_encoded)==0) {
      goto return_do_thing;
    } else {
      if (is_force_update) {
        free ((char *)profile_key_encoded_stored);
  ...
  return_do_thing:
  _RETURN_RESULT_SESN(sesn_ptr, profile_key_encoded_stored, RESULT_TYPE_SUCCESS, RESCODE_BACKEND_DATA_EXISTINGSET)

  return_stored_key:
  syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', uname:'%s', token_provided:'%s', token_stored:'%s'): WARNING: Mismatched profile key: no force update flag: returning stored value", __func__, pthread_self(), sesn_ptr, SESSION_USERNAME(sesn_ptr), profile_key_encoded, profile_key_encoded_stored);
  _RETURN_RESULT_SESN(sesn_ptr, profile_key_encoded_stored, RESULT_TYPE_ERR, RESCODE_BACKEND_DATA_EXISTINGSET);
...
{
  _DbAccountDataAttributeGetText (sesn_ptr, UfsrvUidGetSequenceId(uid_ptr), ACCOUNT_JSONATTR_ACCESS_TOKEN);
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
...
DbBackendSetAccessToken (Session *sesn_ptr, const char *access_token_encoded)
{
  if (likely(strlen(access_token_encoded)<CONFIG_USER_ACCESS_TOKEN_MAX_SIZE_ENCODED)) {
    if ((_DbAccountUpdateData(sesn_ptr, ACCOUNT_JSONATTR_ACCESS_TOKEN, access_token_encoded, SESSION_USERID(sesn_ptr))) == 0) {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_BACKEND_DATA);
    } else {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_BACKEND_DATA);
    }
...
BackendDirectoryNicknameGet (Session *sesn_ptr, const char *nickname)
{
  ...
  if ((redis_ptr->type != REDIS_REPLY_NIL)) {
    ...
    long long sequence_id = redis_ptr->integer;
    freeReplyObject(redis_ptr);
...
{
	if (unlikely(!IS_PRESENT(nickname)))	return false;
...
_DbAccountNicknameGet (Session *sesn_ptr, const char *nickname, UFSRVResult *res_ptr_in)
{
  ...
  #define	COLUMN_UFSRVUID_VALUE ((struct _h_type_text *)result.data[0][1].t_data)->value //binary fields are designated as text.
		...
		if (result.nb_rows>0) {
#if __UF_TESTING
			syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', uid:'%d', rows_sz:'%u'): Found occurrence of nickname", __func__, pthread_self(), sesn_ptr, COLUMN_USERID(result), result.nb_rows);
#endif
...
#define SQL_INSERT_NEW_ACCOUNT "INSERT INTO accounts (number, data, data_user) VALUES ('%s', '%s', '%s') ON DUPLICATE KEY UPDATE data = '%s', data_user='%s'"
...
SetUserKeys (Session *sesn_ptr, const UfsrvUid *uid_ptr, json_object *jobj, int device_id)
{
  ...
  char ufsrvuid_encoded[CONFIG_MAX_UFSRV_ID_ENCODED_SZ+1] = {0};
  UfsrvUidConvertToString (uid_ptr, ufsrvuid_encoded);
...
#define COLUMN_NICKNAME(x)		((struct _h_type_text *)result.data[0][5].t_data)->value
	...
	if (result.nb_rows>=1) {
		if ((strncasecmp("false", COLUMN_FETCHESMSG(result), 1))==0) {
			syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu', fetches_msg:'%c'}: WARNING: COOKIE EXITS BUT ACCOUNT DISABLED FOR USERNAME: '%s'. device_id:'%d'",
...
#ifdef __UF_TESTING
    syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu'}: SUCCESS: UFSRVUID:'%lu', COOKIE VALID FOR USERNAME: '%s', NICKNAME:'%s'. device_id:'%d'",
 ...
 * @param jobj_account
 * @param device_id
 * @dynamic_memory Don't retain returned value. Create a copy instead
 * @return
 */
const char *
...
/**
 *
 * @param jobj_account root node for account
 ...
 * @dynamic_memory: ALLOCATES 'ProfileKeyStore.profile_key_decoded *' if 'is_decoded' is flagged
 */
ProfileKeyStore *
GetAccountAttributeForProfileKey (json_object *jobj_account, bool is_decoded, ProfileKeyStore *store_in)
{
	ProfileKeyStore *store = NULL;
	...
	if (IS_STR_LOADED(profile_key_encoded)) {
		if (IS_EMPTY(store_in)) {
			store = calloc (1, sizeof(ProfileKeyStore));
		}	else {
			store = store_in;
		}

		memcpy(store->profile_key_encoded, profile_key_encoded, CONFIG_USER_PROFILEKEY_MAX_SIZE);

		if (is_decoded) {
			int decoded_sz = 0;
			store->profile_key_decoded = base64_decode((const unsigned char *) profile_key_encoded, CONFIG_USER_PROFILEKEY_MAX_SIZE, &decoded_sz);
		}
	}
...
{
#define SQL_SELECT_USERID "SELECT id, ufsrvuid FROM accounts WHERE number='%s';"
#define SQL_QUERY_UFSRVUID	((struct _h_type_blob *)result.data[0][1].t_data) //if attribute has null data this will be null
#define	SQL_QUERY_UFSRVUID_VALUE ((struct _h_type_text *)result.data[0][1].t_data)->value //binary fields are designated as text.
  ...
  free (sql_query_str);
	...
	if (result.nb_rows==0) {
		syslog(LOG_DEBUG, "%s (cid='%lu'): ERROR: COULD NOT FIND CORRESPONDING DB RECORD", __func__, SESSION_ID(sesn_ptr));
...
_DBAccountDataGenerateJson (Session *sesn_ptr, const char *query_str)
{
	...
	int sql_result=h_query_select(THREAD_CONTEXT_DB_BACKEND(THREAD_CONTEXT)/*sesn_ptr->db_backend*/, query_str, &result);
