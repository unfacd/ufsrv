...
#include <user_preference_descriptor_type.h>
#include <include/user_preference_descriptor_type.h>
#include <utils_str.h>
...
static UFSRVResult *_CacheBackendGetRawSessionRecordByUserId(Session *sesn_ptr_carrier, unsigned long user_id,
                                                             unsigned call_flags, UFSRVResult *res_ptr_in);
	  ...
	  //session_id was <0 we search across online registry. this is what clients send us only uid
		...
		if (IS_PRESENT(instance_sesn_ptr_other_user)) {
		  Session *sesn_ptr_other_user = SessionOffInstanceHolder(instance_sesn_ptr_other_user);
			...
			return (_CacheBackendGetRawSessionRecordByUserId(sesn_ptr_carrier, user_id, call_flags, NULL));
		} else {
#ifdef __UF_TESTING
 ...
 *
 * 	@return UFSRVResult:
 *  with the data field set to the redis_ptr object containing the raw redis record.
 *
 *  @dynamic_memory redis_ptr: EXPORTS ONLY IF RESCODE_USER_BACKEND IS THE RESULT CODE VALUE
 */
UFSRVResult *
_CacheBackendGetRawSessionRecordByUserId(Session *sesn_ptr_carrier, unsigned long user_id, unsigned call_flags, UFSRVResult *res_ptr_in)
{
	UFSRVResult *res_ptr	=	NULL;

	if ((IS_EMPTY(res_ptr_in)))	res_ptr	=	&(sesn_ptr_carrier->sservice.result);
	else												res_ptr	=	res_ptr_in;

	if (likely(user_id > 0)) {
    CacheBackendGetRawSessionRecord(sesn_ptr_carrier, user_id, call_flags, res_ptr);
...
#ifdef __UF_TESTING
				syslog(LOG_DEBUG, "%s (pid:'%lu', o_carrier:'%p', uid:'%lu'): RECORD FOR USER IS OF STATUS '0'. RECORD WILL BE OVERRIDEN",  __func__, pthread_self(), sesn_ptr_carrier, user_id);
#endif
				freeReplyObject(redis_ptr);

				_RETURN_RESULT_RES(res_ptr, NULL, RESULT_TYPE_ERR, RESCODE_USER_NOTONLINE)//aka QUIT
			}

			_RETURN_RESULT_RES(res_ptr, redis_ptr,  RESULT_TYPE_SUCCESS, RESCODE_USER_BACKEND)//online via backend verification
		}
	}
...
/**
 * 	@brief: Used in the context of reseting session
 */
void
...
#define SQL_GET_ACCOUNT_DATA_USERNAME "SELECT data_user FROM accounts WHERE number='%s'" // {"prefs_bool": 192, ...}
      ...
      sql_query_str = mdsprintf(SQL_GET_ACCOUNT_DATA_USERNAME, SESSION_USERNAME(sesn_ptr));
		...
		int sql_result=h_query_select(THREAD_CONTEXT_DB_BACKEND(THREAD_CONTEXT), sql_query_str, &result);

		if (sql_result!=H_OK)		goto return_db_error;
		if (result.nb_rows==0)	goto return_empty_set;
		if (IS_EMPTY(((struct _h_type_blob *)result.data[0][0].t_data)))	goto return_db_empty_jsonstr; //column has NULL value
...
UFSRVResult *
IsUserAllowedToChangeUserPrefGroupRoaming(Session *sesn_ptr, UserPreference *pref_ptr, DataMessage *data_msg_ptr_recieved, UfsrvEvent *event_ptr, unsigned long sesn_call_flags) {
  ...
  if (GetUserPreferenceDescriptorById(&user_prefs_table, pref_ptr->pref_id, &pref)) {
    ...
    if (IS_PRESENT((*pref.pref_ops->pref_set)(sesn_ptr, &pref, PREFSTORE_EVERYWHERE, event_ptr))) {
      ...
      MarshalUserPrefGroupRoaming (sesn_ptr, CLIENT_CTX_DATA(&share_list_ctx), data_msg_ptr_recieved, SESSION_CALLFLAGS_EMPTY, event_ptr);
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_NULL_POINTER)
    }
  }
 ...
 */
UFSRVResult *
IsUserAllowedToChangeNickname (Session *sesn_ptr, const char *nickname_new, unsigned long sesn_call_flags, UfsrvEvent *event_ptr)
{
  AccountNicknameValidateForUniqueness(sesn_ptr, &(SESSION_UFSRVUIDSTORE(sesn_ptr)), nickname_new);
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_BACKEND_DATA_EMPTYSET)) {
    UserPreferenceDescriptor 	pref	=	{0};

    GetUserPreferenceNickname (sesn_ptr, PREF_NICKNAME, PREFSTORE_MEM, &pref);
    pref.value.pref_value_str = (char *)nickname_new;

    if (SetUserPreferenceNickname(sesn_ptr, &pref, PREFSTORE_EVERYWHERE, event_ptr)) {
      if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
        if (likely(sesn_call_flags&CALL_FLAG_BROADCAST_SESSION_EVENT)) {
          InterBroadcastUserNicknameMessage(sesn_ptr, CLIENT_CTX_DATA(nickname_new), event_ptr, COMMAND_ARGS__UPDATED);
	...
	return SESSION_RESULT_PTR(sesn_ptr); //captures error from last invocation. Can also capture success
}
 ...
 */
UFSRVResult *
IsUserAllowedToChangeAvatar (Session *sesn_ptr, const char *avatar_id, AttachmentRecord *attachment_record, unsigned long sesn_call_flags, UfsrvEvent *event_ptr)
{
	UserPreferenceDescriptor 	pref												=	{0};
	AttachmentDescriptor 			attachment_descriptor_out 	= {0};

	if (IS_PRESENT(attachment_record)) {
    if (IS_PRESENT(GetAttachmentDescriptorEphemeral(sesn_ptr, avatar_id, false, &attachment_descriptor_out)))
...
static UFSRVResult *_BackendsUpdateForProfileKey (Session *sesn_ptr, const unsigned char *profile_key);

static UFSRVResult *
_BackendsUpdateForProfileKey (Session *sesn_ptr, const unsigned char *profile_key)
{
 ...
 */
UFSRVResult *
IsUserAllowedToShareProfile (Session *sesn_ptr, DataMessage *data_msg_received, UfsrvEvent *event_ptr, unsigned long sesn_call_flags)
{
	UserPreferenceDescriptor 	pref			=	{0};
	...
	if (SESSION_USER_PROFILE_KEY(sesn_ptr)[0] == '\0')	{
    _BackendsUpdateForProfileKey (sesn_ptr, user_command_prefs->vaues_blob.data);
	...
	if (usercommand->n_target_list > 0) {
      ...
      if (unlikely(UfsrvUidIsEqual((const UfsrvUid *)user_record_ptr->ufsrvuid.data, &SESSION_UFSRVUIDSTORE(sesn_ptr))))	continue; //same user
        ...
        if (!SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_SHARELIST_PRESENT)) {
          ...
          if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
            MarshalUserPrefProfile(sesn_ptr, CLIENT_CTX_DATA((&share_list_ctx)), data_msg_received, event_ptr);
...
UFSRVResult *
IsUserAllowedToShareNetstate(Session *sesn_ptr, DataMessage *data_msg_received, UfsrvEvent *event_ptr, unsigned long sesn_call_flags)
{
  UserPreferenceDescriptor 	pref			=	{0};
  UserCommand *usercommand						=	data_msg_received->ufsrvcommand->usercommand;
  UserPreference *user_command_prefs 	=	usercommand->prefs[0];
    ...
    for (size_t i = 0; i < usercommand->n_target_list; i++) {
      ...
      if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
      	if (!SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_SHARELIST_PRESENT)) {
					...
					if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
						MarshalUserPrefNetstate(sesn_ptr, CLIENT_CTX_DATA((&share_list_ctx)), data_msg_received, 0, event_ptr);

						InterBroadcastUserShareListMessage(sesn_ptr, CLIENT_CTX_DATA((&share_list_ctx)), event_ptr, command_arg);

						_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS,
																RESCODE_UFSRV_INTERBROADCAST)//todo: we exiting the loop (probably OK because list should not contain more than one
					}
				} else {
      		//todo: return indicator of prior presence: ACCEPTED_NOOP
      	}
...
UFSRVResult *
IsUserAllowedToShareReadReceipt (Session *sesn_ptr, DataMessage *data_msg_received, UfsrvEvent *event_ptr, unsigned long sesn_call_flags)
{
  UserPreferenceDescriptor 	pref			=	{0};
  UserCommand *usercommand						=	data_msg_received->ufsrvcommand->usercommand;
  UserPreference *user_command_prefs 	=	usercommand->prefs[0];
  unsigned command_arg 								= usercommand->header->args;//!=COMMAND_ARGS__DELETED
    ...
    for (size_t i = 0; i < usercommand->n_target_list; i++) {
      ...
      if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
        if (!SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_SHARELIST_PRESENT)) {
          ...
          if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
            MarshalUserPref (sesn_ptr, CLIENT_CTX_DATA((&share_list_ctx)), data_msg_received, event_ptr);
...
UFSRVResult *
IsUserAllowedToShareBlocked (Session *sesn_ptr, DataMessage *data_msg_received, UfsrvEvent *event_ptr, CallbackCommandMarshaller command_marshaller, unsigned long sesn_call_flags)
{
  UserPreferenceDescriptor 	pref			=	{0};
  UserCommand *usercommand						=	data_msg_received->ufsrvcommand->usercommand;
  UserPreference *user_command_prefs 	=	usercommand->prefs[0];
        ...
        if (!SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_SHARELIST_PRESENT)) {
          ...
          if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
            ...
            if (IS_PRESENT(command_marshaller)) {
              INVOKE_COMMAND_MARSHALLER(command_marshaller, sesn_ptr, ctx_ptr, data_msg_received, event_ptr);
            }
...
UFSRVResult *
IsUserAllowedToShareContacts (Session *sesn_ptr, DataMessage *data_msg_received, UfsrvEvent *event_ptr, CallbackCommandMarshaller command_marshaller, unsigned long sesn_call_flags)
{
  UserPreferenceDescriptor 	pref			=	{0};
  UserCommand *usercommand						=	data_msg_received->ufsrvcommand->usercommand;
  UserPreference *user_command_prefs 	=	usercommand->prefs[0];
...
UFSRVResult *
IsUserAllowedToChangeUnsolicitedContactAction (Session *sesn_ptr, UserPreference *pref_ptr, DataMessage *data_msg_ptr_recieved, UfsrvEvent *event_ptr, CallbackCommandMarshaller command_marshaller, unsigned long sesn_call_flags)
{
  ...
  if (GetUserPreferenceDescriptorById(&user_prefs_table, pref_ptr->pref_id, &pref)) {
    ...
    if (IS_PRESENT((*pref.pref_ops->pref_set)(sesn_ptr, &pref, PREFSTORE_EVERYWHERE, event_ptr))) {
      ...
      if (IS_PRESENT(command_marshaller)) {
        INVOKE_COMMAND_MARSHALLER(command_marshaller, sesn_ptr, ctx_ptr, data_msg_ptr_recieved, event_ptr);
      }
...
JsonFormatStateSyncForSessionState (Session *sesn_ptr,  enum SessionState session_state, struct json_object *jobj_out) {
  ...
  UfsrvUidConvertToString(&SESSION_UFSRVUIDSTORE(sesn_ptr), uid_encoded);
...
JsonFormatStateSync (Session *sesn_ptr, enum DigestMode digest_mode, bool reload_flag, struct json_object *jobj_out)
{
  ...
  UfsrvUidConvertToString (&SESSION_UFSRVUIDSTORE(sesn_ptr_reloaded), uid_encoded);
  ...
  json_object_object_add (jobj, ACCOUNT_JSONATTR_EID, json_object_new_int64(SESSION_EID(sesn_ptr_reloaded)));
	...
	profile_key=DbBackendGetProfileKey(sesn_ptr, &(SESSION_UFSRVUIDSTORE(sesn_ptr_reloaded)), false/*is_decoded*/);
  if (IS_STR_LOADED(profile_key)) {
    json_object_object_add (jobj,ACCOUNT_JSONATTR_PROFILE_KEY, json_object_new_string(profile_key));
    free ((void *)profile_key);
  }
...
/**
 * 	@brief: This is basic profile information for a given user.
 */
struct json_object *
JsonFormatUserProfile (Session *sesn_ptr, unsigned long userid,  enum DigestMode digest_mode, bool reload_flag, struct json_object *jobj_out)
{
	Session *sesn_ptr_reloaded;
  InstanceHolderForSession *instance_sesn_ptr_reloaded = NULL;

	if (reload_flag)	{
	  if ((instance_sesn_ptr_reloaded	=	SessionInstantiateFromBackend (sesn_ptr, userid, CALL_FLAG_SNAPSHOT_INSTANCE|CALL_FLAG_ATTACH_FENCE_LIST_TO_SESSION))) {
	    sesn_ptr_reloaded = SessionOffInstanceHolder(instance_sesn_ptr_reloaded);
	  } else sesn_ptr_reloaded = NULL;
	}
	else 							sesn_ptr_reloaded	=	sesn_ptr;
...
//	struct	json_object *jobj_location			= JsonFormatUserLocation(sesn_ptr_reloaded, NULL);
  ...
  json_object_object_add (jobj, ACCOUNT_JSONATTR_USERNAME, json_object_new_string(SESSION_USERNAME(sesn_ptr_reloaded)));//old "number"
...
JsonValueFormatForSharedLists(Session *sesn_ptr, json_object *jobj_out) {
  ...
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
    ...
    if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_BACKEND_DATA)) {
            ...
            json_object_object_add(jobj, "type", json_object_new_int(atoi(list_type_str)));
...
_AddSharedUser(ClientContextData *ctx_ptr, ClientContextData  *userid_container)
{
	...
	if (IS_PRESENT(sesn_ptr)) {
	  ...
	  UfsrvUidConvertToString((const UfsrvUid *)&SESSION_UFSRVUIDSTORE(sesn_ptr), ufsrvuid_encoded);
