...
_SendToSocket (InstanceHolderForSession *, SocketMessage *sock_msg_ptr, const char *msg, unsigned flag);
inline static ssize_t
_DispatchSocketBuffer (Session *sesn_ptr, SocketMessage *sock_msg_ptr);
static int
_SocketMessageDecode (InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sm_ptr, unsigned flag);
static int
_SocketMessageEncode (InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sm_ptr, unsigned flag);
 ...
 * 	@locked: MessageQueue must be locked by caller
...
_ReadIntoMessageQueueWithNoSessionLock(Session *sesn_ptr, int blocksz)
{
	...
	if (SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_SECURE)) {
		...
		sm_ptr->raw_msg_size = read(SESSION_SOCKETFD(sesn_ptr), sm_ptr->_raw_msg, blocksz);//todo potential data-race condition as we read off the socket value where we don't hold lock on it, it could be in the middle of being destroyed by other thread due to IA error
	}
...
*/
ssize_t
ReadFromSocket(InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sock_msg_ptr, unsigned flag)
{
	int 		no_error = 1;
	int 		i;
	bool 		blocking_io = false;
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (flag&SOCKMSG_READSOCKET) {
		...
		//this lock ensures one thread at a time owns the socket, but multiple threads can queue for it (provided they lock the queue)
		...
		if (res.result_type == RESULT_TYPE_SUCCESS) {
			...
			if (!(sm_ptr_consolidated->sm_errno == 0)) {
				...
				error_consolidation:
				ErrorFromSocket(instance_sesn_ptr, CALLFLAGS_EMPTY);//don't consolidate queue, as we are not passing SOCKMSG_CONSOLIDATE_INSESSION
			...
			//too bad we just bail out the session
      ...
      sm_ptr->raw_msg_size = 0;
		...
		if (raw_msg_size > 0) {
			...
			if (sm_ptr->missing_msg_size > 0) {
				...
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): JOINED FRAME FRAGMENTS: new msg size: '%lu' ", __func__, pthread_self(), SESSION_ID(sesn_ptr), sm_ptr->raw_msg_size);
			  ...
			  //we read'0'
				...
				if (sm_ptr->raw_msg_size == 0) {
				  //this is the consolidated SocketMessage
					...
					ErrorFromSocket(instance_sesn_ptr, 0);
...
//		}
	...
	return (_SocketMessageDecode(instance_sesn_ptr, sm_ptr, flag));

}

static int _SocketMessageDecode(InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sm_ptr, unsigned flag)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
...
static int _SocketMessageEncode(InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sm_ptr, unsigned flag)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
 ...
 */
int
SendTextMessage(InstanceHolderForSession *instance_sesn_ptr, const char *msg, size_t msglen)
{
	TransmissionMessage tmsg;
	tmsg.type = TRANSMSG_TEXT;
	tmsg.len = msglen;
	tmsg.msg = (void *)strbufdup(msg, msglen);
 ...
 */
ssize_t
SendToSocket(InstanceHolderForSession *instance_sesn_ptr, TransmissionMessage *tmsg_ptr, unsigned flag)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//we must flush out queue first to preserve sequence of message generation
	if ((DispatchSocketMessageQueue(instance_sesn_ptr, 0)) < 0) {
		...
		if (sm_ptr_aux) {
			if (tmsg_ptr->type == TRANSMSG_PROTOBUF) {
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ADDING PROTOBUF TO OUT MessageQueue: RAW SocketMessage: size: '%lu'", __func__, pthread_self(), SESSION_ID(sesn_ptr), tmsg_ptr->len);

				sm_ptr_aux->raw_msg_size  = tmsg_ptr->len;
				sm_ptr_aux->_raw_msg      = tmsg_ptr->msg_packed;//should not free in the calling environment
			} else if (tmsg_ptr->type == TRANSMSG_TEXT) {//msg)//msg:cpy 1 must be freed in calling function
				//save msg into SocketMessage
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ADDING TEXT TO OUT MessageQueue: RAW SocketMessage: size: '%lu'", __func__, pthread_self(), SESSION_ID(sesn_ptr), tmsg_ptr->len);

				sm_ptr_aux->raw_msg_size  = tmsg_ptr->len;
				sm_ptr_aux->_raw_msg      = tmsg_ptr->msg_packed;//msg:cpy 2
			} else if (tmsg_ptr->type == TRANSMSG_SOCKMSG) {
				SocketMessage *sm_ptr;
				sm_ptr = (SocketMessage *)tmsg_ptr->msg;

				if (sm_ptr->flag&SOCKMSG_ENCODED && sm_ptr->flag&SOCKMSG_WSFRAMED) {

					syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ADDTING SOCKMSG TO OUT MessageQueue: FULLY ENCODED SocketMessage: size: '%ld'", __func__,pthread_self(), sesn_ptr->session_id, sm_ptr->processed_msg_size);
					...
					sm_ptr->processed_msg_size      = 0;//don't free in the calling function
					sm_ptr_aux->flag                = sm_ptr->flag;
				} else if (sm_ptr->flag&SOCKMSG_DONTWSFRAME) {
					//extract text
					sm_ptr_aux->processed_msg_size = sm_ptr->processed_msg_size;//-written;///-written added
					sm_ptr_aux->_processed_msg = sm_ptr->_processed_msg;
				} else {
					syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): NOT ADDING TO OUT MessageQueue: MSG FLAG UNRECOGNISED: size: '%ld'", __func__,pthread_self(), sesn_ptr->session_id, sm_ptr->processed_msg_size);
				}
			} else {
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ERROR: COULD NOT FLUSH OUT MESSAGEQUEUE: ERROR ADDING TO QUEUE: WRONG PARAMS.", __func__,pthread_self(), sesn_ptr->session_id);
			...
			syslog(LOG_DEBUG, "%s (pid:'%lu' th_ctx:'%p', cid:'%lu'): ERROR: COULD NOT FLUSH OUT MESSAGEQUEUE: QUEUEING MESSAGE: msg size='%ld'. Queue size (nEntries='%lu')", __func__,pthread_self(), NULL, SESSION_ID(sesn_ptr), sm_ptr_aux->processed_msg_size, sesn_ptr->message_queue_out.queue.nEntries);
		}
	...
	if (tmsg_ptr->type == TRANSMSG_TEXT) {//msg)//msg:cpy 1 must be freed in calling function
...
#ifdef __UF_FULLDEBUG
		syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ADDING TEXT TO OUT MessageQueue: RAW SocketMessage: msg: '...' size: '%lu'", __func__, pthread_self(), SESSION_ID(sesn_ptr), /*tmsg_ptr->msg_packed,*/ tmsg_ptr->len);
#endif
		sesn_ptr->ssptr->socket_msg_out.raw_msg_size  = tmsg_ptr->len;
		...
		if (sm_ptr->flag&SOCKMSG_ENCODED && sm_ptr->flag&SOCKMSG_WSFRAMED) {
#ifdef __UF_FULLDEBUG
			syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ADDING SOCKMSG TO OUT MessageQueue: PREVIOUSLY FULLY ENCODED SocketMessage: size: '%ld'", __func__, pthread_self(), SESSION_ID(sesn_ptr), sm_ptr->processed_msg_size);
#endif
			//relay buffer out
		...
		if (sm_ptr->flag&SOCKMSG_DONTWSFRAME) {
			...
			syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): UNABLE TO PROCESS PREPACKED SOCKET MESSAGE: MSG FLAG UNRECOGNISED: size: '%ld'", __func__, pthread_self(), SESSION_ID(sesn_ptr), sm_ptr->processed_msg_size);

			return -2;
		}
	} else {
		syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ERROR: COULD NOT DETERMINE TRANSMISSION MSG TYPE: '%d'.", __func__, pthread_self(), SESSION_ID(sesn_ptr), tmsg_ptr->type);
	...
	return _SendToSocket(instance_sesn_ptr, &(sesn_ptr->ssptr->socket_msg_out), NULL, flag);
...
//
inline static ssize_t
_SendToSocket(InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sock_msg_ptr, const char *msg, unsigned flag)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if ((flag&SOCKMSG_DONTWSFRAME) || ((sesn_ptr->ssptr->socket_msg_out.flag&SOCKMSG_ENCODED) && (sesn_ptr->ssptr->socket_msg_out.flag&SOCKMSG_ENCODED))) {
		...
		int rc = 0;
		if ((rc = _SocketMessageEncode(instance_sesn_ptr, SESSION_OUTSOCKMSG_TRANS_PTR(sesn_ptr), flag)) < 0) {
...
//
//	@brief sends the Websocket framed message to destination. Implements nonblocking sockets behavior. If blocked it will iterate one more time before writing the
...
_DispatchSocketBuffer(Session *sesn_ptr, SocketMessage *sock_msg_ptr)
{
	...
	__unused int myerrno = 0;

#pragma region reading block
  while (written < write_upto) {// || loop_counter<2)
		...
		if (SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_SECURE)) {
			...
			result = send(sesn_ptr->ssptr->sock, sm_ptr->_processed_msg + (written), (write_upto - written), MSG_NOSIGNAL);
		}
		...
		if (result == -1) {//error
			if (SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_SECURE)) {
				...
				if (errno == EAGAIN || errno == EWOULDBLOCK)	blocking_io = true;
...
#pragma endregion
	...
	if (written == write_upto) {
	...
	} else if ((written < write_upto) && (blocking_io)) {//((myerrno == EAGAIN || myerrno==EWOULDBLOCK || myerrno == EINPROGRESS)))// we have to rely on this check because of the loop ret_value==-1 is not error if the first iteration resturned some bytes
		if (written >= 0) {//we have written less than what we wanted
			...
			if (sm_ptr_aux) {
				...
				syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): QUEUEING PARTIAL MESSAGE: msg size='%ld'. written='%ld' Queue size (nEntries='%lu')", __func__,pthread_self(), sesn_ptr->session_id, sm_ptr_aux->processed_msg_size, written, sesn_ptr->message_queue_out.queue.nEntries);

				return 0;//queued
			}
		} else {
			syslog(LOG_DEBUG, "%s (pid:'%lu' o:'%p' cid:'%lu'): WON'T QUEUE PARTIAL MESSAGE ('0' written):  written='%ld' Queue size (nEntries='%lu')", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), written, sesn_ptr->message_queue_out.queue.nEntries);
		}
	} else {
		//other i/o issues
		if ((result == -1) && (blocking_io == false)) {//(!(myerrno==EAGAIN) || !(myerrno==EWOULDBLOCK) || !(myerrno==EINPROGRESS)))
			syslog(LOG_DEBUG, "%s (pid:'%lu', o:'%p', cid:'%lu', socket_fd:'%d', error:'%s'): ERROR WRITING to remote end (write attempt: '%d'). Amount written: '%ld'", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), SESSION_SOCKETFD(sesn_ptr), strerror(errno), loop_counter, written);

			shutdown(sesn_ptr->ssptr->sock, SHUT_RDWR);//disallow in both directions
...
//
int DispatchSocketMessageQueue(InstanceHolderForSession *instance_sesn_ptr, size_t entries)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	while (loop_counter++ <= counter) {
		...
		SocketMessage *sm_ptr = RemoveFromQueue(&(sesn_ptr->message_queue_out.queue), 1, USE_DEFAULT_RECLAIMER);//don't remove the front
		if (sm_ptr) {
			size_t before_msg_size = sm_ptr->processed_msg_size - sm_ptr->written_msg_size;//remeber current size

			size_t after_msg_size = _SendToSocket(instance_sesn_ptr, sm_ptr, NULL, 0);
			if (after_msg_size == before_msg_size) {
				syslog(LOG_DEBUG, "%s: (pid:'%lu' o:'%p' cid:'%lu'): BEFORE MSG_SIZE='%lu'. AFTER MSG_SIZE='%lu'", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), before_msg_size, after_msg_size);

				sm_ptr = RemoveFromQueue(&(sesn_ptr->message_queue_out.queue), SOCKMSG_DONTWSFRAME, USE_DEFAULT_RECLAIMER);
				free(sm_ptr);
			} else {
				syslog(LOG_DEBUG, "%s: (pid:'%lu' o:'%p' cid:'%lu'): NOTICE: BEFORE MSG_SIZE='%lu'. AFTER MSG_SIZE='%lu': MESSAGE NOT REMOVED FROM QUEUE", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), before_msg_size, after_msg_size);
...
void ErrorFromSocket(InstanceHolderForSession *instance_sesn_ptr, unsigned call_flags)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	_SocketMessageDecode(instance_sesn_ptr, SESSION_INSOCKMSG_TRANS_PTR(sesn_ptr), call_flags);
