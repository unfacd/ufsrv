...
struct ProtocolCallbacks {
	...
	UFSRVResult *(*session_init_callback)(ClientContextData *, unsigned);//called each time a new session is allocated physical storage, but with no InstanceHolder
	UFSRVResult *(*session_reset_callback)(InstanceHolder *, unsigned);//called each time a new session is reset flag is set for for recycling
	UFSRVResult *(*handshake_callback)(InstanceHolder *, SocketMessage *, unsigned, int **);
	UFSRVResult *(*post_handshake_callback)(InstanceHolder *, SocketMessage *, unsigned);
	UFSRVResult *(*msg_callback)(InstanceHolder *, SocketMessage *, unsigned, size_t);//incoming
	UFSRVResult *(*msg_out_callback)(InstanceHolder *, SocketMessage *, unsigned, size_t);//outgoing
	UFSRVResult *(*msg_decode_callback)(InstanceHolder *, SocketMessage *, unsigned);
	UFSRVResult *(*msg_encode_callback)(InstanceHolder *, SocketMessage *, unsigned);
	UFSRVResult *(*service_timeout_callback)(InstanceHolder *, time_t, unsigned long); //session idled
	UFSRVResult *(*error_callback)(InstanceHolder *, unsigned);
	UFSRVResult *(*recycler_error_callback)(InstanceHolder *, unsigned);
	UFSRVResult *(*close_callback)(InstanceHolder *);
	UFSRVResult *(*msgqueue_topics_callback)(UFSRVResult *);//supply msg queue topcs to subscribe to
	UFSRVResult *(*generate_session_id_callback)(UFSRVResult *, ClientContextData *);
 };
...
struct Protocol {
		...
		void * (*protocol_threadx)(void *); //main processor thread the gets incoming requests
...
#define PROTO_PROTOCOL_CLLBACKS_GENERATE_SESSION_ID(x) (x->protocol_callbacks.generate_session_id_callback)
...
#define _PROTOCOL_CLLBACKS_GENERATE_SESSION_ID(x, y) ((x+y))->protocol_callbacks.generate_session_id_callback
#define _PROTOCOL_CLLBACKS_GENERATE_SESSION_ID_INVOKE(x, y, ...) ((x+y)->protocol_callbacks.generate_session_id_callback)(__VA_ARGS__)
