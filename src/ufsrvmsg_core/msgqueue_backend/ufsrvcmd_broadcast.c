...
_HandleInterBroadcast(MessageQueueMsgPayload *mqp_ptr, MessageQueueMessage *mqm_ptr)
{
  ...
  WorkersConfigDescriptor *jobworkers_config  = GetJobWorkersConfigurationDescriptor();
	...
	statsd_timing(pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key), "delegator.ufsrv.job.msgqueue.inter_broadcast_handler.elapsed_time", (timer_end-timer_start));
...
int WorkerThreadMsgQueueParserExecutor(MessageContextData *context_ptr)
{
  ...
  WorkersConfigDescriptor *jobworkers_config = GetJobWorkersConfigurationDescriptor();
	...
	DestructMessageQueueMsgPayload(mqp_ptr, mqm_ptr, false);//object gets freed in
	timer_end = GetTimeNowInMicros();
	statsd_timing(pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key), "delegator.ufsrv.job.msgqueue.intra_broadcast_handler.elapsed_time", (timer_end-timer_start));
	return rc;
 ...
 */
UFSRVResult *
UfsrvInterBroadcastMessage(Session *sesn_ptr, MessageQueueMessage *msgqueue_ptr, unsigned event_type)
{
  msgqueue_ptr->origin							=	masterptr->serverid;

	UFSRVResult 					*res_ptr		=	NULL;
	MessageQueueBackend 	*mq_ptr			=	THREAD_CONTEXT_MSGQUEUE_CACHEBACKEND;
	redisReply 						*redis_ptr	=	NULL;

	size_t packed_sz = message_queue_message__get_packed_size(msgqueue_ptr);
	uint8_t packed_msg[packed_sz];
	message_queue_message__pack(msgqueue_ptr, packed_msg);

	switch (msgqueue_ptr->command_type)
	{
		case UFSRV_FENCE:
...
#define SESSION_CALL_FLAGS (CALL_FLAG_LOCK_SESSION|CALL_FLAG_HASH_SESSION_LOCALLY|					\
  ...
  unsigned  long uid = UfsrvUidGetSequenceId((const UfsrvUid *) cmd_header_ptr->ufsrvuid.data);
	if (IS_PRESENT((instance_sesn_ptr_localuser = SessionInstantiateFromCacheBackendWithDbFallback(NULL, (const UfsrvUid *) cmd_header_ptr->ufsrvuid.data, SESSION_CALL_FLAGS)))) {
	  sesn_ptr_localuser = SessionOffInstanceHolder(instance_sesn_ptr_localuser);
