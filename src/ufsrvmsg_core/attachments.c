...
{
#define SQL_INSERT_ATTACHMENT "INSERT INTO attachments (blob_id, `key`, digest, blurhash, caption, mimetype, key_size, digest_size, size, flags, fid, userid, device_id, id_events) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%lu', '%lu', '%lu', '%u', '%lu', '%lu', '%d', '%lu') "

  char *sql_query_str = mdsprintf(SQL_INSERT_ATTACHMENT, attachment_ptr->id, attachment_ptr->key_encoded, attachment_ptr->digest_encoded, IS_STR_LOADED(attachment_ptr->blurhash)?attachment_ptr->blurhash:NULL, IS_STR_LOADED(attachment_ptr->caption)?attachment_ptr->caption:NULL, attachment_ptr->mime_type, attachment_ptr->key_sz, attachment_ptr->digest_sz, attachment_ptr->size, attachment_ptr->flags, fid, userid, device_id, attachment_ptr->eid);
...
#define SQL_SELECT_ATTACHMENT_DOWNLOAD 	"SELECT mimetype, size, `key`, key_size, digest, digest_size, id_events, blurhash, caption, flags FROM attachments WHERE blob_id = '%s';"
...
#define QUERY_RESULT_FLAGS(x)		        ((struct _h_type_int *)	result.data[x][9].t_data)->value
...
#define QUERY_RESULT_CAPTION_NOTNULL(x)	  (IS_PRESENT((struct _h_type_blob *)result.data[x][8].t_data))
  ...
  char *sql_query_str = mdsprintf(SQL_SELECT_ATTACHMENT_DOWNLOAD, blob_id);
  ...
  memcpy(attachment_ptr->mime_type, QUERY_RESULT_MIMETYPE(0), QUERY_RESULT_MIMETYPE_LEN(0) >= TINYTINYBUF? TINYTINYBUF-1 : QUERY_RESULT_MIMETYPE_LEN(0));
	if (flag_fully_populate) {
    memcpy(attachment_ptr->id, blob_id, strlen(blob_id));
		if (QUERY_RESULT_KEY_LEN(0) > 0) {
			...
			attachment_ptr->key_encoded = strbufdup(QUERY_RESULT_KEY(0), QUERY_RESULT_KEY_LEN(0));
		...
		if (QUERY_RESULT_DIGEST_NOTNULL(0) && QUERY_RESULT_DIGEST_LEN(0) > 0) {
			...
			//for encoded keys hitting the extreme end it will be truncated. But reality key sizes are smaller than the generous MBUF, so we should be fine
			attachment_ptr->digest_encoded  = strbufdup(QUERY_RESULT_DIGEST(0), QUERY_RESULT_DIGEST_LEN(0));
			attachment_ptr->digest_sz			  =	QUERY_RESULT_DIGESTSIZE(0);
		}

		if (QUERY_RESULT_BLURHASH_NOTNULL(0)) {
      attachment_ptr->blurhash = strbufdup(QUERY_RESULT_BLURHASH(0), QUERY_RESULT_BLURHASH_LEN(0));
		}

    if (QUERY_RESULT_CAPTION_NOTNULL(0)) {
      attachment_ptr->caption = strbufdup(QUERY_RESULT_CAPTION(0), QUERY_RESULT_CAPTION_LEN(0));
    }

    attachment_ptr->flags = QUERY_RESULT_FLAGS(0);
		//todo: other fields
	}
...
_CacheLocalLruGetAttachment(Session *sesn_ptr, const char *blob_id)
{
  ...
  instance_attachment_ptr = (InstanceHolderForAttachmentDescriptor *) LockingLruGet(&AttachmentsLruCache, blob_id, (LruClientData **) &instance_attachment_ptr_evicted);
...
_CacheLocalLruSetAttachmentItem(Session *sesn_ptr,  InstanceHolderForAttachmentDescriptor *instance_attachment_ptr_in)
{
	...
	LruClientData *data_ptr_returned = LockingLruSet(&AttachmentsLruCache, (LruClientData *) instance_attachment_ptr_in);
