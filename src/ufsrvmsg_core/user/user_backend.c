...
static int			_DbCreateAccount(Session *sesn_ptr, json_object *jobj_device, json_object *jobj_kbs, json_object *jobj_donations, json_object *jobj_userdata, enum AccountRegoStatus);
...
static UFSRVResult *_DbAccountDataDeviceAttributeGetText(unsigned long, int device_id, const char *attribute_name);
...
static const uintptr_t _AttributeValueGetterByText(json_object *jobj_data, const char *attribute_name);
static const uintptr_t _AttributeValueGetterByBoolean(json_object *jobj_data, const char *attribute_name);
static const uintptr_t _AttributeValueGetterByInt(json_object *jobj_data, const char *attribute_name);
...
DbSetGcmId(Session *sesn_ptr, const UfsrvUid *uid_ptr, int device_id, const char *gcm_id)
{
	...
	jobj_account = DbGetAccountInJson(uid_ptr);
...

	{
		...
		for (i=0; i<array_size; i++) {
			...
			if (device_id == json_object_get_int(json__get(jobj_device, "id"))) {
				...
				fetches_message = json_object_get_boolean(json__get(jobj_device, "fetches_messages"));
				if (fetches_message == 0) {
#if __UF_TESTING
					syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', uname:'%s'}: Received gcm_id for an inactive account: Activating...", __func__, pthread_self(), sesn_ptr, SESSION_USERNAME(sesn_ptr));
#endif
...
{
  _DbAccountDataDeviceAttributeGetText(UfsrvUidGetSequenceId(uid_ptr), device_id, ACCOUNT_JSONATTR_GCM_ID);
	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
	  char *cm_token = ((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
...
{
  _DbAccountDataDeviceAttributeGetText(UfsrvUidGetSequenceId(uid_ptr), device_id, ACCOUNT_JSONATTR_CREATED);
	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
		unsigned long when = strtoul((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA, NULL, 10);
...
//attribute to be replaced must already be in the set., otherwise JSON_INSERT is required
int
 ...
 */
static UFSRVResult *
_DbAccountDataDeviceAttributeGetText(unsigned long userid, int device_id, const char *attribute_name)
{
  ...
  struct _h_result result;
  char *sql_query_str = mdsprintf(SQL_SELECT_ACCOUNT_ATTRIBUTE_STRING, attribute_name, userid);
 ...
 */
UFSRVResult *
DbAccountDataAttributeGetTextByUsername(const char *username, const char *attribute_name)
{
...
{
	//IMPORTANT: USING JSON_UNQUOTE turns the value from json string,  i.e. "value", to my sql string, i.e. value, but the library then return blob type,
	//as opposed to string type. Without UNQUOTE we get string type, but we have to remove the opening and closing " manually
	#define SQL_SELECT_ACCOUNT_ATTRIBUTE_STRING "SELECT  JSON_UNQUOTE(JSON_EXTRACT(data, '$.authenticated_device.%s')) FROM accounts WHERE id=%lu"
...
{
  json_object *jobj_account = DbGetAccountInJson(uid_ptr);
...
_DbAccountAttributeSetJsonObject(Session *sesn_ptr, const UfsrvUid *uid_ptr, int device_id_in, const char *attr_name, struct json_object *jobj)
{
  json_object *jobj_account = DbGetAccountInJson(uid_ptr);
	...
	{
		//fetch device id and update there as well
		json_object *jobj_devices_array = json__get(jobj_account, "devices");
		int array_size = json_object_array_length(jobj_devices_array);
		int i;
		...
		for (i=0; i<array_size; i++) {
			json_object *jobj_device = json_object_array_get_idx(jobj_devices_array, i);
			if (device_id == json_object_get_int(json__get(jobj_device, "id"))) {
				json_object_object_del(jobj_device, attr_name);
...
#define SQL_UPSERT_PENDING_ACCOUNT "INSERT INTO pending_accounts (number, password, cookie, `returning`, verification_code) VALUES ('%s', '%s', '%s', '%d', '%lu') ON DUPLICATE KEY UPDATE verification_code = '%lu', cookie = '%s'"
  ...
  char *sql_query_str = mdsprintf(SQL_UPSERT_PENDING_ACCOUNT, creds_ptr->username, creds_ptr->password, cookie, creds_ptr->rego_status, vcode_ptr->code, vcode_ptr->code, cookie);
...
DbCreateNewAccount(Session *sesn_ptr,  const char *username, const char *e164number, const char *password, const char *nonce)
{
	...
	if ((rego_status = GetAccountRegistrationStatus(username)) != REGOSTATUS_UNKNOWN) {
#ifdef __UF_FULLDEBUG
	...
	char *cookie = GenerateCookie(CONFIG_MAX_REGOCOOKIE_SZ);
   ...
   DbDeletePendingAccountByUsername(username); //remove stale record if any, which can happen during botched registrations
 ...
 */
int
DbSetPendingAccountRegoStatus(const PendingAccount *pacct_ptr)
{
#define SQL_INSERT_KEY "UPDATE ufsrv.pending_accounts SET `returning` = %d where cookie='%s'"

  char *sql_query_str = mdsprintf(SQL_INSERT_KEY, PENDINGACCOUNT_PTR_REGO_STATUS(pacct_ptr), PENDINGACCOUNT_PTR_COOKIE(pacct_ptr));
	...
	if (sql_result != H_OK) {
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: ERROR: COULD EXECUTE QUERY: '%s'", __func__, pthread_self(), sql_query_str);
	}
...
#define SQL_DELETE_USER_ACCOUNT "DELETE FROM accounts WHERE `id` = '%lu'"
  ...
  char *sql_query_str = mdsprintf(SQL_DELETE_USER_ACCOUNT, userid);
 ...
 */
int
DbDeletePendingAccount(const char *cookie)
{
 ...
 */
int
DbDeletePendingAccountByUsername(const char *username)
{
#define SQL_DELETE_PENDING_ACCOUNT "DELETE FROM pending_accounts WHERE number = '%s'"

  char *sql_query_str;
  sql_query_str = mdsprintf(SQL_DELETE_PENDING_ACCOUNT, username);
...
/**
 * @brief Retrieve registration code stored in db backend.
 * @param pending_acct_ptr User-allocated structure holding pending registration cookie as a pending account identifier.
 * If successful, \ref PendingAccount.verification_code.code and \ref PendingAccount.rego_status are provided to the caller.
 */
UFSRVResult * __attribute__((nonnull()))
DbGetPendingAccountVerificationCode(PendingAccount *pending_acct_ptr)
{
#define SQL_GET_VERIFICATION_CODE "SELECT verification_code, `returning` FROM pending_accounts WHERE cookie='%s'"
...
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: FOUND VERIFICATION CODE: '%lu', RETURNING: '%d'", __func__, pthread_self(), pending_acct_ptr->verification_code.code, pending_acct_ptr->rego_status);
#endif
    ...
    THREAD_CONTEXT_RETURN_RESULT_SUCCESS(NULL, RESCODE_BACKEND_DATA)
		...
		syslog(LOG_DEBUG, "%s {}: ERROR: COULD NOT LOCATE USER RECORD", __func__);
    THREAD_CONTEXT_RETURN_RESULT_ERROR(NULL, RESCODE_BACKEND_DATA_EMPTYSET)
	}
...
IsPendingCookieValid(const char *cookie)
{
#define SQL_GET_VERIFICATION_CODE "SELECT cookie from pending_accounts WHERE cookie = '%s'"
  struct _h_result result;
  char *sql_query_str;

  sql_query_str = mdsprintf(SQL_GET_VERIFICATION_CODE, cookie);
  ...
  if (result.nb_rows > 0) {
   bool is_true = strlen((char *)(((struct _h_type_blob *)result.data[0][0].t_data)->value)) > 0;
...
{
  json_object *jobj_userdata = json_object_new_object();
  ...
  json_object_object_add(jobj_userdata, ACCOUNT_JSONATTR_GEOLOC_TRIGGER, json_object_new_int(GEOLOC_TRIGGER_NEIGHBOURHOOD));
  json_object_object_add(jobj_userdata, ACCOUNT_JSONATTR_HOMEBASE_GEOLOC, json_object_new_string(CONFIG_DEFAULT_PREFS_STRING_VALUE));
  json_object_object_add(jobj_userdata, ACCOUNT_JSONATTR_BASELOC_ZONE, json_object_new_int(BASELOC_ZONE_NEIGHBOURHOOD));
  ...
  json_object *jobj_userdata_personal_profile = ProvideDefaultProfilePersonalJsonToken();
  json_object_object_add(jobj_userdata, ACCOUNT_JSONATTR_PERSONAL, jobj_userdata_personal_profile);
...
static KbsDescriptor *
_GetKbsTokenForAccountUpgrade(unsigned long userid, KbsDescriptor *kbs_descriptor_ptr)
{
  return DbAccountGetKbs(userid, false, false, true, kbs_descriptor_ptr);
}
 ...
 *	@param jobj_by_user json payload provided by user
 *
...
UpgradePendingAccountByJson(Session *sesn_ptr, PendingAccount *pacct_ptr, struct json_object *jobj_by_user, bool flag_nuke_old)
{
  ...
  DbAccountGetUfsrvUid(sesn_ptr, SESSION_USERNAME(sesn_ptr), &uid);
  ...
  if (IS_PRESENT(uid_ptr)) {
    ...
    rego_status = GetAccountRegistrationStatus(PENDINGACCOUNT_PTR_USERNAME(pacct_ptr));

    DbBackendGetProfileKey(uid_ptr, KEY_B64_SERIALISED, &key_store);
    if (IS_STR_LOADED(key_store.serialised)) profile_key_encoded = key_store.serialised;

    //if empty, indicates a non-returning user
    if (!IS_EMPTY(_GetKbsTokenForAccountUpgrade(UfsrvUidGetSequenceId(uid_ptr), &kbs_descriptor)) && kbs_descriptor.raw.is_set) {
      jobj_kbs = kbs_descriptor.raw.jobj;
    }

    //if empty, indicates a non-returning user
    if (!IS_EMPTY(GetDonationsTokenForAccountUpgrade(UfsrvUidGetSequenceId(uid_ptr), &donations_descriptor)) && donations_descriptor.raw.is_set) {
      jobj_donations = donations_descriptor.raw.jobj;
    }
  ...
  //indicates a non-returning, brand-new user
  if (IS_EMPTY(jobj_kbs)) {
    jobj_kbs = ProvideDefaultKbsJsonToken();
  }

  //indicates a non-returning, brand-new user
  if (IS_EMPTY(jobj_donations)) {
    jobj_donations = ProvideDefaultDonationsJsonToken(true);
  }
  ...
  if (IS_STR_LOADED(PENDINGACCOUNT_PTR_E164_NUMEBR(pacct_ptr)) &&
	...
	json_object *jobj_device = json_object_new_object();
	json_object_object_add(jobj_device, ACCOUNT_JSONATTR_ID, json_object_new_int(1));
  json_object_object_add(jobj_device, ACCOUNT_JSONATTR_PROFILE_KEY, json_object_new_string(profile_key_encoded)); //temporary -> to be removed at account creation
...
//	json_object_object_add (jobj_device, ACCOUNT_JSONATTR_E164NUMBER, json_object_new_string(IS_PRESENT(e164number)?e164number:CONFIG_DEFAULT_PREFS_STRING_VALUE)); //allow e164 per device
  ...
  json_object *jobj_userdata = _GenerateUserDataJsonDescriptor(sesn_ptr, uid_ptr, true);
    ...
    DbDeleteDonationsSubscriptionPipeline(UfsrvUidGetSequenceId(uid_ptr));
    DbDeleteDonationsSubscription(UfsrvUidGetSequenceId(uid_ptr));
  ...
  json_object_get(jobj_device);//increment refcount
  json_object_get(jobj_kbs);//increment refcount
  json_object_get(jobj_donations);//increment refcount

	//otherwise we retain the old UID if exists against the same number
	if (_DbCreateAccount(sesn_ptr, jobj_device, jobj_kbs, jobj_donations, jobj_userdata, rego_status) == 0) {
    ...
    if ((json_object_put(jobj_kbs)) != 1) {
      syslog(LOG_DEBUG, "%s {pid:'%lu' o:'%p', cid:'%lu'}: ERROR: MEMORY LEAK JSON KBS WAS NOT FREED", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr));
    }

    if ((json_object_put(jobj_donations)) != 1) {
      syslog(LOG_DEBUG, "%s {pid:'%lu' o:'%p', cid:'%lu'}: ERROR: MEMORY LEAK JSON DONATIONS WAS NOT FREED", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr));
    }
    ...
    if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_BACKEND_DATA)) {
      ...
      char e164number_updated[CONFIG_E164_NUMBER_SZ_MAX + 1] = {0};
		...
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {//from BackendDirectoryContactTokenSet
			...
			if ((SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) && (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_SIGNON))) {
				...
				_RETURN_RESULT_SESN(sesn_ptr, authacct_ptr, RESULT_TYPE_SUCCESS, RESCODE_USER_SIGNON)
			}
		}
	} else {
		if ((json_object_put(jobj_userdata)) != 1) {
...
UFSRVResult *
CachebackendDelPendingAccountCookie(const char *cookie)
{
	...
	if (!(redis_ptr = (*pers_ptr->send_command)(NULL, REDIS_CMD_PENDINGACCTCOOKIE_DEL, cookie))) {
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: ERROR COULD NOT DEL COOKIE: BACKEND CONNECTIVITY ERROR", __func__, pthread_self());

    THREAD_CONTEXT_RETURN_RESULT_ERROR(NULL, RESCODE_BACKEND_CONNECTION)
	}

	if (redis_ptr->type == REDIS_REPLY_INTEGER && redis_ptr->integer == 1) {
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: SUCCESS COOKIE:'%s' DELETED...", __func__, pthread_self(), cookie);

		freeReplyObject(redis_ptr);

    THREAD_CONTEXT_RETURN_RESULT_SUCCESS(NULL, RESCODE_PROG_NULL_POINTER)
	} else {
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: ERROR COULD NOT DEL NONCE: REPLY ERROR '%s'", __func__, pthread_self(), redis_ptr->str);
...
/**
 * @brief Query the backend to determine if the pending cookie is still present, indicating the associated account has not been verified yet.
 * @param sesn_ptr
 * @param cookie cookie key to verify
 * @param is_stored_code_required if set to true, return the stored value associated with the query
 * @return The value stored against the cookie: the verification code
 * @dynamic_memory: ALLOCCATES str value upon success (if @param is_stored_code_required is set)
 */
UFSRVResult *
CachebackendGetPendingAccountCookie(const char *cookie, bool is_stored_code_required)
{
	redisReply *redis_ptr;
  PersistanceBackend	*pers_ptr		=	THREAD_CONTEXT_SESSION_CACHEBACKEND;

  if (!(redis_ptr = (*pers_ptr->send_command)(NULL, REDIS_CMD_PENDINGACCTCOOKIE_GET, cookie))) {//half day
		syslog(LOG_DEBUG, "%s (pid:'%lu', cookie:'%s'): ERROR COULD ISSUE GET COMMAND: BACKEND CONNECTIVITY ERROR", __func__, pthread_self(), cookie);

    THREAD_CONTEXT_RETURN_RESULT_ERROR(NULL, RESCODE_BACKEND_RESOURCE_NULL)
	}

	if (redis_ptr->type == REDIS_REPLY_STRING) {
		const char *stored_value = NULL;
		if (is_stored_code_required) stored_value = strdup(redis_ptr->str);

		freeReplyObject(redis_ptr);

    THREAD_CONTEXT_RETURN_RESULT_SUCCESS(stored_value, RESCODE_BACKEND_DATA)
	} else {
		syslog(LOG_DEBUG, "%s (pid:'%lu', cookie:'%s'): ERROR: PENDING ACCOUNT COOKIE GET FAILED FROM CACHEBACKEND: '%s' REPLY CODE:'%d'", __func__, pthread_self(), cookie, redis_ptr->str, redis_ptr->type);
...
GetAccountVerificationStatus(Session *sesn_ptr, PendingAccount *pacct_ptr_out)
{
	...
	CachebackendGetPendingAccountCookie(pacct_ptr_out->cookie, false);
	if (THREAD_CONTEXT_UFSRV_RESULT_TYPE_SUCCESS) { //user could still have verified
	...
	DbGetPendingAccountVerificationCode(pacct_ptr_out);
  ...
  return GetAccountRegistrationStatus(SESSION_USERNAME(sesn_ptr));

  return_account_unverified:
  return  REGOSTATUS_PENDING;
}

static UFSRVResult *_PendingAccountVerifyRegistrationCode(PendingAccount *pacct_ptr, int verification_code_provided);
/**
 * @brief
 * @note does not change state of data.
 * @param pacct_ptr[IN/OUT] user provided to pass registration data
 * @param verification_code_provided
 * @return
 */
static UFSRVResult *
_PendingAccountVerifyRegistrationCode(PendingAccount *pacct_ptr, int verification_code_provided)
{
...
//TODO: SMS VERIFICATION DISABLED FOR BETA TESTING
  ...
  //with this test first, because it indicates if the pending account has expired or not
  CachebackendGetPendingAccountCookie(pacct_ptr->cookie, true);
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    int registration_code_stored = atoi((const char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
    if (test_account || verification_code_provided == registration_code_stored) {
      syslog(LOG_DEBUG, "%s {pid:'%lu', test_account:'%d'}: SUCCESS PENDING COOKIE:'%s' WITH Stored verification code value:'%s'", __func__, pthread_self(), test_account, pacct_ptr->cookie, (const char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
    } else {
      //note: we compare against user supplied, not stored, which is done below
      syslog(LOG_DEBUG, "%s {pid:'%lu', code_provided:'%i', code_stored:'%i'}: ERROR PENDING COOKIE:'%s' AND SUPPLIED VERIFICATION CODE DOES NOT MATCH", __func__, pthread_self(), verification_code_provided, atoi((const char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA), pacct_ptr->cookie);

      free(THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
      THREAD_CONTEXT_RETURN_RESULT_ERROR(NULL, RESCODE_ACCOUNT_REGISTRATION_CODE_MISMATCH)
    }
  ...
  DbGetPendingAccountVerificationCode(pacct_ptr);
  if (THREAD_CONTEXT_UFSRV_RESULT_TYPE_SUCCESS) {
    if (test_account || pacct_ptr->verification_code.code == verification_code_provided) {
  #ifdef _UF_TESTING
      syslog (LOG_DEBUG, "%s (cid:'%lu', returning:'%d'): VERIFICATION CODES BACKEND MATCH: '%d'", __func__, SESSION_ID(sesn_ptr), pacct_ptr->rego_status, verification_code_provided);
  #endif
      return THREAD_CONTEXT_UFSRV_RESULT_PTR;
    } else {
      THREAD_CONTEXT_RETURN_RESULT_ERROR(NULL, RESCODE_ACCOUNT_REGISTRATION_CODE_MISMATCH)
  ...
  return THREAD_CONTEXT_UFSRV_RESULT_PTR; //from DbGetPendingAccountVerificationCode

}

/**
 * 	@brief Process a request to verify account registration using code previosuly generated and sent to user's registration id source. Currently only emails are supported.
 * 	@param pacct_ptr: must be loaded with pending cookie
 *
 */
bool
PendingAccountVerifyByRegistrationIdSource(PendingAccount *pacct_ptr, int verification_code_provided)
{
  _PendingAccountVerifyRegistrationCode(pacct_ptr, verification_code_provided);
  if (THREAD_CONTEXT_UFSRV_RESULT_TYPE_SUCCESS) {
...
/**
 * 	@brief Check if a given pending registration is ready to be upgraded to a full account.
 *
 * 	To be  upgradeable: 1)the registration code must match; and 2) the account must have been previously verified from registration id source.
 * 	The verification process: 1)check if cookie is valid, 2)check if user already exists, 3)check verification code
 ...
 *
 * 	@param pending_account_ptr[inout]: User-allocated structure to hold pass on-and-back registration data. Must be loaded with pending cookie when called with this function.
 *  @param verification_code_provided[in] verification code previously generated for the account
 *  @param rego_lock_provided[in] registration lock as provided by account holder. This will only be checked if user has previously verified registration code.
 */
enum AccountUpgradeStatus __attribute__((nonnull(1)))
UpgradePendingAccountIfPossible(PendingAccount *pending_account_ptr, int verification_code_provided, const char *rego_lock_provided, void (^on_upgradeable)(const char *))
{
  enum AccountUpgradeStatus upgrade_status = UPGRADE_STATUS_REGO_CODE_ERROR;

  _PendingAccountVerifyRegistrationCode(pending_account_ptr, verification_code_provided);
  if (THREAD_CONTEXT_UFSRV_RESULT_TYPE_ERR) goto return_status;

  if (PENDINGACCOUNT_PTR_REGO_STATUS(pending_account_ptr) == REGOSTATUS_VERIFIED) {
    RegistrationLockState registration_lock_state = {0};
    MatchRegistrationLockIfSet(pending_account_ptr->username, rego_lock_provided, &registration_lock_state);
    if (registration_lock_state.is_set) {
      if (!IS_STR_LOADED(rego_lock_provided))  {
        upgrade_status = UPGRADE_STATUS_REGO_LOCK_SET;//account has rego_lock, which user did not provide
        goto return_status;
      }

      if (!registration_lock_state.is_identical) {
        upgrade_status = UPGRADE_STATUS_REGO_LOCK_MISMATCH;//account has rego_lock, which user could not match
        goto return_status;
      }
    }

    if (IS_PRESENT(on_upgradeable)) on_upgradeable(PENDINGACCOUNT_PTR_COOKIE(pending_account_ptr));
    upgrade_status = UPGRADE_STATUS_UPGRADED;
  }
...
/**
 * 	@brief: return contacts which are shared with the user
 *
...
{
	size_t contacts_count = json_object_array_length(jobj_contacts);
...
#ifdef __UF_TESTING
	syslog(LOG_DEBUG, "%s (cid: '%lu'): RECEIVED contacts: '%lu'", __func__, SESSION_ID(sesn_ptr), json_object_array_length(jobj_contacts));
#endif

	size_t 				actually_processed	=	contacts_count;
	...
	{
		size_t 							i;
		size_t							tokens_processed __unused,
			...
			if ((RedisGetReply(sesn_ptr, pers_ptr, (void*)&replies[i]) == REDIS_OK)) {
				if ((replies[i] != NULL) && (replies[i]->type != REDIS_REPLY_NIL)) {
					if (unlikely(replies[i]->len == 0)) {
						syslog(LOG_ERR, "%s {pid:'%lu', o:'%p', cid:'%lu', i:'%lu', redis_type:'%d'}: SEVERE DATA ERROR: RECORD PASSED AS NON-NIL BUT CONTAINS ZERO-LENGTH DATA", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), i, replies[i]->type);
		...
		for (i=0; i<tokens_matched_idx; i++) {
			...
			if (IS_PRESENT(userid)) {
				...
				*userid++ = '\0';
				...
				if (IS_PRESENT(JsonFormatUserProfile(sesn_ptr, &(UfsrvUidSequenceIdPair){.ufsrvuid=EMPTY_UFSRVUID, .uid=strtoul(userid, NULL, 10)}, DIGESTMODE_CONTACTS_SHARING, true, jobj_token))) {
...
#ifdef __UF_TESTING
			syslog(LOG_DEBUG, LOGSTR_BACKENDCACHE_SHARED_CONTACTS_REPORT, __func__, pthread_self(), sesn_ptr, 0UL, LOGCODE_BACKENDCACHE_SHARED_CONTACTS_FOUNDNONE, "No shared contacts found");
#endif
...
{
  DbAccountDataAttributeGetText(UfsrvUidGetSequenceId(uid_ptr), ACCOUNT_JSONATTR_ACCESS_TOKEN);
//  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    char *access_token_encoded = (char *) THREAD_CONTEXT_UFSRV_RESULT_USERDATA;//SESSION_RESULT_USERDATA(sesn_ptr);
    if (*access_token_encoded == '*')	 {
      free (access_token_encoded);
      return NULL;
    }
    ...
    if (!is_decoded) {
      ...
      int decoded_sz = 0;
      unsigned char *access_token_decoded = base64_decode((const unsigned char *) THREAD_CONTEXT_UFSRV_RESULT_USERDATA, CONFIG_USER_PROFILEKEY_MAX_SIZE, &decoded_sz);
      free (THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
...
DbBackendSetAccessToken(Session *sesn_ptr, const char *access_token_encoded)
{
  if (likely(strlen(access_token_encoded) < CONFIG_USER_ACCESS_TOKEN_MAX_SIZE_ENCODED)) {
    if ((DbAccountUpdateDataByText(ACCOUNT_JSONATTR_ACCESS_TOKEN, access_token_encoded, SESSION_USERID(sesn_ptr))) == 0) {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_BACKEND_DATA)
    } else {
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_BACKEND_DATA)
    }
 ...
 */
static int
_DbCreateAccount(Session *sesn_ptr, json_object *jobj_device, json_object *jobj_kbs, json_object *jobj_donations, json_object *jobj_userdata, enum AccountRegoStatus rego_status)
{
	unsigned long verification_code = 0UL;
...
#define SQL_INSERT_NEW_ACCOUNT "INSERT INTO accounts (number, `uuid`, data, data_user) VALUES ('%s', unhex(replace(uuid(),'-','')), '%s', '%s') ON DUPLICATE KEY UPDATE data = '%s', data_user = '%s'"
  ...
  //profile_commitment and profile_version do not currently come through during rego, unlike profile_key, but that may change in the future
...
//  json_object_object_add(jobj_account, ACCOUNT_JSONATTR_REGO_LOCK, json_object_new_int(CONFIG_DEFAULT_PREFS_REGO_LOCK));//TBD
//		json_object_object_add (jobj_account, ACCOUNT_JSONATTR_E164NUMBER, json_object_new_string(e164number));

  if (IS_PRESENT(jobj_kbs)) {
    json_object_object_add(jobj_account, ACCOUNT_JSONATTR_KBS, jobj_kbs);
  }

  if (IS_PRESENT(jobj_donations)) {
    json_object_object_add(jobj_account, ACCOUNT_JSONATTR_DONATIONS, jobj_donations);
  }

  if (rego_status > REGOSTATUS_PENDING)	{
    json_object_object_add(jobj_device, "is_returning", json_object_new_boolean(1));
  }
  json_object_object_add(jobj_account, "authenticated_device", jobj_device);

  //attach master device to array of devices
  json_object *jarray_accounts = json_object_new_array();
  ...
  char *sql_query_str = mdsprintf(SQL_INSERT_NEW_ACCOUNT, username, json_account_str, json_userdata_str, json_account_str, json_userdata_str);
...
static UFSRVResult *_DbBackendGetAccount(DbOpDescriptor *dbop_descriptor);
static char *_UserAccountDbOpQueryProvider(intptr_t values[]);
static int _UserAccountDbOpTransformer(DbOpDescriptor *dbop_descriptor);

static UFSRVResult *
_DbBackendGetAccount(DbOpDescriptor *dbop_descriptor)
{
  GetDbResultForQuery(THREAD_CONTEXT_DB_BACKEND, dbop_descriptor);
  return ReturnUfsrvResultFromDbOpDescriptor(dbop_descriptor);
}

//{ "prefs_bool": 192, "nickname": "Yulla", "avatar": "*", "e164number": "+15551234567", "rego_pin": 0, "geoloc_trigger": 0, "baseloc_zone": 0, "unsolicited_contact": "1" }
//{"number": "usergoodvery@gmail.com", "identity_key": "*", "access_token": "nVWBUwjpB3IExB/w9Zq/Zg==", "profile_key": "qjE09q7qkOsMeSIFTD3LTs/CA9TX4kyO6n1q2D6BknA=", "profile_commitment": "AFS+2R4Qn1Y6mZM88hR9mqBbyemoc9T3ccZo8Zb5C4pqJAQjOhQ8FdBEtqagTiH6fgHR0ZVyAF0nLfp9MbM4ckACVNTnYGW7/v2T93MkMowx7iLjggOBHwH/E5PVbwK+UQ==", "profile_version": "c3d5e0d5dc4097083e93d505d468dfa720b346dc377c1c019cf3d130812b6102", "rego_status": 5,
// "authenticated_device": {"id": 1, "registration_id": 12579, "authentication_token": "tchm4JW6tG31su04/EpsqrdgybA=", "salt": "0a9b061d49701c0711d551fa43b1c3f7c80db4e4e37e0e0ed26caaae1da8fbd5", "number": "usergoodvery@gmail.com", "created": 93825006651152, "lastseen": 1670666721774, "user_agent": "*", "is_returning": true, "cookie": "70dffd9a20dca4cb574c7649c024e408f7ca6a429a9e3e7ec5d63b32fce2bf27", "signed_prekey": {"keyId": 14376903, "publicKey": "BR/OIX1XN5npfeN1+0ETl3GurpzlxpOJHy0GTg5nBqpK", "signature": "RxB9c7kwDGJ0gd+EaTtw3eKzZK6jvpKgvke6NyBcA34h9gxjJeG/lwDX/Y4S9sNwTCuXREuMTEmEetAK1PAaCA"}, "identityKey": "Bci7Z3iioV0BIEZ1V48RKns/shbXSt5dVZbQxJGYmTNR", "gcm_id": "eZfOv8SWSZ6SiW_NaX4Pov:APA91bEDB9o0309DZAehRAiqBJVtAKV_SpN9WexGad698F_xOXmw9UwQEaVZdr9BZrejJZdimyCNPuvaBRXlPSceGrV-y0ZffAC1eeh_Y8QGG6kAf8inqxgia4wZ1gKtb3QUw_YFLgSL", "fetches_messages": true}, "devices": [{"id": 1, "registration_id": 12579, "authentication_token": "tchm4JW6tG31su04/EpsqrdgybA=", "salt": "0a9b061d49701c0711d551fa43b1c3f7c80db4e4e37e0e0ed26caaae1da8fbd5", "number": "usergoodvery@gmail.com", "created": 93825006651152, "lastseen": 1670666721774, "user_agent": "*", "is_returning": true, "cookie": "70dffd9a20dca4cb574c7649c024e408f7ca6a429a9e3e7ec5d63b32fce2bf27", "signed_prekey": {"keyId": 14376903, "publicKey": "BR/OIX1XN5npfeN1+0ETl3GurpzlxpOJHy0GTg5nBqpK", "signature": "RxB9c7kwDGJ0gd+EaTtw3eKzZK6jvpKgvke6NyBcA34h9gxjJeG/lwDX/Y4S9sNwTCuXREuMTEmEetAK1PAaCA"}, "identityKey": "Bci7Z3iioV0BIEZ1V48RKns/shbXSt5dVZbQxJGYmTNR", "gcm_id": "eZfOv8SWSZ6SiW_NaX4Pov:APA91bEDB9o0309DZAehRAiqBJVtAKV_SpN9WexGad698F_xOXmw9UwQEaVZdr9BZrejJZdimyCNPuvaBRXlPSceGrV-y0ZffAC1eeh_Y8QGG6kAf8inqxgia4wZ1gKtb3QUw_YFLgSL", "fetches_messages": true}]}
static char *
_UserAccountDbOpQueryProvider(intptr_t values[])
{
#define SQL_SELECT_USER_ACCOUNT 	 "SELECT ufsrvuid, uuid, uuid_serialised," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.number'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.profile_key'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.profile_commitment'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.profile_version'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.rego_status'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.authenticated_device.cookie'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.prefs_bool'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.nickname'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.avatar')),"   \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.e164number'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.rego_pin'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.geoloc_trigger'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.baseloc_zone'))," \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data_user, '$.unsolicited_contact')), "    \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.rego_lock')), "    \
                                    "JSON_UNQUOTE(JSON_EXTRACT(data, '$.identity_key')) " \
                                    "FROM accounts WHERE id = '%lu'"
  char *sql_query_str = mdsprintf(SQL_SELECT_USER_ACCOUNT, (unsigned long)values[0]);

  return sql_query_str;
#undef SQL_SELECT_USER_ACCOUNT
}
 ...
 */
static int
_UserAccountDbOpTransformer(DbOpDescriptor *dbop_descriptor)
{
#define COLUMN_UFSRVUID		((uint8_t *)((struct _h_type_blob *)result->data[0][0].t_data)->value)
#define COLUMN_UUID		((uint8_t *)((struct _h_type_blob *)result->data[0][1].t_data)->value)
#define COLUMN_UUID_LENGTH	((struct _h_type_blob *)result->data[0][1].t_data)->length
#define COLUMN_UUID_SERIALISED	((struct _h_type_text *)result->data[0][2].t_data)->value
#define COLUMN_USERNAME	((struct _h_type_blob *)result->data[0][3].t_data)->value
#define COLUMN_USERNAME_LEN (((struct _h_type_blob *)result->data[0][3].t_data)->length)
#define COLUMN_PROFILE_KEY	((struct _h_type_blob *)result->data[0][4].t_data)->value
#define COLUMN_PROFILE_KEY_LEN (((struct _h_type_blob *)result->data[0][4].t_data)->length)
#define COLUMN_PROFILE_COMMITMENT	((struct _h_type_blob *)result->data[0][5].t_data)->value
#define COLUMN_PROFILE_COMMITMENT_LEN (((struct _h_type_blob *)result->data[0][5].t_data)->length)
#define COLUMN_PROFILE_VERSION	((struct _h_type_blob *)result->data[0][6].t_data)->value
#define COLUMN_PROFILE_VERSION_LEN (((struct _h_type_blob *)result->data[0][6].t_data)->length)
#define COLUMN_REGO_STATUS	((struct _h_type_int *)result->data[0][7].t_data)->value
#define COLUMN_COOKIE	((struct _h_type_blob *)result->data[0][8].t_data)->value
#define COLUMN_COOKIE_LEN (((struct _h_type_blob *)result->data[0][8].t_data)->length)
#define COLUMN_PREFS_BOOL	((struct _h_type_int *)result->data[0][9].t_data)->value
#define COLUMN_PREFS_NICKNAME	((struct _h_type_blob *)result->data[0][10].t_data)->value
#define COLUMN_PREFS_NICKNAME_LEN (((struct _h_type_blob *)result->data[0][10].t_data)->length)
#define COLUMN_PREFS_AVATAR	((struct _h_type_blob *)result->data[0][11].t_data)->value
#define COLUMN_PREFS_AVATAR_LEN (((struct _h_type_blob *)result->data[0][11].t_data)->length)
#define COLUMN_PREFS_E164NUMBER	((struct _h_type_blob *)result->data[0][12].t_data)->value
#define COLUMN_PREFS_E164NUMBER_LEN (((struct _h_type_blob *)result->data[0][12].t_data)->length)
#define COLUMN_PREFS_REGO_PIN	((struct _h_type_int *)result->data[0][13].t_data)->value
#define COLUMN_IDENTITY_KEY	((struct _h_type_blob *)result->data[0][14].t_data)->value
#define COLUMN_IDENTITY_KEY_LEN (((struct _h_type_blob *)result->data[0][14].t_data)->length)
#define COLUMN_PREFS_GEOLOC_TRIGGER	((struct _h_type_int *)result->data[0][14].t_data)->value
#define COLUMN_PREFS_BASELOC_ZONE	((struct _h_type_int *)result->data[0][15].t_data)->value
#define COLUMN_PREFS_UNSOLICITED_CONTACTS	((struct _h_type_blob *)result->data[0][16].t_data)->value
#define COLUMN_PREFS_UNSOLICITED_CONTACTS_LEN (((struct _h_type_blob *)result->data[0][16].t_data)->length)
#define COLUMN_REGO_LOCK	((struct _h_type_int *)result->data[0][17].t_data)->value


  struct _h_result *result = &dbop_descriptor->result;
  Session *sesn_ptr_loaded = SessionOffInstanceHolder(dbop_descriptor->ctx_data);

  GetAccountAttributeForProfileKeyRawMode(COLUMN_PROFILE_KEY, COLUMN_PROFILE_KEY_LEN, SESSION_USER_PROFILE_KEY(sesn_ptr_loaded));
  memcpy(&SESSION_UFSRVUIDSTORE(sesn_ptr_loaded), COLUMN_UFSRVUID, CONFIG_MAX_UFSRV_ID_SZ);
  GenerateUserPrefsBooleanFromStorage(sesn_ptr_loaded, COLUMN_PREFS_BOOL);
  SESSION_USERNICKNAME(sesn_ptr_loaded) = strbufdup(COLUMN_PREFS_NICKNAME, COLUMN_PREFS_NICKNAME_LEN);
  SESSION_USERNAME(sesn_ptr_loaded) = strbufdup(COLUMN_USERNAME, COLUMN_USERNAME_LEN);
  SESSION_USERE164NUMBER(sesn_ptr_loaded) = strbufdup(COLUMN_PREFS_E164NUMBER, COLUMN_PREFS_E164NUMBER_LEN);
  SESSION_USERAVATAR(sesn_ptr_loaded) = strbufdup(COLUMN_PREFS_AVATAR, COLUMN_PREFS_AVATAR_LEN);
...
#undef COLUMN_USERNAME_LEN
#undef COLUMN_PROFILE_KEY
#undef COLUMN_PROFILE_COMMITMENT
#undef COLUMN_PROFILE_VERSION
#undef COLUMN_REGO_STATUS
#undef COLUMN_COOKIE
#undef COLUMN_PREFS_BOOL
#undef COLUMN_PREFS_NICKNAME
#undef COLUMN_PREFS_AVATAR
#undef COLUMN_PREFS_E164NUMBER
#undef COLUMN_PREFS_REGO_PIN
#undef COLUMN_IDENTITY_KEY
#undef COLUMN_PREFS_GEOLOC_TRIGGER
#undef COLUMN_PREFS_BASELOC_ZONE
#undef COLUMN_PREFS_UNSOLICITED_CONTACTS
#undef COLUMN_PREFS_REGO_LOCK
}

/**
 * @brief Retrieve stored account information and load into session. No local caching.
 * @param userid sequence id part of of the UfsrvUid identifier
 * @param db_descriptor_in pre-allocated descriptor
 * @return instantiated session loaded with account data.
 */
InstanceHolderForSession *
DbBackendGetAccountForSessionByUserId(unsigned long userid, DbOpDescriptor *db_descriptor)
{
  InstanceHolderForSession *instance_sesn_ptr = NULL;
  if (IS_EMPTY(db_descriptor->ctx_data)) {
    instance_sesn_ptr = RecyclerGet(SessionPoolTypeNumber(), NULL, CALL_FLAG_SNAPSHOT_INSTANCE);
    if (unlikely(IS_EMPTY(instance_sesn_ptr))) {
      syslog(LOG_DEBUG, LOGSTR_INCONSISTENT_STATE, __func__, pthread_self(), NULL, 0UL, LOGCODE_PROTO_INCONSISTENT_STATE, "Could not get Session *");

      return NULL;
    }
    db_descriptor->ctx_data = CLIENT_CTX_DATA(instance_sesn_ptr);
  } else {
    instance_sesn_ptr = (InstanceHolderForSession *)db_descriptor->ctx_data;
  }


  db_descriptor->transformer.transform = _UserAccountDbOpTransformer;
//  db_descriptor->finaliser.finalise    = GetDefaultDbOpResultFinaliser();//don't set it, otherwise it will be automatically called after transformation
  ...
  db_descriptor->query_statement_provider.provide = _UserAccountDbOpQueryProvider;
  db_descriptor->query_statement_provider.values  = (intptr_t[]){DBOP_QUERY_PROVIDER_VALUE(userid), 0};
  db_descriptor->query_statement_provider.finalise = GetDefaultQueryStatementProviderFinalser();

  _DbBackendGetAccount(db_descriptor);
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
...
{
  struct json_object *jobj_account = DbGetAccountInJson(uid_ptr);
	if (!jobj_account) {
		syslog(LOG_DEBUG, "%s {o:'%p', cid:'%lu'}: ERROR: COULD NOT GENERATE JSON OBJECT FOR ACCOUNT:'%lu'", __func__, sesn_ptr, SESSION_ID(sesn_ptr), UfsrvUidGetSequenceId(uid_ptr));
    ...
    //todo 10/2/25 devops: delete all fences created by user
...

	{
		//fetch device id and update there as well
		struct json_object *jobj_devices_array = json__get(jobj_account, "devices");
		size_t array_size = json_object_array_length(jobj_devices_array);

		syslog(LOG_DEBUG, "%s (o:'%p', cid:'%lu'): DEVICES ARRAY CONTAIN:'%lu' DEVICES IN IT", __func__, sesn_ptr, SESSION_ID(sesn_ptr), array_size);
		...
		for (size_t i=0; i<array_size; i++) {
			...
			//if (device_id==json_object_get_int(json__get(jobj_device, "id")))//not applicable
			{
				...
				_DbUpdateAccountInJson(sesn_ptr, UfsrvUidGetSequenceId(uid_ptr), jobj_account);////todo 10/3/25 devops: This is broken as it overwrites whole node. need to use JSON_MERGE_PATCH
 ...
 *     In this case, you have to to invoke the finaliser yourself, otherwise you're are leaking memory.
 * @param username
 * @param dbop_descriptor
 * @return
 */
...
UFSRVResult *
DbAccountGetUfsrvUid(Session *sesn_ptr, const char *username, UfsrvUid *uid_ptr_out)
{
#define SQL_SELECT_UFSRVUID 	 "SELECT ufsrvuid FROM accounts WHERE number = '%s'"
#define COLUMN_UFSVUID		((uint8_t *)((struct _h_type_blob *)result.data[0][0].t_data)->value)

  struct _h_result result;
  char *sql_query_str = mdsprintf(SQL_SELECT_UFSRVUID, username);
  ...
  memcpy(uid_ptr_out->data, COLUMN_UFSVUID, CONFIG_MAX_UFSRV_ID_SZ);
...
/**
 * @brief Get identity key
 * @param uid_ptr UfsrvUid under which identity key is stored
 * @param device_id currently defaults to 0 (authenticated device)
 * @return Result carrier with
 * @dynamic_memory EXPORTS char * in UFSRVResult.result_user_data
 */
UFSRVResult *
DbAccountIdentityKeyGet(const UfsrvUidSequenceIdPair *uid_pair_ptr, int device_id)
{
	_DbAccountDataDeviceAttributeGetText(uid_pair_ptr->uid, device_id, ACCOUNT_JSONATTR_IDENTITY_KEY2);
	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
		{
			//TODO: update cache
		}

		return THREAD_CONTEXT_UFSRV_RESULT(ufsrv_thread_context);//success
	}

  return THREAD_CONTEXT_UFSRV_RESULT(ufsrv_thread_context);//contains error

}

/**
 * @brief Fetches the signed prekey record (json structure) containing signed pre key attributes. The attributes are returned verbatim
 ...
 */
UFSRVResult *
DbAccountSignedPreKeyGet(const UfsrvUid *uid_ptr, int device_id)
{
	//TODO: we should separate this out and perform proper cache update
	_DbAccountDataDeviceAttributeGetText(UfsrvUidGetSequenceId(uid_ptr), 0, ACCOUNT_JSONATTR_SIGNED_PREKY);
	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
...
/**
 * 	@brief returns the structure associated with user's signed key on a particular device
...
{
	UFSRVResult *res_ptr = DbAccountSignedPreKeyGet(uid_ptr, device_id);
	if (_RESULT_TYPE_SUCCESS(res_ptr)) {
...
AccountSignedPreKeyGet(Session *sesn_ptr, const UfsrvUid *uid_ptr, int device_id)
{
	...
	if (IS_PRESENT(jobj_prekey)) {
		...
		skey_record_ptr->key.public_key = strdup(json_object_get_string(json__get(jobj_prekey, ACCOUNT_JSONATTR_PUBLIC_KEY)));
  ...
  UfsrvUidConvertSerialise(uid_ptr, ufsrvuid_encoded);
	...
	for (i=0; i<jobj_array_len; i++) {
		...
		if (IS_PRESENT(jobj_entry)) {
			...
			public_key = json_object_get_string(json__get(jobj_entry, ACCOUNT_JSONATTR_PUBLIC_KEY));
			if (key_id > 0 && IS_STR_LOADED(public_key)) {
				DbSetKeys(sesn_ptr, ufsrvuid_encoded, device_id, key_id, public_key, 0);
			} else {
				syslog(LOG_DEBUG, LOGSTR_ACCOUNT_PREKEY_ERROR, __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), LOGCODE_ACCOUNT_PREKEY_ERROR, "PreKey entity was missing");
			}
		}
	}

	//signedPreKey
	struct json_object *jobj_signed_prekey = json__get(jobj, ACCOUNT_JSONATTR_SIGNED_PRE_KEY);
	...
	//IdentityKey
	const char *jobj_str_idkey = json_object_get_string(json__get(jobj, ACCOUNT_JSONATTR_IDENTITY_KEY2));
	if (jobj_str_idkey) {
		UFSRVResult *res_ptr = _DbAccountAttributeSetText(sesn_ptr, &(SESSION_UFSRVUIDSTORE(sesn_ptr)), -1/*device_id*/, ACCOUNT_JSONATTR_IDENTITY_KEY2, jobj_str_idkey);
		if (_RESULT_TYPE_SUCCESS(res_ptr)) {
			//success
		} else {
...
#define SQL_INSERT_KEY "INSERT INTO ufsrv.keys (number, device_id, key_id, public_key, last_resort) VALUES ('%s', '%lu', '%lu', '%s', '%d')"
  ...
  char *sql_query_str = mdsprintf(SQL_INSERT_KEY, ufsrvuid, device_id, key_id, public_key, last_resort_key_flag);
...
//#define QUERY_RESULT_REGO_ID(x)	((struct _h_type_int *)result.data[x][6].t_data)->value
	...
	account_key_ptr->public_key = strbufdup(QUERY_RESULT_KEY_PUBKEY(0), QUERY_RESULT_KEY_PUBKEY_LEN(0));
...
#define SQL_DELETE_KEY "DELETE FROM ufsrv.keys WHERE key_id = '%d'"
  ...
  char *sql_query_str = mdsprintf(SQL_DELETE_KEY, key_id);
...
#define SQL_DELETE_DEVICE_KEYS "DELETE FROM ufsrv.keys WHERE number = '%s' AND device_id = '%d'"
  ...
  char *sql_query_str = mdsprintf(SQL_DELETE_DEVICE_KEYS, ufsrvuid, device_id);
...
#define QUERY_RESULT_COUNT	((struct _h_type_int *)result.data[0][0].t_data)->value
  ...
  char *sql_query_str = mdsprintf(SQL_SELECT_KEY_COUNT, ufsrvuid, device_id);
...
AccountKeyRecordDestruct(AccountKeyRecord *account_key_ptr, bool self_destruct)
{
	...
	if (self_destruct)	{
    free(account_key_ptr);
    account_key_ptr = NULL;
  }
...
AccountKeyRecordSignedDestruct(AccountKeyRecordSigned *saccount_key_ptr, bool self_destruct)
{
	...
	if (self_destruct)	{
    free (saccount_key_ptr);
    saccount_key_ptr = NULL;
  }
...
#define COLUMN_NICKNAME(x)		((struct _h_type_text *)result.data[0][5].t_data)->value
  ...
  char	*sql_query_str = mdsprintf(SQL_GET_SIGNON_COOKIE, cookie);
	...
	if (result.nb_rows >= 1) {
		if ((strncasecmp("false", COLUMN_FETCHESMSG(result), 1)) == 0) {
			syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu', fetches_msg:'%c'}: WARNING: COOKIE EXITS BUT ACCOUNT DISABLED FOR USERNAME: '%s'. device_id:'%li'", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), *COLUMN_FETCHESMSG(result), COLUMN_USERNAME(result), COLUMN_DEVICEID(result));
...
#ifdef __UF_TESTING
    syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu'}: SUCCESS: UFSRVUID:'%lu', COOKIE VALID FOR USERNAME: '%s', NICKNAME:'%s'. device_id:'%li'", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), UfsrvUidGetSequenceId(&acct_ptr->ufsrvuid), COLUMN_USERNAME(result), COLUMN_NICKNAME(result), COLUMN_DEVICEID(result));
#endif
 ...
 *  @param authacct_ptr[INOUT] preallocated to hold account information.
 ...
 *
 *	@returns RESULT_TYPE_SUCCESS, RECODE_USER_AUTHENTICATION: User authenticated with existing cookie. AuthenticatedAccount * returned
 *	@returns RESULT_TYPE_SUCCESS, RESCODE_USER_SIGNON: User authenticates and signed on. New cookie generated
...
#define SQL_QUERY_UUID_SERIALISED (((struct _h_type_text *)result.data[0][8].t_data)->value)
  ...
  char *sql_query_str = mdsprintf(SQL_SELECT_AUTHENTICATION_CREDS, userid);
  ...
  if (CALLGFLAG_IS_SET(call_flags, CALL_FLAG_USER_SIGNON)) {
    ...
    authacct_ptr->username  = strbufdup((char *)SQL_QUERY_USERNAME, SQL_QUERY_USERNAME_SZ );//username;
    ...
    if (IS_PRESENT(SQL_QUERY_E164NUMBER_DATA) && IS_PRESENT(SQL_QUERY_E164NUMBER)) {
      authacct_ptr->e164number = strbufdup((char *)SQL_QUERY_E164NUMBER, SQL_QUERY_E164NUMBER_SZ);
    }

    if (IS_PRESENT(SQL_QUERY_UUID)) {
      memcpy(authacct_ptr->uuid.raw.by_value, SQL_QUERY_UUID, UUID_SIZE);
    }
    ...
    if (SQL_QUERY_COOKIE) {
      cookie_stored = strbufdup((char *)SQL_QUERY_COOKIE, SQL_QUERY_COOKIE_SZ);
    ...
    if (IS_PRESENT(cookie_stored)) {
      ...
      authacct_ptr->password  = password;
      authacct_ptr->username  = strbufdup((char *)SQL_QUERY_USERNAME, SQL_QUERY_USERNAME_SZ);
      if ((SQL_QUERY_UFSRVUID)) {
        memcpy(authacct_ptr->ufsrvuid.data, SQL_QUERY_UFSRVUID_VALUE, CONFIG_MAX_UFSRV_ID_SZ);
      }
      ...
      h_clean_result(&result);
      _RETURN_RESULT_SESN(sesn_ptr, authacct_ptr, RESULT_TYPE_SUCCESS, RECODE_USER_AUTHENTICATION);//this indicates existing cookie
    } else {
      h_clean_result(&result);
	...
	_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RECODE_USER_AUTHENTICATION);
...
#define SQL_QUERY_E164NUMBER_SZ   (((struct _h_type_blob *)result.data[0][2].t_data)->length)
  ...
  char *sql_query_str = mdsprintf(SQL_SELECT_AUTHENTICATION_CREDS, username);
...
#define SQL_UPDATE_ACCOUNT_DATA "UPDATE accounts SET data='%s' WHERE id = '%lu'"
  ...
  char *sql_query_str           = mdsprintf(SQL_UPDATE_ACCOUNT_DATA, account_json_str, userid);
...
/**
 * 	@brief Look up the rego_status property to determine if user has  1)registered and 2)active account etc..
 */
enum AccountRegoStatus
GetAccountRegistrationStatus(const char *username)
{
  DbAccountDataAttributeGetTextByUsername(username, ACCOUNT_JSONATTR_REGO_STATUS);
	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
		enum AccountRegoStatus rego_status = atoi((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
		free(THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
		return rego_status;
	}
...
/**
 * 	@brief Look up the registration lock.
 *
 * 	This works early in the registration flow when the userid is still not known, hence look up using registration username.
 * 	@param username registration id (email)
 * 	@dynamic_memory EXPORTS char *
 */
char *
GetAccountRegistrationLock(const char *username)
{
  DbAccountDataAttributeGetTextByUsername(username, "kbs.rego_lock");
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
   return ((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
  }
...
/**
 * @brief Check if registration lock is set for a user account.
 * @param username identifying registration id (currently email)
 * @param on_rego_set optional closure to run if rego lock is set to further contextualise validity test as relevant for user
 * @return true is rego lock is set or whatever validity logic enclosed by user (only if lock is already set)
 */
bool
IsAccountRegistrationLockSet(const char *username, void (^on_rego_set)(const char *))
{
  bool is_rego_lock_set = false;
  char *rego_lock = GetAccountRegistrationLock(username);
  if (IS_STR_LOADED(rego_lock)) {
    is_rego_lock_set = true;
    if (IS_PRESENT(on_rego_set)) {
      on_rego_set(rego_lock);
    }
    free(rego_lock);
  }

  return is_rego_lock_set;
}

/**
 * @brief retrieve and match account's registration lock
 * @param username[in] registration identifier
 * @param rego_lock_provided[in] account holder provided rego lock to be matched
 * @param registration_lock_state_ptr_provided[inout]
 * @dynamic_memory EXPORTS 'RegistrationLockState *' if none was provided by user to hold result
 * @return
 */
RegistrationLockState *
MatchRegistrationLockIfSet(const char *username, const char *rego_lock_by_user, RegistrationLockState *registration_lock_state_ptr_provided)
{
  RegistrationLockState *registration_lock_state_ptr;

  if (IS_EMPTY(registration_lock_state_ptr_provided)) {
    registration_lock_state_ptr = calloc(1, sizeof(RegistrationLockState));
  } else {
    registration_lock_state_ptr = registration_lock_state_ptr_provided;
  }

  bool is_rego_lock_set   = false;
  int not_identical_state = 1;

  char *rego_lock_stored = GetAccountRegistrationLock(username);
  if (IS_STR_LOADED(rego_lock_stored)) {
    is_rego_lock_set = true;
    if (IS_STR_LOADED(rego_lock_by_user)) {
      not_identical_state = strncasecmp(rego_lock_by_user, rego_lock_stored, MEDIUMBUF);
    }
    free(rego_lock_stored);
  }

  registration_lock_state_ptr->is_set = is_rego_lock_set;
  registration_lock_state_ptr->is_identical = not_identical_state? false : true;

  return registration_lock_state_ptr;
}
 ...
 */
int
UpdateAccountRegistrationLock(unsigned long userid, const char *rego_lock)
{
  return DbAccountUpdateDataByText("kbs.rego_lock", rego_lock, userid);
 ...
 */
char *
GetAccountRegistrationPinHash(unsigned long userid)
{
  DbAccountDataUserAttributeGetText(userid, "kbs.argon2_hash");
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    return ((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
  }
...
GetAccountRegistrationPinHashByUsername(const char *username)
{
  DbAccountDataAttributeGetTextByUsername(username, "kbs.argon2_hash");
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    return ((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
  }
...
bool
IsKbsPinHashValid(const char *pin_hash)
{
  if (strlen(pin_hash) > MEDIUMBUF) return false;

  return true;
}

bool
IsKbsMasterkeyValid(const char *masterkey)
{
  if (strlen(masterkey) > MEDIUMBUF) return false;

  return true;
}

bool
IsKbsRegoLockValid(const char *rego_lock)
{
  if (strlen(rego_lock) > MEDIUMBUF) return false;

  return true;
}

bool
IsKbsCiphertextValid(const char *ciphertext)
{
  if (strlen(ciphertext) > MEDIUMBUF) return false;

  return true;
}

/**
 * @brief Retrieve stored value for masterkey
 * @param username account identifying username (used at re-registration where ufsrvuid is not known yet)
 * @return stored value for masterkey or NULL on error
 * @dynamic_memory EXPORTS 'char *'
 */
char * __attribute__((nonnull()))
GetAccountKbsMasterKey(const char *username)
{
  DbAccountDataAttributeGetTextByUsername(username, "kbs.masterkey");
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    return ((char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA);
  }
...
/**
 * @brief For re-registyration purpose
 * @param pin_hash_to_check aragon2 hashed pin
 * @param username Since this is done at registration (where ufsrvuid is not known yet) signup username is used.
 * @return true if matching
 */
bool
IsKbsPinHashMatching(const char *pin_hash_to_check, const char *username)
{
  char *pin_hash_stored = GetAccountRegistrationPinHashByUsername(username);
  bool is_matching = IS_STR_LOADED(pin_hash_stored) && (0 == strncmp(pin_hash_stored, pin_hash_to_check, MEDIUMBUF));
  free(pin_hash_stored);
...
_AttributeValueGetterByText(json_object *jobj_data, const char *attribute_name)
{
  return (uintptr_t)json_object_get_string(json__get(jobj_data, attribute_name));
}

__unused inline static const uintptr_t
_AttributeValueGetterByBoolean(json_object *jobj_data, const char *attribute_name)
{
  return (uintptr_t)json_object_get_boolean(json__get(jobj_data, attribute_name));
}

__unused inline static const uintptr_t
_AttributeValueGetterByInt(json_object *jobj_data, const char *attribute_name)
{
...
{
  const char *cm_token = (const char *) _GetAccountDataDeviceAttributeByJsonObject(jobj_account, device_id, ACCOUNT_JSONATTR_GCM_ID, _AttributeValueGetterByText);
...
#define	SQL_QUERY_UUID_SERIALISED_VALUE ((struct _h_type_text *)result.data[0][3].t_data)->value //binary fields are designated as text.
  ...
  char *sql_query_str = mdsprintf(SQL_SELECT_USERID, username);
#ifdef __UF_TESTING
	syslog(LOG_DEBUG, "%s (): GENERATED SQL QUERY: '%s'", __func__, sql_query_str);
#endif
  ...
  if ((SQL_QUERY_UUID)) {
    memcpy(uid_descriptor_ptr->uuid.raw.by_value, SQL_QUERY_UUID_VALUE, UUID_SIZE);
  }
...
#define SQL_SELECT_USERNAME "SELECT number FROM accounts WHERE id='%lu';"
  ...
  char	*sql_query_str = mdsprintf(SQL_SELECT_USERNAME, userid);
...
{
	DbAccountDataUserAttributeGetText(userid, ACCOUNT_JSONATTR_E164NUMBER);
