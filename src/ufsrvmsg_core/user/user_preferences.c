 ...
 * 	@WARNING: String value is pointer reference. make sure sesn_msg_pref_ptr stays in scope until string value is no longer needed
 */
void
	...
	switch (pref_ptr->pref_value_type)
		{
			...
			case PREFVALUETYPE_BLOB:
				pref_ptr->value.pref_value_blob = (void *)&(sesn_msg_pref_ptr->vaues_blob); //this contains payload+length
				break;
...
HandleIntraCommandForSessionPreference(Session *sesn_ptr, SessionMessage *sesn_msg_ptr)
{
	...
	if (sesn_msg_ptr->n_prefs > 0) {
		...
		syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu', uname:'%s'}: ERROR: SessionMessage DID NOT CONTAIN VALID PREFS DEFINITION", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), SESSION_USERNAME(sesn_ptr));
		_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER);
	}
...
json_object *
JsonValueFormatForGenericInteger(Session *sesn_ptr, UserPreferenceDescriptor *preference_descriptor, json_object *jobj_out, PrefsStore prefs_store)
{
  json_object	*jobj;
  if (IS_PRESENT(jobj_out))	jobj = jobj_out;
  else											jobj = json_object_new_object();
  ...
  GetUserPreferenceInteger(sesn_ptr, preference_descriptor->pref_id, PREFSTORE_CACHED, &pref);
...
json_object *
JsonValueFormatForGenericString(Session *sesn_ptr, UserPreferenceDescriptor *preference_descriptor, json_object *jobj_out, PrefsStore prefs_store)
{
  json_object	*jobj;
  if (IS_PRESENT(jobj_out))	jobj = jobj_out;
  else											jobj = json_object_new_object();
  ...
  GetUserPreferenceString(sesn_ptr, preference_descriptor->pref_id, prefs_store, &pref);
  if (IS_PRESENT(pref.value.pref_value_str)) {
    return PreferenceStringJsonValueFormatter(&pref, jobj);
  }

  if (IS_EMPTY(jobj_out)) json_object_put(jobj);
...
JsonFormatUserPreferences(Session *sesn_ptr, enum PrefType pref_type, enum DigestMode digest_mode, json_object *jobj_out)
{
  ...
  for (size_t i = 0; i<pref_rego->master_pref_size; i++) {
    ...
    if (likely(IS_PRESENT(pref_descriptor->pref_validate) && IS_PRESENT((pref_descriptor->pref_value_formatter)))) {
    	...
    	if ((jobj_pref = (*pref_descriptor->pref_value_formatter)(CLIENT_CTX_DATA(sesn_ptr), (UserPreferenceDescriptor *)pref_descriptor, NULL, PREFSTORE_DEFAULT))) {
