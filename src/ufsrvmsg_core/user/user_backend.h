...
#include <ufsrvmsg_core/user/registration_lock_state_type.h>
...
enum AccountUpgradeStatus {
    UPGRADE_STATUS_UPGRADED,
    UPGRADE_STATUS_REGO_LOCK_SET,
    UPGRADE_STATUS_REGO_LOCK_MISMATCH,
    UPGRADE_STATUS_REGO_CODE_ERROR
};
...
UFSRVResult *DbAccountDataAttributeGetTextByUsername(const char *username, const char *attribute_name);
...
enum AccountRegoStatus GetAccountRegistrationStatus(const char *username);
char *GetAccountRegistrationLock(const char *username);
bool IsAccountRegistrationLockSet(const char *username, void (^on_rego_set)(const char *));
RegistrationLockState *MatchRegistrationLockIfSet(const char *username, const char *rego_lock_by_user, RegistrationLockState *registration_lock_state_ptr_provided);
int UpdateAccountRegistrationLock(unsigned long userid, const char *rego_lock);
InstanceHolderForSession *DbBackendGetAccountForSessionByUserId(unsigned long userid, DbOpDescriptor *db_descriptor_in);
char *GetAccountRegistrationPinHash(unsigned long userid);
char * __attribute__((nonnull())) GetAccountRegistrationPinHashByUsername(const char *username);
bool IsKbsPinHashValid(const char *pin_hash);
bool IsKbsMasterkeyValid(const char *masterkey);
bool IsKbsRegoLockValid(const char *rego_lock);
bool IsKbsCiphertextValid(const char *ciphertext);
bool IsKbsPinHashMatching(const char *pin_hash_to_check, const char *username);
char *GetAccountKbsMasterKey(const char *username);
...
int DbDeletePendingAccount(const char *username);
int DbDeletePendingAccountByUsername(const char *username);
int DbSetPendingAccountRegoStatus(const PendingAccount *);
...
UFSRVResult *DbGetPendingAccountVerificationCode(PendingAccount *pending_acct_ptr);
bool __attribute__((nonnull())) IsPendingCookieValid(const char *cookie);

UFSRVResult *DbAccountGetUfsrvUid(Session *sesn_ptr, const char *username, UfsrvUid *uid_ptr_out);
...
enum AccountUpgradeStatus UpgradePendingAccountIfPossible(PendingAccount *pending_account_ptr, int verification_code_provided, const char *rego_lock_provided, void (^on_upgradeable)(const char *));
bool PendingAccountVerifyByRegistrationIdSource(PendingAccount *pacct_ptr, int verification_code_provided);
UFSRVResult *CachebackendGetPendingAccountCookie(const char *cookie, bool is_return_code);
UFSRVResult *CachebackendDelPendingAccountCookie(const char *cookie);
...
UFSRVResult *DbAccountIdentityKeyGet(const UfsrvUidSequenceIdPair *uid_pair_ptr, int device_id);
UFSRVResult *DbAccountSignedPreKeySet(Session *sesn_ptr, const UfsrvUid *, int device_id, struct json_object *);
UFSRVResult *DbAccountSignedPreKeyGet(const UfsrvUid *, int device_id);
