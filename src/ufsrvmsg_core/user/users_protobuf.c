...
{
	if (IS_STR_LOADED(user_rec_ptr->username)) user_rec_ptr->username	=	flag_ref_mode ? (SESSION_USERNAME(sesn_ptr)) : (strdup(SESSION_USERNAME(sesn_ptr)));

  ProvideUfsrvUidInProto((UfsrvUid *) &SESSION_UFSRVUIDSTORE(sesn_ptr), &(user_rec_ptr->ufsrvuid), true);
 ...
 * @dynamic_memory: ALLOCATES 'unsigned char *' into UserRecord when 'flag_by_ref' is set true in calling ProvideUfsrvUidInProto()
 */
inline static void
...
{
	if (IS_PRESENT(GetUfsrvUid(ufsrvuid_request_ptr->uid, &(ufsrvuid_request_ptr->ufsrvuid), false, NULL))) {
    ProvideUfsrvUidInProto(&(ufsrvuid_request_ptr->ufsrvuid), &(user_rec_ptr->ufsrvuid), false);
  }
	user_rec_ptr->username = "*"; //this is not needed

}

UserRecord *
ProvideUserRecordFromUseridInProto(Session *sesn_ptr_carrier, unsigned long userid, UserRecord *user_record_supplied)
{
 ...
 */
UserRecord *
ProvideUserRecordFromUsernameInProto(Session *sesn_ptr, const char *username, UserRecord *user_record_supplied, bool flag_digest_mode)
{
 ...
 */
UserRecord *
ProvideUserRecordFromSessionInProto(Session *sesn_ptr_source, UserRecord *user_record_supplied, bool flag_digest_mode, bool flag_ref_mode)
{
 ...
 */
UserRecord *
ProvideUserRecordForSelfInProto(const Session *sesn_ptr, bool flag_digest_mode)
{
...
ProvideSessionMessageUserPreferenceInProto(Session *sesn_ptr, CollectionDescriptor *pref_collection_ptr, CollectionDescriptor *collection_ptr_out)
{
	...
	for (size_t i=0; i<pref_collection_ptr->collection_sz; i++) {
		...
		msgpref_ptr->type     = (enum _PreferenceType)pref_descriptor_ptr->pref_value_type;
 ...
 * @param is_by_ref if set reassign uid buffer by reference
 * @dynamic_memory: EXPORTS
 */
void
ProvideUfsrvUidInProto(UfsrvUid *uid_ptr, ProtobufCBinaryData *proto_ptr, bool is_by_ref)
{
 ...
 *
 *
 * @dynamic_memory: EXPORTS 'char *' IdentityCertificate.sendere164
 * @dynamic_memory: EXPORTS 'unsigned char *': IdentityCertificate.identitykey.data
 */
UFSRVResult *
ProvideIdentityCertificate(Session *sesn_ptr, IdentityCertificate *cert_identity_ptr, ServerCertificate *cert_server_ptr)
{
  UFSRVResult  *res_ptr = DbAccountIdentityKeyGet(&(UfsrvUidSequenceIdPair){.ufsrvuid=&(SESSION_UFSRVUIDSTORE(sesn_ptr)), .uid=UfsrvUidGetSequenceId(&(SESSION_UFSRVUIDSTORE(sesn_ptr)))}, 0);
  if (unlikely(IS_EMPTY(_RESULT_USERDATA(res_ptr)))) {
    goto error_identity_key;
  }

  char *ufsrvuid_encoded = NULL;
  UfsrvUidConvertSerialise(&SESSION_UFSRVUIDSTORE(sesn_ptr), ufsrvuid_encoded);
  const unsigned char *identity_key = _RESULT_USERDATA(res_ptr);
  ...
  cert_identity_ptr->sendere164           = ufsrvuid_encoded;
 ...
 */
UFSRVResult *
ProvideSenderCertificate(Session *sesn_ptr, SenderCertificateContext *cert_ctx_ptr)
{
  //Always send MASTER_CONF_SERVER_PUBLICKEY_SERIALISED because it's prefixed with byte[0] containing DJB type value 0x05
  if (GetSignedServerCertificate(&(MASTER_CONF_SERVER_PRIVATEKEY), &(MASTER_CONF_SERVER_PUBLICKEY_SERIALISED), cert_ctx_ptr->cert_server_ptr, cert_ctx_ptr->cert_key_ptr, SERVER_KEYID)) {
    SESSION_RETURN_RESULT(sesn_ptr, NULL, RESULT_TYPE_ERR, RECODE_NONE)
  }

  ProvideIdentityCertificate(sesn_ptr, cert_ctx_ptr->cert_identity_ptr, cert_ctx_ptr->cert_server_ptr);
  ...
  error_key_cert:
  free(cert_ctx_ptr->cert_identity_ptr->sendere164);
  free(cert_ctx_ptr->cert_identity_ptr->identitykey.data);
  free(certificate_packed);
  SESSION_RETURN_RESULT(sesn_ptr, NULL, RESULT_TYPE_ERR, RECODE_NONE)
}
