 ...
 */
ProfileKeyStore *
DbBackendGetProfileKey(const UfsrvUid *uid_ptr, enum ProfileKeyFormattingCode key_formatting_code, ProfileKeyStore *key_store_out)
{
  DbAccountDataAttributeGetText(UfsrvUidGetSequenceId(uid_ptr), ACCOUNT_JSONATTR_PROFILE_KEY);
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    char *profile_key_encoded = (char *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA;
...
/**
 * @brief Set the profile key in the DB
 * @param sesn_ptr User session for which key is being set
 * @param profile_key_encoded profile key in b64 encoded format.
 * @return
 */
UFSRVResult *
DbBackendSetProfileKey(Session *sesn_ptr, const char *profile_key_encoded)
{
  if (likely(strlen(profile_key_encoded) <= CONFIG_USER_PROFILEKEY_MAX_SIZE_ENCODED)) {
    if ((DbAccountUpdateDataByText(ACCOUNT_JSONATTR_PROFILE_KEY, profile_key_encoded, SESSION_USERID(sesn_ptr))) == 0) {
...
/**
 * Extra logic to check if stored profile key matches provided one.
 * If stored value is not set, provided value will be stored in its place.
 * If stored value is set and doesn't match, stored value is returned, unless 'is_force_update' is true, in which case provided value will be
 * stored in its place.
 * @param sesn_ptr
 * @param profile_key_encoded Provided profile key value to be checked against stored one. Must be CONFIG_USER_ACCESS_TOKEN_MAX_SIZE
...
DbBackendSetProfileKeyIfNecessary(Session *sesn_ptr, const char *profile_key_encoded, bool is_force_update)
{
  ...
  DbBackendGetProfileKey(&(SESSION_UFSRVUIDSTORE(sesn_ptr)), KEY_B64_SERIALISED, &key_store);
  ...
  _RETURN_RESULT_SESN(sesn_ptr, strbufdup(key_store.serialised, CONFIG_USER_PROFILEKEY_MAX_SIZE_ENCODED), RESULT_TYPE_ERR, RESCODE_BACKEND_DATA_EXISTINGSET);
...
/**
 * @brief Decode and copy profile key into provided raw store.
 * @param profile_key_encoded[in] profile key b64 encoded
 * @param profile_key_encoded_sz[in] str length
 * @param profile_key_raw[inout] pre-allocated raw storage
 * @return raw profile key
 */
unsigned char * __attribute__((nonnull(1, 3)))
GetAccountAttributeForProfileKeyRawMode(const char *profile_key_encoded, size_t profile_key_encoded_sz, unsigned char *profile_key_raw)
{
  ProfileKeyStore *key_store = NULL;

  if (IS_STR_LOADED(profile_key_encoded)) {
    int decoded_sz = 0;
    profile_key_raw = base64_decode((const unsigned char *)profile_key_encoded, profile_key_encoded_sz, &decoded_sz);

    if (decoded_sz == CONFIG_USER_PROFILEKEY_MAX_SIZE) return profile_key_raw;

    syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', profile_key:'%s', decoded_sz:'%d'): ERROR: ERROR DECODING KEY", __func__, pthread_self(), THREAD_CONTEXT_PTR, profile_key_encoded, decoded_sz);
    return NULL;
  }

  syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', encoded_profile:'%s'): ERROR: ENCODED KEY WAS NOT SET", __func__, pthread_self(), THREAD_CONTEXT_PTR, profile_key_encoded);
  return NULL;
}

/**
 * @brief Load stored user preferences into provided session.
 * @param jobj_account[in] Raw json object containing account information.
 * @param session_out[inout] Session to load attributes into
 * @return
 */
UFSRVResult * __attribute__((nonnull(1, 2)))
GetAccountAttributes(json_object *jobj_account,  Session *session_out)
{
  const char *stored_attribute = json_object_get_string(json__get(jobj_account, ACCOUNT_JSONATTR_NICKNAME));
  if (IS_STR_LOADED(stored_attribute)) {
    SESSION_USERNICKNAME(session_out) = strdup(stored_attribute);
  }

  stored_attribute= json_object_get_string(json__get(jobj_account, ACCOUNT_JSONATTR_NUMBER));
  if (IS_STR_LOADED(stored_attribute)) {
    SESSION_USERNAME(session_out) = strdup(stored_attribute);
  }

  stored_attribute= json_object_get_string(json__get(jobj_account, ACCOUNT_JSONATTR_E164NUMBER));
  if (IS_STR_LOADED(stored_attribute)) {
    SESSION_USERE164NUMBER(session_out) = strdup(stored_attribute);
  }

  stored_attribute= json_object_get_string(json__get(jobj_account, ACCOUNT_JSONATTR_AVATAR));
  if (IS_STR_LOADED(stored_attribute)) {
    SESSION_USERAVATAR(session_out) = strdup(stored_attribute);
  }

  _RETURN_RESULT_SESN(session_out, NULL, RESULT_TYPE_SUCCESS, RECODE_NONE)
}
 ...
 */
__unused const char * __attribute__((nonnull(1)))
...
//  db_descriptor->finaliser.finalise    = GetDefaultDbOpResultFinaliser();//don't set it, otherwise it will be automatically called after transformation
  ...
  db_descriptor->query_statement_provider.provide = _UuidDbOpQueryProvider;
  db_descriptor->query_statement_provider.values  = (intptr_t[]){DBOP_QUERY_PROVIDER_VALUE(username), 0};
...
//  db_descriptor->finaliser.finalise    = GetDefaultDbOpResultFinaliser();//don't set it, otherwise it will be automatically called after transformation
  ...
  db_descriptor->query_statement_provider.provide = _UuidDbOpQueryProviderByUserId;
  db_descriptor->query_statement_provider.values  = (intptr_t[]){DBOP_QUERY_PROVIDER_VALUE(uid), 0};
  db_descriptor->query_statement_provider.finalise = GetDefaultQueryStatementProviderFinalser();
...
{
  UserProfileAuthDescriptor *profile_descriptor = NULL;
...
static UFSRVResult *sRetrieveUserProfileAuthDescriptor(unsigned long userid, UserProfileAuthDescriptor *profile_descriptor, DbOpDescriptor *dbop_descriptor);
static int sUserProfileAuthDescriptorDpOpTransformer(DbOpDescriptor *dbop_descriptor);
static char *sUserProfileAuthProfileDescriptorQueryProvider(intptr_t values[]);
...
GetProfileKeyCredential(UfsrvUid *ufsrv_uid, ProfileCredentialRequest *profile_cred_request, char *profile_version, ProfileCredentialResponse *profile_cred_response_in)
{
  ...
  sRetrieveUserProfileAuthDescriptor(UfsrvUidGetSequenceId(ufsrv_uid), &profile_auth_descriptor, &db_descriptor);
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
    ...
    SignalFfiError  *error_code = signal_server_secret_params_issue_profile_key_credential_deterministic(&profile_cred_response->response.raw,
                                                                                                         &MASTER_CONF_SERVER_PRIVATE_PARAMS,
                                                                                                         &random_bytes,
                                                                                                         &profile_cred_request->request.raw,
                                                                                                         profile_auth_descriptor.uuid.raw.by_ref,
                                                                                                         &profile_auth_descriptor.commitment.raw);
    ...
    syslog(LOG_ERR, "%s: (th_ctx:'%p, ffi_result:'%d', commitment:'%s', uuid:'%s'): ERROR ISSUING ProfileKeyCredential...", __func__, THREAD_CONTEXT_PTR, signal_error_get_type(error_code), profile_auth_descriptor.commitment.serialised_ref, profile_auth_descriptor.uuid.serialised.by_ref);
 ...
 */
static UFSRVResult *
sRetrieveUserProfileAuthDescriptor(unsigned long userid, UserProfileAuthDescriptor *profile_descriptor, DbOpDescriptor *dbop_descriptor)
{
  dbop_descriptor->ctx_data = profile_descriptor;
  dbop_descriptor->transformer.transform = sUserProfileAuthDescriptorDpOpTransformer;

  dbop_descriptor->query_statement_provider.provide= sUserProfileAuthProfileDescriptorQueryProvider;
  dbop_descriptor->query_statement_provider.values = (intptr_t[]){DBOP_QUERY_PROVIDER_VALUE(userid), 0};
  dbop_descriptor->query_statement_provider.finalise = GetDefaultQueryStatementProviderFinalser();
...
static  char *
sUserProfileAuthProfileDescriptorQueryProvider(intptr_t values[])
{
 ...
 */
static int
sUserProfileAuthDescriptorDpOpTransformer(DbOpDescriptor *dbop_descriptor)
{
...
#define SQL_UPDATE_ACCOUNT_DATA_STRING 	 "UPDATE accounts SET data = JSON_REPLACE(data, '$.%s', '%s'),  data = JSON_REPLACE(data, '$.%s', '%s') WHERE id =' %lu'"
  ...
  char *sql_query_str = mdsprintf(SQL_UPDATE_ACCOUNT_DATA_STRING, ACCOUNT_JSONATTR_PROFILE_COMMITMENT, (*profile_descriptor->commitment.accessor.get_serialised)(profile_descriptor), ACCOUNT_JSONATTR_PROFILE_VERSION, (*profile_descriptor->version.accessor.get_serialised)(profile_descriptor), userid);
