...
//todo consolidate with ufsrvuid.c:UfsrvUidRawSystemUser()
static Session ufsrv_system_user={.session_id=1,
                                  .sservice.user.user_details.uid.data={0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
...
static inline UFSRVResult *_DbBackendBootstrapIntoBrandNewSession(Session *sesn_ptr, unsigned long userid_provided, bool);
...
static bool _SetupLocalCachingForBrandNewSession(InstanceHolderForSession *instance_sesn_ptr);

static UFSRVResult *_CacheBackendLoadUfsrvUid(unsigned long user_id, UFSRVResult *res_ptr_in);
...
/**
 * 	@brief: User must load context s necessary, but this is not thread safe. Todo: Consolidate with ufsrvuid.c:UfsrvUidRawSystemUser()
 */
 ...
 */
UfsrvUid *
GetUfsrvUid(unsigned long user_id, UfsrvUid *uid_ptr_out, bool flag_by_ref, bool *flag_is_local)
{
  UfsrvUid  *uid_ptr;
  ...
  if (likely(user_id > 0)) {
    ...
    if (IS_EMPTY((instance_sesn_ptr_local))) {
      _ALLOCATE_STORAGE_IF_NECESSARY
       ...
       if (IS_PRESENT(CacheBackendInstateUfsrvUid(user_id, CALLFLAGS_EMPTY, uid_ptr))) {
...
/**
* 	@brief: Soft suspension is the fist phase of cycling a Session out of service. Soft suspension keeps Sessions data structures intact for quick resumption
* 	but otherwise network access is completely unavailable.
...
_SessionSuspendSoft(InstanceHolderForSession *instance_sesn_ptr_target)
{
	...
	if (_PROTOCOL_CLLBACKS_RESET_SESSION(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr_target))))) {
		UFSRVResult *res_ptr = _PROTOCOL_CLLBACKS_RESET_SESSION_INVOKE(protocols_registry_ptr,
																...
																instance_sesn_ptr_target, SOFT_SUSPENSE);//soft reset
...
ResetSessionData(InstanceHolderForSession *instance_sesn_ptr_target)
{
  Session *sesn_ptr_target = SessionOffInstanceHolder(instance_sesn_ptr_target);
	...
	if (IS_PRESENT(sesn_ptr_target->dsptr)) memset(sesn_ptr_target->dsptr, 0, sizeof(Socket));
	...
	if (recycle_flag == HARD_SUSPENSE) {
		...
		if (_PROTOCOL_CLLBACKS_RESET_SESSION(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr))))) {
			UFSRVResult *res_ptr = _PROTOCOL_CLLBACKS_RESET_SESSION_INVOKE(protocols_registry_ptr,
																	...
																	instance_sesn_ptr, HARD_SUSPENSE);
...
//		memset (&sesn_ptr->ssptr->protocol_header, 0, sizeof(ProtocolHeaderWebsocket));
    ...
    memset(SESSION_UFSRVUID(sesn_ptr), 0, CONFIG_MAX_UFSRV_ID_SZ);
		...
		if (IS_PRESENT(sesn_ptr->dsptr)) {
      memset (sesn_ptr->dsptr, 0, sizeof(Socket));
		}
	}
...
/**
 * 	@brief Fetch raw session record from redis cache backend.
 * 	@worker_thread: ufsrv and session if sesn_ptr_this is provided, otherwise ufsrvworker's is used
 ...
 */
UFSRVResult *
CacheBackendGetRawSessionRecordByUserId(unsigned long user_id, unsigned long call_flags, UFSRVResult *res_ptr_in)
{
	UFSRVResult *res_ptr;
	...
	if (user_id > 0) {
		...
		redis_ptr = (*pers_ptr->send_command)(NO_SESSION, REDIS_CMD_USER_SESSION_RECORD_GET_ALL, user_id);

		if (IS_EMPTY(redis_ptr)) {
			syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p'}: ERROR COULD NOT GET REDIS RESPONSE for UID '%lu'", __func__, pthread_self(), THREAD_CONTEXT_PTR, user_id);

			_RETURN_RESULT_RES(res_ptr, NULL, RESULT_TYPE_BACKENDERR, RESCODE_BACKEND_CONNECTION)
		}

		if (redis_ptr->type == REDIS_REPLY_ERROR) {
		   syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p'}: REDIS_REPLY_ERROR COULD NOT GET REDIS RESPONSE for UID '%lu'", __func__, pthread_self(), THREAD_CONTEXT_PTR, user_id);
		...
		if (redis_ptr->type == REDIS_REPLY_NIL) {
		   syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p'}: COULD NOT RETRIEVE RECORD FOR UID '%lu'",  __func__, pthread_self(), THREAD_CONTEXT_PTR, user_id);
		...
		//this is necessary because the HMGET command will return an empty named array
	   if (!(redis_ptr->element[0]->str)) {
		   syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', uid:'%lu', err:'%s', type:'%d', elements:'%lu'}: ERROR: EMPTY SET FOR UID",  __func__, pthread_self(), THREAD_CONTEXT_PTR, user_id, redis_ptr->str, redis_ptr->type, redis_ptr->elements);
...
static UFSRVResult *
_CacheBackendLoadUfsrvUid(unsigned long user_id, UFSRVResult *res_ptr_in)
{
  UFSRVResult *res_ptr;
  ...
  if (likely(user_id > 0)) {
    ...
    redis_ptr = (*pers_ptr->send_command)(NULL, REDIS_CMD_USER_GET_UFSRVUID, user_id);
...
UfsrvUid *
CacheBackendInstateUfsrvUid(unsigned long user_id, unsigned long sesn_call_flags, UfsrvUid *uid_ptr_out)
{
  UFSRVResult res = {0};
  UfsrvUid *uid_ptr;
  ...
  UFSRVResult *res_ptr = _CacheBackendLoadUfsrvUid(user_id, &res);
...
/**
 * 	@brief: Fetch a raw session record from the cache backend using the provided cookie.
 * 	via a call to SessionGetFromBackendRaw(), which the user responsible for freeing redis reply.
 * 	Redis reply instantiated here is destroyed here as well, once the session id is extracted from it and fed into the contained function.
 *
 * 	@param sesn_ptr_this: current connected session which is servicing request
 * 	@param session_cookie: cookie provided via some incoming session
 * 	@locks: NONE
 * 	@blocks: redis write
 * 	@dynamic_memory creates redisReply * which is is freed here
 ...
 */
UFSRVResult *
CacheBackendGetRawSessionRecordByCookie(const char *session_cookie, unsigned long call_flags)
{
	UFSRVResult			*res_ptr = THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT);;
	PersistanceBackend	*pers_ptr = THREAD_CONTEXT_PERSISTANCE_CACHEBACKEND(THREAD_CONTEXT);
	redisReply			*redis_ptr;

	if (!(redis_ptr = (*pers_ptr->send_command)(NO_SESSION, REDIS_CMD_COOKIE_SESSION_GET, session_cookie))) {
	...
	if (user_id_str) {
		*user_id_str = '\0';
		...
		syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p'}: ERROR PARSING VALUE OF COOKIE:%%session_cookie: USER ID WAS NOT ENCODED: str:'%s'", __func__, pthread_self(), THREAD_CONTEXT_PTR, redis_ptr->str);
	...
	return (CacheBackendGetRawSessionRecordByUserId(userid, call_flags, NULL));
    ...
    sesn_ptr_other_user = (Session *)_RESULT_USERDATA(res_ptr_aux);
		...
		if (call_flags&CALL_FLAG_LOCK_SESSION) {
			...
			if (_RESULT_TYPE_EQUAL(THREAD_CONTEXT.res_ptr, RESULT_TYPE_SUCCESS)) {
				if (_RESULT_CODE_EQUAL(THREAD_CONTEXT.res_ptr, RESCODE_PROG_LOCKED_BY_THIS_THREAD)) res_code = RESCODE_PROG_LOCKED_BY_THIS_THREAD;
			}
			else {_RETURN_RESULT_SESN(sesn_ptr_carrier, NULL, RESULT_TYPE_ERR, RESCODE_PROG_WONTLOCK)}
		}
		...
		if ((instance_sesn_ptr_other_user = CacheBackendInstantiateRawSessionRecord(sesn_ptr_carrier, redis_ptr, call_flags | CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION, NULL)))  {//recycler instance
		  sesn_ptr_other_user = SessionOffInstanceHolder(instance_sesn_ptr_other_user);
...
//		return sesn_ptr_other_user;//session can be locked depending on flag which we passed on
		if (SESSION_RESULT_CODE(sesn_ptr_other_user) == RESCODE_PROG_LOCKED_BY_THIS_THREAD) if (IS_PRESENT(lock_state)) *lock_state = true;
...
/**
 * 	@brief Given a user id return a Session representation for them. The Session can an actual active one or just a dead data carrier
 * 	.Search is performed locally, cache backend and finally db.
 *  @return InstanceHolderForSession
 * 	@locks: sesn_ptr_other_user
 */
UFSRVResult *
GetSessionForThisUserByUserId(Session *sesn_ptr, unsigned long uid, bool *lock_state, unsigned long call_flags_sesn)
{
  ...
  //region return from locally found session
  if (IS_PRESENT(instance_sesn_ptr_other_user)) {
	  sesn_ptr_other_user = SessionOffInstanceHolder(instance_sesn_ptr_other_user);

		//bingo...
		if (call_flags_sesn & CALL_FLAG_LOCK_SESSION) {
			if (!(call_flags_sesn & CALL_FLAG_LOCK_SESSION_BLOCKING)) 	SessionLockRWCtx(THREAD_CONTEXT_PTR, sesn_ptr_other_user, _LOCK_TRY_FLAG_TRUE, __func__);
			else 																								        SessionLockRWCtx(THREAD_CONTEXT_PTR, sesn_ptr_other_user, _LOCK_TRY_FLAG_FALSE, __func__);
		...
		if (call_flags_sesn & CALL_FLAG_ATTACH_FENCE_LIST_TO_SESSION && SESNSTATUS_IS_SET(sesn_ptr_other_user->stat, SESNSTATUS_FENCELIST_LAZY)) {
  ...
  //endregion
	...
	} else if (_RESULT_CODE_EQUAL(res_ptr_aux, RESCODE_USER_BACKEND)) { //session instance found in the backend: instantiate one locally
		redisReply *redis_ptr = (redisReply *)res_ptr_aux->result_user_data;

		if ((instance_sesn_ptr_other_user = CacheBackendInstantiateRawSessionRecord(sesn_ptr, redis_ptr, call_flags_sesn | CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION, NULL))) {
		  sesn_ptr_other_user = SessionOffInstanceHolder(instance_sesn_ptr_other_user);
...
/**
 * 	@brief: wrapper function to instantiate Session from cache backend with the option to fall back to DB Backend when flagged.
 * 	USE WITH CAUTION AS IT PULLS IN AN INSTANCE FROM THE RECYCLER. MAKE SURE THE SESSION DOES NOT HAVE LOCAL REPRESENTATION
 * 	OF ANY SORT: CONNECTED OR REMOTE.
 * 	@param ufsrvuid[in] if loaded from DB backend, this value will be copied into the newly instantiated session.
 * 	@param sesn_call_flags[in] CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION: if session was not found in cache backend, rebuild from db backend
 *
...
SessionInstantiateFromCacheBackendWithDbFallback(Session *sesn_ptr_this, const UfsrvUid *ufsrvuid, unsigned long sesn_call_flags)
{
  //first: try cache backend, which relies on userid
  CacheBackendGetRawSessionRecordByUserId(UfsrvUidGetSequenceId(ufsrvuid), CALLFLAGS_EMPTY, THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT));
  if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
   ...
   InstanceHolderForSession *instance_sesn_ptr_remote = CacheBackendInstantiateRawSessionRecord(sesn_ptr_this, redis_ptr_user, sesn_call_flags | CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION, NULL);
  ...
  //fall through: session to be loaded from db backend
  if (sesn_call_flags&CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION) {
    InstanceHolderForSession *instance_sesn_ptr_fetched = RecyclerGet(SessionPoolTypeNumber(), NULL, CALL_FLAG_SNAPSHOT_INSTANCE);
    if (unlikely(IS_EMPTY(instance_sesn_ptr_fetched))) {
    ...
    Session *sesn_ptr_fetched = SessionOffInstanceHolder(instance_sesn_ptr_fetched);

    DbBackendInstantiateSession(sesn_ptr_fetched, ufsrvuid);
    if (SESSION_RESULT_TYPE_ERROR(sesn_ptr_fetched)) {
      syslog(LOG_ERR, "%s (pid:'%lu', thx_o:'%p'): ERROR: NO ACCOUNT RECORD EXITS.", __func__, pthread_self(), THREAD_CONTEXT_PTR);
      SessionReturnToRecycler(instance_sesn_ptr_fetched, NO_CONTEXT_DATA, CALLFLAGS_EMPTY);
      goto exit_return_error;
    }

    memcpy(&SESSION_UFSRVUIDSTORE(sesn_ptr_fetched), ufsrvuid, sizeof(UfsrvUid));//since DbBackendInstantiateSession does not retrieve teh full db record

    return instance_sesn_ptr_fetched;
  }
...
/**
 * @brief populate the Session object with attributes stored in the CacheBackend redisReply backend object.
 ...
 *
 * If HASHLOCALLy flag is set, InstantiateSessionObject will hash the new session for session and uid depending on flag
 *
 ...
 * 	if FLAG_TRANSFER_DBDATA is set, copy key db data across.
...
CacheBackendInstantiateRawSessionRecord(Session *sesn_ptr_this, redisReply *redis_ptr, unsigned long sesn_call_flags, InstanceHolderForSession *instance_sesn_ptr_out)
{
  ...
  //region allocate session object and lock
  ...
  //endregion
    ...
    SESSION_USERID_TEMP(sesn_ptr) = UfsrvUidGetSequenceId(&SESSION_UFSRVUIDSTORE(sesn_ptr));
		_DbBackendBootstrapIntoBrandNewSession(sesn_ptr, 0UL, true);
    ...
    SESSION_EID(sesn_ptr) = strtoul(redis_ptr->element[REDIS_KEY_USER_EVENT_COUNTER]->str, NULL, 10);
...
#if 0 //no longer support loading cachebackend for profile key
      ...
      ProfileKeyStore key_store = {0};
		  DbBackendGetProfileKey(&(SESSION_UFSRVUIDSTORE(sesn_ptr)), KEY_RAW, &key_store);
        ...
        syslog(LOG_ERR, "%s: {pid:'%lu', o:'%p'} ERROR: COULD NOT DECODE STORED PROFILE KEY: SESSION INSTANTIATED WITHOUT PROFILE KEY", __func__, pthread_self(), sesn_ptr);
		  }
		}
		...
		memcpy(SESSION_HADDRESS(sesn_ptr), redis_ptr->element[REDIS_KEY_USER_HADDRESS]->str, redis_ptr->element[REDIS_KEY_USER_HADDRESS]->len + 1 > MAXHOSTLEN ? MAXHOSTLEN : redis_ptr->element[REDIS_KEY_USER_HADDRESS]->len + 1);
      ...
      break;
		...
		if (IS_PRESENT(redis_ptr->element[REDIS_KEY_USER_BASELOC]->str) && (*redis_ptr->element[REDIS_KEY_USER_BASELOC]->str != CONFIG_DEFAULT_PREFS_CHAR_VALUE)) {
      UpdateHomebaseGeoLocDynamicAssignment(sesn_ptr, (const char *) redis_ptr->element[REDIS_KEY_USER_BASELOC]->str, CALLFLAGS_EMPTY);
		}

		/*if (IS_PRESENT(redis_ptr->element[REDIS_KEY_HOME_BASELOC]->str) && (*redis_ptr->element[REDIS_KEY_HOME_BASELOC]->str != CONFIG_DEFAULT_PREFS_CHAR_VALUE)) {
      UpdateHomebaseGeoLocUserPrefAssignment(sesn_ptr, (const char *) redis_ptr->element[REDIS_KEY_USER_BASELOC]->str, CALLFLAGS_EMPTY);
		}*/

		if (IS_PRESENT(redis_ptr->element[REDIS_KEY_LOCATION_USER]->str) && (*redis_ptr->element[REDIS_KEY_LOCATION_USER]->str != CONFIG_DEFAULT_PREFS_CHAR_VALUE)) {
      if ((ParseCacheBackendStoredLocationDescription(SESSION_ULOCATION_BYUSER_PTR(sesn_ptr), redis_ptr->element[REDIS_KEY_LOCATION_USER]->str, false)) == 0) {
		...
		if (IS_PRESENT(redis_ptr->element[REDIS_KEY_LOCATION_SERVER]->str) && (*redis_ptr->element[REDIS_KEY_LOCATION_SERVER]->str != CONFIG_DEFAULT_PREFS_CHAR_VALUE)) {
      if ((ParseCacheBackendStoredLocationDescription(SESSION_ULOCATION_BYSERVER_PTR(sesn_ptr), redis_ptr->element[REDIS_KEY_LOCATION_SERVER]->str, false)) == 0) {
		...
		if (!(SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_SNAPSHOT))) {
			UpdateSessionGeoFenceDataByFid(sesn_ptr, strtoul(redis_ptr->element[REDIS_KEY_CURRENT_GEOFENCE]->str? :"0", NULL, 10), strtoul(redis_ptr->element[REDIS_KEY_PAST_GEOFENCE]->str? :"0", NULL, 10));
		}
      ...
      SESSION_USERAVATAR(sesn_ptr) = strdup(redis_ptr->element[REDIS_KEY_USER_AVATAR]->str);
		...
		if (redis_ptr->element[REDIS_KEY_GUARDIAN_ID]->type == REDIS_REPLY_NIL) {
      GuardianRecordDescriptor guardian_descriptor = {.originator.uid = SESSION_USERID(sesn_ptr), .status=GUARDIAN_STATUS_LINKED};
      DbBackendGetGuardianRecordForOriginator(&guardian_descriptor);
      if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) SESSION_USERGUARDIAN_UID(sesn_ptr) = guardian_descriptor.guardian.uid;
    } else SESSION_USERGUARDIAN_UID(sesn_ptr) = strtoul(redis_ptr->element[REDIS_KEY_GUARDIAN_ID]->str, NULL, 10);
...
GenerateSessionIdGlobally(void)
{
	unsigned long id = 0;
	...
	if (redis_ptr->elements != _REDIS_UID_FIELDS_COUNT) {//empty set
...
{
	return ((unsigned long)labs((rand() * rand()) << 1));
...
/**
 * @brief A jump function to invoke protocol sensitve session id generation
 * @param protocol_id
 * @param res_ptr Result carrier
 * @param context_data User context data
 * @return
 */
unsigned long
GenerateSessionId(int protocol_id, UFSRVResult *res_ptr, ClientContextData *context_data)
{
  if (_PROTOCOL_CLLBACKS_GENERATE_SESSION_ID(protocols_registry_ptr, protocol_id)) {
    res_ptr = _PROTOCOL_CLLBACKS_GENERATE_SESSION_ID_INVOKE(protocols_registry_ptr, protocol_id, res_ptr, context_data);
    if (_RESULT_TYPE_SUCCESS(res_ptr)) {
      return (unsigned long) (uintptr_t) _RESULT_USERDATA(res_ptr);
    }
  }
...
type_instantiator
GetSessionInstantiator()
{
  return (type_instantiator) InstantiateSessionObject2;
}

/**
 * @brief A generic Session instantiator that is unrelated to the underlying recycler subsystem lifecycle or protocol
 * lifecycle.
 * param protocol_id: >=0 protocol id which can be used as index. If -1 no protocol is associated with this session
 */
Session *
InstantiateSessionObject(Socket *ssptr, Socket *dsptr, unsigned call_flags, int protocol_id)
{
	Session *sesn_ptr;
	...
	if (protocol_id > 0) {
	  if ((sesn_ptr->session_id = GenerateSessionId(protocol_id, &sesn_ptr->sservice.result, NULL)) == 0)	goto final_clean_up;
	}
	...
	if (rc == 0) {
//	  sesn_ptr->session_id = GetSessionId(protocol_id, &sesn_ptr->sservice.result, NULL);
    ...
    if (protocol_id >= 0) {
      ...
      //should not do this as this is not recycler managed
 ...
 */
int
InstantiateSessionObject2(Session **sesn_ptr_in, unsigned long call_flags, int protocol_id)
{
	Session *sesn_ptr = NULL;
...
_s_DestructSocketMessageQueue(Session *sesn_ptr, MessageQueue *msg_que_ptr)
{
	...
	syslog(LOG_DEBUG, "DestructSocketMessageQueue (pid:'%lu' cid:'%lu'): DESTRUCTING Queue (msg count='%lu')...", pthread_self(), sesn_ptr->session_id, msg_que_ptr->queue.nEntries);
	...
	syslog(LOG_DEBUG, "DestructSocketMessageQueue (pid:'%lu' cid:'%lu'): DESTRUCTED Queue (msg count='%lu')...", pthread_self(), sesn_ptr->session_id, msg_que_ptr->queue.nEntries);
...
DestructSocketMessage(SocketMessage *sock_msg_ptr)
{
  ...
  if (sock_msg_ptr->frame_index_multiples > 0) free(sock_msg_ptr->frame_index);
...
HandleSessionReturnHandshake(InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sock_msg_ptr, unsigned long sesn_call_flags)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	res_ptr_aux = ProcessOutgoingWsHandshake(instance_sesn_ptr, sock_msg_ptr);
...
AuthenticateForNonCookieHashedSession(InstanceHolderForSession *instance_sesn_ptr_transient)
{
	...
	if (_RESULT_TYPE_SUCCESS(res_ptr)) {
        ...
        instance_sesn_ptr_processed = (InstanceHolderForSession *)res_ptr->result_user_data; break;
			...
			case RECODE_USER_AUTHENTICATION:
		...
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_processed)) {
			_RETURN_RESULT_SESN(sesn_ptr_processed, instance_sesn_ptr_processed, RESULT_TYPE_SUCCESS, RESCODE_USER_INITIALISED);
		}
 ...
 * 	User must ensure session's is de-allocated of any previous values.
 */
void
TransferBasicSessionDbBackendData(Session *sesn_ptr, AuthenticatedAccount *authacct_ptr)
{
	...
	SESSION_USERNAME(sesn_ptr)				=	authacct_ptr->username; //@dynamic_memory: transferred reference
	if (IS_STR_LOADED(SESSION_USERNICKNAME(sesn_ptr)))	free(SESSION_USERNICKNAME(sesn_ptr));
	SESSION_USERNICKNAME(sesn_ptr)		=	authacct_ptr->nickname;	//@dynamic_memory: transferred reference
 ...
 * 	This list may expand into the future. Must ensure any hashing dependencies are resolved here, eg userid is hashed against Session instance
 * 	etc...
...
//	if (SESSION_USERID(sesn_ptr_target)!=SESSION_USERID(sesn_ptr_source)) {
  if (!UfsrvUidIsEqual(&SESSION_UFSRVUIDSTORE(sesn_ptr_target), &SESSION_UFSRVUIDSTORE(sesn_ptr_source)))  {
    ...
    if (strcmp(SESSION_USERNICKNAME(sesn_ptr_source), SESSION_USERNICKNAME(sesn_ptr_target)) != 0) {
      ...
      SESSION_USERNICKNAME(sesn_ptr_target) = SESSION_USERNICKNAME(sesn_ptr_source);  //@dynamic_memory: transfered reference
...
_DbBackendVerifyAndInitialiseSession(InstanceHolderForSession *instance_sesn_ptr_transient)
{
  ...
  Session *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient);
	...
	//let's make sure we are supposed to be talking to this fellow. We should hold a cookie of sorts if legit user
	...
	if (!_RESULT_TYPE_SUCCESS(res_ptr)) {
		...
		ProcessOutgoingWsHandshake(instance_sesn_ptr_transient, NULL);
    UfsrvCommandInvokeUserCommand(&(InstanceContextForSession) {instance_sesn_ptr_transient, sesn_ptr_transient}, NULL, NULL, NULL, NULL, uACCOUNT_VERIFIED_V1_IDX);

    _RETURN_RESULT_SESN(sesn_ptr_transient, instance_sesn_ptr_transient, RESULT_TYPE_ERR, RECODE_USER_AUTHENTICATION)
	}
...
_AuthenticateUserIdHashedSession(InstanceHolderForSession *instance_sesn_ptr_transient, InstanceHolderForSession *instance_sesn_ptr_uid_hashed, unsigned long sesn_call_flags)
  ...
  *   IMPORTANT: There is a danger multiple sessions are instantiated for the same userid, because cookie hash is the only place that uniquely identifies this user.
...
_AuthenticateDbBackendValidatedCookieSession(InstanceHolderForSession *instance_sesn_ptr_transient, redisReply *redis_ptr, unsigned long sesn_call_flags)
 {
 	...
 	//this could be hoaxed, as the server may have crashed before updating the true value
  ...
  Session *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient);
 	...
 	if ((instance_sesn_ptr_migrated = CacheBackendInstantiateRawSessionRecord(sesn_ptr_transient, redis_ptr, (CALL_FLAG_TRANSFER_DB_USERDATA | CALL_FLAG_LOAD_DB_BACKEND_FOR_SESSION), NULL))) {
...
#undef ABORT_ON_FAILURE
  ...
  * 	so we onboard them into the system and have them fully setup.
  *
...
_AuthenticateBrandNewSession(InstanceHolderForSession *instance_sesn_ptr_transient)
 {
   Session *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient);
	...
	_DbBackendBootstrapIntoBrandNewSession(sesn_ptr_transient, 0UL, true);
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_transient)) {
		if (!_SetupLocalCachingForBrandNewSession(instance_sesn_ptr_transient))	goto exit_error;
      ...
      SESNSTATUS_UNSET(SESSION_STATUS(sesn_ptr_transient), SESNSTATUS_TRANSIENT);
			...
			_RETURN_RESULT_SESN(sesn_ptr_transient, instance_sesn_ptr_transient, RESULT_TYPE_SUCCESS, RECODE_USER_AUTHENTICATION)
		}
	}
 ...
 /**
  * @brief Hash key identifying attributes locally.
  * @param instance_sesn_ptr Session containing identifying attributes
  * @return
  */
static bool
_SetupLocalCachingForBrandNewSession(InstanceHolderForSession *instance_sesn_ptr)
 {
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
...
/**
 * @param sesn_ptr_transient: Most likely this session is freshly instantiated
 * @param userid_provided if flag_auth_data_loaded is false, use provided id to load account data into transient session
 * @param flag_auth_data_loaded: session already carries fresh db backend authentication data
 *
 ...
 */
static UFSRVResult *
_DbBackendBootstrapIntoBrandNewSession(Session *sesn_ptr_transient, unsigned long userid_provided, bool flag_auth_data_loaded)
{
	const char				  *scratch;
	struct json_object	*jobj_account	= NULL;

  unsigned long userid = flag_auth_data_loaded ? SESSION_USERID(sesn_ptr_transient) : userid_provided;
	jobj_account = DbGetAccountInJsonByUserId(userid);

	if (unlikely(IS_EMPTY(jobj_account)))	_RETURN_RESULT_SESN(sesn_ptr_transient, NULL, RESULT_TYPE_ERR, RESCODE_USER_INITIALISED)

	json_object *jobj_userprefs = NULL;
	DbBackendGetUserPrefs(sesn_ptr_transient, userid);
  if (SESSION_RESULT_IS_SUCCESS_WITH_BACKEND_DATA(sesn_ptr_transient))  {
    ...
    PopulateUserPrefsFromRawJson(sesn_ptr_transient, jobj_userprefs);
  ...
  scratch = GetAccountAttributeForCloudMessaging(jobj_account, AUTHENTICATED_DEVICE);
	...
	ProfileKeyStore profilekey_store = {0};
  GetAccountAttributeForProfileKeyByStore(jobj_account, true, &profilekey_store);
  if (profilekey_store.raw_sz > 0)	{
    memcpy(SESSION_USER_PROFILE_KEY(sesn_ptr_transient), profilekey_store.raw, CONFIG_USER_PROFILEKEY_MAX_SIZE);
    free(profilekey_store.raw);
  }
          //Add more fields...

  json_object_put(jobj_account);

	_RETURN_RESULT_SESN(sesn_ptr_transient, sesn_ptr_transient, RESULT_TYPE_SUCCESS, RESCODE_USER_INITIALISED)

}

/**
 * @brief Load session with account and profile information stored in the DB Backend (as opposed to Cache Backend)
 * @param sesn_ptr_transient[inout] Pre-allocated session object to store data into
 * @param ufsrvuid
 * @return
 * @todo convert this to use: DbBackendGetAccountForSessionByUserId()
 */
UFSRVResult *
DbBackendInstantiateSession(Session *sesn_ptr_transient, const UfsrvUid *ufsrvuid)
{
  const char				  *scratch;

  unsigned long userid = UfsrvUidGetSequenceId(ufsrvuid);
  struct json_object	*jobj_account = DbGetAccountInJsonByUserId(userid);
  if (unlikely(IS_EMPTY(jobj_account)))	_RETURN_RESULT_SESN(sesn_ptr_transient, NULL, RESULT_TYPE_ERR, RESCODE_USER_INITIALISED)

  struct json_object	*jobj_datauser = DbGetAccountUserDataInJsonByUserId(userid);
  if (unlikely(IS_EMPTY(jobj_datauser)))	{
    json_object_put(jobj_account);
    _RETURN_RESULT_SESN(sesn_ptr_transient, NULL, RESULT_TYPE_ERR, RESCODE_USER_INITIALISED)
  }

  const char *profile_key = json_object_get_string(json__get(jobj_account, ACCOUNT_JSONATTR_PROFILE_KEY));
  GetAccountAttributeForProfileKeyRawMode(profile_key, strlen(profile_key),SESSION_USER_PROFILE_KEY(sesn_ptr_transient));
  ...
  GetAccountAttributes(jobj_account,  sesn_ptr_transient);////todo 9/3/25 devops: does this conflict with PopulateUserPrefsFromRawJson() above? nickname, avatar are reloaded here
  //Add more fields...

  json_object_put(jobj_account);
  json_object_put(jobj_datauser);
...
AuthenticateForBackendCookieHashedSession(InstanceHolderForSession *instance_sesn_ptr_transient, UFSRVResult *res_ptr, SocketMessage *sock_msg_ptr)
{
  ...
  //check if it's possible to locate user internally and multiplex instance_sesn_ptr_backend accordingly
#pragma region diagnostics
  if (!(IS_EMPTY(instance_sesn_ptr_sesn_hashed)) && !(IS_EMPTY(instance_sesn_ptr_ui_hashed))) {
    ...
    //cookie is related to another Session id in the backend We only know of its id, so we'll instantiate it from provided raw cache backend record
...
#pragma endregion
	...
	if (IS_EMPTY(sesn_ptr_backend)) {
		//rebuild the session since we have no in-memory data on it
    SESSION_USERID_TEMP(sesn_ptr_transient) = user_id;
		//TODO: SHOULDN'T this be handled same as new user initialisation?
		UFSRVResult *res_ptr_nonlocal = _InstateAuthenticatedNonLocalUser(instance_sesn_ptr_transient, sock_msg_ptr, redis_ptr);
		int rescode = _RESULT_CODE(res_ptr_nonlocal);

		if (_RESULT_TYPE_ERROR(res_ptr_nonlocal)) {
			InstanceHolderForSession *instance_sesn_ptr_processed = (InstanceHolderForSession *)res_ptr_nonlocal->result_user_data;
    ...
    return  res_ptr_nonlocal;
	} else {
		//refresh user session from newly fetched backend data
		UFSRVResult *res_ptr_local = _RefreshAuthenticatedLocalUser(instance_sesn_ptr_transient, instance_sesn_ptr_backend, sock_msg_ptr, redis_ptr);
    int rescode = _RESULT_CODE(res_ptr_local);

		if (_RESULT_TYPE_ERROR(res_ptr_local)) {
			InstanceHolderForSession *instance_sesn_ptr_processed = (InstanceHolderForSession *)res_ptr_local->result_user_data;
			Session *sesn_ptr_processed = SessionOffInstanceHolder(instance_sesn_ptr_processed);
			if (!IS_EMPTY(instance_sesn_ptr_processed))	{
			  SuspendSession(instance_sesn_ptr_processed, SOFT_SUSPENSE);
        if (sesn_ptr_processed == sesn_ptr_transient) {
 ...
 * 	 but for which we didn't have a local in-memory authenticated session instance.
 *
 * 	So we go ahead and instantiate a new local authenticated Session, and populate it with raw backend data supplied.
 * 	The user is then hand-shaked and allowed through.
 *
 ...
 * 	@param redis_ptr previously fetched full raw cache backend record
 *
...
_InstateAuthenticatedNonLocalUser(InstanceHolderForSession *instance_sesn_ptr_transient, SocketMessage *sock_msg_ptr, redisReply *redis_ptr)
{
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_backend)) {
		...
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_backend)) { //todo: this is confusing; clean up and user res_ptr_error
			_RETURN_RESULT_SESN(sesn_ptr_backend, instance_sesn_ptr_backend, RESULT_TYPE_SUCCESS, RESCODE_USER_INITIALISED);
		}
 ...
 *
 * 	@param instance_sesn_ptr_transient: current connected  transient session for the named user, whose _HASHED_ Session is about to be refreshed locally.
 * 	 This transient session instance will be returned to the pool and the hashed one use moving forward.
 * 	@param instance_sesn_ptr_hashed: currently instantiated, non-connected Session for the user.
 *
...
_RefreshAuthenticatedLocalUser(InstanceHolderForSession *instance_sesn_ptr_transient, InstanceHolderForSession *instance_sesn_ptr_hashed, SocketMessage *sock_msg_ptr, redisReply *redis_ptr)
{
  ...
  if (SESSION_EID(sesn_ptr_transient) != SESSION_EID(sesn_ptr_hashed)) {
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_hashed)) {
		...
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_hashed)) {
			_RETURN_RESULT_SESN(sesn_ptr_hashed, instance_sesn_ptr_hashed, RESULT_TYPE_SUCCESS, RESCODE_USER_INITIALISED)
		}
...
* 	The current connected session sesn_ptr is in transient state: oneway incoming handshaked and not fully authenticated yet.
 *
 ...
 * 	test then that points out to inconsistency. local hashed will be destroyed and a new one recreated from backend.
 *
 * 	@param	instance_sesn_ptr_transient:	connected session in transient state
 *
 * 	@param	sesn_ptr_hashed: Session belonging to the same user found under cookie
 *
 *	@locked sesn_ptr: in main loop
...
AuthenticateForCookieHashedSession(InstanceHolderForSession *instance_sesn_ptr_transient, InstanceHolderForSession *instance_sesn_ptr_hashed, SocketMessage *sock_msg_ptr)
{
  ...
  Session     *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient);
	...
	//todo: ATOMIC no lock on sesn_ptr_hashed so this read-off is technically ripe for race condition. Relying on the atomicity of 64bit read off
  ...
  //region session is locked and unresponsive
  if (_RESULT_TYPE_EQUAL(THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT), RESULT_TYPE_ERR)) {
		if (time(NULL) - SESSION_WHEN_SERVICE_STARTED(sesn_ptr_hashed) > CONFIG_SESSION_INSERVICE_MAX_TIMEOUT) {
			syslog(LOG_DEBUG, "%s (pid:'%lu',o:'%p', o_hashed:'%p', cid:'%lu'): ERROR: SESSION cid:'%lu' IN LOCKED STATE AND NOT RESPONSIVE AND MAY NOT BE ABLE TO BE RECOVERED: SESSION WILL BE FORCIBLY DESTROYED AND RECREATED",
			...
			//TODO: Should this be picked by the periodic CheckSessionIdle? the problem we can re-initialise client if it has trace in the system we must clean up first
  ...
  //endregion
	...
	if (sesn_ptr_hashed->stat&SESNSTATUS_CONNECTED) {
		...
		//haproxy  may still hang on to the connection, even though the client is coming with a fresh connection. Perhaps safer to terminate the connected
		if ((IsSocketAlive(SESSION_SOCKETFD(sesn_ptr_hashed))) == 0) {
#if __UF_TESTING
			syslog(LOG_NOTICE, "%s (pid:'%lu', o:'%p', cid:'%lu'): WARNING: DOUBLE SIGNON ATTEMPT: HASHED SESSION IS CONNECTED AND ON LIVE SOCKET (o_hashed:'%p', cid_hashed:'%lu', socketfd:'%d'): SUSPENDING HASHED/CONNECTED SESSION...",
			...
			//TODO: OR we just leave it where it is and Send protocol reply message for double connection
  ...
  CacheBackendGetRawSessionRecordByUserId(SESSION_USERID(sesn_ptr_hashed), CALLFLAGS_EMPTY, THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT)); //fetch a fresh cache backend record (no db refresh)
		...
		if (THREAD_CONTEXT_UFSRV_RESULT_CODE_EQUAL(THREAD_CONTEXT, RESCODE_BACKEND_DATA)) {
		  redis_ptr = (redisReply *)THREAD_CONTEXT_UFSRV_RESULT_USERDATA;
			if (SESNSTATUS_IS_SET(sesn_ptr_hashed->stat, SESNSTATUS_REMOTE))
				return (_InstateRemoteSessionAsConnected(instance_sesn_ptr_transient, instance_sesn_ptr_hashed, redis_ptr));
		} else if (THREAD_CONTEXT_UFSRV_RESULT_CODE_EQUAL(THREAD_CONTEXT, RESCODE_BACKEND_DATA_EMPTYSET)) {
 ...
 *
 *	@locked sesn_ptr: in main loop
 *	@locked sesn_ptr_hashed: in caller's environment. Which subsequently unlocked here after beind reset for rebuild from backend record
...
_InstateRemoteSessionAsConnected(InstanceHolderForSession *instance_sesn_ptr_transient, InstanceHolderForSession *instance_sesn_ptr_hashed, redisReply *redis_ptr)
{
  ...
  Session *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient),
	...
	if (eid != SESSION_EID(sesn_ptr_hashed)) {
		...
		instance_sesn_ptr_new = CacheBackendInstantiateRawSessionRecord(sesn_ptr_transient, redis_ptr, sesn_call_flags, NULL); //no db refresh?
		...
		if (res.result_code == RESCODE_IO_POLLERROR) {
			...
			if (!IS_EMPTY(instance_sesn_ptr_returned)) {
			  sesn_ptr_returned = SessionOffInstanceHolder(instance_sesn_ptr_returned);
				...
				//since we don't have a return session object to carry the error message anymore
...
_InstateUnconnectedSessionAsConnected(InstanceHolderForSession *instance_sesn_ptr_migrated, InstanceHolderForSession *instance_sesn_ptr_transient, unsigned long call_flags, UFSRVResult *res_ptr_in)
{
  ...
  Session     *sesn_ptr_transient = SessionOffInstanceHolder(instance_sesn_ptr_transient);
	...
	if (!(call_flags&CALL_FLAG_DONT_LOCK_SESSION)) {
		...
		if (_RESULT_TYPE_EQUAL(THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT), RESULT_TYPE_ERR)) {
			_RETURN_RESULT_RES(res_ptr_in, instance_sesn_ptr_transient, RESULT_TYPE_ERR, RESCODE_PROG_LOCKED) //will cause hashed object to be killed
		}
	}
  ...
  sesn_ptr_transient->ssptr = sesn_ptr_migrated->ssptr; //this is a throw away socket
	 ...
	 SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_migrated) = SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_transient);
	 LOAD_NULL(SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_transient));

	 //TODO: WE AUTOMATICALLY GET THE NEW SOCKET ADDRESS, BUT WE LOSE MSGQUEUE FOR EXISTING SOCKET MAY HAVE ENTRIES IN IT
     ...
     SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_transient) = SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_migrated);
     LOAD_NULL(SESSION_PROTOCOL_SESSION_DATA(sesn_ptr_migrated));
...
//		memset (&sesn_ptr_target->ssptr->protocol_header, 0, sizeof(ProtocolHeaderWebsocket));
 ...
 /**
  * @brief Update cache-backend stored user cookie. Also, delete older cookie from cookie cache-backend store if provided.
  * @param sesn_ptr[in] User session
  * @param old_cookie Stale cookie value to be deleted
  * @return
  */
UFSRVResult *
...
/**
 * 	@brief Load a fresh session entry for the user.
 * 	@param old_cookie Stale cookie entry to be removed from cache-backend
 * 	call_flags CALL_FLAG_DONT_BROADCAST_SESSION_EVENT: if set, the session refresh event won't be broadcast to the queue
 */
UFSRVResult *
RefreshBackendCacheForSession(Session *sesn_ptr, const char *old_cookie, unsigned long sesn_call_flags)
{
  ...
  redisReply *redis_ptr = (*pers_ptr->send_command)(sesn_ptr, REDIS_CMD_USERNAME_UID_SET, SESSION_USERNAME(sesn_ptr), SESSION_USERID(sesn_ptr));
	...
	redis_ptr = (*pers_ptr->send_command)(sesn_ptr, REDIS_CMD_USER_SESSION_RECORD_SET_ALL,
                                                              SESSION_USERID(sesn_ptr),
                                                              SESSION_ID(sesn_ptr),
                                                              masterptr->serverid,
                                                              //sesn_ptr->sservice.user.user_details.user_id,
                                                              SESSION_UFSRVUID(sesn_ptr), (size_t)CONFIG_MAX_UFSRV_ID_SZ,
                                                              GetEmptyProfile(), (size_t)CONFIG_USER_PROFILEKEY_MAX_SIZE,
                                                              0UL,//this slot is unused: old 'when' value
                                                              sesn_ptr->ssptr->haddress,
                                                              (IS_STR_LOADED(SESSION_HOMEBASE_GEOLOC_DYNAMIC(sesn_ptr)) ? SESSION_HOMEBASE_GEOLOC_DYNAMIC(sesn_ptr) : CONFIG_DEFAULT_PREFS_STRING_VALUE),
                                                              sesn_ptr->when_serviced_start,
                                                              sesn_ptr->when_suspended,
                                                              0UL,//this slot is unused: old 'resumed' value
                                                              ...
                                                              (IS_STR_LOADED(SESSION_USERPREF_HOMEBASE_GEOLOC(sesn_ptr)) ? SESSION_USERPREF_HOMEBASE_GEOLOC(sesn_ptr) : CONFIG_DEFAULT_PREFS_STRING_VALUE),
                                                              (IS_STR_LOADED(SESSION_USERAVATAR(sesn_ptr))?SESSION_USERAVATAR(sesn_ptr):CONFIG_DEFAULT_PREFS_STRING_VALUE),
                                                              SESSION_USERGUARDIAN_UID(sesn_ptr)
                                                              );
	...
	_RETURN_RESULT_SESN(sesn_ptr, sesn_ptr, RESULT_TYPE_SUCCESS, RESCODE_USER_INITIALISED)
...
/**
 * @brief Generate a unique, sequential eid for user represented by Session.
 * @mem ALLOCATED UfsrvEvent if none passed by user
 * @locked sesn_ptr
 */
UFSRVResult *
RegisterUfsrvEvent(Session *sesn_ptr, EnumEventType event_type, unsigned event_instance_type, void *event_payload, UfsrvEvent *event_ptr_out)
{
	...
	event_ptr->event_cmd_type = MSGCMD_USER;
	event_ptr->originator_ptr = &SESSION_UFSRVUIDSTORE(sesn_ptr);
...
/**
* 	@brief create unique, sequential event id for session.
 * 	@eid_in If provided, unique, sequential eid generate is skipped and provided value is used, instead.
* 	@locked 'Session *'
 * 	@mem ALLOCATES UfsrvEvent if not provided by uses
*/
static UfsrvEvent *
...
/**
 * @brief Generate a unique sequential eid for user represented by Session *
 * @param sesn_ptr user session for which eid is being generated.
 * @return unique sequential eid
 * @locked Session *
 */
...
_GenerateSessionEventId(Session *sesn_ptr)
{
	...
	SESSION_EID(sesn_ptr) = (unsigned long)ev_counter;//todo this shouldn't happen here, as it mandates Session object being locked.
...
SessionLoadEphemeralMode(Session *sesn_ptr)
{
	...
	thread_ctx_ptr				=pthread_getspecific(jobworkers_config->ufsrv_thread_context_key);
	inst_ptr							=pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key);
	pers_ptr							=pthread_getspecific(jobworkers_config->worker_persistance_key);
	mq_ptr								=pthread_getspecific(jobworkers_config->ufsrv_msgqueue_pub_key);
	usrmsg_cachbackend_ptr=pthread_getspecific(jobworkers_config->worker_usrmsg_cachebackend_key);
	fence_cachbackend_ptr	=pthread_getspecific(jobworkers_config->worker_fence_cachebackend_key);
	db_backend						=pthread_getspecific(jobworkers_config->ufsrv_db_backend_key);
...
inline int __attribute__ (( no_sanitize_thread ))
...
void
ThreadContextTransferAccessContextForSession(Session *sesn_ptr)
{
    sesn_ptr->thread_ctx_ptr						=	THREAD_CONTEXT_PTR;
    sesn_ptr->instrumentation_backend	  =	THREAD_CONTEXT_INSTRUMENTATION_BACKEND;
    sesn_ptr->persistance_backend			  =	THREAD_CONTEXT_PERSISTANCE_CACHEBACKEND(THREAD_CONTEXT);
    sesn_ptr->msgqueue_backend					=	THREAD_CONTEXT_MSGQUEUE_CACHEBACKEND;
    sesn_ptr->db_backend								=	THREAD_CONTEXT_DB_BACKEND;
    sesn_ptr->usrmsg_cachebackend			  =	THREAD_CONTEXT_USRMSG_CACHEBACKEND;
    sesn_ptr->fence_cachebackend				=	THREAD_CONTEXT_FENCE_CACHEBACKEND;
}

void
ThreadContextResetAccessContextForSession(Session *sesn_ptr_from)
{
...
{
	WorkersConfigDescriptor *jobworkers_config = GetJobWorkersConfigurationDescriptor();
 return (pthread_getspecific(jobworkers_config->ufsrv_http_request_context_key));
}

static inline UFSRVResult *_InvokeLifecycleCallbackServiceTimeout(InstanceHolderForSession *, time_t now, unsigned long call_flags);

static inline UFSRVResult *
_InvokeLifecycleCallbackServiceTimeout(InstanceHolderForSession *instance_sesn_ptr, time_t now, unsigned long call_flags)
{
	UFSRVResult *res_ptr;
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (_PROTOCOL_CLLBACKS_SERVICE_TIMEOUT(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr))))) {
		res_ptr = _PROTOCOL_CLLBACKS_SERVICE_TIMEOUT_INVOKE(protocols_registry_ptr, PROTO_PROTOCOL_ID(((Protocol *)SESSION_PROTOCOLTYPE(sesn_ptr))), instance_sesn_ptr, now, call_flags);
	} else {
...
__attribute__ ((pure)) ScheduledJobType *
GetScheduledJobTypeForSessionTimeout(void)
{
	static ScheduledJobType job_type_session_timeout={
			...
			.callbacks					=	{
					...
					.on_error				=	NULL,
					.on_run					=	(CallbackOnRun)CheckSessionIdleTime,
...
static InstanceHolderForSession	*_LocalSessionStore[_CONFIGDEFAULT_HASHTABLE_EXPANSION_SZ * 2];
 ...
 */
int
CheckSessionIdleTime(ScheduledJob *scheduled_job, void *arg)
{
	bool expected_running_is_false = false;
	if (!atomic_compare_exchange_strong_explicit(&isCheckSessionIdelTimeRunning, &expected_running_is_false, true, memory_order_acq_rel, memory_order_relaxed)) {
		...
		//statsd_inc(pthread_getspecific(sessions_delegator_ptr->jobworkers.ufsrv_instrumentation_backend_key), "worker.ufsrv.job.idletime.busy", 1.0);
		return 0;
	}
  ...
  long long timer_start = GetTimeNowInMicros();
		...
		statsd_gauge(pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key), "worker.ufsrv.job.idletime.sessions_global_size", (ssize_t)HASHTABLE_ENTRIES(hash));
    ...
    UfsrvConfigRegisterUfsrverActivity(pthread_getspecific(jobworkers_config->worker_persistance_key), time(NULL));
...

		{
			...
			//once only init
			ephemeral_mode_descriptor.thread_ctx_ptr				=	pthread_getspecific(jobworkers_config->ufsrv_thread_context_key);
			ephemeral_mode_descriptor.inst_ptr							=	pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key);
			ephemeral_mode_descriptor.pers_ptr							=	pthread_getspecific(jobworkers_config->worker_persistance_key);
			ephemeral_mode_descriptor.mq_ptr								=	pthread_getspecific(jobworkers_config->ufsrv_msgqueue_pub_key);
			ephemeral_mode_descriptor.usrmsg_cachbackend_ptr=	pthread_getspecific(jobworkers_config->worker_usrmsg_cachebackend_key);
			ephemeral_mode_descriptor.fence_cachbackend_ptr	=	pthread_getspecific(jobworkers_config->worker_fence_cachebackend_key);
			ephemeral_mode_descriptor.db_backend						=	pthread_getspecific(jobworkers_config->ufsrv_db_backend_key);

			//bool recycle_flag=false;
			statsd_gauge(pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key), "worker.ufsrv.job.idletime.sessions_collection_size", numResults);
...
//					}
				...
				if (SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_DEFERRED_RECYCLE) && SESNSTATUS_IS_SET(sesn_ptr->stat, SESNSTATUS_SUSPENDED)) {
					...
					if (SuspendSession (instance_sesn_ptr, HARD_SUSPENSE)) {
						result_code = RESCODE_SESN_HARDSPENDED;
						goto unlock_session;
					} else {
				...
				_InvokeLifecycleCallbackServiceTimeout(instance_sesn_ptr, now, 0);
				...
				if (result_code == RESCODE_SESN_HARDSPENDED)/*session out of circulation*/ {
					SESNSTATUS_SET(sesn_ptr->stat, SESNSTATUS_RECYCLED);
					SessionReturnToRecycler(instance_sesn_ptr, (ContextData *)NULL, 0);
				}
				//>>>>>>>>>>>>>>>>>>>>>>>
		...
		//statsd_gauge(pthread_getspecific(sessions_delegator_ptr->jobworkers.ufsrv_instrumentation_backend_key), "worker.ufsrv.job.idletime.sessions_collection_size", 0);
		...
		statsd_timing(pthread_getspecific(jobworkers_config->ufsrv_instrumentation_backend_key), "worker.ufsrv.job.idletime.elapsed_time", (timer_end-timer_start));
