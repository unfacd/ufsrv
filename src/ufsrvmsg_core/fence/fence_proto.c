...
#include <ufsrvmsg_core/fence/fence_protobuf.h>
...
void
ProvideFenceListSemanticsInProto(Fence *f_ptr, FenceRecord *fence_record_ptr)
{
...
void
ProvideFencePermissionsUserListsInProto(Session *sesn_ptr, Fence *f_ptr, FenceRecord *fence_record_ptr)
{
...
//  user_record_ptr->userid = SESSION_USERID(sesn_ptr); user_record_ptr->has_userid = 1;
  ProvideUfsrvUidInProto(&SESSION_UFSRVUIDSTORE(sesn_ptr), &(user_record_ptr->ufsrvuid), true);

  SESSION_RETURN_RESULT(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RECODE_NONE)
}
...
{
  _InitialiseStorageIfNecessary(f_ptr, permission_ptr);
  ...
  hopscotch_iterator_executor_with_offset(&permission_ptr->permitted_users, (CallbackExecutor) _LoadPermissionList, CLIENT_CTX_DATA(&ctx));
...
void
ProvideFencePermissionsInProto(Session *sesn_ptr, Fence *f_ptr, FenceRecord *fence_record_ptr)
{
  ...
  ProvideFenceListSemanticsInProto(f_ptr, fence_record_ptr);
  ProvideFencePermissionsUserListsInProto(sesn_ptr, f_ptr, fence_record_ptr);

}

#include <ufsrvmsg_core/fence/fence_state.h> //for FPREF_LAST_ALIGNMENT
...
void
ProvideFenceUserPreferencesInProto(Session *sesn_ptr, FenceStateDescriptor *fstate_ptr, FenceRecord *fence_record_ptr)
{
...
_MakeFenceUserPreferenceInProto(FenceUserPreference *user_pref_ptr, FenceStateDescriptor *fstate_ptr, FenceUserPrefsOffsets pref_offset)
{
	...
	urec_ptr_aux->pref_id = (enum _FenceUserPrefs)pref_offset;
...
void
ProvideFencePrivacyModeInProto(Fence *f_ptr, FenceRecord *fence_record_ptr)
{
...
void
ProvideFenceTypeInProto(Fence *f_ptr, FenceRecord *fence_record_ptr)
{
...
void
ProvideFenceDeliveryModeInProto(Fence *f_ptr, FenceRecord *fence_record_ptr)
{
...
void
ProvideFenceJoinModeInProto(Fence *f_ptr, FenceRecord *fence_record_ptr)
{
 ...
 */
FenceRecord *
ProvideFenceRecordInProto(InstanceContextForSession *ctx_ptr, InstanceContextForFence *ctx_f_ptr, FenceRecord *fence_record_ptr_in)
{
  ...
  fence_record_ptr->eid					=	FENCE_FENECE_EVENTS_COUNTER(f_ptr); fence_record_ptr->has_eid = 1;
	...
	fence_record_ptr->location		= ProvideFenceLocationInProto(f_ptr, true);

	ProvideFenceKeyInProto(FENCE_KEY(f_ptr), fence_record_ptr, false);

  ProvideFenceOwnerUfsrvUid(FENCE_OWNER_UID(f_ptr), ^(UfsrvUid *ufsrvuid) {
    if(IS_PRESENT(ufsrvuid)) {
      ProvideUfsrvUidInProto(ufsrvuid, &(fence_record_ptr->owner_uid), false);
      fence_record_ptr->has_owner_uid = 1;
    } else {
      UfsrvUid uid = {0};
      if (IS_PRESENT(GetUfsrvUid(FENCE_OWNER_UID(f_ptr), &uid, false, NULL))) {
        ProvideUfsrvUidInProto(&uid, &(fence_record_ptr->owner_uid), false);
        fence_record_ptr->has_owner_uid = 1;
      }
    }
  });
  ...
  ProvideFencePrivacyModeInProto(f_ptr, fence_record_ptr);
  ProvideFenceDeliveryModeInProto(f_ptr, fence_record_ptr);
  ProvideFenceTypeInProto(f_ptr, fence_record_ptr);
  ProvideFenceJoinModeInProto(f_ptr, fence_record_ptr);
  ProvideFencePermissionsInProto(ctx_ptr->sesn_ptr, f_ptr, fence_record_ptr);
	...
	desc_ptr = ProvideFenceMembersInProto(ctx_ptr, ctx_f_ptr, CALL_FLAG_SESSION_LIST_INCLUDE_REMOTE, 0);//dontlock fence
	if (desc_ptr) {
	...
	desc_ptr = ProvideFenceInviteListInProto(ctx_ptr->sesn_ptr, f_ptr, 0/*@call_flags: not locking fence*/);//fence should be setup with invite list by now
	if (desc_ptr) {
		if (desc_ptr->records_sz > 0) {
  ...
  desc_ptr = ProvideFenceLinkJoinMembersInProto(ctx_ptr, ctx_f_ptr, CALL_FLAG_SESSION_LIST_INCLUDE_REMOTE, 0/*@call_flags: not locking fence*/);
  if (desc_ptr) {
    if (desc_ptr->records_sz > 0) {
      fence_record_ptr->n_linkjoin_members = desc_ptr->records_sz;
      fence_record_ptr->linkjoin_members = desc_ptr->user_records;
    }

    free(desc_ptr);//contained objects are destructed separately in DestructFenceRecordProto()
  }

  desc_ptr = ProvideFenceBannedMembersInProto(ctx_ptr, ctx_f_ptr, CALL_FLAG_SESSION_LIST_INCLUDE_REMOTE, 0/*@call_flags: not locking fence*/);
  if (desc_ptr) {
    if (desc_ptr->records_sz > 0) {
      fence_record_ptr->n_banned_members = desc_ptr->records_sz;
      fence_record_ptr->banned_members = desc_ptr->user_records;
    }
...
/**
 * When using @param flag_by_ref don't lose scope of @param fence_key until the host prototype structure is destructed
 * @param fence_key source key data
 * @param proto_ptr host proto
 * @param flag_by_ref when true pass value by reference.
 */
void
ProvideFenceKeyInProto(unsigned char *fence_key, FenceRecord *fence_record, bool flag_by_ref)
{
  ProtobufCBinaryData *proto_ptr = &fence_record->fkey;

  proto_ptr->len  = GROUP_MASTER_KEY_SIZE;
  if (flag_by_ref) {
    proto_ptr->data = fence_key;
  } else {
    proto_ptr->data = calloc(GROUP_MASTER_KEY_SIZE, sizeof(uint8_t));
    memcpy(proto_ptr->data, fence_key, GROUP_MASTER_KEY_SIZE);
  }

  fence_record->has_fkey = 1;
}

/**
 * @brief Lightweight Fence record definition.
 * @param f_ptr[IN]
 * @param fence_record_ptr_in[IN] User provided allocation for return \ref FenceRecord structure
 * @param isIncludeCname Optionally, include cname if flagged
 * @return
 * @dynamic_memory EXPORTS 'char *'
 * @dynamic_memory EXPORTS 'FenceRecord *' if no user allocation was provided
 */
FenceRecord *
ProvideFenceRecordInProtoAsIdentifier(Session *sesn_ptr, Fence *f_ptr, FenceRecord *fence_record_ptr_in, bool isIncludeCname)
{
	FenceRecord 				*fence_record_ptr = NULL;
	FenceUserRecordsDescription *desc_ptr = NULL;
...
FenceRecord *
ProvideFenceRecordInProtoAsIdentifierByParams(Session *sesn_ptr, unsigned long fid, FenceRecord *fence_record_ptr_in)
{
	FenceRecord 				*fence_record_ptr = NULL;
	FenceUserRecordsDescription *desc_ptr = NULL;
...
ProvideFenceInviteListFromNamesInProto(Session *sesn_ptr, char **members_invited, size_t members_sz, unsigned call_flags)
{
#if 1
	//uses memory vector
	if (members_sz == 0) {
	...
	for (i = 0; i < members_sz; i++) {
		...
		if (IS_OK(
            ProvideUserRecordFromUsernameInProto(sesn_ptr, members_invited[i], urec_ptr_aux, PROTO_USER_RECORD_MINIMAL)))
...
ProvideFenceRecordsListFromFenceEventsInProto(Session *sesn_ptr, FenceEvent **fence_events, size_t fence_events_sz, unsigned long call_flags, CollectionDescriptor *collection_ptr_in)
{
	...
	for (i=0; i<fence_events_sz; i++) {
		...
		urec_ptr_aux->fid = fence_events[i]->ctx_id; urec_ptr_aux->has_fid = 1;
...
/**
 * 	@brief: Build fence's own invite list into proto
 ...
 */
FenceUserRecordsDescription *
ProvideFenceInviteListInProto(Session *sesn_ptr, Fence *f_ptr, unsigned call_flags)
{
	bool fence_lock_already_owned = false;
	//vector based allocation
	//this is not ready: record poulation function is not implemented. Valgrid client code needs to be uncommented in the free functions above
	...
	for (eptr=f_ptr->fence_user_sessions_invited_list.head; eptr; eptr=eptr->next) {
	   //UserRecord *urec_ptr_aux=(UserRecord *)(user_rec_ptr+(processed_counter*sizeof(UserRecord)));//chunk up vector as needed
		...
		if (IS_OK(ProvideUserRecordFromSessionInProto(sesn_ptr_aux, urec_ptr_aux, PROTO_USER_RECORD_MINIMAL, false))) {
			user_records[processed_counter] = urec_ptr_aux;//connect the record to the array
 ...
 */
FenceUserRecordsDescription  *
ProvideFenceMembersInProto(InstanceContextForSession *ctx_ptr, InstanceContextForFence *ctx_f_ptr, unsigned long sesn_call_flags, unsigned long fence_call_flags)
{
	bool fence_lock_already_owned = false;
  ...
  InstateMembersListCacheRecordForFence(sesn_ptr_this, ctx_f_ptr->instance_f_ptr, UNSPECIFIED_UID, UNSPECIFIED_FENCE_LISTTYPE, UNSPECIFIED_FENCE_LISTTYPE, FENCE_CALLFLAG_ATTACH_USER_LIST_TO_FENCE);
...
FenceUserRecordsDescription  *
ProvideFenceLinkJoinMembersInProto(InstanceContextForSession *ctx_ptr, InstanceContextForFence *ctx_f_ptr, unsigned long sesn_call_flags, unsigned long fence_call_flags)
{
  bool fence_lock_already_owned = false;
  Session *sesn_ptr_this = ctx_ptr->sesn_ptr;
  Fence *f_ptr = ctx_f_ptr->f_ptr;
  ...
  if (FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr) == 0) {
    if (fence_call_flags&FENCE_CALLFLAG_LOCK_FENCE) {
      if (!fence_lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr_this));
    }
  ...
  InstateLinkJoiningMembersListCacheRecordForFence(sesn_ptr_this, ctx_f_ptr->instance_f_ptr, UNSPECIFIED_UID, FENCE_CALLFLAG_ATTACH_USER_LIST_TO_FENCE);
  if (SESSION_RESULT_TYPE_EQUAL(sesn_ptr_this, RESULT_TYPE_ERR)) {
    free(desc_ptr);
    return NULL;
  }

  //this should not happen given the above check, unless of data integrity issues: local fence has members, but backend is empty //todo 7/4/24 devops: fix possible data integrity and mismatch
  if (SESSION_RESULT_CODE_EQUAL(sesn_ptr_this, RESCODE_BACKEND_DATA_EMPTYSET)) {
    syslog(LOG_ERR, LOGSTR_FENCE_BACKEND_USERLIST_MISMATCH, __func__, pthread_self(), sesn_ptr_this, f_ptr, FENCE_ID(f_ptr), FENCE_USERS_COUNT(f_ptr), LOGCODE_FENCE_BACKEND_USERLIST_MISMATCH);
  ...
  user_records = calloc(FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr), sizeof(UserRecord *));//we may fill less than that if flag not set: CALL_FLAG_SESSION_LIST_INCLUDE_REMOTE
  user_rec_ptr = calloc(FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr), sizeof(UserRecord));

#if __VALGRIND_DRD
  VALGRIND_CREATE_MEMPOOL(user_records, 0, 1);
  VALGRIND_MAKE_MEM_NOACCESS(user_records, FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr) * (sizeof(UserRecord *)));

  VALGRIND_CREATE_MEMPOOL(user_rec_ptr, 0, 1);
  VALGRIND_MAKE_MEM_NOACCESS(user_rec_ptr, FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr) * (sizeof(UserRecord)));
#endif
...
#ifdef __UF_TESTING
  syslog(LOG_DEBUG, "%s (pid:'%lu', fid:'%lu', users_sz:'%d'): FINISHED PROTO representation.", __func__, pthread_self(), FENCE_ID(f_ptr), FENCE_LINK_JOINING_USER_SESSION_LIST_SIZE(f_ptr));
#endif
...
FenceUserRecordsDescription  *
ProvideFenceBannedMembersInProto(InstanceContextForSession *ctx_ptr, InstanceContextForFence *ctx_f_ptr, unsigned long sesn_call_flags, unsigned long fence_call_flags)
{
  bool fence_lock_already_owned = false;
  Session *sesn_ptr_this = ctx_ptr->sesn_ptr;
  Fence *f_ptr = ctx_f_ptr->f_ptr;
  ...
  if (FENCE_BANNED_USER_SESSION_LIST_SIZE(f_ptr) == 0) {
    if (fence_call_flags&FENCE_CALLFLAG_LOCK_FENCE) {
      if (!fence_lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr_this));
    }
...
#if 0 //todo 7/4/24 devops: implement ProvideFenceBannedMembersInProto()
  InstateBannedMembersListCacheRecordForFence(sesn_ptr_this, ctx_f_ptr->instance_f_ptr, UNSPECIFIED_UID, FENCE_CALLFLAG_ATTACH_USER_LIST_TO_FENCE);
  if (SESSION_RESULT_TYPE_EQUAL(sesn_ptr_this, RESULT_TYPE_ERR)) {
    free(desc_ptr);
    return NULL;
  }
 ...
 * 	This is temporary treatment, as current attachment data for fence avatars are not saved directly in FenceRecord, but needs
 * 	to be retrieved from GroupContext.
 * 	Similar to TEMPProvideAttachmentRecordFromAttachmentPointerInProto but not intended for packing, so it doesn't
...
TransferFenceAvatarAttachmentIfPresent(AttachmentPointer *attachment_pointer_ptr, AttachmentRecord *record_ptr_out)
{
	if (IS_PRESENT(attachment_pointer_ptr)) {
		...
		record_ptr->blurhash      = attachment_pointer_ptr->blurhash;
		record_ptr->caption       = attachment_pointer_ptr->caption;
 ...
 */
CollectionDescriptor *
TEMPProvideAttachmentRecordFromAttachmentPointerInProto (AttachmentPointer *attachment_pointer_ptr, CollectionDescriptor *collection_attachment_records_out)
{
	AttachmentRecord 			*attachment_record_ptr;
	...
	attachment_record_ptr->blurhash     = attachment_pointer_ptr->blurhash;
	attachment_record_ptr->caption      = attachment_pointer_ptr->caption;
...
TEMPAttachmentDescriptorGetFromProto(Session *sesn_ptr, AttachmentPointer *attachment_record, size_t eid, AttachmentDescriptor *attachment_descriptor_ptr_in, bool flag_encode_key)
{
  ...
  if (IS_STR_LOADED(attachment_record->blurhash) && ((sz = strlen(attachment_record->blurhash)) <= CONFIGDEFAULT_MAX_IMAGE_BLURHASH_SZ)) {
    attachment_ptr->blurhash = strbufdup(attachment_record->blurhash,  sz);
  }

  if (IS_STR_LOADED(attachment_record->caption) && ((sz = strlen(attachment_record->caption)) <= CONFIGDEFAULT_MAX_IMAGE_CAPTION_SZ)) {
    attachment_ptr->caption = strbufdup(attachment_record->caption,  sz);
  }
...
AttachmentDescriptorGetFromProto(Session *sesn_ptr, AttachmentRecord *attachment_record, size_t eid, AttachmentDescriptor *attachment_descriptor_ptr_in, bool flag_encode_key)
{
  ...
  if (IS_STR_LOADED(attachment_record->caption) && ((sz = strlen(attachment_record->caption)) <= CONFIGDEFAULT_MAX_IMAGE_CAPTION_SZ)) {
	...
	attachment_ptr->size      = attachment_record->size;
	attachment_ptr->eid       = eid;
  attachment_record->flags  = attachment_record->flags;
...
LocationRecord *
ProvideUserLocationInProto(const User *user_ptr, bool flag_digest_mode)
{
	...
	location_record_ptr = ProvideLocationDescriptionInProto(location_desc_ptr, flag_digest_mode, true, NULL);

	if (IS_FALSE(flag_digest_mode))	{
    if (user_ptr->user_details.homebase_geoloc_dynamic) location_record_ptr->baseloc = strdup(user_ptr->user_details.homebase_geoloc_dynamic);
  }
...
LocationRecord *
ProvideFenceLocationInProto(const Fence *f_ptr, bool flag_digest_mode)
{
	if (unlikely(IS_EMPTY(f_ptr)))	return NULL;

  LocationRecord *location_record_ptr = ProvideLocationDescriptionInProto(&(f_ptr->fence_location.fence_location), flag_digest_mode, true, NULL);
 ...
 */
LocationRecord *
ProvideLocationDescriptionInProto(const LocationDescription *loc_ptr, bool flag_digest_mode, bool flag_dup_mode, LocationRecord *location_record_ptr_out)
{
...
DestructFenceRecordProto(FenceRecord *fence_record_ptr, bool flag_self_destruct)
{
  ...
  if (IS_PRESENT(fence_record_ptr->by_user.data))	{
    free(fence_record_ptr->by_user.data);
  }

  if (IS_PRESENT(fence_record_ptr->fkey.data))	{
    free(fence_record_ptr->fkey.data);
  }
 ...
 * 	FENCE RECORDS< WHO ARE ALLOCATED VIA A SINGLE VECTOR I.E. NOT INDIVIDUALLY.
 *
...
DestructFenceRecordsProto(FenceRecord **fence_records_ptr, unsigned count, bool flag_self_destruct, bool self_destruct_record)
{
	if (fence_records_ptr && count) {
		...
		for (; idx>=0; idx--) {
			...
			if (IS_PRESENT(fence_record_ptr->cname)) free(fence_record_ptr->cname);
			if (IS_PRESENT(fence_record_ptr->fname)) free(fence_record_ptr->fname);
