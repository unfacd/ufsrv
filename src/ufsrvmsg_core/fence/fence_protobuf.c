...
MakeAttachmentRecordInProto(AttachmentDescriptor *attachment_ptr, AttachmentRecord *attachment_record_ptr_out, bool flag_dup)
{
  ...
  } else if (IS_STR_LOADED(attachment_ptr->key_encoded)) {
  ...
  } else if (IS_STR_LOADED(attachment_ptr->digest_encoded)) {
    int decoded_sz_out									=	0;
  ...
  if (attachment_ptr->flags > 0) {
    attachment_record_ptr->flags = attachment_ptr->flags;
    attachment_record_ptr->has_flags = 1;
  }
