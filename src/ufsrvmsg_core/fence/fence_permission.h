...
//SADD <%fid>:<%set_name> <%uid>
#define REDIS_CMD_FENCE_PERMISSION_SET_ADD_USER 			"SADD %lu:%s %lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_DEL_USER 			"SREM %lu:%s %lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_GET_USERS	    "SMEMBERS %lu:%s"
#define REDIS_CMD_FENCE_PERMISSION_SET_ISMEMBER	      "SISMEMBER %lu:%s %lu"
#define REDIS_CMD_FENCE_PERMISSION_SET_DEL_USERS	    "DEL %lu:%s"
...
int LoadListSemanticsForBackendPersistence(const Fence *f_ptr);
void MapPermissionsListSemanticsFromBackendPersistence (Fence *f_ptr, int list_semantics_persisted);
...
UFSRVResult *CacheBackendPermissionMembersIsMember(long fid, FencePermission *permission_ptr, unsigned long userid_target);
...
size_t InvokeFencePermissionListIteratorExecutor(FencePermission *permission_ptr, CallbackExecutor executor, ClientContextData *ctx_data_ptr, bool is_initialise);

static inline void
_InitialiseStorageIfNecessary( Fence *f_ptr, FencePermission *permission_ptr)
{
	//lazy initialisation
...
/**
 * 	@brief Check for the presence of a user on a given permission ACL, but without regards to list semantics
 */
static inline bool
IsUserOnFencePermissionList(Session *sesn_ptr, Fence *f_ptr, FencePermission *permission_ptr)
{
  ...
  unsigned long userid = UfsrvUidGetSequenceId(&SESSION_UFSRVUIDSTORE(sesn_ptr));
	...
	return (hopscotch_lookup_with_offset(FENCE_PERMISSION_PERMITTED_USERS_PTR(permission_ptr),
                                       PermissionListItemExtractorCallback, (uint8_t *) &userid,
                                       CONFIG_FENCE_PERMISSIONS_KEYOFFSET(Session, sservice.user.user_details.user_id)) != NULL);
}

/**
 * @bruef Use permission ACL semantics to check if a user has a permission.
 * @return
 */
static inline bool
...
/**
 * @brief Turn off fence's permission member listing whitelisting mode.
 *
 * This turns off the whitelisting mode (the default), so the permission's ACL is now in so called "black list mode", meaning
 * the permission is granted, unless blacklisted. So if a given user is present in the permission's members list \ref FencePermission.permitted_users
 * then the user is disallowed action guarded by the permission.
 * @see SetFencePermissionWhiteListSemantics()
 * @param sesn_ptr
 * @param f_ptr
