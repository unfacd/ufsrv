...
static FenceEvent *_CreateFenceEvent(Fence *f_ptr, int, unsigned lock_flag, FenceEvent *fe_ptr_out);
...
GetFenceEventId(Fence *f_ptr, unsigned lock_flag)
{
	...
	unsigned long fence_event_counter = f_ptr->fence_events.event_id;
 ...
 */
FenceEvent *
RegisterFenceEvent(Fence *f_ptr, unsigned event_type,  void *event_payload, unsigned lock_flag,  FenceEvent *fe_ptr_out)
{
	FenceEvent *fe_ptr = NULL;

		//1)instantiate message
	if (event_type == EVENT_TYPE_FENCE_CREATED) fe_ptr = _CreateFenceEvent(f_ptr, 1, lock_flag, fe_ptr_out);//seed with 1
	else fe_ptr = _CreateFenceEvent(f_ptr, 0, lock_flag, fe_ptr_out);//increment backend
...
#ifdef __UF_TESTING
	syslog(LOG_DEBUG, "%s {pid:'%lu', fid:'%lu', eid:'%lu'}: FenceEvent: Added..", __func__, pthread_self(), FENCE_ID(f_ptr), fe_ptr->eid);
#endif
 ...
 */
FenceEvent *
RegisterFenceEventWithFid(unsigned long fence_id, unsigned event_type,  void *event_payload, FenceEvent *fe_ptr_out)
{
	FenceEvent	*fe_ptr	= NULL;
	Fence				fence		= {0};
	...
	if (event_type == EVENT_TYPE_FENCE_CREATED)
		fe_ptr = _CreateFenceEvent(&fence, 1, 0, fe_ptr_out);//seed with 1
	else
		fe_ptr = _CreateFenceEvent(&fence, 0, 0, fe_ptr_out);//increment backend
...
#ifdef __UF_TESTING
	syslog(LOG_DEBUG, "%s {pid:'%lu', bid:'%lu', eid:'%lu'}: FenceEvent: Added..", __func__, pthread_self(), fence_id, fe_ptr->eid);
#endif
...

{
	...
	if (reset_counter_flag)	f_ptr->fence_events.event_id=0;
...
GenerateFenceEventId(Fence *f_ptr, unsigned lock_flag)
{
  extern __thread ThreadContext ufsrv_thread_context;
  ...
  if (!(IS_EMPTY(f_ptr))) {
    ...
    f_ptr->fence_events.event_id = (unsigned long)ev_counter;
 ...
 */
static FenceEvent *
_CreateFenceEvent(Fence *f_ptr, int eid_in, unsigned lock_flag, FenceEvent *fe_ptr_out)
{
	FenceEvent *fe_ptr = NULL;
	if (IS_PRESENT(fe_ptr_out))	fe_ptr = fe_ptr_out;
	else												fe_ptr = malloc(sizeof(FenceEvent));

	//generate at the cache backend
	if (eid_in == 0) {
		fe_ptr->eid = GenerateFenceEventId(f_ptr, lock_flag);
		if (fe_ptr->eid <= 0) {
			syslog(LOG_INFO, LOGSTR_FENCE_EVENET_ID_ERROR_BACKEND, __func__, pthread_self(), f_ptr, FENCE_ID(f_ptr), fe_ptr->eid,  LOGCODE_FENCE_EVENET_ID_ERROR_BACKEND);
...
{
#define SQL_INSERT_NEW_INCOMING_MESSAGE "INSERT INTO events (eid, ctxid, cmd_type, event_type, timestamp, originator) VALUES ('%lu', '%lu', '%u', '%u', '%lu', '%lu')"

  char *sql_query_str;
  sql_query_str = mdsprintf(SQL_INSERT_NEW_INCOMING_MESSAGE, event_ptr->eid, event_ptr->ctx_id, event_ptr->event_cmd_type, event_ptr->event_type, event_ptr->when, UfsrvUidGetSequenceId(event_ptr->originator_ptr));
...
//SELECT a.id, a.id_events, a.fid, b.eid, b.status, b.originator FROM messages AS a INNER JOIN events AS b ON a.id_events = b.id WHERE b.eid = 177;
/**
...
{
#define SQL_GET_EVENT "SELECT a.id, a.id_events, a.fid, b.eid, b.status, b.originator FROM messages AS a INNER JOIN events AS b ON a.id_events = b.id WHERE b.id = '%lu'"
...
#define COLUMN_EID(x)	          ((struct _h_type_int *)result.data[x][3].t_data)->value
#define COLUMN_STATUS(x)	      ((struct _h_type_int *)result.data[x][4].t_data)->value
#define COLUMN_USERID_ORIGINATOR(x)    ((struct _h_type_int *)result.data[x][5].t_data)->value

  struct _h_result result;

  char *sql_query_str = mdsprintf(SQL_GET_EVENT, event_descriptor_ptr_out->ufsrv_event.gid);

#if __UF_TESTING
  syslog(LOG_DEBUG, "%s {th_ctx:'%p', gid:'%lu'}: GENERATED SQL QUERY: '%s'", __func__, THREAD_CONTEXT_PTR, event_descriptor_ptr_out->ufsrv_event.gid, sql_query_str);
#endif

  int sql_result = h_query_select(THREAD_CONTEXT_DB_BACKEND, sql_query_str, &result);

  if (sql_result != H_OK) {
    syslog(LOG_DEBUG, "%s {th_ctx:'%p', gid:'%lu'}: ERROR: COULD EXECUTE QUERY: '%s'", __func__, THREAD_CONTEXT_PTR, event_descriptor_ptr_out->ufsrv_event.gid, sql_query_str);
  ...
  //we should ever only find 1 or zero really
  if (result.nb_rows > 0) {
    event_descriptor_ptr_out->ufsrv_event.eid = COLUMN_ID_EVENTS(0);
    event_descriptor_ptr_out->ufsrv_event.ctx_id = COLUMN_FID(0);
    event_descriptor_ptr_out->status = COLUMN_STATUS(0);
    event_descriptor_ptr_out->userid_originator = COLUMN_USERID_ORIGINATOR(0);
...
#ifdef __UF_TESTING
    syslog(LOG_DEBUG, "%s {th_ctx:'%p', gid:'%lu'}: ERROR: COULD RETRIEVE MESSAGE", __func__, THREAD_CONTEXT_PTR, event_descriptor_ptr_out->ufsrv_event.gid);
#endif
  }
...
int
DbBackendUpdateEventStatus(unsigned long event_rowid, enum EventStatus event_status)
{
//#define SQL_UPDATE_EVENT_STATUS "INSERT INTO events (id) VALUES ('%lu') ON DUPLICATE KEY UPDATE status = '%d'"
#define SQL_UPDATE_EVENT_STATUS "UPDATE events SET status = '%d' WHERE id = '%lu'"

  char *sql_query_str = mdsprintf(SQL_UPDATE_EVENT_STATUS, event_status, event_rowid);
  ...
  if (sql_result != H_OK) {
    syslog(LOG_DEBUG, "%s (pid:'%lu', th_ctx:'%p'): ERROR: COULD NOT EXECUTE UPDATE STATEMENT: '%s'", __func__, pthread_self(), THREAD_CONTEXT_PTR, sql_query_str);
  }
