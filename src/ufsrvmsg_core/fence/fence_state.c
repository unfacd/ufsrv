...
/**
 * @file fence_state.c
 * @brief User settable set of fence preferences that are only relevant for the user's state, not the fence as an entity.
 */
...
#include <user/user_preferences.h>
...
static UserPreferenceDescriptor *_SetFenceUserPreferenceBoolean(PairedSessionFenceState *paired_ptr, UserPreferenceDescriptor *pref_ptr_in, PrefsStore pref_store, UfsrvEvent *event_ptr);
...
static inline bool	_GetBooleanPrefByOffset(FenceStateDescriptor *fence_state_ptr, FenceUserPrefsOffsets pref_offset);
static inline void	_SetBooleanPrefByOffset(FenceStateDescriptor *fence_state_ptr, FenceUserPrefsOffsets pref_offset, bool value);
...
static const UserPreferenceDescriptor fence_prefs_table[] = {
		...
		{.pref_id=0,										            .pref_name="", 									          .pref_value_type=PREFVALUETYPE_INVALID, .value={0}, NULL,  	 																			 NULL, .pref_ops=NULL}//leave this trap
};
...
{
	return _GetBooleanPrefByOffset(paired_ptr->fence_state_ptr, offset);
}
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
		...
		for (size_t i=0; i<redis_ptr->elements; i++) {//must be fully aligned with pref_table order
			if (GetUserPreferenceDescriptorById(&user_fence_prefs_table, (UserPrefsOffsets)i, &pref)) {
				...
				_SetBooleanPrefByOffset(paired_ptr->fence_state_ptr, (FenceUserPrefsOffsets)i, bool_value);
			}
		}
	}
...
CacheBackendGetAllFenceUserPreferencesByJson(Session *sesn_ptr, unsigned long userid)
{
  ...
  CacheBackendGetFencesListSizeForUser(SESSION_USERID(sesn_ptr));

	if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA)	fences_list_sz = (uintptr_t)THREAD_CONTEXT_UFSRV_RESULT_USERDATA;
	if (fences_list_sz > 0) {
...
_SetFenceUserPreferenceBoolean(PairedSessionFenceState *paired_ptr, UserPreferenceDescriptor *pref_ptr_in, PrefsStore pref_store, UfsrvEvent *event_ptr)
{
	...
	_SetBooleanPrefByOffset(paired_ptr->fence_state_ptr, pref_ptr_in->pref_id, pref_ptr_in->value.pref_value_bool);

  RegisterUfsrvEvent(paired_ptr->session_ptr, (EnumEventType)MSGCMD_SESSION, 0, NULL, event_ptr); //todo: set session event instance type
...
_GetFenceUserPreferenceBoolean(PairedSessionFenceState *paired_ptr,  UserPreferenceDescriptor *pref_ptr_in, PrefsStore pref_store, UserPreferenceDescriptor *pref_ptr_out)
{
	...
	{
		case PREFSTORE_MEM:
			pref_ptr_in->value.pref_value_bool	= _GetBooleanPrefByOffset(paired_ptr->fence_state_ptr, pref_ptr_in->pref_id);
			return pref_ptr_in;
 ...
 */
static inline bool
_GetBooleanPrefByOffset(FenceStateDescriptor *fence_state_ptr, FenceUserPrefsOffsets pref_offset)
{
...
_SetBooleanPrefByOffset(FenceStateDescriptor *fence_state_ptr, FenceUserPrefsOffsets pref_offset, bool value)
{
...
_ProcessIntraMessageCommandForFenceUserPref(Session *sesn_ptr, FenceUserPreference *sesn_msg_pref_ptr, SessionMessage *sesn_msg_ptr)
{
	...
	if (IS_EMPTY((instance_fstate_ptr = IsUserMemberOfFenceByFenceId(&SESSION_FENCE_LIST(sesn_ptr),
                                                                   sesn_msg_ptr->fences[0]->fid, false)))) {
  ...
  FenceStateDescriptor *fence_state_ptr = FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
	...
	if (GetUserPreferenceDescriptorById(&user_fence_prefs_table, (UserPrefsOffsets)sesn_msg_pref_ptr->pref_id, &pref)) {
...
UFSRVResult *
IsUserAllowedToChangeFenceUserPrefProfileSharing(InstanceContextForSession *ctx_ptr, FenceUserPreference *sesn_msg_pref_ptr, DataMessage *data_msg_ptr_received, UfsrvEvent *event_ptr, CallbackCommandMarshaller command_marshaller)
{
	InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
	...
	if (IS_EMPTY((instance_fstate_ptr = IsUserMemberOfFenceByFenceId(&SESSION_FENCE_LIST(sesn_ptr),
                                                                   command_ptr->fences[0]->fid, false)))) {
  ...
  FenceStateDescriptor 	*fence_state_ptr = FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
	...
	if (GetUserPreferenceDescriptorById(&user_fence_prefs_table, (UserPrefsOffsets)sesn_msg_pref_ptr->pref_id, &pref)) {
        ...
        _INVOKE_COMMAND_MARSHALLER(command_marshaller, ctx_ptr, &share_list_ctx, data_msg_ptr_received, NULL, event_ptr);
		  }
			_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_NULL_POINTER)
		}
	}
