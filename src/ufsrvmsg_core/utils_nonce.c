 ...
 * 	@param sesn_ptr_this: represents the currently in-service session that is invoking the command. If Null, the request is
...
BackEndGenerateNonce(Session *sesn_ptr_carrier, time_t expiry_in, const char *prefix, const char *value)
{
  ...
  if (strlen(nonce_hashed) == 0) {
    syslog(LOG_DEBUG, "%s (pid:'%lu' cid='%lu'): ERROR: COULD NO GENERATE SHA-1 for NONCE:'%s' for COUNTER:'%lld'", __func__, pthread_self(), SESSION_ID(sesn_ptr_carrier), nonce_unhashed, nonce_counter);
