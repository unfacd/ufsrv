...
typedef struct ShareListContextData {
    ...
    ShareListItemKeyStoreValue *key_store_value;
} ShareListContextData;

typedef struct HopscotchHashtable * (*ShareListInitialiserCallback)(struct HopscotchHashtable *, size_t);
...
typedef struct ShareListTypeOps {
  ShareListInitialiserCallback            initialiser_callback;
...
#define REDIS_CMD_SHARELIST_ADD "ZADD SHL_%i:%lu %lu %lu"
//list type enum, uid, uid(of allowed user)
#define REDIS_CMD_SHARELIST_REM "ZREM SHL_%i:%lu %lu"
#define REDIS_CMD_SHARELIST_GETALL "ZRANGE SHL_%i:%lu 0 -1"
#define REDIS_CMD_SHARELIST_SIZE "ZCARD SHL_%i:%lu"
#define REDIS_CMD_SHARELIST_REM_ALL	"DEL SHL_%i:%lu"

//As per the comment above, this the shared list, which contains all users who shared their permissions with this
//user. unlike sharing, this list contains all sharelist types. It is not loaded into Sessions memory.
//%uid, %uid_other, %type_enum
#define REDIS_CMD_SHAREDLIST_ADD "ZADD SHDL_%lu 0 %lu:%i"
#define REDIS_CMD_SHAREDLIST_REM "ZREM SHDL_%lu %lu:%i"
#define REDIS_CMD_SHAREDLIST_GETALL "ZRANGE SHDL_%lu 0 -1"
#define REDIS_CMD_SHAREDLIST_REM_ALL	"DEL SHDL_%lu"
...
ShareListInitialiserCallback GetShareListInitialiser(enum EnumShareListType type);
...
UFSRVResult *CacheBackendGetShareList(ShareList *shlist_ptr, unsigned long userid);
UFSRVResult *CacheBackendGetSharedList(Session *sesn_ptr);
UFSRVResult *CacheBackendGetShareListAndCheckUser(const ShareList *shlist_ptr, unsigned long userid_host, unsigned long userid_to_check);
UFSRVResult *CacheBackendGetShareListSize(Session *sesn_ptr, ShareList *shlist_ptr);
UFSRVResult *InstateShareListForUsers(Session *sesn_ptr, ShareList *shlist_ptr, bool flag_init);
UFSRVResult *InstateShareListForItems(Session *sesn_ptr, ShareList *shlist_ptr, bool flag_init);
UFSRVResult *LoadInitialiseShareListForUsersIfNecessary(Session *sesn_ptr, ShareList *shlist_ptr);
UFSRVResult *LoadInitialiseShareListForItemsIfNecessary(Session *sesn_ptr, ShareList *shlist_ptr);
bool IsUserOnShareListProvided(Session *sesn_ptr_target, ShareList *shlist_ptr);
bool IsItemOnShareListProvided(ShareList *shlist_ptr, ShareListItemKeyStoreValue *shlist_item);
UFSRVResult *AddUserToShareList(Session *sesn_ptr, ShareList *shlist_ptr, InstanceHolderForSession *instance_sesn_ptr_target, unsigned long sesn_call_flags);
UFSRVResult *AddItemToShareList(Session *sesn_ptr, ShareList *shlist_ptr, ShareListItemKeyStoreValue *item_key_value, unsigned long sesn_call_flags);
...
UFSRVResult *RemoveItemFromShareList(Session *sesn_ptr, ShareList *shlist_ptr, ShareListItemKeyStoreValue *item_key_value, unsigned long sesn_call_flags);
...
UFSRVResult *CacheBackendClearSessionSharingLists(Session *sesn_ptr, unsigned long call_flags);
...
inline static bool IsShareLisInitialisedBlockedFence(const Session *sesn_ptr) {return SESSION_LISTS_INIT_STATE_BLOCKED_FENCE(sesn_ptr);}
...
inline static void SetShareListInitialisedBlockedFence(Session *sesn_ptr, bool value) {SESSION_LISTS_INIT_STATE_BLOCKED_FENCE(sesn_ptr)=value;}
