...
#define SESNSTATUS_CONNECTION_LISTENER (0x1UL<<25UL)//session is of type connection listener

typedef InstanceHolder InstanceHolderForSession;
#define AS_INSTANCE_HOLDER_FOR_SESSION(x, y) InstanceHolderForSession *x = (InstanceHolderForSession *)(y)
...
//Inform seesion handler for on_read/on_write whether invocation is related to pre or post event
enum IOHandlerEventStage {
  PRE_HANDLER = 1, //before event happened
  POST_HANDLER //after event happened
};
...
#define AS_PROTOCOL_SESSION_DATA(x) ((ProtocolSessionData *)(x))
...
typedef struct MessageQueue MessageQueue;
 ...
 typedef struct Session {
  ...
  unsigned long net_address_hash; //hash of ip:port for source, connecting end
  Socket *dsptr; /* destination socket */
  Socket *ssptr; /* source socket */

  //TODO: phase out and migrate into thread context
  ...
  //session has the option of specifying per-session handlers for i/o polling events, which called from within protocols
  struct {
    int (*on_read)(InstanceHolder *, enum IOHandlerEventStage);
    int (*on_write)(InstanceHolder *, enum IOHandlerEventStage);
  } io_event_handlers;
...
typedef struct InstanceContextForSession {
  ...
  Session *sesn_ptr;
  bool lock_already_owned,
       is_locked;
} InstanceContextForSession;

#define AS_INSTANCE_CONTEXT_FOR_SESSION(x) ((InstanceContextForSession *)(x))
...
#define AS_CARRIER_SESSION(x)                 (x) //for semantic clarity
...
#define SESSION_LISTS_INIT_STATE_BLOCKED_FENCE(x)	(SESSION_LISTS_INIT_STATE(x).blocked_fence)
...
#define SESSION_USERGUARDIAN_UID(x)	((x)->sservice.user.user_details.user_preferences.guardian_uid)
...
#define SESSION_PROTOCOL_SESSION_DATA(x) ((x)->protocol_session_data)
...
#define SESSION_NETADDRESS_HASH(x)	(x)->net_address_hash
...
#define SESSION_HOMEBASE_GEOLOC_DYNAMIC(x)						(x)->sservice.user.user_details.homebase_geoloc_dynamic
#define SESSION_USERPREF_HOMEBASE_GEOLOC(x)				(x)->sservice.user.user_details.user_preferences.homebase_geoloc
#define SESSION_USERPREF_BASELOC_ZONE(x)				(x)->sservice.user.user_details.user_preferences.baseloc_anchor_zone
#define SESSION_USERPREF_GEOLOC_TRIGGER(x)				(x)->sservice.user.user_details.user_preferences.geoloc_trigger
...
#define SESSION_USERPREF_SHLIST_BLOCKED_FENCE(x)	((x)->sservice.user.user_details.user_preferences.sharelist_blocked_fence)
#define SESSION_USERPREF_SHLIST_BLOCKED_FENCE_PTR(x)	&(SESSION_USERPREF_SHLIST_BLOCKED_FENCE((x)))
...
#define SESSION_EVENT_HANDLER_ON_READ(x)	((x)->io_event_handlers.on_read)
#define SESSION_EVENT_HANDLER_ON_WRITE(x)	((x)->io_event_handlers.on_write)
