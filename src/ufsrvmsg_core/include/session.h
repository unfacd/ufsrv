...
#include "../type_providers/session_provider.h"
...
#define CALL_FLAG_LOCATION_BY_SERVER									(0x1UL<<30UL)
...
REDIS_KEY_USER_HADDRESS, 				REDIS_KEY_USER_BASELOC, /* automatically calculated by backend */
...
REDIS_KEY_HOME_BASELOC, /* set by user */					REDIS_KEY_USER_AVATAR,
...
unsigned long GenerateSessionId(int protocol_id, UFSRVResult *res_ptr, ClientContextData *context_data);
unsigned long GenerateSessionIdGlobally(void);
unsigned long GenerateSessionIdLocally(void);
Session *SessionGet(int protocol_id, unsigned call_flas);
Session *InstantiateSessionObject(Socket *ssptr, Socket *dsptr, unsigned call_flags, int protocol_id);
type_instantiator GetSessionInstantiator();
int InstantiateSessionObject2(Session **sesn_ptr_in, unsigned long call_flags, int protocol_id);
...
UfsrvUid *GetUfsrvUid(unsigned long user_id, UfsrvUid *uid_ptr_out, bool flag_by_ref, bool *flag_is_local);
...
int CheckSessionIdleTime(ScheduledJob *, void *);
...
UFSRVResult *GetSessionForThisUserByUserId(Session *sesn_ptr, unsigned long uid, bool *lock_state, unsigned long call_flags_sesn);
InstanceHolderForSession *SessionInstantiateFromCacheBackendWithDbFallback(Session *sesn_ptr_this, const UfsrvUid *ufsrvuid, unsigned long sesn_call_flags);
...
UFSRVResult *CacheBackendGetRawSessionRecordByUserId(unsigned long user_id, unsigned long call_flags, UFSRVResult *res_ptr_in);
UfsrvUid *CacheBackendInstateUfsrvUid(unsigned long user_id, unsigned long sesn_call_flags, UfsrvUid *uid_ptr_out);
...
UFSRVResult *DbBackendInstantiateSession(Session *sesn_ptr_transient, const UfsrvUid *ufrsvuid);
  ...
  */
  ...
  SessionLockRDCtx(ThreadContext *thread_ctx_ptr, Session *sesn_ptr, int try_flag)
  {
    ...
    if (try_flag) {
      ...
      if (lock_state == 0) {
        ...
        syslog(LOG_DEBUG, "%s (pid:'%lu', cid:'%lu', o:'%p', ctx:'%p'): ERROR: COULD NOT acquire TRY-READ lock for Session events (errno='%d'):  '%s'", __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0, errno, err_str);
      }
    } else {
      ...
      if (lock_state == 0) {
        ...
        char *err_str = thread_error(errno);
        syslog(LOG_DEBUG, "%s (pid='%lu' cid='%lu' o='%p): ERROR: COULD NOT acquire READ lock for Session events (errno='%d'):  '%s'", __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, errno, err_str);
      }
    }
  ...
  SessionLockRWCtx(ThreadContext *thread_ctx_ptr, Session *sesn_ptr, int try_flag, const char *func)
  {
    ...
    if (try_flag) {
      ...
      if (lock_state == 0) {
  #if defined  (__UF_FULLDEBUG_LOCKING) || defined(__UF_FULLDEBUG)
        syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu' o:'%p', ctx:'%p', try:1, func:'%s'): SUCCESS: TRY-WRITE/READ lock for Session events acquired...", __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0L, func);
  #endif
        if (likely(IS_PRESENT(thread_ctx_ptr))) PutIntoLockedObjectsStore(thread_ctx_ptr->ht_ptr, (void *)sesn_ptr);
        ...
        syslog(LOG_DEBUG, "%s (pid:'%lu', cid:'%lu', o:'%p', ctx:'%p', try:1, func:'%s'): ERROR: COULD NOT acquire TRY-WRITE/READ lock for Session events (errno='%d'): '%s'", __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0, func, errno, err_str);
      }
    } else {
      lock_state = pthread_rwlock_wrlock(&(sesn_ptr->session_events.rwlock));
      if (lock_state == 0) {
  #if defined  (__UF_FULLDEBUG_LOCKING) || defined(__UF_FULLDEBUG)
        syslog(LOG_DEBUG, "%s (pid:'%lu', cid:'%lu', o:'%p', ctx:'%p', func:'%s'): SUCCESS: WRITE/READ lock for Session events acquired...", __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0L, func);
  #endif
        if (IS_PRESENT(thread_ctx_ptr)) PutIntoLockedObjectsStore(thread_ctx_ptr->ht_ptr, (void *)sesn_ptr);
        ...
        syslog(LOG_DEBUG, "%s (pid:'%lu', cid:'%lu', o:'%p', ctx:'%p', func:'%s'): ERROR: COULD NOT acquire WRITE/READ lock for Session events (errno='%d'): '%s'",
            __func__, pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0, func, errno, err_str);
      }
  }
...
#endif
      //todo there is a potential race condition here, as the session object lock could be acquired for writing and object written to
      //as we perform comparison on object presence (via address equality check).
      //we'll consider this as harmless for now, hence the __attribute__ (( no_sanitize_thread )) for the function below
      //alternatively, the unlock should be placed after object store removal op
      if (IS_PRESENT(thread_ctx_ptr)) RemoveFromLockedObjectsStore(thread_ctx_ptr->ht_ptr, (void *)sesn_ptr);
      _RETURN_RESULT_RES(res_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_LOCKED)
    } else {
      char *err_str = thread_error(errno);
      syslog(LOG_DEBUG, "SessionUnLock: (pid:'%lu', cid:'%lu', o:'%p', ctx:'%p', func:'%s'): ERROR: COULD NOT RELEASE WRITE/READ lock for Session events (errno='%d'): '%s'", pthread_self(), SESSION_ID(sesn_ptr), sesn_ptr, IS_PRESENT(thread_ctx_ptr)?thread_ctx_ptr:0, func, errno, err_str);
    }
...
//----------------------------
  ...
  static  inline int
  MessageQueueLock(Session *sesn_ptr, MessageQueue *msg_que_ptr, int try_flag)
  {
    int lock_state;
    ...
    if (try_flag) {
      ...
      if (lock_state == 0) {
#if defined  (__UF_FULLDEBUG_LOCKING) || defined(__UF_FULLDEBUG)
        syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu'  o:'%p'}: SUCCESS: MUTEX TRY-LOCK for MessageQueue events acquired...", __func__, pthread_self(), 0L, SESSION_ID(sesn_ptr), sesn_ptr);
#endif
      } else {
        char *err_str = thread_error(errno);
        syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu'  o:'%p'}: ERROR: COULD NOT acquire MUTEX lock for MessageQueue (errno='%d'):  '%s'", __func__, pthread_self(), 0UL, SESSION_ID(sesn_ptr), sesn_ptr, errno, err_str);
      }
    } else {
      lock_state = pthread_mutex_lock(&(msg_que_ptr->mutex));
      if (lock_state == 0) {
#if defined  (__UF_FULLDEBUG_LOCKING) || defined(__UF_FULLDEBUG)
        syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu'  o:'%p'}: SUCCESS:  MUTEX lock for MessageQueue acquired...", __func__, pthread_self(), 0L, SESSION_ID(sesn_ptr), sesn_ptr);
#endif
      } else {
        char *err_str = thread_error(errno);
        syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu'  o:'%p'}: ERROR: COULD NOT acquire MUTEX lock for MessageQueue (errno='%d'): '%s'", __func__, pthread_self(), 0UL, SESSION_ID(sesn_ptr), sesn_ptr, errno, err_str);
  ...
  static inline int
  MessageQueueUnLock(Session *sesn_ptr, MessageQueue *msg_que_ptr)
  {
    int lock_state = pthread_mutex_unlock(&(msg_que_ptr->mutex));
    if (lock_state == 0) {
#if defined  (__UF_FULLDEBUG_LOCKING) || defined(__UF_FULLDEBUG)
      syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu'  o:'%p'}: SUCCESS: RELEASED MUTEX lock for MessageQueue...", __func__, pthread_self(), 0L, SESSION_ID(sesn_ptr), sesn_ptr);
#endif
    } else {
      char *err_str = thread_error(errno);
      syslog(LOG_DEBUG, "%s: {pid'%lu', th_ctx:'%lu', cid:'%lu', o:'%p'}: ERROR: COULD NOT RELEASE MUTEX lock for MessageQueue (errno='%d'):  '%s'", __func__, pthread_self(), 0UL, SESSION_ID(sesn_ptr), sesn_ptr, errno, err_str);
    }
