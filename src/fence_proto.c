...
_LoadPermissionList (PermissionProtoBuilderExecutorContext *ctx_ptr, ClientContextData  *userid_container)
{
	...
	Session *sesn_ptr = userid_container;
...
static void
_MakeFencePermissionsUserListsInProto(Session *sesn_ptr, Fence *f_ptr, FencePermission *permission_ptr,
																			FenceRecord__Permission *fence_record_permission)
{
...
#include <fence_state.h> //for FPREF_LAST_ALIGNMENT
 ...
 */
FenceRecord *
MakeFenceRecordInProto (Session *sesn_ptr, Fence *f_ptr, FenceRecord *fence_record_ptr_in)
{
	if (unlikely(IS_EMPTY(sesn_ptr) || IS_EMPTY(f_ptr)))	return NULL;
  ...
  fence_record_ptr->eid					=	FENCE_FENECE_EVENTS_COUNTER(f_ptr); fence_record_ptr->has_eid=1;
	...
	UfsrvUid uid = {0};
  if (IS_PRESENT(GetUfsrvUid(sesn_ptr, FENCE_OWNER_UID(f_ptr), &(uid), false, NULL))) {
  ...
  MakeFencePermissionsInProto(sesn_ptr, f_ptr, fence_record_ptr);
	if (IS_STR_LOADED(FENCE_AVATAR(f_ptr)))    MakeFenceAvatarInProto (sesn_ptr, fence_record_ptr, FENCE_AVATAR(f_ptr), &attach_descriptor, NULL);

	desc_ptr = MakeFenceMembersInProto (sesn_ptr,  f_ptr, CALL_FLAG_SESSION_LIST_INCLUDE_REMOTE, 0);//dontlock fence
	if (desc_ptr) {
	...
	desc_ptr = MakeFenceInviteListInProto(sesn_ptr, f_ptr, 0/*@call_flags: not locking fence*/);//fence should be setup with invite list by now
	if (desc_ptr) {
		if (desc_ptr->records_sz>0) {
...
MakeFenceInviteListFromNamesInProto (Session *sesn_ptr, char **members_invited, size_t members_sz, unsigned call_flags)
{
	...
	{
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p'}: NOTICE: Received empty Invite Members List.",
				__func__, pthread_self(), sesn_ptr);
#endif
	...
	{
		urec_ptr_aux=MakeUserRecordFromUsernameInProto (sesn_ptr, members_invited[i], NULL, PROTO_USER_RECORD_MINIMAL);
		if (urec_ptr_aux)	user_records[processed_counter]=urec_ptr_aux;//connect to the array
...
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p'}: FINISHED PROTO representation for Invited members list. User count '%lu'",
				__func__, pthread_self(), sesn_ptr, desc_ptr->records_sz);
#endif
	...
	{
		//free (user_rec_ptr);//not applicable
		free (user_records);
		free (desc_ptr);
 ...
 */
FenceUserRecordsDescription  *
MakeFenceMembersInProto (Session *sesn_ptr_this,  Fence *f_ptr, unsigned long sesn_call_flags, unsigned long fence_call_flags)
{
	bool fence_lock_already_owned = false;
...
#if 0
  //this clears the lazy bit
  if (GetUsersListCacheRecordForFence (sesn_ptr_this, f_ptr, FENCE_CALLFLAG_ATTACH_USER_LIST_TO_FENCE))
  {
    syslog(LOG_DEBUG, "%s {pid:'%lu', fid:'%lu', users_sz:'%d'}: Loaded Fence's user list from backend", __func__, pthread_self(),
      FENCE_ID(f_ptr), FENCE_USERS_COUNT(f_ptr));
  }
  else
  {
...
#endif
  GetMembersListCacheRecordForFence (sesn_ptr_this, InstanceHolderFromClientContext(f_ptr), 0, UNSPECIFIED_FENCE_LISTTYPE, UNSPECIFIED_FENCE_LISTTYPE, FENCE_CALLFLAG_ATTACH_USER_LIST_TO_FENCE);
 ...
 */
void
DestructFenceRecordsProto (FenceRecord **fence_records_ptr, unsigned count, bool flag_self_destruct)
{
	if (fence_records_ptr && count)
	{
		...
		free (fence_records_ptr[0]);//assuming vector...
  ...
  if ((collection_attachments->collection_sz > 0) && IS_PRESENT(collection_attachments->collection[0])) {
    ...
    for (size_t i=0; i<collection_attachments->collection_sz; i++) {
      ...
      if (IS_PRESENT((GetAttachmentDescriptorEphemeral(sesn_ptr, attachment_record_ptr->id, false, attachment_descriptor_ptr)))) {
        syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', blocb_id:'%s'}: ERROR: ATTACHMENT ID ALREADY EXISTS", __func__, pthread_self(), sesn_ptr, attachment_record_ptr->id);

        collection_attachments_out->collection[i] = NULL; //slot is unavailble
