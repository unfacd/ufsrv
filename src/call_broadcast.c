...
inline static int _VetrifyCallCommandForIntra	(MessageQueueMessage *mqm_ptr, bool flag_free_unpacked);


int
HandleIntraBroadcastForCall (MessageQueueMsgPayload *mqp_ptr, MessageQueueMessage *mqm_ptr, UFSRVResult *res_ptr, unsigned long call_flags)
{
	int				rc					= 0;
	long long timer_start	=	GetTimeNowInMicros();
	long long timer_end;

	if ((rc=_VetrifyCallCommandForIntra(mqm_ptr, false))<0)	goto return_final;
  ...
  Session *sesn_ptr_local_user = SessionOffInstanceHolder(instance_sesn_ptr_local_user);
	...
	CommandCallbackControllerCallCommand (sesn_ptr_local_user, NULL, mqm_ptr->wire_data, mqp_ptr);
 ...
 */
inline static int
_VetrifyCallCommandForIntra	(MessageQueueMessage *mqm_ptr, bool flag_free_unpacked)
{
	int rc=1;

	if (unlikely(IS_EMPTY((mqm_ptr->wire_data->ufsrvcommand->callcommand))))				goto return_error_callcommand_missing;
	if (unlikely(IS_EMPTY(mqm_ptr->wire_data->ufsrvcommand->callcommand->header)))	goto return_error_commandheader_missing;
	if (unlikely(IS_EMPTY(mqm_ptr->wire_data->ufsrvcommand->callcommand->fence)))		goto return_error_missing_fence_definition;
