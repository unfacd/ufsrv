 ...
 */
UFSRVResult *
CacheBackendGetShareList (Session *sesn_ptr,  ShareList *shlist_ptr)
{
	...
	if (!(redis_ptr=(*pers_ptr->send_command)(sesn_ptr, SHARELIST_GETALL, shlist_ptr->list_type, SESSION_USERID(sesn_ptr)))) goto return_redis_error;
...
{
	CacheBackendGetShareList (sesn_ptr,  shlist_ptr);
 ...
 */
void
InvokeShareListIteratorExecutor (Session *sesn_ptr, ShareList *shlist_ptr, CallbackExecutor executor, ClientContextData *ctx_ptr, bool is_initialise)
{
	if (is_initialise) {
    LoadInitialiseShareListIfNecessary(sesn_ptr, shlist_ptr);
	}

  hopscotch_iterator_executor (&(shlist_ptr->hashtable), executor, ctx_ptr);
}
