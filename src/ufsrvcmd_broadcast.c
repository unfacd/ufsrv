...
static UfsrvCommandBroadcast ufsrvcmd_broadcasts[UFSRV_MAX_BROADCAST_ID] = {
		{UFSRV_SESSION, 				_INTERCOMMAND_SESSION, 		{DEFAULT_CALLFLAGS_VERFIER_INTER, HandleInterBroadcastForSession, 		_VerifyInterMessageQueueCommand}		},
		{UFSRV_FENCE, 					_INTERCOMMAND_FENCE,			{DEFAULT_CALLFLAGS_VERFIER_INTER, HandleInterBroadcastForFence,				_VerifyInterMessageQueueCommand}		},
		{UFSRV_MSG,							_INTERCOMMAND_MSG,				{DEFAULT_CALLFLAGS_VERFIER_INTER, HandleInterBroadcastForUserMessage, _VerifyInterMessageQueueCommand}		},
		{UFSRV_LOC,							_INTERCOMMAND_LOC,				{DEFAULT_CALLFLAGS_VERFIER_INTER, HandleInterBroadcastForLocation,		_VerifyInterMessageQueueCommand}		},
		{UFSRV_USER,						_INTERCOMMAND_USER,				{DEFAULT_CALLFLAGS_VERFIER_INTER, HandleInterBroadcastForUser,				_VerifyInterMessageQueueCommand}		},
		{UFSRV_CALL,						_INTERCOMMAND_CALL,				{0, 															NULL,																NULL}																},
		{UFSRV_SYS,							_INTERCOMMAND_SYS,				{0, 															NULL,																NULL}																},
		{UFSRV_RECEIPT,					_INTERCOMMAND_RECEIPT,		{0, 															NULL,																NULL}																},
		{UFSRV_SYNC,						_INTERCOMMAND_SYNC,				{0, 															NULL,																NULL}																},
//------------------------------------------- SYMMETRY LINE ------------------------------------------------------------//
		{UFSRV_INTRA_SESSION,		_INTRACOMMAND_SESSION,		{DEFAULT_CALLFLAGS_VERFIER_INTRA,	HandleIntraBroadcastForSession,			_VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_FENCE,			_INTRACOMMAND_FENCE,			{DEFAULT_CALLFLAGS_VERFIER_INTRA, HandleIntraBroadcastForFence,				_VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_MSG,				_INTRACOMMAND_MSG,				{DEFAULT_CALLFLAGS_VERFIER_INTRA, HandleIntraBroadcastForUserMessage,	_VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_LOC,				_INTRACOMMAND_LOC,				{0, 															NULL,																NULL}																},
		{UFSRV_INTRA_USER,			_INTRACOMMAND_USER,				{DEFAULT_CALLFLAGS_VERFIER_INTRA, HandleIntraBroadcastForUser,				_VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_CALL,			_INTRACOMMAND_CALL,				{DEFAULT_CALLFLAGS_VERFIER_INTRA, HandleIntraBroadcastForCall,			  _VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_RECEIPT,		_INTRACOMMAND_RECEIPT,		{DEFAULT_CALLFLAGS_VERFIER_INTRA, HandleIntraBroadcastForReceipt,			  _VerifyIntraMessageQueueCommand}		},
		{UFSRV_INTRA_SYNC,			_INTRACOMMAND_SYNC,				{DEFAULT_CALLFLAGS_VERFIER_INTRA, NULL,			  NULL}		},
		{UFSRV_INTRA_SYS,				_INTRACOMMAND_SYS,				{0, 															NULL,																NULL}																},
...
_HandleStagedEncodedIntraMessage (redisReply *redis_ptr_staged_message, size_t reqid_target, bool *reqid_target_found)
{
	...
	rc=ufsrvcmd_broadcasts[command_type].ops.broacast_verifier(&mqp_staged, &mqm_ptr_staged, ufsrvcmd_broadcasts[command_type].ops.verfier_callflags);
	if (rc==MSGQUE_SUCCESS) {
		rc=ufsrvcmd_broadcasts[command_type].ops.broacast_handler(&mqp_staged, mqm_ptr_staged, &res, 0);

		//lucky strike!
		if (reqid_target==reqid_parsed) {
			*reqid_target_found=true;
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s (pid:'%lu', channle_name:'%s', reqid_target:'%lu', reqid_processed:'%lu'): NOTICE: PROCESSED TARGET REQID", __func__,	pthread_self(), ufsrvcmd_broadcasts[command_type].topic_name, reqid_target, reqid_parsed);
#endif
		} else {
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s (pid:'%lu', channle_name:'%s', reqid_target:'%lu', reqid_processed:'%lu'): NOTICE: PROCESSED OTHER REQID", __func__,	pthread_self(), ufsrvcmd_broadcasts[command_type].topic_name, reqid_target, reqid_parsed);
#endif
		}
	...
	syslog(LOG_ERR, "%s (pid:'%lu', channle_name:'%s', verification_rc:'%d'): ERROR: COMMADN VERIFICATION ", __func__,	pthread_self(), ufsrvcmd_broadcasts[command_type].topic_name, rc);
...
_HandleInterBroadcast (MessageQueueMsgPayload *mqp_ptr, MessageQueueMessage *mqm_ptr)
{
	...
	rc=ufsrvcmd_broadcasts[mqm_ptr->command_type].ops.broacast_verifier(mqp_ptr, &mqm_ptr, ufsrvcmd_broadcasts[mqm_ptr->command_type].ops.verfier_callflags);
	if (rc == MSGQUE_SUCCESS) {
		rc = ufsrvcmd_broadcasts[mqm_ptr->command_type].ops.broacast_handler(mqp_ptr, mqm_ptr, &res, 0);
	}
...
int WorkerThreadMsgQueueParserExecutor (MessageContextData *context_ptr)
{
	...
	if (mqm_ptr->has_broadcast_semantics &&
		...
		if ((_VerifyBroadcastTargeting(mqm_ptr, ufsrvcmd_broadcasts[mqm_ptr->command_type].ops.verfier_callflags)) != MSGQUE_SUCCESS)	goto return_final_no_timing;

		return (_HandleInterBroadcast(mqp_ptr, mqm_ptr));
	}

	intra_handler_block:
	if ((_VerifyBroadcastTargeting(mqm_ptr, ufsrvcmd_broadcasts[mqm_ptr->command_type+UFSRVCMDID_INTER_INTRA_CONVERSION_FACTOR].ops.verfier_callflags))!=MSGQUE_SUCCESS)	goto return_final_no_timing;
  ...
  InstanceHolderForSession *instance_sesn_ptr_localuser;
	...
	if (IS_PRESENT((instance_sesn_ptr_localuser = LocallyLocateSessionById(cmd_header_ptr->cid)))) {
	  sesn_ptr_localuser = SessionOffInstanceHolder(instance_sesn_ptr_localuser);
		...
		__unused bool lock_already_owned = (_RESULT_CODE_EQUAL(THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT), RESCODE_PROG_LOCKED_THIS_THREAD));
...
#define SESSION_CALL_FLAGS (CALL_FLAG_LOCK_SESSION|CALL_FLAG_HASH_SESSION_LOCALLY|					\
	...
	if (IS_PRESENT((instance_sesn_ptr_localuser = SessionInstantiateFromBackend(NULL, cmd_header_ptr->cid, SESSION_CALL_FLAGS)))) {
	  sesn_ptr_localuser = SessionOffInstanceHolder(instance_sesn_ptr_localuser);
