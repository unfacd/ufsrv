...
#include <session_type.h>//remove
#include <include/instance_type.h>
...
inline static int RecycledSessions_RdLock (unsigned);
inline static int RecycledSessions_WrLock (unsigned);
inline static void RecycledSessions_UnLock (void);

//when we allocate types we allocate by factor of that many at once
#define _RECYECLER_INIT_TYPE_COUNT 10
...
static RecyclerPoolHandle *_TypePoolExpandAllocationGroup (RecyclerPoolDefinition *pool_ptr);
 ...
 */
RecyclerPoolHandle *
RecyclerInitTypePool (const char *typename, size_t blocksz, size_t expansion_threshold, RecyclerPoolOps *pool_ops_ptr)
{
	...
	if (recycler_ptr->count_typeslots == recycler_ptr->count_types) {
		...
		expanded_recylers = realloc(recycler_ptr->recyclers, (recycler_ptr->count_typeslots + _RECYECLER_INIT_TYPE_COUNT) * sizeof(RecyclerPoolDefinition *));

		if (expanded_recylers) {
			recycler_ptr->recyclers = expanded_recylers;
			recycler_ptr->count_typeslots += _RECYECLER_INIT_TYPE_COUNT;
	...
	if (pool_ptr) {
		...
		pool_ptr->pool_memory2 = calloc(_CONF_SESNMEMSPECS_ALLOC_GROUPS(masterptr), (sizeof(void *)));
		...
		pool_ptr->allocation_groups = calloc(_CONF_SESNMEMSPECS_ALLOC_GROUPS(masterptr), sizeof(TypePoolAllocationGroup));

		//create the first allocation group, which attaches itself to the first available index in pool_memory
		RecyclerPoolHandle *pool_handle_ptr = _TypePoolExpandAllocationGroup (pool_ptr);
...
_TypePoolExpandAllocationGroup (RecyclerPoolDefinition *pool_ptr)
{
	if	(unlikely((pool_ptr->alocated_groups_sz >= _CONF_SESNMEMSPECS_ALLOC_GROUPS(masterptr)))) {
		syslog (LOG_DEBUG, LOGSTR_RECYCLER_MAX_ALLOC_CAPACITY_REACHED, __func__, pthread_self(),  _CONF_SESNMEMSPECS_ALLOC_GROUPS(masterptr), pool_ptr->expasnion_threshold, LOGCODE_RECYCLER_MAX_ALLOC_CAPACITY_REACHED);
...
RecyclerPoolTypeEnvelop *_DeQueue (RecyclerPoolHandle *pool_handle, TypePoolAllocationGroup *alloc_group_ptr)
{
	if (unlikely(alloc_group_ptr == NULL)) {
		syslog(LOG_DEBUG, LOGSTR_NULL_PARAM, __func__, pthread_self(), LOGCODE_RECYCLER_UNDEFINED_GROUPALLOC, "Undefined AllocationGroup *");
...
RecyclerPut(const unsigned type, InstanceHolder *client_data, ContextData *context_data, unsigned long call_flags)
{
	...
	if (unlikely(IS_EMPTY(pool_ptr))) {
		syslog(LOG_DEBUG, LOGSTR_NULL_PARAM, __func__, pthread_self(), LOGCODE_RECYCLER_UNDEFINED_POOLDEF, "Undefined RecyclerPoolDefinition *");
...
RecyclerGet (const unsigned type, ContextData *context_data, unsigned long call_flags)
{
	...
	//TODO: potentially aggressive revise, but we need to ensure the state of the leased items do not change after we indetified one
	...
	//but we could still operate on a snapsot, becase whenexpansion happens that happens afer the RECYCLER_POOL_ALLOCATED_GROUPS_SZ index we read here so that is OK concurency wise
	pthread_spin_lock(&(pool_ptr->spin_lock));

	//get the first Allocation group that has available, unleased entry
	...
	if (IS_EMPTY(alloc_group_ptr)) {
		//TODO: TRIGGER EXPANSION and check position spinlock for pool below, as we'd need to keep it locked during exapnsion
		...
		if (_TypePoolExpandAllocationGroup(pool_ptr))	goto find_available_allocation_group;
		else	goto exit_error;//unlocks spin_lock
	}

	pthread_spin_unlock(&(pool_ptr->spin_lock));//-1
	...
	else*/
	{
		...
		p = (char *)env_ptr;
		p += sizeof(RecyclerPoolTypeEnvelop);
		client_data = (ClientContextData *)p; //pool allocation slot
    ...
    ListItemInstance *list_item_ptr = AddThisInstanceToList (&(env_ptr->instances_list), client_data);
        ...
        RemoveFromInstancesList (&(env_ptr->instances_list), list_item_ptr);
 ...
 */
ClientContextData *GetClientContextData (InstanceHolder *instance_holder_ptr)
{
	...
	int lock_state;
		...
		if (try_flag)
		{
			...
			else
			{
				...
				syslog(LOG_NOTICE, "%s: ERROR: COULD NOT acquire TRY-READ lock for Session events (errno='%d'):  '%s'",__func__,
						 errno, error_str);
			}
		}
		else
...
RecycledSessions_WrLock (unsigned try_flag)
{
	...
	if (try_flag)
	{
		...
		else
		{
			...
			syslog(LOG_NOTICE, "%s: ERROR: COULD NOT acquire TRY-WR lock for Session events (errno='%d'):  '%s'",
