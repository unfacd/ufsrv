...
//AA+ thsi is now implicitly initialised wne a new session is created, especially the dictionary setup for request object
	#if 0
	onion_request *req;
	...
	syslog(LOG_DEBUG, "Create request %p", req);

	//AA-
	/*if (op){
		if (op->request_init){
			if (op->request_init(req)<0){
				syslog(LOG_DEBUG, "Invalid request, closing");
...
static void unlink_files(void *p, const char *key, const char *value, int flags){
	syslog(LOG_DEBUG, "%s: Unlinking temporal file %s", __func__, value);
	unlink(value);
}
...
void onion_request_free(onion_request *req){
  //ONION_DEBUG0("Free request %p", req);
	...
	//AA+ this cause a carsh under certain not understood confitionds ith slowhttptest tool
	if (req->free_list){
		onion_ptr_list_foreach(req->free_list, free);
		onion_ptr_list_free(req->free_list);
	}

	//AA- we dont free it is not used as pointer. same true for response
	//free(req);
}
  ...
  const char *v=ov;
	...
	do{ // Check all possible sessions
		...
		//	ONION_DEBUG("At -1: %c %d (%p %p)",v[-1],is_alnum(v[-1]),v,ov);
			...
			v+=10;
			r=mystrdup(v); // Maybe allocated more memory, not much anyway.
			char *p=r;
			while (*p!='\0' && *p!=';') p++;
			*p='\0';
...
//NOT USED ANYMORE check msg_callback in protocol_http.c
/**
...
const char *onion_request_get_client_description(onion_request *req){
	if (!req->connection.cli_info && req->connection.cli_len){
		...
		if (getnameinfo((struct sockaddr *)&req->connection.cli_addr, req->connection.cli_len, tmp, sizeof(tmp)-1,
			...
			tmp[sizeof(tmp)-1]='\0';
			req->connection.cli_info=mystrdup(tmp);
		}
		else
			req->connection.cli_info=NULL;
	}
...
onion_dict* onion_request_get_cookies_dict(onion_request* req){
	...
	if (!ccookies)
		return req->cookies;
	char *cookies=mystrdup(ccookies); // I prepare a temporary string, will modify it.
	char *val=NULL;
	char *key=NULL;
	char *p=cookies;
	...
	while(*p){
		if (*p!=' ' && !key && !val){
			...
			*p=0;
			...
			syslog(LOG_DEBUG, "Add cookie <%s>=<%s> %d", key, val, first);
			val=NULL;
			key=NULL;
		}
		p++;
