...
int net_sockopt_blocking_set(int fd, bool blocking)
{
	...
	if (0 != ioctlsocket(fd, FIONBIO, &noblock)) {
		err = WSAGetLastError();
		syslog(LOG_INFO, "nonblock set: fd=%d err=%d (%m)\n",
			      fd, err, err);
	}
	return err;
#else
	...
	if (-1 == flags) {
		err = errno;
		syslog(LOG_INFO, "sockopt set: fnctl F_GETFL: (%i)\n", err);
		goto out;
	}
	...
	if (-1 == fcntl(fd, F_SETFL, flags)) {
		err = errno;
		syslog(LOG_INFO, "sockopt set: fcntl F_SETFL non-block (%i)\n", err);
	}
...
int net_sockopt_reuse_set(int fd, bool reuse)
{
	...
	if (-1 == setsockopt(fd, SOL_SOCKET, SO_REUSEADDR,
			     BUF_CAST &r, sizeof(r))) {
		syslog(LOG_INFO, "SO_REUSEADDR: %m\n", errno);
		return errno;
	}
#endif
