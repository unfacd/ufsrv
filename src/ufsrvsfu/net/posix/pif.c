...
int net_if_getaddr4(const char *ifname, int af, struct sa *ip)
{
	...
	if (error) {
		syslog(LOG_ERR, "get_ifaddr: getaddrinfo(): %s\n", gai_strerror(error));
...
int net_if_list(net_ifaddr_h *ifh, void *arg)
{
	...
	if (0 > (sockfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP))) {
		err = errno;
		syslog(LOG_ERR, "interface list: socket(): (%d)\n", err);
		goto out;
	}
	...
	if (0 != ioctl(sockfd, SIOCGIFCONF, &ifc)) {
		err = errno;
    syslog(LOG_ERR, "interface list: ioctl SIOCFIFCONF: %d\n", err);
		goto out;
	}
	...
	for (ifr = ifc.ifc_req;
		...
		if (err) {
      syslog(LOG_ERR, "if_list: sa_set_sa %d\n", err);
			break;
		}
