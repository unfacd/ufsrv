...
int udp_get_soc(struct udp_sock *udp_soc)
{
  return udp_soc->fd;
}

void udp_read(struct udp_sock *us, int fd)
{
  //AA+
  if (fd == -1) fd = us->fd;
	...
	if (n < 0) {
		err = errno;
		syslog(LOG_DEBUG, "%s (errno:'%d'): ERROR RECVFROM: %s", __func__, errno, strerror(errno) );
		if (EAGAIN == err)
			goto out;
	...
	syslog(LOG_DEBUG, "%s (bytes:'%lu': Received bytes: CALLING HELPER...", __func__ , n);
	/* call helpers */
	le = us->helpers.head;
	while (le) {
...
__unused static void udp_read_handler(int flags, void *arg)
{
...
__unused static void udp_read_handler6(int flags, void *arg)
{
	struct udp_sock *us = arg;
	       ...
	       udp_recv_h *rh, void *arg)
{
	...
	if (error) {
		...
		DEBUG_WARNING("listen: getaddrinfo: wsaerr=%d\n",
			      WSAGetLastError());
#endif
		syslog(LOG_INFO, "listen: getaddrinfo: %s:%s (%s)\n", addr, serv, gai_strerror(error));
		err = EADDRNOTAVAIL;
		goto out;
	}
	...
	for (r = res; r; r = r->ai_next) {
		...
		char bufx[512]={0};
		re_snprintf(bufx, sizeof bufx, "%J", r->ai_addr);
		syslog(LOG_INFO, "listen: for: fd:'%d', af=%d addr=%s\n", fd, r->ai_family, bufx);

		err = net_sockopt_blocking_set(fd, false);
		if (err) {
			syslog(LOG_ERR, "udp listen: nonblock set: %d\n", err);
			(void)close(fd);
			continue;
		}

		//AA- can only bind to an address of local interface (will fail when used with remote addresses)
		/*if (bind(fd, r->ai_addr, SIZ_CAST r->ai_addrlen) < 0) {
			err = errno;
			syslog(LOG_ERR, "listen: bind(): %d ()\n", err);
			(void)close(fd);
			continue;
		}*/
      ...
      syslog(LOG_ERR, "socket %d: IPV6_V6ONLY is %d\n", fd, on);
			if (on) {
				us->fd6 = fd;
				continue;
			}
...
int udp_thread_attach(struct udp_sock *us)
{
	...
	if (-1 != us->fd) {
//		err = fd_listen(us->fd, FD_READ, udp_read_handler, us);//AA- attach handler at when sockets is ready for event
		if (err)
			goto out;
	}
