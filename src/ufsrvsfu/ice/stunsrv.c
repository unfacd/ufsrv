			    ...
			    struct ice_cand *rcand)
{
	...
	if (cp) {
		...
		if (err) {
			printf("failed to allocate candpair:"
				      " lcand=%p rcand=%p (%m)\n",
				      lcand, rcand, err);
			return;
		}
			    ...
			    uint32_t prio, bool use_cand, bool tunnel)
{
	...
	if (!lcand) {
		printf("{%s.%u} local candidate not found"
			      " (checklist=%u) (src=%J)\n",
			      icem->name, comp->id,
	...
	if (!cp) {
		cp = icem_candpair_find_rcand(icem, rcand);
		if (!cp) {
			printf("{%s.%u} candidate pair not found:"
				      " source=%J\n",
			    ...
			    bool use_cand)
{
	...
	rcand = icem_cand_find(&icem->rcandl, comp->id, src);
	if (!rcand) {
		printf("lite: could not find remote candidate\n");
		return 0;
	}
	...
	if (!lcand) {
		printf("lite: could not find local candidate\n");
		return 0;
	}
	...
	err = icem_candpair_alloc(&cp, icem, lcand, rcand);
	if (err) {
		printf("lite: failed to created candidate pair\n");
		return err;
	}
		    ...
		    struct stun_msg *req, size_t presz)
{
	...
	err = re_regex(attr->v.username, strlen(attr->v.username),
		       "[^:]+:[^]+", &lu, &ru);
	if (err) {
		printf("could not parse USERNAME attribute (%s)\n",
			      attr->v.username);
		goto unauth;
	}
