					     ...
					     const struct sa *dest)
{
	...
	if (!lcand) {
		printf("no such local candidate: %J\n", mapped);
		return NULL;
	}
	if (!rcand) {
		printf("no such remote candidate: %J\n", dest);
		return NULL;
	}
			   ...
			   const struct sa *laddr)
{
	if (!icem_cand_find(&icem->lcandl, cp->lcand->compid, laddr)) {
		...
		if (err) {
			printf("failed to add PRFLX: %m\n", err);
		}
	}

	cp = construct_valid_pair(icem, cp, laddr, &cp->rcand->addr);
	if (!cp) {
		printf("{%s} no valid candidate pair for %J\n",
			      icem->name, laddr);
		return;
	}
			       ...
			       const struct stun_msg *msg, void *arg)
{
	...
	case 0: /* Success case */
		...
		if (!attr) {
			printf("no XOR-MAPPED-ADDR in response\n");
...
int icem_conncheck_send(struct ice_candpair *cp, bool use_cand, bool trigged)
{
	...
	if (!icem->rpwd) {
		printf("no remote password!\n");
	}

	if (cp->ct_conn) {
		printf("send_req: CONNCHECK already Pending!\n");
		return EBUSY;
	}
	...
	case ICE_CAND_TYPE_PRFLX:
		...
		err = stun_request(&(InstanceContext){}, &cp->ct_conn, icem->stun, icem->proto,
	...
	default:
		printf("unknown candidate type %d\n", lcand->type);
		err = EINVAL;
		break;
	}
