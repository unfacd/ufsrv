			...
			struct list *authl, struct list *addl, void *arg)
{
	...
	if (!rr) {
		printf("no SRV entry, trying A lookup on \"%s\"\n",
			   dns->domain);
	...
	if (err) {
		printf("SRV: A lookup failed (%m)\n", err);
		goto out;
	}

	printf("SRV handler: doing A/AAAA lookup..\n");
			 ...
			 stun_dns_h *dnsh, void *arg)
{
	...
	/* Port specified - use AAAA or A lookup */
		...
		printf("resolving A query: (%s)\n", domain);

		err = a_or_aaaa_query(dns, domain);
		if (err) {
			printf("%s: A/AAAA lookup failed (%m)\n",
				      domain, err);
			goto out;
		}
	}
