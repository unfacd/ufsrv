...
//AA deallocate client transaction
		     ...
		     const struct stun_unknown_attr *ua)
{
	...
	case STUN_CLASS_SUCCESS_RESP://AA retrieve stored client transaction for which this response was received (matching ct->tid)
...
int stun_ctrans_request(InstanceContext *instance_context, struct stun_ctrans **ctp, struct stun *stun, int proto,
			...
			size_t keylen, stun_resp_h *resph, void *arg)
{
	...
	case IPPROTO_UDP:
		...
		//AA+
		AS_INSTANCE_CONTEXT_FOR_SESSION(instance_context)->sesn_ptr->ssptr->sock = udp_get_soc((struct udp_sock *)ct->sock);
		int res = EnableIoEventsNotification(GetSessionWorkerEventsHandler(), AS_INSTANCE_CONTEXT_FOR_SESSION(instance_context)->instance_sesn_ptr);
		//

		ct->txc = 1;
		syslog(LOG_DEBUG, "%s {fd:'%d'}: Sending STUN request...", __func__, udp_get_soc((struct udp_sock *)ct->sock));
 ...
 out:
	if (!err) {
		...
		ct->resph = resph;
		ct->arg   = arg;
		syslog(LOG_DEBUG, "%s (err:'%d'): PREPARING TO READ RESPONSE", __func__, err);
//		udp_read((struct udp_sock *)ct->sock, -1);//AA+ a lift-out from event async read
	}
	else {
		syslog(LOG_DEBUG, "%s: STUN SEND ERROR", __func__ );
		mem_deref(ct);}
