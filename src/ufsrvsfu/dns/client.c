...
static int reply_recv(struct dnsc *dnsc, struct mbuf *mb)
{
	...
	if (q->type == DNS_QTYPE_AXFR) {
		...
		if (dq.hdr.rcode == DNS_RCODE_OK && dq.hdr.nans > 0 &&
			...
			printf("waiting for last SOA record in reply\n");
			goto out;
		}
	}
...
static void tcp_recv_handler(struct mbuf *mbrx, void *arg)
{
 ...
 next:
	...
	if (mbuf_get_left(mbrx) > 0) {
		printf("%lu bytes of tcp data left\n", mbuf_get_left(mbrx));
		goto next;
	}
...
{
  printf("ERROR tcp_estab_handler DISABLE\n");
	/*struct tcpconn *tc = arg;
	struct le *le = list_head(&tc->ql);
	int err = 0;

	printf("connection (%J) established\n", &tc->srv);
...
static bool tcpconn_fail_handler(struct le *le, void *arg)
{
	...
	if (q->ntx >= *q->srvc) {
		printf("all servers failed, giving up!!\n");
			 ...
			 const struct sa *srv)
{
	struct tcpconn *tc;
	int err = ENOMEM;
  printf("ERROR tcpconn_alloc DISABLED");
...
static int send_tcp(struct dns_query *q)
{
	...
	printf("ERROR send_tcp DISABLED");
	/*if (!q)
		return EINVAL;
...
static int send_udp(struct dns_query *q)
{
	...
	int err = ETIMEDOUT;
	uint32_t i;
  printf("ERROR send_udp DISABLED");
/*
	if (!q)
		return EINVAL;
