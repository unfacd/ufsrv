...
ProcessOutgoingWsHandshake (InstanceHolderForSession *instance_sesn_ptr, SocketMessage *sock_msg_ptr)
{
  ...
  SocketMessage	*sm_ptr;
    ...
    sprintf((char *)sm_ptr->_processed_msg, SERVER_HANDSHAKE_HYBI, sha1, "base64", SESSION_ID(sesn_ptr), UfsrvUidGetSequenceId(&SESSION_UFSRVUIDSTORE(sesn_ptr)));
  }
...
HTTP/1.1 101 Switching Protocols#015#012Upgrade: websocket#015#012Connection: Upgrade#015#012Sec-WebSocket-Accept: rjrmoqL0bOAvDJ4+K+cC1ftpqvg=#015#012Sec-WebSocket-Protocol: base64#015#012#015
...
#ifdef __UF_TESTING
	syslog(LOG_DEBUG, "%s (pid:'%lu', th_ctx'%p', o:'%p' cid:'%lu'): Final handshake response: '%s'", __func__, pthread_self(), NULL, sesn_ptr, SESSION_ID(sesn_ptr), sm_ptr->_processed_msg);
#endif
  ...
  int ret_value = SendToSocket(instance_sesn_ptr, &tms, SOCKMSG_DONTWSFRAME);
  if (ret_value > 0) {
    ...
    sesn_ptr->stat |= SESNSTATUS_HANDSHAKED;

    _RETURN_RESULT_SESN(sesn_ptr, sesn_ptr, RESULT_TYPE_SUCCESS, RESCODE_PROTOCOL_WSHANDSHAKE)
  }

  if (ret_value == -1) {
    //i/o error with suspension
    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_IO_SUSPENDED)
  }
