...

{
    ...
    if (b64_sz <= 125) {
        ...
        target[1]|= srclength;
        payload_offset = 2;
    } else if ((b64_sz > 125) && (b64_sz < MAXBUF)) { //64k
		  target[1] |= 126;
				//set length into the next bytes
		  set_16bit (srclength, target + 2);
      payload_offset = 4;
    } else if (b64_sz < 9223372036854775807) {
	    syslog (LOG_NOTICE, "Sending frames larger than 65535 bytes not supported");
	    //target[1] = (char) 127;
		  //payload_offset = 10;
		// not supported yet
...
parse_handshake(Session *sesn_ptr, char *handshake)
{
	...
	if (handshake_len < CONFIG_WEBSOCKET_HANDSHAKE_MIN_SZ || handshake_len > CONFIG_WEBSOCKET_HANDSHAKE_MAX_SZ || (bcmp(handshake, "GET ", 4) != 0)) {
...
//	headers->key3[0] = '\0';
	...
	//Sec-WebSocket-Key:
	if ((idx = wsi->u.hdr.ah->frag_index[WSI_TOKEN_KEY]) && (wsi->u.hdr.ah->frags[idx].len > 0)) {
    headers->key1 = calloc(wsi->u.hdr.ah->frags[idx].len + 1, sizeof (char));
...
void
gen_sha1(const char *key, char *target)
{
  SHA_CTX c;
