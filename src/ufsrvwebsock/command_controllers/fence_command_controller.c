...
#include <ufsrvmsg_core/include/misc.h>
...
inline static UFSRVResult *_CommandControllerFenceDescription(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received);
...
inline static UFSRVResult *_CommandControllerFenceInviteRejected(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr);
inline static UFSRVResult *_CommandControllerFenceInviteRejectedSet(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr);
inline static UFSRVResult *_CommandControllerFenceInviteDeleted(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr);
inline static UFSRVResult *_CommandControllerFenceInviteDeletedSet(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr);
...
inline static UFSRVResult *_WireMarshalFenceStateSync(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_target, Fence *f_ptr, Envelope *command_envelope_ptr);
...
inline static UFSRVResult *_MarshalFenceDescriptionUpdate(InstanceContextForSession *ctx_ptr, InstanceHolderForFence *, WebSocketMessage *, DataMessage *data_msg_ptr, unsigned long call_flags, FenceEvent *fence_event_ptr);
...
_PrepareMarshalMessageForFence(MarshalMessageEnvelopeForFence *envelope_ptr, Session *sesn_ptr, Fence *f_ptr, FenceEvent *event_ptr, DataMessage *data_msg_ptr_orig, enum _FenceCommand__CommandTypes command_type, enum _CommandArgs command_arg)
{
  ...
  ProvideFenceRecordInProtoAsIdentifier(sesn_ptr, f_ptr, envelope_ptr->fence_record, false);
	...
	envelope_ptr->fence_command->originator		= ProvideUserRecordFromSessionInProto(sesn_ptr, envelope_ptr->user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
	...
	if (IS_PRESENT(data_msg_ptr_orig) && IS_PRESENT(data_msg_ptr_orig->ufsrvcommand->fencecommand)) {
...
/**
 * 	@brief: Main command controller for Fence related messages.
 *
 * 	This is invoked in the context of wire data message arriving via the msgqueue bus. The message is in raw wire format (proto).
 ...
 * 	@param data_msg_ptr_received: The raw DataMessage protobuf as provided by the sending user. This message will have been previously verified
 ...
 */
UFSRVResult *
CommandCallbackControllerFenceCommand(InstanceHolderForSession *instance_sesn_ptr_local_user, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received)
{
	void *command_types [] = {
					&&FENCE_COMMAND_JOIN, //0
					&&FENCE_COMMAND_LEAVE,
					...
					&&FENCE_COMMAND_TTL,
					&&FENCE_COMMAND_FKEY,
					&&FENCE_COMMAND_JOIN_MODE, //10
					&&FENCE_COMMAND_TAGS,
					&&FENCE_COMMAND_MAKE,
					&&FENCE_COMMAND_STATE,
					&&FENCE_COMMAND_EXPIRY, //14
					&&FENCE_COMMAND_MUTE,
          ...
          &&FENCE_COMMAND_PERMISSION_LIST_SEMANTICS,//22
          &&FENCE_COMMAND_INVITE_REJECTED,
          &&FENCE_COMMAND_INVITE_DELETED,
          &&FENCE_COMMAND_JOIN_REQUEST, //25
          &&FENCE_COMMAND_DESCRIPTION //26
	}; //ALIGN WITH PROTOBUF which is index at 0. For each entry in the array above, add corresponding label below, which will cause the label to resolve

	Session *sesn_ptr_local_user = SessionOffInstanceHolder(instance_sesn_ptr_local_user);
	CommandHeader *command_header = data_msg_ptr_received->ufsrvcommand->fencecommand->header;
	...
	FENCE_COMMAND_JOIN:
		return (_CommandControllerFenceJoin(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_LEAVE:
		return (_CommandControllerFenceLeave(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_INVITE:
		return (_CommandControllerFenceInvite(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_BLOCK:
		goto return_unknown;

	FENCE_COMMAND_FNAME:
		return (_CommandControllerFenceName(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_BANNER:
		goto return_unknown;

	FENCE_COMMAND_AVATAR:
		return (_CommandControllerFenceAvatar(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_MAXUSERS:
		return (_CommandControllerFenceMaxMembers(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
	...
	return (_CommandControllerFenceJoinMode(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
	...
	FENCE_COMMAND_STATE:
		return (_CommandControllerFenceStateSync(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_EXPIRY:
		return (_CommandControllerFenceMessageExpiry(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_MUTE:
		goto return_unknown;

	FENCE_COMMAND_PERMISSION:
		return (_CommandControllerFencePermission(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
	...
	FENCE_COMMAND_PRIVACY_MODE:
		return (_CommandControllerFencePrivacyMode(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

	FENCE_COMMAND_DELIVERY_MODE:
		return (_CommandControllerFenceDeliveryMode(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
  ...
  FENCE_COMMAND_INVITE_REJECTED:
  return (_CommandControllerFenceInviteRejected(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
  goto return_unknown;

  FENCE_COMMAND_INVITE_DELETED:
  return (_CommandControllerFenceInviteDeleted(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
  goto return_unknown;

  FENCE_COMMAND_JOIN_REQUEST:
  return (CommandControllerFenceLinkJoin(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));

  FENCE_COMMAND_DESCRIPTION:
  return (_CommandControllerFenceDescription(instance_sesn_ptr_local_user, wsm_ptr_received, data_msg_ptr_received));
	...
	SESSION_RETURN_RESULT (sesn_ptr_local_user, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_UNKNOWN_TYPE)
...

{
  ...
  //join an existing fence
    ...
    unsigned long call_flags_fence = FENCE_CALLFLAG_JOIN|FENCE_CALLFLAG_KEEP_FENCE_LOCKED|FENCE_CALLFLAG_LOCK_FENCE_BLOCKING;
    if (fcmd_ptr->header->args == COMMAND_ARGS__LINK_BASED) {
      call_flags_fence |= FENCE_CALLFLAG_JOIN_LINKJOIN;
      join_type = JT_LINK_BASED;
    }

    res_ptr = IsUserAllowedToJoinFenceById(instance_sesn_ptr, fence_record_ptr->fid, fence_record_ptr->description, call_flags_fence, &fence_lock_already_owned);

    //region error handling
    if (SESSION_RESULT_TYPE_ERROR(sesn_ptr)) {
			if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST) || SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_USER_FENCE_KEY)) {
				//no fence lock
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, SESSION_RESULT_CODE(sesn_ptr), data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
				return SESSION_RESULT_PTR(sesn_ptr);
			}

      if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_LINKJOIN_NONCE) || SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_JOIN_LINKJOIN_LIST)) {
        ...
        return SESSION_RESULT_PTR(sesn_ptr);
			...
			if (SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_INVITATION_LIST)) {
				if (!fence_lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, (Fence *)SESSION_RESULT_USERDATA(sesn_ptr), SESSION_RESULT_PTR(sesn_ptr));
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_INVITATION_LIST, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
				return SESSION_RESULT_PTR(sesn_ptr);
			}
    ...
    //endregion
      ...
      f_ptr = FENCESTATE_FENCE(fstate_ptr);
	...
	} else if (IS_STR_LOADED(fence_record_ptr->fname)) {
		...
		syslog(LOG_DEBUG, "%s {pid:'%lu'}: ERROR: RECEIVED INCOMPLETE OR ILL-FORMATTED FENCE COMMAND MESSAGE: fid and/or fname missing", __func__, pthread_self());
		goto exit_catch_all;
	}
...
//				res_ptr->result_user_data = FENCESTATE_INSTANCE_HOLDER(FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr)); //no need for state
		...
		case RESCODE_FENCE_DOESNT_EXIST:
			return HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                                   sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_DOESNT_EXIST, data_msg_ptr->ufsrvcommand
                                                                                                                                                        ->fencecommand
                                                                                                                                                        ->header
                                                                                                                                                        ->command, NULL);
...
MarshalGeoFenceJoinToUser(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_target, FenceStateDescriptor *fence_state_ptr, WebSocketMessage *wsm_ptr, unsigned call_flags)
{
  ...
  ProvideUserRecordFromSessionInProto(GetUfsrvSystemUser(), &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
	fence_command.originator		=	&user_record_originator;

  ProvideUfsrvUidInProto(&(fence_state_ptr->invited_by), &(fence_record.by_user), false); //dynamically allocates buffer
	fence_record.has_by_user	=	1;
	fence_record.by_user_when		=	fence_state_ptr->when_invited;	fence_record.has_by_user_when	=	1;

	fence_records[0] = ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {FENCESTATE_INSTANCE_HOLDER(fence_state_ptr), f_ptr}, &fence_record);
  ProvideFenceUserPreferencesInProto(sesn_ptr, fence_state_ptr, &fence_record);
	...
	UFSRVResult *res_ptr = UfsrvCommandInvokeUserCommand(ctx_ptr, ctx_ptr_target, wsm_ptr, NULL, (void *) &ufsrv_descpription/*&ufsrv_command*/, uFENCE_V1_IDX);
...
_MarshalNewFenceJoin(InstanceContextForSession *ctx_ptr, UFSRVResult *res_ptr, WebSocketMessage *wsm_ptr_received, FenceRecord *fence_record_ptr, DataMessage *data_msg_ptr_received, CollectionDescriptor *invited_eids_collection_ptr)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {instance_f_ptr, f_ptr}, &fence_record);
...
_MarshalNewFenceReJoin(InstanceContextForSession *ctx_ptr, const InstanceContextForFenceStateDescriptor  *instance_ctx_fstate_ptr, WebSocketMessage *wsm_ptr_orig, FenceRecord *fence_record_ptr, DataMessage *data_msg_ptr_received)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr, instance_ctx_fstate_ptr->instance_f_ptr, &fence_record);
  ProvideFenceUserPreferencesInProto(sesn_ptr, instance_ctx_fstate_ptr->fstate_ptr, &fence_record);
...
MarshalFenceJoinToUser(InstanceContextForSession *ctx_ptr, FenceStateDescriptor *fence_state_ptr, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {FENCESTATE_INSTANCE_HOLDER(fence_state_ptr), f_ptr}, &fence_record);
  ProvideFenceUserPreferencesInProto(sesn_ptr, fence_state_ptr, &fence_record);

	if (IS_PRESENT(data_msg_ptr)) {
		size_t 	legacy_message_encoded_sz = data_message__get_packed_size(data_msg_ptr);
    if (legacy_message_encoded_sz > 0) {
      uint8_t legacy_message_encoded[legacy_message_encoded_sz];
      data_message__pack(data_msg_ptr, legacy_message_encoded);
      command_envelope.legacymessage.data = legacy_message_encoded;
      command_envelope.legacymessage.len = legacy_message_encoded_sz;
      command_envelope.has_legacymessage = 1;
    } else {
      syslog (LOG_DEBUG, "%s {pid:'%lu', o:'%p'}: ERROR: COULD NOT OBTAIN PACKED MESSAGE SIZE", __func__, pthread_self(), sesn_ptr);
    }
	}
  ...
  UfsrvCommandInvokeUserCommand(ctx_ptr, NULL, wsm_ptr_orig, NULL, &ufsrv_description, uFENCE_V1_IDX);
...
MarshalFenceJoinInvitedToUser(InstanceContextForSession *ctx_ptr, FenceStateDescriptor *fence_state_ptr, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {FENCESTATE_INSTANCE_HOLDER(fence_state_ptr), f_ptr}, &fence_record); //we have to reinitialise record with full load, because it is anew fence join
  ProvideFenceUserPreferencesInProto(sesn_ptr, fence_state_ptr, &fence_record);
...
MarshalFenceStateSyncForJoin(InstanceContextForSession *ctx_ptr, Session *sesn_ptr_newly_joined, InstanceHolderForFence *instance_f_ptr, unsigned call_flags)
{
  ...
  Fence *f_ptr = FenceOffInstanceHolder(instance_f_ptr);
	...
	UserRecord user_record_originator;
	fence_command.originator	= ProvideUserRecordFromSessionInProto(sesn_ptr_newly_joined, &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
...
//			_WireMarshalFenceStateSync(sesn_ptr, NULL, f_ptr, &command_envelope, false);
//			fence_command.originator=originator_ptr;
//			header.has_when_client	=	0;
		} else {
      _WireMarshalFenceStateSync(ctx_ptr, &(InstanceContextForSession) {(InstanceHolderForSession *) eptr->whatever, sesn_ptr_listed}, f_ptr, &command_envelope);
		}
	}
...

{
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (IS_EMPTY(instance_f_ptr)) {
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_DOESNT_EXIST, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
	...
	if (SESSION_RESULT_TYPE_ERROR(sesn_ptr)) {
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_MEMBERSHIP, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
...
/**
 * 	@brief: Marshals a Fence state sync message with emphasis on leave to Fence participants, enabling clients to refresh their data models.
...
MarshalFenceStateSyncForLeave(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_newly_left, InstanceContextForFence *ctx_f_ptr, DataMessage *data_msg_ptr_orig, unsigned call_flags)
{
	...
	FenceRecord *fence_record_ptr = ProvideFenceRecordInProto(ctx_ptr, ctx_f_ptr, NULL);

	if (!IS_EMPTY(sesn_ptr_newly_left))	originator_ptr = ProvideUserRecordForSelfInProto(sesn_ptr_newly_left, PROTO_USER_RECORD_MINIMAL);
	...
	header.eid									=	f_ptr->fence_events.event_id; header.has_eid = 1;
  ...
  _WireMarshalFenceStateSync(ctx_ptr, NULL, f_ptr, &command_envelope);
    ...
    _WireMarshalFenceStateSync(ctx_ptr,
                               &(InstanceContextForSession) {(InstanceHolderForSession *) eptr->whatever, SessionOffInstanceHolder((InstanceHolderForSession *) eptr->whatever)}, f_ptr, &command_envelope);
  }
...
_CommandControllerFenceInvite(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_recieved, DataMessage *data_msg_ptr)
{
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	switch (data_msg_ptr->ufsrvcommand->fencecommand->header->args)
	{
		...
		case COMMAND_ARGS__DELETED://user removed a member from invite list
			res_ptr = _CommandControllerFenceInviteDeleted(instance_sesn_ptr, wsm_ptr_recieved, data_msg_ptr);
			break;
    ...
    case COMMAND_ARGS__REJECTED_USER://user rejected invitation offer
      res_ptr = _CommandControllerFenceInviteRejected(instance_sesn_ptr, wsm_ptr_recieved, data_msg_ptr);
    break;
...
inline static UFSRVResult *
_CommandControllerFenceInviteRejected(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_recieved, DataMessage *data_msg_ptr)
{
  Fence									*f_ptr		=	NULL;
  UFSRVResult 					*res_ptr	=	NULL;
  ...
  switch (data_msg_ptr->ufsrvcommand->fencecommand->header->args)
  {
    case COMMAND_ARGS__SET:
      res_ptr = _CommandControllerFenceInviteRejectedSet(instance_sesn_ptr, wsm_ptr_recieved, data_msg_ptr);
      break;

    default:
      syslog (LOG_DEBUG, "%s {pid:'%lu', o:'%p', arg:'%d'}: ERROR: UNKNOWN FENCE INVITE-REJECTED COMMAND ARG...", __func__, pthread_self(), sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->header->args);
  }
...
inline static UFSRVResult *
_CommandControllerFenceInviteDeleted(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_recieved, DataMessage *data_msg_ptr)
{
  Fence									*f_ptr		=	NULL;
  UFSRVResult 					*res_ptr	=	NULL;
  ...
  {
    case COMMAND_ARGS__SET:
      res_ptr = _CommandControllerFenceInviteDeletedSet(instance_sesn_ptr, wsm_ptr_recieved, data_msg_ptr);
      break;

    default:
      syslog (LOG_DEBUG, "%s {pid:'%lu', o:'%p', arg:'%d'}: ERROR: UNKNOWN FENCE INVITE -DELETE COMMAND ARG...", __func__, pthread_self(), sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->header->args);
  }
...
static bool IsFenceOwnerInvitedBack(Session *sesn_ptr_inviter, UserRecord **user_records, unsigned long uid_owner, size_t invited_members_sz);

/**
 * @brief Users are allowed to invite fence owner back even if they don't have permissions to change fence membership state (as per rules codified below)
 * @param sesn_ptr_inviter Target user that is being checked. Session must be locked.
 * @param user_records collection of invited user records passed by inviter
 * @param uid_owner uid of of fence owner
 * @param invited_members_sz number of invited users on user_records collection
 * @return true if fence owner invite-back rules are satisfied
 */
static bool __attribute__((nonnull(1, 2), access(read_only, 1),  access(read_only, 2)))
IsFenceOwnerInvitedBack(Session *sesn_ptr_inviter, UserRecord **user_records, unsigned long uid_owner, size_t invited_members_sz)
{
  if (invited_members_sz == 1) {
    const UserRecord *invited_user_record_ptr = user_records[0];
    unsigned long userid_invited = UfsrvUidGetSequenceId((const UfsrvUid *) invited_user_record_ptr->ufsrvuid.data);
    if (userid_invited == uid_owner) {
      bool IsUserBlocked = IsUserOnShareList(sesn_ptr_inviter, userid_invited, SHARELIST_BLOCKED, CALLFLAGS_EMPTY);
...
#define _FENCE_CALL_FLAGS_FENCE_INVITE	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//locks by default
	IsUserAllowedToChangeFence(sesn_ptr,fcmd_ptr->fences[0]->fid, fcmd_ptr->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_FENCE_INVITE);
	...
	if (SESSION_RESULT_TYPE_ERROR(sesn_ptr))	{
      ...
      if (SESSION_RESULT_CODE(sesn_ptr) == RESCODE_FENCE_OWNERSHIP) {
        instance_fstate_ptr = (InstanceHolderForFenceStateDescriptor *)SESSION_RESULT_USERDATA(sesn_ptr);
				...
				//Exception is made if user being invited is the fence owner. todo: no check if owner is already a member
        if (IsFenceOwnerInvitedBack(sesn_ptr, fcmd_ptr->fences[0]->invited_members, FENCE_OWNER_UID(f_ptr), fcmd_ptr->fences[0]->n_invited_members))  {
          SESSION_RESULT_USERDATA(sesn_ptr) = instance_fstate_ptr; //important to re-instate
          goto process_invited;
        }

        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, fstate_ptr, wsm_ptr_received, data_msg_ptr, rescode, fcmd_ptr->header
                                                                                                                                      ->command, NULL);
      } else { //RESCODE_FENCE_MEMBERSHIP
				f_ptr		=	FenceOffInstanceHolder((InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr));
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=(InstanceHolderForFence *) SESSION_RESULT_USERDATA(sesn_ptr)}), wsm_ptr_received, data_msg_ptr, rescode, fcmd_ptr->header
                                                                                                                                                                                                                                           ->command, NULL);
      }
  ...
  fstate_ptr          = FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
	...
	MarshalFenceInvitation(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, FENCESTATE_INSTANCE_HOLDER(fstate_ptr), wsm_ptr_received, data_msg_ptr, &(invited_collections_for_result.first), &(invited_collections_for_result.second), CALLFLAGS_EMPTY);
...
#undef _FENCE_CALL_FLAGS_FENCE_INVITE
}

/**
 * @brief User rejected an incoming invite
 */
inline static UFSRVResult *
_CommandControllerFenceInviteRejectedSet(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr)
{
  bool 									fence_already_locked = false;
  InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
  Fence                                 *f_ptr = NULL;
...
#define _FENCE_CALL_FLAGS_INVITE_REJECTED	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

  //locks by default
  IsUserAllowedToRejectFence(sesn_ptr,fcmd_ptr->fences[0]->fid, fcmd_ptr->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_INVITE_REJECTED);
  ...
  if (SESSION_RESULT_TYPE_ERROR(sesn_ptr))	{
    ...
    if (!SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
      //FENCE LOCKED...
      if (SESSION_RESULT_CODE(sesn_ptr) == RESCODE_FENCE_OWNERSHIP) {
//        instance_f_ptr = (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);//this branch is not supported
//        f_ptr		=	FenceOffInstanceHolder(instance_f_ptr);
//
//        HandleFenceCommandError(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_received, data_msg_ptr, rescode, fcmd_ptr->header->command, NULL);
      } else { //RESCODE_FENCE_MEMBERSHIP //user not on invite list
        instance_f_ptr =  (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
        f_ptr		=	FenceOffInstanceHolder(instance_f_ptr);
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_received, data_msg_ptr, rescode, fcmd_ptr->header
                                                                                                                                                                                             ->command, NULL);
      }
  ...
  instance_f_ptr = (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
  f_ptr =  FenceOffInstanceHolder(instance_f_ptr);
  if (unlikely(IS_EMPTY((instance_fstate_ptr = IsUserMemberOfThisFence(SESSION_INVITED_FENCE_LIST_PTR(sesn_ptr), f_ptr, false)))))	goto return_error_not_invited;
  ...
  RemoveUserFromInvitedList(instance_sesn_ptr, fstate_ptr, &fence_event, FENCE_CALLFLAG_WRITEBACK_DATA_TO_BACKEND);

  InstanceContextForFence fence_context = {.f_ptr=f_ptr, .instance_f_ptr=instance_f_ptr, .is_locked=true, .lock_already_owned=fence_already_locked};
  MarshalFenceInvitationRejected(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, fstate_ptr, &fence_context, &uid_invited_by, &fence_event, wsm_ptr_received, data_msg_ptr, 0);
  if (fence_context.is_locked && !fence_context.lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, FENCESTATE_FENCE(fstate_ptr), SESSION_RESULT_PTR(sesn_ptr));

  return SESSION_RESULT_PTR(sesn_ptr);

  return_error_not_invited://no retrieved FenceStateDescriptor
  HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                        sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_received, data_msg_ptr, RESCODE_FENCE_MEMBERSHIP, fcmd_ptr->header->command, NULL);
  if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
  return SESSION_RESULT_PTR(sesn_ptr);


#undef _FENCE_CALL_FLAGS_INVITE_REJECTED
}

/**
 * @brief A user removed  from invite list (uninvited); either by inviter (e.g admin) or by invitee
 * @param instance_sesn_ptr the requesting user, or invitee
 */
inline static UFSRVResult *
_CommandControllerFenceInviteDeletedSet(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr)
{
  InstanceHolderForFenceStateDescriptor *instance_fstate_ptr = NULL;
  FenceStateDescriptor	                *fstate_ptr;
  FenceCommand					                *fcmd_ptr = data_msg_ptr->ufsrvcommand->fencecommand;

#define _FENCE_CALL_FLAGS_INVITE_DELETE	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
  size_t collection_sz = 1;
  FenceRecord  *fence_record;
  if (fcmd_ptr->n_fences > 0 && IS_PRESENT(fcmd_ptr->fences)) {
    fence_record = fcmd_ptr->fences[0];
    if (IS_PRESENT(fence_record) && fence_record->n_invited_members > 0 && IS_PRESENT(fence_record->invited_members)) collection_sz = fence_record->n_invited_members;
    else goto return_parameters_error;
    if (collection_sz > CONFIGDEFAULT_FENCE_LIST_MAX_SZ) {
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_received,
                              data_msg_ptr, RESCODE_FENCE_INVITATION_LIST, fcmd_ptr->header
                                                                                   ->command, NULL);
      _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_INVITE_SIZE)
    }
  } else {
    return_parameters_error:
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_received,
                            data_msg_ptr, RESCODE_FENCE_MISSING_PARAM, fcmd_ptr->header
                                                                               ->command, NULL);
    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_MISSING_PARAM)

  }

  InstanceContextForFence instance_fence_ctx = {0};
  UFSRVResult  results_uninvited[collection_sz]; memset(results_uninvited, '\0', sizeof(results_uninvited));
  InstanceContextForSession sessions_uninvited[collection_sz]; memset(sessions_uninvited, '\0', sizeof(sessions_uninvited));
  CollectionDescriptorPair uninvited_users_collection = {0};
  uninvited_users_collection.first.collection			= AS_COLLECTION_TYPE(results_uninvited);
  uninvited_users_collection.first.collection_sz	=	collection_sz;
  uninvited_users_collection.second.collection		= AS_COLLECTION_TYPE(sessions_uninvited);
  uninvited_users_collection.second.collection_sz	= collection_sz;
  uninvited_users_collection.source.collection    = AS_COLLECTION_TYPE(fence_record->invited_members);
  uninvited_users_collection.source.collection_sz	= collection_sz;

  IsUserAllowedToRemoveInvitedUser(sesn_ptr,fcmd_ptr->fences[0]->fid, &instance_fence_ctx, AS_CONTEXT_DATA(&uninvited_users_collection), _FENCE_CALL_FLAGS_INVITE_DELETE);

  //>>> FENCE SHOULD BE IN RW LOCKED STATE

  Fence                                 *f_ptr          = instance_fence_ctx.f_ptr;
  InstanceHolderForFence                *instance_f_ptr = instance_fence_ctx.instance_f_ptr;
  for (size_t idx=0; idx<collection_sz; idx++) {
    UFSRVResult *result = (UFSRVResult  *)(uninvited_users_collection.first.collection + (idx * sizeof(UFSRVResult)));
    InstanceContextForSession *instance_sesn_ctx_uninvited = (InstanceContextForSession *)(uninvited_users_collection.second.collection + (idx * sizeof(InstanceContextForSession)));
    if (result->result_type == RESULT_TYPE_SUCCESS) {
      instance_sesn_ctx_uninvited->sesn_ptr = SessionOffInstanceHolder(instance_sesn_ctx_uninvited->instance_sesn_ptr);

      SessionLockRWCtx(THREAD_CONTEXT_PTR, instance_sesn_ctx_uninvited->sesn_ptr, _LOCK_TRY_FLAG_FALSE, __func__);
      if (_RESULT_TYPE_EQUAL(THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT), RESULT_TYPE_ERR)) {
        continue;
      }
      instance_sesn_ctx_uninvited->is_locked = true;
      instance_sesn_ctx_uninvited->lock_already_owned = (_RESULT_CODE_EQUAL(THREAD_CONTEXT_UFSRV_RESULT(THREAD_CONTEXT), RESCODE_PROG_LOCKED_BY_THIS_THREAD));

      if (unlikely(IS_EMPTY((instance_fstate_ptr = IsUserMemberOfThisFence(SESSION_INVITED_FENCE_LIST_PTR(instance_sesn_ctx_uninvited->sesn_ptr), f_ptr, false)))))	{
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_received, data_msg_ptr, RESCODE_FENCE_MEMBERSHIP, fcmd_ptr->header->command, NULL);
        if (!instance_sesn_ctx_uninvited->lock_already_owned) {
          SessionUnLockCtx(THREAD_CONTEXT_PTR, instance_sesn_ctx_uninvited->sesn_ptr, __func__);
          instance_sesn_ctx_uninvited->is_locked = false;
        }
        continue;
      }

      //sesn_uninvited WR LOCKED
      fstate_ptr          = FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
      UfsrvUid uid_invited_by = {0};
      memcpy(&uid_invited_by, &(fstate_ptr->invited_by), sizeof(UfsrvUid));

      FenceEvent fence_event = {0};	fence_event.event_type = EVENT_TYPE_FENCE_USER_UNINVITED;
      RemoveUserFromInvitedList(instance_sesn_ctx_uninvited->instance_sesn_ptr, fstate_ptr, &fence_event, FENCE_CALLFLAG_WRITEBACK_DATA_TO_BACKEND);

      MarshalFenceInvitationDeleted(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, instance_sesn_ctx_uninvited, fstate_ptr, &instance_fence_ctx, &uid_invited_by, &fence_event, wsm_ptr_received, data_msg_ptr, FENCE_CALLFLAG_KEEP_FENCE_LOCKED);
      if (!instance_sesn_ctx_uninvited->lock_already_owned) {
        SessionUnLockCtx(THREAD_CONTEXT_PTR, instance_sesn_ctx_uninvited->sesn_ptr, __func__);
      }
    } else {
        ...
        if (rescode != RESCODE_FENCE_DOESNT_EXIST) {
          //FENCE LOCKED...
          if (rescode == RESCODE_FENCE_OWNERSHIP) {
  //        instance_f_ptr = (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);//this branch is not supported
  //        f_ptr		=	FenceOffInstanceHolder(instance_f_ptr);
  //
  //        HandleFenceCommandError(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_received, data_msg_ptr, rescode, fcmd_ptr->header->command, NULL);
          ...
          continue;
        } else goto  return_with_error;//irrecoverable error
  ...
  return_unblock:
  if (instance_fence_ctx.is_locked && !instance_fence_ctx.lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
  _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RECODE_NONE)

#undef _FENCE_CALL_FLAGS_INVITE_DELETE
}
...
#define _FENCE_CALL_FLAGS_INVITE_SYNC	(FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//locks by default
	FindFence(sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->cname, &fence_lock_already_owned, _FENCE_CALL_FLAGS_INVITE_SYNC);
...
//			HandleFenceCommandError (sesn_ptr, &((FenceStateDescriptor){.fence=f_ptr}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
//			FenceEventsUnLock(f_ptr);
  ...
  HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                        sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_holder_ptr_fence}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
	if (!fence_lock_already_owned)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
	_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, rescode)
...
#undef _FENCE_CALL_FLAGS_INVITE_SYNC
...
_MarshalFenceInvitationReceipt(InstanceContextForSession *ctx_ptr, Fence *f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, CollectionDescriptor *invited_eids_collection_ptr, CollectionDescriptor *rejected_collection_ptr)
{
	...
	WireMarshalFenceCommandToUser(ctx_ptr, NULL, f_ptr, wsm_ptr_received, &command_envelope,  uFENCE_V1_IDX);
...
MarshalFenceInvitation(InstanceContextForSession *ctx_ptr, InstanceHolderForFence *instance_f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, CollectionDescriptor *invited_eids_collection_ptr, CollectionDescriptor *unchanged_collection_ptr, unsigned call_flags)
{
  ...
  Fence *f_ptr = FenceOffInstanceHolder(instance_f_ptr);
	...
	FenceRecord *fence_record_ptr	= ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {instance_f_ptr, f_ptr}, NULL);//MakeFenceRecordInProtoAsIdentifier
	fence_records[0]							=	fence_record_ptr;
	fence_command.fences					=	fence_records;
	fence_command.n_fences				=	1;
	...
	if (IS_PRESENT(data_msg_ptr)) {
      ...
      res_ptr = _MarshalFenceInvitationFromWireSource(ctx_ptr, f_ptr, wsm_ptr_received, &command_envelope, data_msg_ptr, invited_eids_collection_ptr, call_flags);
		...
		if ((IS_PRESENT(unchanged_collection_ptr)) && (unchanged_collection_ptr->collection_sz > 0)) {
			res_ptr = _MarshalUnchangedFenceInvitationFromWireSource(ctx_ptr, f_ptr, &command_envelope, data_msg_ptr, unchanged_collection_ptr, call_flags);
		}
	} else {
		...
		fence_command.originator	= ProvideUserRecordFromSessionInProto(GetUfsrvSystemUser(), &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
  ...
  assert(IS_PRESENT(res_ptr));
	return res_ptr;

}

/**
 * @brief Send an invitation rejection message by the invitation-receiving user.
 * The semantics are slightly different when the inviter, or admin initiates the request
 * (AKA invite deletion). See @ref MarshalFenceInvitationDeleted.
 * @param ctx_ptr Session context for the user who initiated this message
 * @param f_ptr Fence under consideration
 * @param uid_invited_by user sequence id of inviter
 * @param wsm_ptr_received WSM context for the user who initiated this message
 * @param data_msg_ptr DataMessage for the user who initiated this message
 * @param call_flags
 * @locked f_ptr, sesn_ptr
 * @unlocks f_ptr
 * @return
 */
UFSRVResult *
MarshalFenceInvitationRejected(InstanceContextForSession *ctx_ptr, FenceStateDescriptor	*fstate_ptr, InstanceContextForFence *instance_fence_ctx, UfsrvUid *uid_invited_by, FenceEvent *event_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, unsigned call_flags)
{
  Session *sesn_ptr = ctx_ptr->sesn_ptr;
  Fence *f_ptr = FENCESTATE_FENCE(fstate_ptr);

  _GENERATE_FENCE_COMMAND_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForFence(&envelope_marshal, sesn_ptr, f_ptr, event_ptr, data_msg_ptr, FENCE_COMMAND__COMMAND_TYPES__INVITE_REJECTED, COMMAND_ARGS__ACCEPTED);
  ...
  GetSessionForThisUserByUserId(sesn_ptr, UfsrvUidGetSequenceId(uid_invited_by), &lock_already_owned, sesn_call_flags);
  InstanceHolderForSession *instance_sesn_ptr_invited_by = (InstanceHolderForSession *)SESSION_RESULT_USERDATA(sesn_ptr);

  //>>>>>>> INVITED_BY SESSION NOT LOCKED

  if (IS_PRESENT(instance_sesn_ptr_invited_by)) {
    UserRecord	*user_records_invited[1];
    user_records_invited[0]							=	fence_command.originator;
    fence_record.invited_members				=	user_records_invited; fence_record.n_invited_members =	1;

    _MarshalFenceInvitation(ctx_ptr, NULL, f_ptr, NULL, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));

    header.args = COMMAND_ARGS__SYNCED;
    FenceRawSessionList raw_session_list = {0};
    GetRawMemberUsersListForFence(sesn_ptr, instance_fence_ctx->instance_f_ptr, FENCE_CALLFLAG_INCLUDE_REMOTE_SESSIONS, &raw_session_list);//no locking
    if (raw_session_list.sessions_sz > 0) {
      for (size_t i = 0; i < raw_session_list.sessions_sz; i++) {
        ...
        _MarshalFenceInvitation(ctx_ptr, &(InstanceContextForSession){.instance_sesn_ptr=raw_session_list.sessions[i], .sesn_ptr=sesn_ptr_listed, .is_locked=false}, f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
      }

      DestructFenceRawSessionList(&raw_session_list, false);
    }

    FenceRawSessionList raw_session_list_invited = {0};
    GetRawInvitedUsersListForFence(sesn_ptr, instance_fence_ctx, FENCE_CALLFLAG_INCLUDE_REMOTE_SESSIONS, &raw_session_list_invited);//no locking
    if (!instance_fence_ctx->lock_already_owned)	{
      FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, instance_fence_ctx->f_ptr, SESSION_RESULT_PTR(sesn_ptr));
      instance_fence_ctx->is_locked = false;
    }

    if (raw_session_list_invited.sessions_sz > 0) {
      for (size_t i = 0; i < raw_session_list_invited.sessions_sz; i++) {
        Session *sesn_ptr_listed = SessionOffInstanceHolder(raw_session_list_invited.sessions[i]);
        _MarshalFenceInvitation(ctx_ptr, &(InstanceContextForSession){.instance_sesn_ptr=raw_session_list_invited.sessions[i], .sesn_ptr=sesn_ptr_listed, .is_locked=false}, f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
      }

      DestructFenceRawSessionList(&raw_session_list_invited, false);
    }

  } else {
    syslog(LOG_ERR, "%s {pid:'%lu', o:'%p', fid:'%lu', uid_invited_by:'%lu'} ERROR: SESSION FOR INVITED-BY USER COULD NOT BE LOADED", __func__, pthread_self(), ctx_ptr->sesn_ptr, FENCE_ID(f_ptr), UfsrvUidGetSequenceId(uid_invited_by));
    HandleFenceCommandError(ctx_ptr, fstate_ptr, wsm_ptr_received, data_msg_ptr, RESCODE_FENCE_MEMBERSHIP, header.command, NULL);
  }
...
/**
 * @brief Send an invitation deletion (uninvite) message which can be by or the original inviter, the admin.
 * The message will be sent to current members, plus invited members. The semantics are slightly different when the invitee
 * deletes a message (AKA rejection). See @ref MarshalFenceInvitationRejected.
 * @param ctx_ptr Session context for the user who initiated this message
 * @param sesn_ctx_uninvited Session context for the user being uninvited
 * @param f_ptr Fence under consideration
 * @param uid_invited_by user sequence id of inviter
 * @param wsm_ptr_received WSM context for the user who initiated this message
 * @param data_msg_ptr DataMessage for the user who initiated this message
 * @param call_flags
 * @locked f_ptr, sesn_ptr, sesn_ptr_uninvited
 * @unlocks f_ptr unless FENCE_CALLFLAG_KEEP_FENCE_LOCKED is passed
 * @return
 */
UFSRVResult *
MarshalFenceInvitationDeleted(InstanceContextForSession *ctx_ptr, InstanceContextForSession *sesn_ctx_uninvited, FenceStateDescriptor	*fstate_ptr, InstanceContextForFence *instance_fence_ctx, UfsrvUid *uid_invited_by, FenceEvent *event_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, unsigned fence_call_flags)
{
  Session *sesn_ptr = ctx_ptr->sesn_ptr;

  _GENERATE_FENCE_COMMAND_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForFence(&envelope_marshal, sesn_ptr, instance_fence_ctx->f_ptr, event_ptr, data_msg_ptr, FENCE_COMMAND__COMMAND_TYPES__INVITE_DELETED, COMMAND_ARGS__ACCEPTED);
  ...
  if (IS_PRESENT(instance_sesn_ptr_invited_by)) {
    UserRecord user_record_uninvited = {0};
    UserRecord	*user_records_uninvited[1];
    user_records_uninvited[0]			=	ProvideUserRecordFromSessionInProto(sesn_ctx_uninvited->sesn_ptr, &user_record_uninvited, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
    fence_record.invited_members	=	user_records_uninvited; fence_record.n_invited_members =	1;

    _MarshalFenceInvitation(ctx_ptr, NULL, instance_fence_ctx->f_ptr, NULL, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
    ...
    if (raw_session_list.sessions_sz > 0) {
      for (size_t i = 0; i < raw_session_list.sessions_sz; i++) {
        ...
        _MarshalFenceInvitation(ctx_ptr, &(InstanceContextForSession){.instance_sesn_ptr=raw_session_list.sessions[i], .sesn_ptr=sesn_ptr_listed, .is_locked=false}, instance_fence_ctx->f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
      }
    ...
    if (raw_session_list_invited.sessions_sz > 0) {
      for (size_t i = 0; i < raw_session_list_invited.sessions_sz; i++) {
        Session *sesn_ptr_listed = SessionOffInstanceHolder(raw_session_list_invited.sessions[i]);
        _MarshalFenceInvitation(ctx_ptr, &(InstanceContextForSession){.instance_sesn_ptr=raw_session_list_invited.sessions[i], .sesn_ptr=sesn_ptr_listed, .is_locked=false}, instance_fence_ctx->f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
      }

      DestructFenceRawSessionList(&raw_session_list_invited, false);
    }

    //lastly, inform the impacted user (as it won't be present in the invite list above)
    _MarshalFenceInvitation(ctx_ptr, sesn_ctx_uninvited, instance_fence_ctx->f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
  } else {
    syslog(LOG_ERR, "%s {pid:'%lu', o:'%p', fid:'%lu', uid_invited_by:'%lu'} ERROR: SESSION FOR INVITED-BY USER COULD NOT BE LOADED", __func__, pthread_self(), ctx_ptr->sesn_ptr, FENCE_ID(instance_fence_ctx->f_ptr), UfsrvUidGetSequenceId(uid_invited_by));
    HandleFenceCommandError(ctx_ptr, fstate_ptr, wsm_ptr_received, data_msg_ptr, RESCODE_FENCE_MEMBERSHIP, header.command, NULL);
  }
...
/**
 * @brief Marshal fence invite-list update message for a user blocking a fence
 * @warning Although the logic to process invited_by is present below, it's currently ignored from actual processing.
 * @param ctx_ptr
 * @param fstate_ptr
 * @param fence_context
 * @param uid_invited_by if known
 * @param event_ptr
 * @param wsm_ptr_received
 * @param data_msg_ptr
 * @param call_flags
 * @locks NONE
 * @locked WR @ref Fence f_ptr
 * @unlocks @ref Fence f_ptr
 * @return
 */
UFSRVResult *
MarshalFenceInvitationBlocked(InstanceContextForSession *instance_sesn_ctx, FenceStateDescriptor	*fstate_ptr, InstanceContextForFence *instance_fence_ctx, UfsrvUid *uid_invited_by, FenceEvent *event_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, unsigned call_flags)
{
  Session *sesn_ptr = instance_sesn_ctx->sesn_ptr;
  Fence *f_ptr = FENCESTATE_FENCE(fstate_ptr);

  _GENERATE_FENCE_COMMAND_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForFence(&envelope_marshal, sesn_ptr, f_ptr, event_ptr, data_msg_ptr, FENCE_COMMAND__COMMAND_TYPES__INVITE_DELETED, COMMAND_ARGS__ACCEPTED);
  ...
  if (true || IS_PRESENT(instance_sesn_ptr_invited_by)) {
    UserRecord *user_records_invited[1];
    user_records_invited[0] = fence_command.originator;
    fence_record.invited_members = user_records_invited;
    fence_record.n_invited_members = 1;

    _MarshalFenceInvitation(instance_sesn_ctx, NULL, f_ptr, NULL, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
    ...
    if (raw_session_list.sessions_sz > 0) {
      for (size_t i = 0; i < raw_session_list.sessions_sz; i++) {
        ...
        _MarshalFenceInvitation(instance_sesn_ctx,
                                &(InstanceContextForSession) {.instance_sesn_ptr=raw_session_list.sessions[i], .sesn_ptr=sesn_ptr_listed, .is_locked=false},
                                f_ptr, wsm_ptr_received, &command_envelope, UfsrvUidGetInstanceId(uid_invited_by));
      }
...
_MarshalFenceInvitation(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_target, Fence *f_ptr, WebSocketMessage *wsm_ptr_received,  Envelope *command_envelope_ptr, unsigned long eid)
{
  ...
  ProvideUfsrvUidInProto(sesn_ptr_target? &SESSION_UFSRVUIDSTORE(sesn_ptr_target) : &SESSION_UFSRVUIDSTORE(sesn_ptr), &(command_header_ptr->ufsrvuid), true); //ufsrvuid by reference
...
_MarshalFenceStateSyncForInvite(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_newly_invited, Fence *f_ptr, FenceEvent *fence_event_ptr, unsigned call_flags)
{
  ...
  ProvideFenceRecordInProtoAsIdentifier(sesn_ptr, f_ptr, &fence_record, false);

	UserRecord	*user_records_invited[1];
	UserRecord 	user_record_invited;

  ProvideUserRecordFromSessionInProto(ctx_ptr_newly_invited->sesn_ptr, &user_record_invited, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
	...
	for (eptr=f_ptr->fence_user_sessions_list.head; eptr; eptr=eptr->next) {
	  Session *sesn_ptr_listed = SessionOffInstanceHolder(eptr->whatever);
        ...
        _WireMarshalFenceStateSync(ctx_ptr, &(InstanceContextForSession) {(InstanceHolderForSession *) eptr->whatever,
                                                                          sesn_ptr_target}, f_ptr, &command_envelope);
      }
	}
...
_MarshalFenceInviteUnchanged(InstanceContextForSession *ctx_ptr, unsigned long userid, Fence *f_ptr, DataMessage *data_msg_ptr_orig, unsigned call_flags)
{
  ...
  ProvideUserRecordFromUseridInProto(sesn_ptr, userid, &user_record_invited);
	user_records_invited[0]							=	&user_record_invited;
	fence_record.invited_members				=	user_records_invited; fence_record.n_invited_members =	1;
...
_MarshalFenceSyncInviteForSelf(InstanceContextForSession *ctx_ptr_inviter, InstanceContextForSession *ctx_ptr_newly_invited, FenceStateDescriptor *fence_state_ptr, FenceEvent *fence_event_ptr, unsigned call_flags)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr_newly_invited,
                            &(InstanceContextForFence) {FENCESTATE_INSTANCE_HOLDER(fence_state_ptr), f_ptr},
                            &fence_record);

  ProvideUfsrvUidInProto(&(SESSION_UFSRVUIDSTORE(sesn_ptr_inviter)), &(fence_record.by_user), true); //ufsrvid by reference
	fence_record.has_by_user = 1;
	fence_record.by_user_when						=	fence_state_ptr->when_invited; fence_record.has_by_user_when = 1;
	fence_record.has_eid	=	0; //todo: this is hack because we dont have access to historic eids at this stage

  _WireMarshalFenceStateSync(ctx_ptr_newly_invited, NULL, f_ptr, &command_envelope);

	fence_record.by_user.data = NULL; //ufsrvid by reference
 ...
 * 	2)user declining an invite (this is now handled by explicit reject fence command)
 *
...
//  UserRecord				user_record_originator;
  fence_command.originator						= ProvideUserRecordFromSessionInProto(sesn_ptr_originator == NULL? GetUfsrvSystemUser() : sesn_ptr_originator, &_user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
  ProvideFenceRecordInProtoAsIdentifier(sesn_ptr_originator, f_ptr, &fence_record, false);

	UserRecord	*user_records_uninvited[1];
	UserRecord 	user_record_uninvited;

  ProvideUserRecordFromSessionInProto(sesn_ptr_uninvited, &user_record_uninvited, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
    ...
    _WireMarshalFenceStateSync(ctx_ptr_originator, NULL, f_ptr, &command_envelope);
	} else {
    //system initiated
    _WireMarshalFenceStateSync(ctx_ptr_originator, ctx_ptr_uninvited, f_ptr, &command_envelope);
  }
...
//			if	(sesn_ptr_target==sesn_ptr_uninvited)	continue;	//this user already got the msg; nothing to sync
    ...
    _WireMarshalFenceStateSync(ctx_ptr_originator, &(InstanceContextForSession) {(InstanceHolderForSession *) eptr->whatever, sesn_ptr_target}, f_ptr, &command_envelope);
	}
...
_VerifyInviteCommand(InstanceContextForSession *ctx_ptr, FenceStateDescriptor *fence_state_ptr, DataMessage *data_msg_ptr, WebSocketMessage *wsm_ptr_received, unsigned long fence_call_flags)
{
	...
	if (fence_record_ptr->n_invited_members <= 0 || fence_record_ptr->n_invited_members > CONFIG_MAX_INVITE_SET_SIZE) {
      ...
      HandleFenceCommandError(ctx_ptr, &((FenceStateDescriptor) {.instance_holder_fence=FENCESTATE_INSTANCE_HOLDER(fence_state_ptr)}),
                              wsm_ptr_received, data_msg_ptr, RESCODE_FENCE_INVITE_SIZE, data_msg_ptr->ufsrvcommand
                                                                                                     ->fencecommand
                                                                                                     ->header
                                                                                                     ->command, NULL);
			_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_INVITE_SIZE)
...
_CommandControllerFenceName(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr)
{
  ...
  InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
	...
	FenceCommand	*fcmd_ptr;
#define _FENCE_CALL_FLAGS_FENCE_NAME	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//locks by default
	IsUserAllowedToChangeFence(sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_FENCE_NAME);
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr))	{
	  ...
	  fstate_ptr           =  FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
        ...
        fstate_ptr = FenceStateDescriptorOffInstanceHolder((InstanceHolderForFenceStateDescriptor *)SESSION_RESULT_USERDATA(sesn_ptr));
        f_ptr           = FENCESTATE_FENCE(fstate_ptr);
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
			} else { //RESCODE_FENCE_MEMBERSHIP
			  InstanceHolderForFence *instance_f_ptr = (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
        f_ptr		=	FenceOffInstanceHolder(instance_f_ptr);
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
      }
			if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
			goto exit_catch_all;
		}
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                            sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
			goto exit_catch_all;
		}
	...
	if (IS_STR_LOADED((fname_new = fcmd_ptr->fences[0]->fname))) {
		IsUserAllowedToChangeFenceName(sesn_ptr, fstate_ptr, fname_new, _FENCE_CALL_FLAGS_FENCE_NAME, &fence_event);
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
			exit_success:
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_orig, data_msg_ptr, SESSION_RESULT_CODE(sesn_ptr), FENCE_COMMAND__COMMAND_TYPES__FNAME, NULL);
	} else {
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_orig, data_msg_ptr, RESCODE_PROG_MISSING_PARAM, FENCE_COMMAND__COMMAND_TYPES__FNAME, NULL);
	}
...
#undef _FENCE_CALL_FLAGS_FENCE_NAME
}
  ...
  Session *sesn_ptr = ctx_ptr->sesn_ptr;
    ...
    Session *sesn_ptr_listed = SessionOffInstanceHolder(raw_session_list.sessions[i]);
		if (SESSION_ID(sesn_ptr) == SESSION_ID(sesn_ptr_listed)) {
		  ...
		  WireMarshalFenceCommandToUser(ctx_ptr, &(InstanceContextForSession){raw_session_list.sessions[i], sesn_ptr_listed}, f_ptr, wsm_ptr_received, &command_envelope,  uFENCE_V1_IDX);
		}
	}
...
//// FENCE DESCRIPTION
inline static UFSRVResult *
_CommandControllerFenceDescription(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received)
{
  bool fence_already_locked = false;
  InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
  FenceStateDescriptor *fstate_ptr;
  FenceEvent 		fence_event 			= {0};
  FenceCommand	*fcmd_ptr;
#define _FENCE_CALLFLAGS_DESCRIPTION	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_CHECK_PERMESSIONS|FENCE_CALLFLAG_CHECK_PERM_PRESENTATION|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

  //locks by default unless flagged otherwise
  IsUserAllowedToChangeFence(sesn_ptr, data_msg_ptr_received->ufsrvcommand->fencecommand->fences[0]->fid, NULL, &fence_already_locked, _FENCE_CALLFLAGS_DESCRIPTION);
  ...
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr))	{
    ...
    if (SESSION_RESULT_TYPE_ERROR(sesn_ptr) && !SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
      ...
      if (SESSION_RESULT_CODE(sesn_ptr) == RESCODE_FENCE_OWNERSHIP) {
        fstate_ptr = FenceStateDescriptorOffInstanceHolder((InstanceHolderForFenceStateDescriptor *)SESSION_RESULT_USERDATA(sesn_ptr));
        f_ptr           = FENCESTATE_FENCE(fstate_ptr);
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_received, data_msg_ptr_received, rescode, data_msg_ptr_received->ufsrvcommand->fencecommand->header->command, NULL);
      } else { //RESCODE_FENCE_MEMBERSHIP or other
        InstanceHolderForFence *instance_f_ptr = (InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
        f_ptr		=	FenceOffInstanceHolder(instance_f_ptr);
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_received, data_msg_ptr_received, rescode, data_msg_ptr_received->ufsrvcommand->fencecommand->header->command, NULL);
      }
      if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
      goto exit_catch_all;
    }
    ...
    if (IS_EMPTY(SESSION_RESULT_USERDATA(sesn_ptr)) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
      //fence doesn't exist. NO lock
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_received, data_msg_ptr_received, rescode, data_msg_ptr_received->ufsrvcommand->fencecommand->header->command, NULL);
      goto exit_catch_all;
    }
  ...
  fcmd_ptr = data_msg_ptr_received->ufsrvcommand->fencecommand;
  const char *description_new = fcmd_ptr->fences[0]->description; //can be NULL
  IsUserAllowedToChangeFenceDescription(sesn_ptr, fstate_ptr, description_new, _FENCE_CALLFLAGS_DESCRIPTION, &fence_event);
  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
    _MarshalFenceDescriptionUpdate(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, FENCESTATE_INSTANCE_HOLDER(fstate_ptr), wsm_ptr_received, data_msg_ptr_received, 0, &fence_event);
    if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, FENCESTATE_FENCE(fstate_ptr), SESSION_RESULT_PTR(sesn_ptr));

    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_NULL_POINTER)
  }
  else {
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, fstate_ptr, wsm_ptr_received, data_msg_ptr_received, SESSION_RESULT_CODE(sesn_ptr), FENCE_COMMAND__COMMAND_TYPES__DESCRIPTION, NULL);
  }
...
#undef _FENCE_CALLFLAGS_DESCRIPTION
}

inline static UFSRVResult *
_MarshalFenceDescriptionUpdate(InstanceContextForSession *ctx_ptr, InstanceHolderForFence *instance_f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_recieved, unsigned long call_flags, FenceEvent *fence_event_ptr)
{
  Fence *f_ptr = FenceOffInstanceHolder(instance_f_ptr);
  Session *sesn_ptr = ctx_ptr->sesn_ptr;

  _GENERATE_FENCE_COMMAND_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForFence(&envelope_marshal, sesn_ptr, f_ptr, fence_event_ptr, data_msg_ptr_recieved, FENCE_COMMAND__COMMAND_TYPES__DESCRIPTION, COMMAND_ARGS__UPDATED);

  //actual delta
  fence_record.description = FENCE_DESCRIPTION(f_ptr);//by reference and could be NULL
...
_CommandControllerFenceAvatar(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr)
{
	...
	InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
	FenceEvent 		fence_event 			= {0};
#define _FENCE_CALL_FLAGS_FENCE_AVATAR	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//locks by default
	IsUserAllowedToChangeFence (sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_FENCE_AVATAR);
...
#undef _FENCE_CALL_FLAGS_FENCE_AVATAR
}
...
_MarshalFenceAvatarUpdate(InstanceContextForSession *ctx_ptr, InstanceHolderForFence *instance_f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, unsigned long call_flags, FenceEvent *fence_event_ptr)
{
  ...
  TEMPProvideAttachmentRecordFromAttachmentPointerInProto(data_msg_ptr_received->group->avatar, &collection_attachment_records);
//	fence_command.attachments														=	attachment_records;
//	fence_command.n_attachments													=	1;
  fence_record.avatar                                 = &attachment_record;
	//end delta
...

{
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
    ...
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                          sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_INVALID_MAXMEMBERS, data_msg_ptr->ufsrvcommand
                                                                                                                                                     ->fencecommand
                                                                                                                                                     ->header
                                                                                                                                                     ->command, NULL);
    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_MISSING_PARAM)
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr))	{
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                            sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=FENCESTATE_INSTANCE_HOLDER(fstate_ptr)}), wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_PERMISSION, data_msg_ptr->ufsrvcommand
                                                                                                                                                                                                                                    ->fencecommand
                                                                                                                                                                                                                                    ->header
                                                                                                                                                                                                                                    ->command, _RestoreFenceMaxMembers);
			if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, FENCESTATE_FENCE(fstate_ptr), SESSION_RESULT_PTR(sesn_ptr));
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                            sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=(InstanceHolderForFence *) SESSION_RESULT_USERDATA(sesn_ptr)}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand
                                                                                                                                                                                                                                         ->fencecommand
                                                                                                                                                                                                                                         ->header
                                                                                                                                                                                                                                         ->command, _RestoreFenceMaxMembers);
			if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
...
static void
_RestoreFenceMaxMembers(FenceStateDescriptor * _Nonnull fence_state_ptr, FenceRecord * _Nonnull fence_record_ptr)
{
...
//	maxmembers_by_user = fcmd_ptr->fences[0]->maxmembers;
//	if (maxmembers_by_user<0 || maxmembers_by_user > INT_MAX) {
//		HandleFenceCommandError (sesn_ptr, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_INVALID_MAXMEMBERS, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
//		goto exit_catch_all;
//	}
//
//	//locks by default
...
//			HandleFenceCommandError (sesn_ptr, &((FenceStateDescriptor){.fence=FENCESTATE_FENCE(fence_state_ptr)}), wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_PERMISSION, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceMaxMembers);
...
//			HandleFenceCommandError (sesn_ptr, &((FenceStateDescriptor){.fence=f_ptr}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceMaxMembers);
//			if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
//			goto exit_catch_all;
//		}
//		if (IS_EMPTY(SESSION_RESULT_USERDATA(sesn_ptr)) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST))
//		{
//			//fence doesn't exist. NO lock
//			HandleFenceCommandError (sesn_ptr, NULL, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
//			goto exit_catch_all;
//		}
//
//		exit_catch_all:
...
//		{
//			header.args	=	COMMAND_ARGS__ACCEPTED;//self
//			WireMarshalFenceCommandToUser(sesn_ptr, NULL, f_ptr, &command_envelope,  uFENCE_V1_IDX);
//			header.args	=	COMMAND_ARGS__UPDATED;//restore for others
//		}
//		else
//		{
//			header.cid	=	SESSION_ID(raw_session_list.sessions[i]);
//			WireMarshalFenceCommandToUser(sesn_ptr, raw_session_list.sessions[i], f_ptr, &command_envelope,  uFENCE_V1_IDX);
//		}
//	}
//
//	DestructFenceRawSessionList (&raw_session_list, false);
...
//	//locks by default
    ...
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr), wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_PERMISSION, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceDeliveryMode);
    if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, FENCESTATE_FENCE(fstate_ptr), SESSION_RESULT_PTR(sesn_ptr));

    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_FENCE_PERMISSION)
	} else {
    //No FenceStateDescriptor object is available
		InstanceHolderForFence *instance_f_ptr		=	(InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
		int 	rescode		=	SESSION_RESULT_CODE(sesn_ptr);

		if (SESSION_RESULT_TYPE_ERROR(sesn_ptr) && !SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
			//FENCE LOCKED... membership
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceDeliveryMode);
		...
		if (IS_EMPTY(SESSION_RESULT_USERDATA(sesn_ptr)) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
			//fence doesn't exist. NO lock
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
			goto exit_catch_all;
		}
...
//
  ...
  HandleFenceCommandError(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_MISSING_PARAM, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
		goto exit_error;

	exit_invalid_delivery_mode:
  HandleFenceCommandError(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_INVALID_DELIVERY_MODE, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
	goto exit_error;
...
_MarshalFenceDeliveryModeUpdate(InstanceContextForSession *ctx_ptr, InstanceHolderForFence *instance_f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, unsigned long call_flags, FenceEvent *fence_event_ptr)
{
  ...
  Session *sesn_ptr = ctx_ptr->sesn_ptr;
	...
	//actual delta
  ProvideFenceDeliveryModeInProto(f_ptr, &fence_record);
	//end delta
...
#define _FENCE_CALL_FLAGS_FENCEEXPIRY	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	//locks by default
	IsUserAllowedToChangeFence(sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_FENCEEXPIRY);
    ...
    InstanceHolderForFence *instance_f_ptr = NULL;
      ...
      if (SESSION_RESULT_CODE(sesn_ptr) == RESCODE_FENCE_OWNERSHIP) {
        ...
        fstate_ptr = FenceStateDescriptorOffInstanceHolder(instance_fstate_ptr);
        f_ptr           = FENCESTATE_FENCE(fstate_ptr);
      } else { //RESCODE_FENCE_MEMBERSHIP no access to FenceStateDescriptor
        instance_f_ptr		=	(InstanceHolderForFence *)SESSION_RESULT_USERDATA(sesn_ptr);
        f_ptr = FenceOffInstanceHolder(instance_f_ptr);
      }
      ...
      if (SESSION_RESULT_CODE(sesn_ptr) == RESCODE_FENCE_OWNERSHIP) {
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, fstate_ptr, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceExpiryTimer);
      } else { //RESCODE_FENCE_MEMBERSHIP
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=instance_f_ptr}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, _RestoreFenceExpiryTimer);
      }
			if (!fence_already_locked)	FenceEventsUnLockCtx(THREAD_CONTEXT_PTR, f_ptr, SESSION_RESULT_PTR(sesn_ptr));
			goto exit_catch_all;
		}
...
#define _FENCE_CALL_FLAGS_FENCEPERMISSION	(FENCE_CALLFLAG_CHECK_FENCEOWNERSHIP|FENCE_CALLFLAG_KEEP_FENCE_LOCKED)
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
    ...
    Fence *f_ptr    = NULL;
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                            sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=(InstanceHolderForFence *) SESSION_RESULT_USERDATA(sesn_ptr)}), wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand
                                                                                                                                                                                                                                         ->fencecommand
                                                                                                                                                                                                                                         ->header
                                                                                                                                                                                                                                         ->command, NULL/*_RestoreFenceExpiryTimer*/);
		...
		if (IS_EMPTY(SESSION_RESULT_USERDATA(sesn_ptr)) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
			//fence doesn't exist. NO lock
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                            sesn_ptr}, NULL, wsm_ptr_orig, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand
                                                                                                                              ->fencecommand
                                                                                                                              ->header
                                                                                                                              ->command, NULL);
			goto exit_catch_all;
    ...
    HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                          sesn_ptr}, fstate_ptr, wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_PERMISSION, data_msg_ptr->ufsrvcommand
                                                                                                                                                   ->fencecommand
                                                                                                                                                   ->header
                                                                                                                                                   ->command, NULL);
		goto	exit_unlock;
	...
	if (IS_PRESENT(sesn_ptr_target) && sesn_ptr_target != sesn_ptr) {
			...
			if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr_target)) {
				...
				_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_NULL_POINTER)
			} else if (SESSION_RESULT_CODE_EQUAL(sesn_ptr_target, RESCODE_FENCE_PERMISSION_MEMBERSHIP)) {
        HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr,
                                                              sesn_ptr}, &((FenceStateDescriptor) {.instance_holder_fence=FENCESTATE_INSTANCE_HOLDER(fstate_ptr)}), wsm_ptr_orig, data_msg_ptr, RESCODE_FENCE_PERMISSION_MEMBERSHIP, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
			}
...
_MarshalFencePermissionUpdate(InstanceContextForSession *ctx_ptr,  FencePermissionContextData *context_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, unsigned long call_flags, FenceEvent *fence_event_ptr)
{
  ...
  ProvideUserRecordFromSessionInProto(sesn_ptr_target, &user_record, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
	...
	fence_permission.type								=	(FenceRecord__Permission__Type)context_ptr->permission_ptr->type; //aligned with protobuf enum
	fence_command.type									=	(FenceRecord__Permission__Type)context_ptr->permission_ptr->type; fence_command.has_type = 1;
...
_CommandControllerFenceStateSyncSynced(InstanceHolderForSession *instance_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr)
{
	...
	FenceStateDescriptor 	*fstate_ptr           = NULL;
  InstanceHolderForFenceStateDescriptor *instance_fstate_ptr;
#define _FENCE_CALL_FLAGS_STATE_SYNC	(FENCE_CALLFLAG_KEEP_FENCE_LOCKED)

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	//locks by default, only checking for membership
	IsUserAllowedToChangeFence(sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->cname, &fence_already_locked, _FENCE_CALL_FLAGS_STATE_SYNC);
	...
	if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr))	{
      ...
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr},
		...
		if (IS_EMPTY(SESSION_RESULT_USERDATA(sesn_ptr)) && SESSION_RESULT_CODE_EQUAL(sesn_ptr, RESCODE_FENCE_DOESNT_EXIST)) {
			//fence doesn't exist. NO lock
      HandleFenceCommandError(&(InstanceContextForSession) {instance_sesn_ptr, sesn_ptr}, NULL, wsm_ptr_received, data_msg_ptr, rescode, data_msg_ptr->ufsrvcommand->fencecommand->header->command, NULL);
			goto exit_catch_all;
		}
...
#undef _FENCE_CALL_FLAGS_STATE_SYNC
}
...
MarshalFenceStateSync(InstanceContextForSession *ctx_ptr, FenceStateDescriptor *fstate_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, unsigned long call_flags)
{
  ...
  ProvideFenceRecordInProto(ctx_ptr, &(InstanceContextForFence) {FENCESTATE_INSTANCE_HOLDER(fstate_ptr), f_ptr}, &fence_record); //we need more info on the fence than just identifier
  ProvideFenceUserPreferencesInProto(sesn_ptr, fstate_ptr, &fence_record);
...
_MarshalServerRequestForFenceStateSync(InstanceContextForSession *ctx_ptr, Fence *f_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, unsigned long call_flags)
{
  ...
  UfsrvCommandInvokeUserCommand(ctx_ptr, NULL, wsm_ptr_received, NULL, &ufsrv_description, uFENCE_V1_IDX);
 ...
 */
inline static UFSRVResult *
_WireMarshalFenceStateSync(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_target, Fence *f_ptr, Envelope *command_envelope_ptr)
{
  Session *sesn_ptr = ctx_ptr->sesn_ptr;
  Session *sesn_ptr_target = IS_PRESENT(ctx_ptr_target)? ctx_ptr_target->sesn_ptr : NULL;
...
#ifdef __UF_TESTING
	syslog (LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu', cid_target:'%lu' uname_target:'%s', fid:'%lu'} Syncing fence State ", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), SESSION_ID((sesn_ptr_target?sesn_ptr_target:sesn_ptr)), SESSION_USERNAME((sesn_ptr_target?sesn_ptr_target:sesn_ptr)), FENCE_ID(f_ptr));
#endif
 ...
 */
UFSRVResult *
WireMarshalFenceCommandToUser(InstanceContextForSession *ctx_ptr, InstanceContextForSession *ctx_ptr_target, Fence *f_ptr, WebSocketMessage *wsm_ptr_received, Envelope *command_envelope_ptr, unsigned req_cmd_idx)
{
	CommandHeader *command_header_ptr	=	command_envelope_ptr->ufsrvcommand->header;
	Session *sesn_ptr                 = ctx_ptr->sesn_ptr;
	Session *sesn_ptr_target          = IS_PRESENT(ctx_ptr_target) ? ctx_ptr_target->sesn_ptr : NULL;
  ...
  UfsrvCommandInvokeUserCommand(ctx_ptr, ctx_ptr_target, wsm_ptr_received, NULL, &ufsrv_description, req_cmd_idx);
...
_BuildErrorHeaderForFenceCommand(FenceCommand *fence_command_ptr, FenceCommand *fence_command_ptr_incoming, int errcode, int command_type)
{
    ...
    case RESCODE_USER_FENCE_KEY:
      header_ptr->args_error	=	FENCE_COMMAND__ERRORS__WRONG_KEY; 	    header_ptr->has_args_error	=	1;
      header_ptr->args				=	COMMAND_ARGS__REJECTED;									header_ptr->has_args				=	1;
      break;
    ...
    case RESCODE_FENCE_PERMISSION_PRESENTATION:
		case RESCODE_FENCE_PERMISSION_MEMBERSHIP:
    ...
    case RESCODE_FENCE_PERMISSION_PROHIBITED:
    ...
    case RESCODE_FENCE_JOIN_LINKJOIN_LIST:
      header_ptr->args_error	=	FENCE_COMMAND__ERRORS__BY_REQUEST;    header_ptr->has_args_error	=	1;
      header_ptr->args				=	COMMAND_ARGS__REJECTED;								header_ptr->has_args				=	1;
      break;

    case RESCODE_FENCE_LINKJOIN_NONCE:
      header_ptr->args_error	=	FENCE_COMMAND__ERRORS__LINKJOIN_NONCE;header_ptr->has_args_error	=	1;
      header_ptr->args				=	COMMAND_ARGS__REJECTED;								header_ptr->has_args				=	1;
      break;

    case RESCODE_FENCE_BANNED_LIST:
      header_ptr->args_error	=	errcode; 	                            header_ptr->has_args_error	=	1;
      header_ptr->args				=	COMMAND_ARGS__REJECTED;								header_ptr->has_args				=	1;
      break;
 ...
 */
UFSRVResult *
HandleFenceCommandError(InstanceContextForSession *ctx_ptr, FenceStateDescriptor * _Nullable fence_state_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, int rescode, FenceCommand__CommandTypes command_type, CallbackRestoreFenceValue _Nullable restore_callback)
{
	Envelope 					command_envelope	= ENVELOPE__INIT;
	CommandHeader 		header						= COMMAND_HEADER__INIT;
	UfsrvCommandWire	ufsrv_command			= UFSRV_COMMAND_WIRE__INIT;
	...
	if (IS_PRESENT(fence_state_ptr)) {
		fence_records[0]										= ProvideFenceRecordInProtoAsIdentifier(sesn_ptr, FENCESTATE_FENCE(fence_state_ptr), &fence_record, false);
    ...
    if (IS_PRESENT(restore_callback))	(*restore_callback)(fence_state_ptr, &fence_record);
		...
		fence_records[0]										= ProvideFenceRecordInProtoAsIdentifierByParams(sesn_ptr, data_msg_ptr->ufsrvcommand->fencecommand->fences[0]->fid, &fence_record);
    if (IS_PRESENT(restore_callback))	(*restore_callback)(NULL, &fence_record);
	}
	...
	return (WireMarshalFenceCommandToUser(ctx_ptr, NULL,  IS_PRESENT(fence_state_ptr)?FENCESTATE_FENCE(fence_state_ptr):NULL, wsm_ptr_received, &command_envelope,  uFENCE_V1_IDX));
