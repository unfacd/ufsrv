...
#include <include/incoming_message_descriptor_type.h>
#include <include/command_base_context_type.h>
...
#include <ufsrvmsg_core/include/ufsrv_events.h>
...
#include <include/message_command_broadcast.h>
...
#include <ufsrvmsg_core/include/attachments.h>
...
#define _GENERATE_MESSAGECOMMAND_ENVELOPE_INITIALISATION() \
	...
	__block Envelope												command_envelope		=	ENVELOPE__INIT;	\
...
inline static void _PrepareMarshalMessageForUserMessage(MarshalMessageEnvelopeForUser *envelope_ptr, Session *sesn_ptr, Fence *f_ptr, UfsrvEvent *event_ptr, DataMessage *data_msg_ptr_orig, enum _MessageCommand__CommandTypes command_type, enum _CommandArgs command_arg);
inline static UFSRVResult *_UfsrvCommandControllerMessageSay(InstanceContextForSession *ctx_sesn_ptr_local_user, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr_received, ParsedMessageDescriptor *msg_descriptor_ptr_out);
...
inline static UFSRVResult *_UfsrvCommandControllerMessageIntroUserResponse(InstanceContextForSession *ctx_sesn_ptr_local_user, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr, ParsedMessageDescriptor *msg_descriptor_ptr_out);
inline static UFSRVResult *_UfsrvCommandControllerMessageReported(CommandBaseContext *cmd_base_ctx);
static UFSRVResult *_UfsrvCommandControllerMessageRevoked(CommandBaseContext *cmd_base_ctx);
...
inline static UFSRVResult *_MarshalMessageToFence(InstanceContextForSession *ctx_sesn_ptr, WebSocketMessage *wsm_ptr_orig, DataMessage *data_msg_ptr_received, InstanceContextForFence *, FenceEvent *);
...
inline static UFSRVResult *_MarshalIntroMessageUserResponse(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, UfsrvEvent *event_ptr);
inline static UFSRVResult *_MarshalMessageCommandResponseToUser(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, Envelope *command_envelope_ptr);
...
static UFSRVResult *_HandleMessageCommandError(CommandBaseContext *cmd_base_ctx, int rescode, int command_type);
...
inline static void
_PrepareMarshalMessageForUserMessage(MarshalMessageEnvelopeForUser *envelope_ptr, Session *sesn_ptr, Fence *f_ptr, UfsrvEvent *event_ptr, DataMessage *data_msg_ptr_orig, enum _MessageCommand__CommandTypes command_type, enum _CommandArgs command_arg) {
...
//  envelope_ptr->message_command->originator = MakeUserRecordFromSessionInProto (sesn_ptr, envelope_ptr->originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
  ...
  envelope_ptr->envelope->sourceufsrvuid		=	UFSRV_SOURCE_ORIGIN;
...
CommandCallbackControllerMessageCommand(InstanceContextForSession *ctx_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, ParsedMessageDescriptor *msg_descriptor_ptr_out)
{
  ...
  goto *command_types[command_header->command];//computed goto statement
  ...
  MESSAGE_COMMAND__COMMAND_TYPES__REVOKE:
  return _UfsrvCommandControllerMessageRevoked(COMMAND_BASE_CONTEXT(&command_ctx));

  MESSAGE_COMMAND__COMMAND_TYPES__INTRO_USER_RESPONSE:
  return _UfsrvCommandControllerMessageIntroUserResponse(ctx_sesn_ptr, wsm_ptr_received, data_msg_ptr_received, msg_descriptor_ptr_out);
 ...
 * @param data_msg_ptr_received Contains command specific parameters and controls as packaged by sender
...
_UfsrvCommandControllerMessageSay(InstanceContextForSession *ctx_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, ParsedMessageDescriptor *msg_descriptor_ptr_out)
{
  ...
  msgcmd_ptr = data_msg_ptr_received->ufsrvcommand->msgcommand;
  ...
  if (fid > 0) {
    ...
    if (F_ATTR_IS_SET(FENCE_ATTRIBUTES(f_ptr), F_ATTR_BROADCAST_ONEWAY)) {
      if (SESSION_USERID(sesn_ptr) != FENCE_OWNER_UID(f_ptr)) {
        syslog(LOG_DEBUG, "%s (pid:'%lu', co:'%p', cid:'%lu', fo:'%p', fid:'%lu', uid_owner:'%lu', uid_sender:'%lu'): ERROR: F_ATTR_BROADCAST_ONE-WAY IS SET", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), f_ptr, fid, FENCE_OWNER_UID(f_ptr), SESSION_USERID(sesn_ptr));
    ...
    if (IS_EMPTY((RegisterFenceEvent(f_ptr, EVENT_TYPE_FENCE_USR_MSG, NULL, 0, &fence_event)))) {
    ...
    bool is_fence_public = IsFencePublic(f_ptr);

    DbBackendInsertUfsrvEvent((UfsrvEvent *) &fence_event);//generates gid for event
    if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
      ...
      UFSRVResult *res_ptr = _MarshalMessageToFence(ctx_sesn_ptr, wsm_ptr_received, data_msg_ptr_received, &fence_context, &fence_event);
      ...
      //prepare a packed message for entry into "messages:rawmsg" table
      if (is_fence_public) {
        msg_descriptor_ptr_out->eid = fence_event.eid;
        msg_descriptor_ptr_out->gid = fence_event.gid;
        _RETURN_RESULT_SESN(sesn_ptr,
                            _ParsedMessageDescriptorForMessageCommand(ctx_sesn_ptr, data_msg_ptr_received, msg_descriptor_ptr_out),
...
/**
 * @brief Main controller for Intro messages user response messages. A response message is sent to the user who originated the intro request.
 * @param sesn_ptr
 * @param wsm_ptr_orig
 * @param data_msg_ptr
 * @param mqp_ptr
 ...
 */
inline static UFSRVResult *
_UfsrvCommandControllerMessageIntroUserResponse(InstanceContextForSession *ctx_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr, ParsedMessageDescriptor *msg_descriptor_ptr_out)
{
  MessageCommand	*msgcmd_ptr;
  UFSRVResult 	*res_ptr			= NULL;
  ...
  //"to" must be ufsrvuid (no handle type for responses)
  IntroMessageRecord *record_ptr = msgcmd_ptr->intro;
  if (msgcmd_ptr->header->eid_client <= 0 ) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', eid_client:'%lu'}: ERROR: EID_CLIENT NOT SET...", __func__, pthread_self(), THREAD_CONTEXT_PTR, sesn_ptr, msgcmd_ptr->header->eid_client);

    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER)
  }

  if (!IS_STR_LOADED(record_ptr->to)  || strlen(record_ptr->to) > CONFIG_MAX_UFSRV_ID_ENCODED_SZ) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p'}: ERROR: COMMAND CONTAINED INVALID TO-UFSRVUID...", __func__, pthread_self(), THREAD_CONTEXT_PTR, sesn_ptr);

    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER)
  }

  //todo: check: originator must match the sender of this message to ensure receiving end has a proper IntroResponse message
  if (!IS_STR_LOADED(record_ptr->originator)  || strlen(record_ptr->originator) > CONFIG_MAX_UFSRV_ID_ENCODED_SZ) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p'}: ERROR: COMMAND CONTAINED INVALID ORIGINATOR-UFSRVUID...", __func__, pthread_self(), THREAD_CONTEXT_PTR, sesn_ptr);

    _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER)
  }

  //todo: retrieve eid_client and confirm to-recipient has actually sent the original intro, for which this user is responding to
  bool		lock_already_owned    = false;
  unsigned long sesn_call_flags	=	(CALL_FLAG_LOCK_SESSION|CALL_FLAG_LOCK_SESSION_BLOCKING|
                                     CALL_FLAG_HASH_SESSION_LOCALLY|CALL_FLAG_HASH_UID_LOCALLY| CALL_FLAG_HASH_USERNAME_LOCALLY|
                                     CALL_FLAG_ATTACH_FENCE_LIST_TO_SESSION|CALL_FLAG_REMOTE_SESSION);
...
//  GetSessionForThisUserByUserId(sesn_ptr, UfsrvUidGetSequenceId((UfsrvUid *)user_ptr->ufsrvuid.data), &lock_already_owned, sesn_call_flags);
  GetSessionFromUserHandle(AS_CARRIER_SESSION(sesn_ptr), record_ptr->to, &lock_already_owned, sesn_call_flags);
  InstanceHolderForSession *instance_sesn_ptr_target = (InstanceHolderForSession *)SESSION_RESULT_USERDATA(sesn_ptr);
  ...
  UfsrvEvent event = {0};
  RegisterUfsrvEvent(sesn_ptr, EVENT_TYPE_USER_INTRO, MESSAGE_COMMAND__COMMAND_TYPES__INTRO_USER_RESPONSE, NULL, &event);//note eid is being generated for the user who accepted ie this session (not target)

  if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr)) {
    InterBroadcastUserMessage(sesn_ptr, CLIENT_CTX_DATA(msgcmd_ptr), (FenceEvent *) &event, msgcmd_ptr->header->args);

    _MarshalIntroMessageUserResponse(ctx_sesn_ptr, instance_sesn_ptr_target, wsm_ptr_received, data_msg_ptr, &event);
...
IsUserAllowedToReportMessage(CommandBaseContext *cmd_base_ctx, CommandMarshallerCallback command_marshaller, unsigned long call_flags)
{
  ...
  if (record_ptr->gid > 0 && record_ptr->originator.len == 16 && IS_PRESENT(record_ptr->originator.data)) {
    ...
    DbBackendGetEventDescriptorByGid((event_descriptor.ufsrv_event.gid=record_ptr->gid, &event_descriptor));
    if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
      _FIND_FENCE(event_descriptor.ufsrv_event.ctx_id);
      //fence locked
      FenceEvent fence_event = {0};
      if (IS_EMPTY((RegisterFenceEvent(f_ptr, EVENT_TYPE_MSG_REPORTED, NULL, 0, &fence_event)))) {
        rescode = RESCODE_FENCE_EVENT_GENERATION; goto return_unlock_fence;
      }

      fence_event.originator_ptr  = &SESSION_UFSRVUIDSTORE(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr));
      fence_event.session_id	    =	SESSION_ID(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr));
      ...
      if (DbBackendUpdateMessageStatus(event_descriptor.ufsrv_event.gid, uid, EVENT_STATUS_REPORTED) == H_OK) {
        DbBackendUpdateEventFlagger(event_descriptor.ufsrv_event.gid, uid, msgcmd_ptr->header->when);
...
static UFSRVResult *_IsUserAllowedToRevokeMessage(CommandBaseContext *cmd_base_ctx, CommandMarshallerCallback command_marshaller, unsigned long call_flags);
static UFSRVResult *_MarshalMessageRevoked(CommandBaseContext *cmd_base_ctx);

static UFSRVResult *
_UfsrvCommandControllerMessageRevoked(CommandBaseContext *cmd_base_ctx) {
  MessageCommandContext *cmd_ctx_ptr = (MessageCommandContext *) cmd_base_ctx;
  MessageCommand *msgcmd_ptr = cmd_base_ctx->data_msg_ptr_received->ufsrvcommand->msgcommand;
  if (IS_EMPTY(msgcmd_ptr->revoked) || msgcmd_ptr->n_revoked < 1) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', n_revoked:'%lu'}: ERROR: COMMAND CONTAINED INVALID REVOKED CONTENT RECORD...", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), msgcmd_ptr->n_revoked);

    _RETURN_RESULT_SESN(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER)
  }

  _IsUserAllowedToRevokeMessage(cmd_base_ctx, _MarshalMessageRevoked, CALLFLAGS_EMPTY);

  if (SESSION_RESULT_TYPE_SUCCESS(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr))) return SESSION_RESULT_PTR(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr));

  int rescode = SESSION_RESULT_CODE(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr));
    switch (rescode) {
      case RESCODE_MESSAGE_REVOKED: //message was already revoked
      case RESCODE_BACKEND_RESOURCE_OWNER: //user did not author the message
      case RESCODE_BACKEND_RESOURCE_NULL: //resource was not found
      case RESCODE_FENCE_EVENT_GENERATION: //system processing error
        _HandleMessageCommandError(cmd_base_ctx, rescode, MESSAGE_COMMAND__COMMAND_TYPES__REVOKE);
    }

  return SESSION_RESULT_PTR(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr));//error
}

#include <uflib/utils_time.h>
__pure inline static bool
IsRevokedMessageTimingThresholdLapsed(long long when_sent)
{
  if (GetTimeNowInMillis() - when_sent >= DAY_TO_MILLI_SECONDS(1)) return true;
 ...
 */
static UFSRVResult *
_IsUserAllowedToRevokeMessage(CommandBaseContext *cmd_base_ctx, CommandMarshallerCallback command_marshaller, unsigned long call_flags)
{
  unsigned rescode = RESCODE_PROG_NULL_POINTER;
  bool fence_lock_already_owned = false;
  Fence *f_ptr = NULL;
  MessageCommandContext *cmd_ctx_ptr = (MessageCommandContext *) cmd_base_ctx;
  MessageCommand	*msgcmd_ptr = cmd_ctx_ptr->command_base_context.data_msg_ptr_received->ufsrvcommand->msgcommand;
  RevokedMessageRecord *revoked_message_ptr_received = msgcmd_ptr->revoked[0];

  if (revoked_message_ptr_received->gid > 0 && revoked_message_ptr_received->when_sent > 0) {
    if (IsRevokedMessageTimingThresholdLapsed(revoked_message_ptr_received->when_sent)) {
      rescode = RESCODE_TIME_THRESHOLD_LAPSED;
      syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu', when_sent:'%lu'}: ERROR: THRESHOLD LAPSED FOR MESSAGE..", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), revoked_message_ptr_received->gid, revoked_message_ptr_received->when_sent);
      goto return_error;
    }

    UfsrvEventDescriptor event_descriptor = {0};
    DbBackendGetEventDescriptorByGid((event_descriptor.ufsrv_event.gid=revoked_message_ptr_received->gid, &event_descriptor));
    if (THREAD_CONTEXT_UFSRV_RESULT_IS_SUCCESS_WITH_BACKEND_DATA) {
      if (event_descriptor.status == EVENT_STATUS_REVOKED) {
        syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu'}: ERROR: MESSAGE ALREADY REVOKED..", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), revoked_message_ptr_received->gid);
        rescode = RESCODE_MESSAGE_REVOKED;
        goto return_error;
      }

      unsigned long userid_originator = UfsrvUidGetSequenceId(&SESSION_UFSRVUIDSTORE(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr)));
      if (userid_originator != event_descriptor.userid_originator) {
        syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu', userid_originator:'%lu', stored_userid_originator:'%lu'}: ERROR: NO PERMISSION TO REVOKE MESSAGE", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), revoked_message_ptr_received->gid, userid_originator, event_descriptor.userid_originator);
        rescode = RESCODE_BACKEND_RESOURCE_OWNER;
        goto return_error;
      }
      ...
      FenceEvent fence_event = {0};
      if (IS_EMPTY((RegisterFenceEvent(f_ptr, EVENT_TYPE_MSG_REVOKED, NULL, 0, &fence_event)))) {
        rescode = RESCODE_FENCE_EVENT_GENERATION; goto return_unlock_fence;
      }
      ...
      int result = DbBackendUpdateEventStatus(event_descriptor.ufsrv_event.gid, EVENT_STATUS_REVOKED);

      if (result != H_OK) {
        syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu'}: ERROR: COULD NOT CHANGE EVENT STATUS...", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr),revoked_message_ptr_received->gid);

        rescode = RESCODE_BACKEND_RESOURCE_NULL; goto return_unlock_fence;
      }

      if (IsFencePublic(f_ptr)) {
        if (DbBackendUpdateMessageStatus(event_descriptor.ufsrv_event.gid, 0, EVENT_STATUS_REVOKED) != H_OK) {
          syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu'}: ERROR: COULD NOT CHANGE MESSAGE STATUS: IS MESSAGE DELETED ALREADY?", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), revoked_message_ptr_received->gid);
          //keep going...
        }
      }
      ...
      revoked_message_ptr_received->fid = FENCE_ID(f_ptr); revoked_message_ptr_received->has_fid = 1;
      InterBroadcastUserMessageRevoked((CommandBaseContext *)cmd_ctx_ptr, COMMAND_ARGS__ADDED);//update to COMMAND_ARGS__POSTED
      ...
      syslog(LOG_DEBUG, "%s {pid:'%lu', th_ctx:'%p', o:'%p', gid:'%lu'}: ERROR: COMMAND CONTAINED INVALID GID...", __func__, pthread_self(), THREAD_CONTEXT_PTR, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), revoked_message_ptr_received->gid);
      goto return_error;
    }
...
static UFSRVResult *
_MarshalMessageRevoked(CommandBaseContext *cmd_base_ctx)
{
  MessageCommandContext *cmd_ctx_ptr = (MessageCommandContext *) cmd_base_ctx;
  MessageCommand	*msgcmd_ptr = CMDCTX_DATA_MESSAGE(cmd_ctx_ptr)->ufsrvcommand->msgcommand;
  RevokedMessageRecord *revoked_messaged_record_ptr_received = msgcmd_ptr->revoked[0];

  _GENERATE_MESSAGECOMMAND_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForUserMessage(&envelope_marshal, CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), NULL, CMDCTX_EVENT(cmd_ctx_ptr), CMDCTX_DATA_MESSAGE(cmd_ctx_ptr), MESSAGE_COMMAND__COMMAND_TYPES__REVOKE, COMMAND_ARGS__ACCEPTED);

  RevokedMessageRecord *revoked_message_records[1];
  RevokedMessageRecord revoked_message_record = REVOKED_MESSAGE_RECORD__INIT;
  revoked_message_records[0] = &revoked_message_record;
  message_command.revoked = revoked_message_records; message_command.n_revoked = 1;

  revoked_message_record.gid = revoked_messaged_record_ptr_received->gid;
  revoked_message_record.when_sent = revoked_messaged_record_ptr_received->when_sent; revoked_message_record.has_when_sent = 1;
  ProvideUfsrvUidInProto((UfsrvUid *)&SESSION_UFSRVUIDSTORE(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr)), &revoked_message_record.author, PROTO_USER_RECORD_BYREF); revoked_message_record.has_author = 1;
  message_command.originator = ProvideUserRecordFromSessionInProto(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);

  _MarshalMessageCommandResponseToUser(CMDCTX_SESN_CTX_ORIGINATOR(cmd_ctx_ptr), NULL, CMDCTX_WSM(cmd_ctx_ptr), &command_envelope);
...
#endif
      _MarshalMessageCommandResponseToUser(CMDCTX_SESN_CTX_ORIGINATOR(cmd_ctx_ptr), raw_session_list.sessions[i], CMDCTX_WSM(cmd_ctx_ptr), &command_envelope);
    }
...
_MarshalMessageEffect(CommandBaseContext *cmd_base_ctx)
{
  ...
  if (raw_session_list.sessions_sz == 2) {
    envelope_marshal.header->args = COMMAND_ARGS__SYNCED;
    message_command.originator		= ProvideUserRecordFromSessionInProto(CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr),
                                                                        &user_record_originator,
                                                                        PROTO_USER_RECORD_MINIMAL,
                                                                        PROTO_USER_RECORD_BYREF);
...
_UfsrvCommandControllerMessageReaction(CommandBaseContext *cmd_base_ctx)
{
  ...
  MessageCommand	*msgcmd_ptr = cmd_base_ctx->data_msg_ptr_received->ufsrvcommand->msgcommand;
   ...
   if (IS_EMPTY(msgcmd_ptr->reaction) || msgcmd_ptr->reaction->fid <= 0) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', reaction:'%p'}: ERROR: COMMAND CONTAINED NO VALID REACTION RECORD...", __func__, pthread_self(), CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr), msgcmd_ptr->reaction);
...
inline static Envelope__Type _GetProtocolType(MessageCommand *msg_ptr, void(^on_ready)(Envelope__Type));
inline static Envelope__Type _GetProtocolType(MessageCommand *msg_ptr, void(^on_ready)(Envelope__Type))
{
	if (likely(msg_ptr->n_messages > 0 && IS_PRESENT(msg_ptr->messages[0]))) {
		on_ready((Envelope__Type)msg_ptr->messages[0]->protocol_type);
    return (Envelope__Type)msg_ptr->messages[0]->protocol_type;
	}

  on_ready(ENVELOPE__TYPE__UNKNOWN);
  return ENVELOPE__TYPE__UNKNOWN;
}

/**
 * @brief Marshal IntroContact message between two interacting parties:
 * Direction outgoing: Sender is identified by eid, timestamp on sender's envelop and sender's ufsrvuid
 * Direction incoming: Receiver gets senders eid_client, when_client and sender's s ufsrvuid + other ancillary info provided by user (eg avatar)
 * @param ctx_sesn_ptr
 * @param instance_sesn_ptr_target
 * @param wsm_ptr_received
 * @param data_msg_ptr_received
 * @param event_ptr
 * @return
 */
inline static UFSRVResult *
_MarshalIntroMessage(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, UfsrvEvent *event_ptr)
{
  ...
  _MarshalMessageCommandResponseToUser(ctx_sesn_ptr, NULL, wsm_ptr_received, &command_envelope);

  //only to target user, revealing the ufsrvuid of the user initiated the request
  message_command.header->args  = COMMAND_ARGS__SYNCED;
  message_command.originator    = ProvideUserRecordFromSessionInProto(sesn_ptr, &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);

  Session *sesn_ptr_target  = SessionOffInstanceHolder(instance_sesn_ptr_target);
//  intro_message.to          = msgcmd_ptr->intro->to; //no need for this
  intro_message.originator  = msgcmd_ptr->intro->originator;//todo: we are trusting the user value, should be rewritten by server for the sending user
  intro_message.to          = msgcmd_ptr->intro->to; //the handle as presented by the requester
  ...
  _MarshalMessageCommandResponseToUser(ctx_sesn_ptr, instance_sesn_ptr_target, wsm_ptr_received, &command_envelope);

  return SESSION_RESULT_PTR(sesn_ptr);
}

inline static UFSRVResult *
_MarshalIntroMessageUserResponse(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, UfsrvEvent *event_ptr)
{
  Session *sesn_ptr = SessionOffInstanceHolder(ctx_sesn_ptr->instance_sesn_ptr);

  _GENERATE_MESSAGECOMMAND_FOR_INTRO_ENVELOPE_INITIALISATION();
  _PrepareMarshalMessageForUserMessage(&envelope_marshal, sesn_ptr, CLIENT_CTX_DATA(SessionOffInstanceHolder(instance_sesn_ptr_target)), event_ptr, data_msg_ptr_received, MESSAGE_COMMAND__COMMAND_TYPES__INTRO_USER_RESPONSE, COMMAND_ARGS__SYNCED);

  MessageCommand		*msgcmd_ptr				= data_msg_ptr_received->ufsrvcommand->msgcommand;
  message_command.header->eid_client  = msgcmd_ptr->header->eid_client;   message_command.header->has_eid_client = 1;
  message_command.header->when_client = msgcmd_ptr->header->when_client;  message_command.header->has_when_client = 1;//_PrepareMarshalMessageForUserMessage copied arriving when, but semantics her are different, as we need the timestamp at which introrequest was first sent out
  message_command.intro               = msgcmd_ptr->intro; //shift as is

//  _MarshalMessageCommandResponseToUser(ctx_sesn_ptr, NULL, wsm_ptr_received, &command_envelope);//todo we need to let user know of their updated eid

  //only to user for whom response is targeted, revealing the ufsrvuid of the accepting party
  message_command.originator    = ProvideUserRecordFromSessionInProto(sesn_ptr, &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
...
#define _MARSHALL_GUARDIAN_REQUEST_ACCEPTED \
  ...
  UserRecord  *to_record_ptr =	ProvideUserRecordFromSessionInProto(SessionOffInstanceHolder(raw_session_list.sessions[i]), &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF); \
...
//  command_envelope.type     = _GetProtocolType(msgcmd_ptr); command_envelope.has_type = 1; //important as challenge is e2ee (only for initial request)
  _GetProtocolType(msgcmd_ptr, ^(Envelope__Type type){ command_envelope.type = type;
                                                       command_envelope.has_type = 1;
  });
...
_MarshalGuardianUnLink(CommandBaseContext *cmd_base_ctx)
{
  ...
  _MarshalMessageCommandResponseToUser(CMDCTX_SESN_CTX_ORIGINATOR(cmd_ctx_ptr), CMDCTX_MSG_TARGET_SESN_INSTANCE(cmd_ctx_ptr), CMDCTX_WSM(cmd_ctx_ptr), &command_envelope);
 ...
 */
inline static UFSRVResult *
_MarshalMessageToFence(InstanceContextForSession *ctx_sesn_ptr, WebSocketMessage *wsm_ptr_received, DataMessage *data_msg_ptr_received, InstanceContextForFence *ctx_f_ptr, FenceEvent *fence_event_ptr)
{
  Session *sesn_ptr = ctx_sesn_ptr->sesn_ptr;
  ...
  header.fid = FENCE_ID(f_ptr); header.has_fid = 1;
  ...
  command_envelope.servertimestamp	=	command_envelope.timestamp; command_envelope.has_servertimestamp = 1;
  _GetProtocolType(msgcmd_ptr, ^(Envelope__Type type){ command_envelope.type = type;
      command_envelope.has_type = 1;
  });
  ...
  if (SESSION_USERGUARDIAN_UID(ctx_sesn_ptr->sesn_ptr) > 0 && _GetProtocolType(msgcmd_ptr, NULL) != ENVELOPE__TYPE__CIPHERTEXT) {
      _MarshalMessageCommandSync(ctx_sesn_ptr, data_msg_ptr_received, &(WebSocketMessage){.request=NULL, .type=WEB_SOCKET_MESSAGE__TYPE__REQUEST}, &command_envelope);
  }

  header.command							=	MESSAGE_COMMAND__COMMAND_TYPES__SAY;
  header.args									=	COMMAND_ARGS__ACCEPTED; header.has_args = 1;

  fence_record_ptr										= ProvideFenceRecordInProtoAsIdentifier(sesn_ptr, f_ptr, &fence_record, false);
  ...
  _MarshalFenceMessageToUser(ctx_sesn_ptr, NULL, IS_EMPTY(wsm_ptr_received)? (&(WebSocketMessage){.request=NULL, .type=WEB_SOCKET_MESSAGE__TYPE__REQUEST}) : wsm_ptr_received, FENCE_ID(f_ptr), &command_envelope);
  ...
  header.args									=	COMMAND_ARGS__SYNCED;
	...
	if (F_ATTR_IS_SET(FENCE_ATTRIBUTES(f_ptr), F_ATTR_BROADCAST)) {
		InstanceHolderForSession *instance_sesn_ptr_fence_owner = GetSessionForFenceOwnerNonBlocking(sesn_ptr, FENCE_OWNER_UID(f_ptr));
	...
	if (raw_session_list.sessions_sz > 0) {
      ...
      if (!IsUserOnShareListBlocked(sesn_ptr_listed, sesn_ptr)) {
        _MarshalFenceMessageToUser(ctx_sesn_ptr, raw_session_list.sessions[i],
                                   ...
                                   (&(WebSocketMessage) {.request=NULL, .type=WEB_SOCKET_MESSAGE__TYPE__REQUEST}) : wsm_ptr_received,
...
_MarshalFenceMessageToUser(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, unsigned long fid, Envelope *command_envelope_ptr)
{
  ...
  Session *sesn_ptr_target = IS_PRESENT(instance_sesn_ptr_target)? SessionOffInstanceHolder(instance_sesn_ptr_target) : NULL;
	...
	UfsrvCommandMarshallingDescriptor ufsrv_description = {command_header_ptr->eid, fid, command_header_ptr->when, &EnvelopeMetaData, command_envelope_ptr, command_header_ptr->gid};
  ...
  UfsrvCommandInvokeUserCommand(ctx_sesn_ptr,
                                (IS_PRESENT(instance_sesn_ptr_target)? (&(InstanceContextForSession) {instance_sesn_ptr_target, sesn_ptr_target}) : NULL),
                                IS_EMPTY(wsm_ptr_received)? (&(WebSocketMessage){.request=NULL, .type=WEB_SOCKET_MESSAGE__TYPE__REQUEST}) : wsm_ptr_received,
...
_MarshalMessageCommandSync(InstanceContextForSession *ctx_sesn_ptr, DataMessage *dm_ptr_received, WebSocketMessage *wsm_ptr_received, Envelope *command_envelope_ptr_prepared)
{
  ...
  sync_command.originator = ProvideUserRecordFromSessionInProto(sesn_ptr, &user_record_originator, PROTO_USER_RECORD_MINIMAL, PROTO_USER_RECORD_BYREF);
...
inline static UFSRVResult *
_MarshalMessageCommandResponseToUser(InstanceContextForSession *ctx_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, Envelope *command_envelope_ptr)
{
  Session *sesn_ptr         = SessionOffInstanceHolder(ctx_sesn_ptr->instance_sesn_ptr);
  Session *sesn_ptr_target  = IS_PRESENT(instance_sesn_ptr_target) ? SessionOffInstanceHolder(instance_sesn_ptr_target) : NULL;
 ...
 */
static void
_BuildErrorHeaderForMessageCommand(CommandHeader *header_ptr, CommandHeader *header_ptr_received, int errcode, int command_type)
{
  switch (errcode)
  {
    case RESCODE_USERCMD_MISSING_PARAM:
      ...
      break;
    case RESCODE_MESSAGE_REVOKED: //message was already revoked
      header_ptr->args_error	=	MESSAGE_COMMAND__ERRORS__MESSAGE_ALREADY_REVOKED;
      break;
    case RESCODE_BACKEND_RESOURCE_OWNER: //user did not author the message
      header_ptr->args_error	=	MESSAGE_COMMAND__ERRORS__OWNERSHIP;
      break;
    case RESCODE_BACKEND_RESOURCE_NULL: //resource was not found
      header_ptr->args_error	=	MESSAGE_COMMAND__ERRORS__MESSAGE_NOT_IN_STORAGE;
      break;
    case RESCODE_FENCE_EVENT_GENERATION: //system processing error
      header_ptr->args_error	=	MESSAGE_COMMAND__ERRORS__NO_POSTING_ALLOWED;
      break;
  ...
  header_ptr->has_args_error	=	1;
  header_ptr->args				=	COMMAND_ARGS__REJECTED;									header_ptr->has_args				=	1;
  if (command_type > 0)		header_ptr->command			=	command_type;
  else									  header_ptr->command			=	header_ptr_received->command;//restore original command
  header_ptr->when_client	=	header_ptr_received->when;							header_ptr->has_when_client = header_ptr_received->has_when_client;
  return;
 ...
 */
static UFSRVResult *
_HandleMessageCommandError(CommandBaseContext *cmd_ctx_ptr, int rescode, int command_type)
{
  Envelope 					command_envelope	= ENVELOPE__INIT;
  CommandHeader 		header						= COMMAND_HEADER__INIT;
  UfsrvCommandWire	ufsrv_command			= UFSRV_COMMAND_WIRE__INIT;
  ...
  Session *sesn_ptr                   = BASE_CMDCTX_SESN_ORIGINATOR(cmd_ctx_ptr);
  ...
  _BuildErrorHeaderForMessageCommand(&header, BASE_CMDCTX_DATA_MESSAGE(cmd_ctx_ptr)->ufsrvcommand->msgcommand->header, rescode, command_type);
  ...
  return (_MarshalCommandToUser(BASE_CMDCTX_SESN_INSTANCE_ORIGINATOR(cmd_ctx_ptr), NULL, BASE_CMDCTX_WSM(cmd_ctx_ptr), &command_envelope,  uGETKEYS_V1_IDX));//TODO: temp use of uGETKEYS_V1
}
...
_MarshalCommandToUser(InstanceHolderForSession *instance_sesn_ptr, InstanceHolderForSession *instance_sesn_ptr_target, WebSocketMessage *wsm_ptr_received, Envelope *command_envelope_ptr, unsigned req_cmd_idx)
{
  ...
  UfsrvCommandInvokeUserCommand(&(InstanceContextForSession) {instance_sesn_ptr, SessionOffInstanceHolder(instance_sesn_ptr)},
                                IS_EMPTY(instance_sesn_ptr_target)? NULL : &(InstanceContextForSession) {instance_sesn_ptr_target, SessionOffInstanceHolder(instance_sesn_ptr_target)},
