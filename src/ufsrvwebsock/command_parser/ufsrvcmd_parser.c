...
unsigned char lextable_ufsrvcmd[] = {
	#include "ufsrvwebsock/command_parser/ufsrvcmd_lexer_data.h"
};
...
_UfsrvCommandIndexGet (LexParserState *wsi, unsigned char c)
{
    ...
    if (lextable_ufsrvcmd[wsi->lextable_pos] < FAIL_CHAR) {
      /* terminal state */
      n = ((unsigned int)lextable_ufsrvcmd[wsi->lextable_pos] << 8) | lextable_ufsrvcmd[wsi->lextable_pos + 1];

      //printf("known header %d\n", n); //this may be a match
...
UfsrvCommandIndexGet (Session *sesn_ptr, const char *command_str)
{
	...
	if (len > UFSRVCMD_MAXLEN) {
		syslog(LOG_NOTICE, "%s {pid:'%lu'}: ERROR: COMMAND '%s' has length: '%lu' exceeding UFSRVCMD_MAXLEN: '%d'..", __func__, pthread_self(), command_str, len, UFSRVCMD_MAXLEN);
	...
	while (len--) {
		if ((cmdidx = _UfsrvCommandIndexGet(&lps, *str++)) == -1) {
		  syslog(LOG_NOTICE, "%s {pid:'%lu', th_ctx:'%p'}: ERROR: COMMAND '%s' could be found...", __func__, pthread_self(), NULL, command_str);
		  break;
		}
	}
