...
{
	if (unlikely(sesn_ptr_source==NULL))	return NULL;

	//IMPORTANT SESSION UNLOCKED SO THIS IS READONLY
	UserRecord	*user_record_ptr=NULL;
	if (IS_PRESENT(user_record_supplied))	user_record_ptr=user_record_supplied;
	else	user_record_ptr=calloc(1, sizeof(UserRecord));
...
{
	if (unlikely(sesn_ptr==NULL))	return NULL;
...
{
  if (GetSignedCertificate(cert_ctx_ptr->cert_server_ptr, cert_ctx_ptr->cert_key_ptr)) {
