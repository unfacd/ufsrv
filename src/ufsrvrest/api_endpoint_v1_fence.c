...
#define _THIS_PATH_FENCE	"/V1/Fence"
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
    ...
    if (strlen(full_path) > sizeof(_THIS_PATH_FENCE) + UINT64_LONGEST_STR_SZ) goto request_error; //includes allowance for '/' since we did not deduct -1 from sizeof for '\0' string terminator
    ...
    if (IS_STR_LOADED(fid_point)) {
      ...
      if (fid) {
        ...
        if (IS_PRESENT(jobj_fence)) {
          ...
          onion_response_write(instance_sesn_ptr, HTTPSESN_RESPONSE_PTR(http_ptr), jobj_reply_str_fence, strlen(jobj_reply_str_fence));
...
static long _GetValidateFidFromUrlParam(const char *url_path_and_params, unsigned int max_path_params_sz, unsigned int fid_param_idx);
/**
 * @Helper routine to extract fid from url path. This is a basic surface level validation.
 * @param url_path_and_params full url path and parameters
 * @param max_path_params_sz Maximum number of path tokens (seperated by '/')
 * @param fid_param_idx the positional index (counting from 0) where fid is located on the supplied path
 * @return -1 on error
 */
static long
_GetValidateFidFromUrlParam(const char *url_path_and_params, unsigned int max_path_params_sz, unsigned int fid_param_idx)
{
  unsigned long fid = 0;
  ...
  TokeniseUrlParams((char *)url_path_and_params, &url_params, max_path_params_sz);

  if (url_params.tokens_sz < 4) return fid;

  if (IS_STR_LOADED(url_params.tokens[fid_param_idx]->token)) {
    fid = strtoul(url_params.tokens[fid_param_idx]->token, NULL, 10);
  }
...
/**
 * @brief Endpoint to retrieve basic fence information.
 * @param instance_sesn_ptr
 * @url_param /V1/Fence/Info/<%fid>
 * @return
 */
API_ENDPOINT_V1(FENCE_INFO)
{
#define _THIS_PATH	"/V1/Fence/Info/"

#define FENCE_INFO_MAX_URL_PARAMS_SZ 4
#define FENCE_INFO_URL_TOKEN_FID     3

  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
  HttpSession *http_ptr = (HttpSession *)SESSION_PROTOCOL_SESSION_DATA(sesn_ptr);

  unsigned long fid = _GetValidateFidFromUrlParam((char *)onion_request_get_fullpath(HTTPSESN_REQUEST_PTR(http_ptr)), FENCE_INFO_MAX_URL_PARAMS_SZ, FENCE_INFO_URL_TOKEN_FID);
  if (fid == 0) {
    syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p'}: Missing fid...", __func__, pthread_self(), sesn_ptr);
    goto request_error;
  }
  ...
  if ((flags&OR_METHODS) == OR_GET) {
    ...
    Fence fence = {0};
    RestRequestDescriptor rest_descriptor = {.requester.jobj=jobj, .handler.ctx_data=CLIENT_CTX_DATA(&fence)};
    int handling_result = HandleFenceInfoRequest(fid, SESSION_USERID(sesn_ptr), JsonFormatFenceInfoResponseCallback, &rest_descriptor);
...
/**
 * Endpoint for issuing authentication credentials.
 * @param instance_sesn_ptr
 * @return
 */
...
#define CREDENTIALS_REDEMPTION_PERIOD_MAX 7
  ...
  if ((flags&OR_METHODS) == OR_GET) {
    ...
    AuthenticationCredential auth_credentials[redemption_period + 1]; memset(auth_credentials, 0, sizeof(auth_credentials));
    collection.collection = AS_COLLECTION_TYPE(auth_credentials); collection.collection_sz = redemption_period + 1;//to be all-inclusive of provided end period, add one additional bucket ie loop 8 times
    ...
    int handling_result = HandleCredentialRequest(MASTER_CONF_SERVER_PRIVATE_PARAMS, &uuid, redemption_start_time,
                                                  redemption_end_time, &collection,
                                                  JsonFormatCredentialResponseCallback, &rest_descriptor);
...
API_ENDPOINT_V1(FENCE_ZKGROUP) {
  ...
  if ((flags&OR_METHODS) == OR_GET) {
    ...
    syslog(LOG_DEBUG, "%s: Received X-Group-Authorization header: '%s'", __FUNCTION__, group_authorization);//group public params:auth credentials presentation
    ...
    if (hash_result == 0) {
      uint8_t serialised_hashed_authorization[Base32ProvideEncodedBufferSize(16) + 1]; memset(serialised_hashed_authorization, '\0', sizeof(serialised_hashed_authorization));
...
API_ENDPOINT_V1(FENCE_NEARBY)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (HTTPSESN_JSONDATA(((HttpSession *)SESSION_PROTOCOL_SESSION_DATA(sesn_ptr)))) {
		json_object *jobj = HTTPSESN_JSONDATA(((HttpSession *)SESSION_PROTOCOL_SESSION_DATA(sesn_ptr)));

		longitude = json_object_get_double(json__get(jobj, ACCOUNT_JSONATTR_LONGITUDE));
		latitude = json_object_get_double(json__get(jobj, ACCOUNT_JSONATTR_LATITUDE));

		BufferDescriptor buffer_packed_msg = {0};
		if (IS_PRESENT(ProvideFencesNearByInProtoPacked(sesn_ptr, longitude, latitude, 50, &buffer_packed_msg))) {
