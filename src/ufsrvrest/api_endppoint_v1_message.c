...
inline static int _HandleMessageGetRequest (InstanceContextForSession *ctx_ptr, const UfsrvUid *);
...
API_ENDPOINT_V1(MESSAGE)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if ((flags&OR_METHODS) == OR_POST) {
		...
		return (_HandleMessageGetRequest(&(InstanceContextForSession){instance_sesn_ptr, sesn_ptr}, &(SESSION_UFSRVUIDSTORE(sesn_ptr))));
	} else {
...
inline static int
_HandleMessageGetRequest (InstanceContextForSession *ctx_ptr, const UfsrvUid *uid_ptr)
{
