			...
			if (strlen(verification_code_provided) == CONFIG_MAX_VERIFICATION_CODE_SZ + 1) {//plus 1 for leading '/' in '/xxxxxx'
				*(cookie + CONFIG_MAX_COOKIE_SZ) = '\0'; //'/' gone
				...
				if (PendingAccountVerify(sesn_ptr, &pending_account, atoi(verification_code_provided), 0)) {
				  return onion_shortcut_redirect(instance_sesn_ptr, "http://unfacd.com/sample-page/unfacd-registration-verification/");
//          char *verification_response =
//#include "html_successful_verification.h"
//          ;
//          onion_response_set_length(HTTPSESN_RESPONSE_PTR(http_ptr), strlen(verification_response));
//          onion_response_write(instance_sesn_ptr, HTTPSESN_RESPONSE_PTR(http_ptr), verification_response, strlen(verification_response));
//					goto return_processed;
				}
			}
		}
...
//  } else	goto request_error;
  ...
  if ((flags&OR_METHODS) == OR_GET) {
    if (url_params.tokens_sz <= 4) {
      ...
      if (IS_PRESENT(JsonFormatUserProfile(sesn_ptr, userid, DIGESTMODE_FULL, true, jobj_token))) {
    ...
    if (UserProfileAuthDescriptorFromJson(jobj_msg, &profile_descriptor)) {
      StoreUserProfileAuthDescriptor(SESSION_USERID(sesn_ptr), &profile_descriptor);
      goto request_success;
    }
...
API_ENDPOINT_V1(ACCOUNT_SHARED_CONTACTS)
{
  Session *sesn_ptr     = SessionOffInstanceHolder(instance_sesn_ptr);
	HttpSession *http_ptr = (HttpSession *)SESSION_PROTOCOLSESSION(sesn_ptr);
	json_object *jobj_client = HTTPSESN_JSONDATA(((HttpSession *)SESSION_PROTOCOLSESSION(sesn_ptr)));

	{
		...
		if (true) {//get matched keys from db
			struct json_object *jobj_shared_contacts_array;
			{
				...
				else 		syslog(LOG_DEBUG, "%s {pid:'%lu, o:'%p', cid: '%lu', contacts_sz:'%i'}: RECEIVED contacts array node", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), json_object_array_length(jobj_array_orig));
