		...
		if (SESSION_RESULT_TYPE_SUCCESS(sesn_ptr) && IS_STR_LOADED((char *)SESSION_RESULT_USERDATA(sesn_ptr))) {
			...
			if (IS_PRESENT(userid)) {
				...
				*userid++ = '\0';
				...
				if (IS_PRESENT(JsonFormatUserProfile(sesn_ptr, &(UfsrvUidSequenceIdPair){.ufsrvuid=EMPTY_UFSRVUID, .uid=strtoul(userid, NULL, 10)}, DIGESTMODE_BRIEF, true, jobj_token))) {
...
//	char *userid=strndupa(user_token_req, CONFIG_MAX_UFSRV_ID_ENCODED_SZ+1);
	UfsrvUid uid_raw = {0};
	unsigned long userid_sequence = UfsrvUidGetSequenceId(UfsrvUidCreateFromEncodedText(user_token_req, &uid_raw));

	if (userid_sequence <= 0) {
    if (strncmp(UFSRV_SYSTEMUSER_UID, user_token_req, CONFIG_MAX_UFSRV_ID_ENCODED_SZ) == 0) {
      userid_sequence = 0 ;
    } else {
	...
	if ((flags&OR_METHODS) == OR_GET) {
			struct json_object *jobj_token = json_object_new_object();
			if (IS_PRESENT(JsonFormatUserProfile(sesn_ptr, &(UfsrvUidSequenceIdPair){.ufsrvuid=&uid_raw, .uid=userid_sequence}, DIGESTMODE_BRIEF, true, jobj_token))) {
        const char *json_str_reply = json_object_to_json_string(jobj_token);
