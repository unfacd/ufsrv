...
typedef struct FencesCollectionForSession {
    ...
    CollectionDescriptor linkjoin_fences;
    CollectionDescriptor blocked_fences;
} FencesCollectionForSession;

static inline UFSRVResult *_CacheBackendDeleteGetAffectedInvitedFencesList(Session *sesn_ptr, unsigned long call_flags, CollectionDescriptor *collection_ptr_in);
static inline UFSRVResult *_CacheBackendDeleteGetAffectedMemberFencesList(Session *sesn_ptr, unsigned long call_flags, CollectionDescriptor *collection_ptr_in);
static inline UFSRVResult *_CacheBackendClearSessionAndLinkedFences(Session *sesn_ptr, unsigned long call_flags, FencesCollectionForSession *fence_collection_ptr_in);
...
/**
 * 	@brief Marshals an IntraCommand to deactivate a user. The most crucial part is propagating the command through the messaging queue
 ...
 * 	active connection.
 *
 * 	@param sesn_ptr_carrier: this is <b>not</b> the actual connected Session for the user, just a carrier Session for the  user who issued the request based on http authentication,
 * 	yet this Session has full backend context.
 *  @param uid_ptr Raw ufsrvuid for the user, whose account in being deactivated
 *  @param is_nuke if set, the account record in the database backend is also deleted. This irrecoverable.
 *
 * 	@dynamic_memory: IMPORTS 2x 'FenceEvents **' which gets deallocated locally
 *
 */
UFSRVResult *
DeactivateUserAndPropagate(Session *sesn_ptr_carrier, UfsrvUid *uid_ptr, bool is_nuke, bool is_simulated)
{
	//note we are not loading fences list for user
	bool lock_already_owned = false;
  CacheInvalidateBasicAuth(UfsrvUidGetSequenceId(uid_ptr), INVALIDATE_CACHEBACKEND); //cut off any potential simultaneous incoming requests by the same user
	GetSessionForThisUserByUserId(sesn_ptr_carrier, UfsrvUidGetSequenceId(uid_ptr), &lock_already_owned, CALL_FLAG_LOCK_SESSION|CALL_FLAG_SEARCH_BACKEND|CALL_FLAG_SNAPSHOT_INSTANCE|CALL_FLAG_REMOTE_SESSION);
    ...
    size_t member_fences_collection_sz = 0;
    size_t invited_fences_collection_sz = 0;
    ...
    CacheBackendClearSessionSharingLists(sesn_ptr_deactivated, CALLFLAGS_EMPTY);
    _CacheBackendClearSessionAndLinkedFences(sesn_ptr_deactivated, CALLFLAGS_EMPTY, &fences_collection);
		//retrieve list of fences affected by deactivation as it contains eventids
      ...
      member_fences_collection_sz = fences_collection_ptr->member_fences.collection_sz;
      invited_fences_collection_sz = fences_collection_ptr->invited_fences.collection_sz;
		}

		CollectionDescriptor member_fences_collection		=	{0};
		CollectionDescriptor invited_fences_collection	=	{0};

    ProvideFenceRecordsListFromFenceEventsInProto(sesn_ptr_deactivated, fence_events, member_fences_collection_sz, 0, &member_fences_collection);
    ProvideFenceRecordsListFromFenceEventsInProto(sesn_ptr_deactivated, fence_events_invited, invited_fences_collection_sz, 0, &invited_fences_collection);

		SessionMessage session_msg				= SESSION_MESSAGE__INIT;
		CommandHeader		header						=	COMMAND_HEADER__INIT;

		session_msg.header								=	&header;
    ProvideUfsrvUidInProto(&SESSION_UFSRVUIDSTORE(sesn_ptr_deactivated), &(header.ufsrvuid), true); header.has_ufsrvuid = 1;
    ...
    __unused UFSRVResult *res_ptr = DbAccountDeactivate(sesn_ptr_carrier, uid_ptr, is_nuke);
...
_CacheBackendDeleteGetAffectedInvitedFencesList(Session *sesn_ptr, unsigned long call_flags, CollectionDescriptor *collection_ptr_in)
{
...
static inline
UFSRVResult *
_CacheBackendDeleteGetAffectedMemberFencesList(Session *sesn_ptr, unsigned long call_flags, CollectionDescriptor *collection_ptr_in)
{
	if (unlikely(IS_EMPTY(sesn_ptr)))	return _ufsrv_result_generic_error;
 ...
 */
static inline UFSRVResult *
_CacheBackendClearSessionAndLinkedFences(Session *sesn_ptr, unsigned long call_flags, FencesCollectionForSession *fence_collection_ptr_in)
{
	if (unlikely(IS_EMPTY(sesn_ptr)))	return _ufsrv_result_generic_error;
  ...
  _CacheBackendDeleteGetAffectedMemberFencesList(sesn_ptr, call_flags, &(fences_collection_ptr->member_fences));
  _CacheBackendDeleteGetAffectedInvitedFencesList(sesn_ptr, call_flags, &(fences_collection_ptr->invited_fences));
...
//	static const char *provider_url_str=APIURL_SMS_PROD;
	...
	int result = HttpRequestPostUrl(http_ptr, provider_url_str, post_fields_str, APITOKEN_SMSVOICE_PROD, NULL, NULL, 0L);
...
{
/* To test spam sensitivity: https://www.mail-tester.com/
 * https://documentation.mailgun.com/en/latest/api-sending.html#examples
 * curl -s --user 'api:key-xxxx' \
    https://api.mailgun.net/v3/unfacd.io/messages \
    -F from='Registration <rego@unfacd.io>' \
    -F to=x@ccc.com \
    -F subject='Hello' \
 ...
 */
	...
	asprintf(&post_fields_str, "from=unfacd registration service <rego@unfacd.io>"
													   "&to=%s"
                              "&h:List-Unsubscribe="
	...
	int result = HttpRequestPostUrl(http_ptr, provider_url_str, post_fields_str, API_EMAIL_KEY, NULL, NULL, 0L);
	if (result == 0) {
		syslog(LOG_ERR, "%s (cid='%lu'): ERROR: COULD NOT POST URL '%s'", __func__, SESSION_ID(sesn_ptr), post_fields_str);
		free(post_fields_str);
