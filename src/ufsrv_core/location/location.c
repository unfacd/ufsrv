...
LocationDescription *
DetermineLocationByServerByProto(Session *sesn_ptr, const LocationRecord *location_record_ptr)
{
  char *url_str = NULL;
  ...
  if (location_record_ptr->source == LOCATION_RECORD__SOURCE__BY_UNDEFINED) {
    ...
    double latitude = location_record_ptr->longitude;
...
DetermineUserLocationByServer (Session *sesn_ptr, HttpRequestContext *http_ptr, unsigned long sesn_call_flags)
{
	...
	if (asprintf(&url_str, "http://%s:%d/json/%s", masterptr->geoip_backend.address, masterptr->geoip_backend.port, sesn_ptr->ssptr->haddress ) > 0) {
		...
		UpdateUserLocationAssignment (sesn_ptr, CLIENT_CTX_DATA(http_ptr->jobj), CALL_FLAG_LOCTION_BY_SERVER|CALL_FLAG_WRITEBACK_SESSION_DATA_TO_BACKEND|CALL_FLAG_BROADCAST_SESSION_EVENT);
 ...
 */
UFSRVResult *
UpdateUserLocationAssignment (Session *sesn_ptr, ClientContextData  *location_data_ptr/*json_object *jobj*/, unsigned long call_flags_sesn)
{
...
/**
 * @brief given provided new location state update stored location state for the user
 * @param location_record_ptr Raw protobuf record provided by user
 * @return state describing nature of location change
 */
UFSRVResult *
UpdateLocationByProto (InstanceContextForSession *ctx_ptr, const LocationRecord *location_record_ptr)
{
...
//		if (self_zoned != 0)	USER_ATTRIBUTE_SET(ctx_ptr->sesn_ptr, USERATTRIBUTE_DEFINES_USERZONE);
      ...
      UpdateUserLocationByUserByProto(ctx_ptr->sesn_ptr, location_record_ptr, CALL_FLAG_BROADCAST_SESSION_EVENT|CALL_FLAG_WRITEBACK_SESSION_DATA_TO_BACKEND);
      loc_ptr = (LocationDescription *)SESSION_RESULT_USERDATA(ctx_ptr->sesn_ptr);
      origin = 2;
		} else {
      loc_ptr = DetermineLocationByServerByProto (ctx_ptr->sesn_ptr, location_record_ptr);
      origin = 1;
		}

		//creates a base fence if one doesn't not already exist
		int result;
	   switch ((result = ProcessUserLocation(ctx_ptr->instance_sesn_ptr, loc_ptr, origin)))
	   {
...
#ifdef __UF_TESTING
        syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): NO CHANGE in user location: passed locality: '%s'. Current in-use locality: '%s': No further processing.", __func__, pthread_self(), SESSION_ID(sesn_ptr), locality, sesn_ptr->sservice.user.user_details.user_location.locality);
#endif
        _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_LOCATION_UNCHANGED)
      } else {
#ifdef __UF_TESTING
...
UFSRVResult *
UpdateUserLocationByUserByProto (Session *sesn_ptr, const LocationRecord *location_record_ptr, unsigned long call_flags_sesn)
{
  if (sesn_ptr->sservice.user.user_details.user_location_initialised) {
    ...
    if (IS_STR_LOADED(locality)) {
      if (IS_STR_LOADED(sesn_ptr->sservice.user.user_details.user_location.locality) && strcasecmp(locality, sesn_ptr->sservice.user.user_details.user_location.locality) == 0) {
#ifdef __UF_TESTING
        syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu'}: NO CHANGE in user location: passed locality: '%s'. Current in-use locality: '%s': No further processing.", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), locality, sesn_ptr->sservice.user.user_details.user_location.locality);
#endif
        _RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_LOCATION_UNCHANGED)
      } else {
#ifdef __UF_TESTING
        syslog(LOG_DEBUG, "%s {pid:'%lu', o:'%p', cid:'%lu'}: CHANGE in user location: passed locality: '%s'. current in-use locality: '%s'.", __func__, pthread_self(), sesn_ptr, SESSION_ID(sesn_ptr), locality,  sesn_ptr->sservice.user.user_details.user_location.locality);
#endif

        User *u_ptr=&(sesn_ptr->sservice.user);

        UpdateUserLocationAssignment (sesn_ptr, CLIENT_CTX_DATA(location_record_ptr), CALL_FLAG_LOCTION_BY_USER|CALL_FLAG_WRITEBACK_SESSION_DATA_TO_BACKEND|CALL_FLAG_BROADCAST_SESSION_EVENT);
        UpdateBaseLocAssignment (sesn_ptr, location_record_ptr->baseloc, CALL_FLAG_LOCTION_BY_USER|CALL_FLAG_WRITEBACK_SESSION_DATA_TO_BACKEND|CALL_FLAG_BROADCAST_SESSION_EVENT);
    ...
    //locality was not known
    locality = location_record_ptr->adminarea;
    if (IS_STR_LOADED(locality)) {
      if (IS_STR_LOADED(sesn_ptr->sservice.user.user_details.user_location.admin_area) && strcasecmp(locality, sesn_ptr->sservice.user.user_details.user_location.admin_area) == 0) {
#ifdef __UF_TESTING
        syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): NO CHANGE in user location: passed adminarea: '%s'. current in-use admin: '%s'.", __func__, pthread_self(), SESSION_ID(sesn_ptr), locality, sesn_ptr->sservice.user.user_details.user_location.admin_area);
    ...
    //locality & adminarea were not known
    locality = location_record_ptr->country;
    if (IS_STR_LOADED(locality)) {
      if (IS_STR_LOADED(sesn_ptr->sservice.user.user_details.user_location.country) && strcasecmp(locality, sesn_ptr->sservice.user.user_details.user_location.country) == 0) {
#ifdef __UF_TESTING
        syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): NO CHANGE in user location: passed country: '%s'. current in-use country: '%s'.", __func__, pthread_self(), SESSION_ID(sesn_ptr), locality, sesn_ptr->sservice.user.user_details.user_location.country);
...
static UFSRVResult *_ResolveRoamingModeAndJoin(InstanceHolderForSession *instance_sesn_ptr);
static UFSRVResult *_HandleRoamingModeWandererGeoJoin (InstanceHolderForSession *instance_sesn_ptr);
static UFSRVResult *_HanldeRoamingModeConquererGeoJoin (InstanceHolderForSession *instance_sesn_ptr);
static UFSRVResult *_HanldeRoamingModeJournalerGeoJoin (InstanceHolderForSession *instance_sesn_ptr);
static UFSRVResult *_HandleGeoJoinForWanderer (InstanceHolderForSession *instance_sesn_ptr);
 ...
 */
static UFSRVResult *
_HanldeRoamingModeConquererGeoJoin (InstanceHolderForSession *instance_sesn_ptr)
{
	UFSRVResult *res_ptr;

	Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);

	res_ptr = IsUserAllowedToJoinGeoFence(instance_sesn_ptr, &(sesn_ptr->sservice.user.user_details.user_location));
...
_HandleGeoJoinForWanderer (InstanceHolderForSession *instance_sesn_ptr)
{
  Session *sesn_ptr  = SessionOffInstanceHolder(instance_sesn_ptr);

	UFSRVResult *res_ptr = IsUserAllowedToJoinGeoFence(instance_sesn_ptr, &(sesn_ptr->sservice.user.user_details.user_location));
 ...
 */
static UFSRVResult *
_HandleRoamingModeWandererGeoJoin (InstanceHolderForSession *instance_sesn_ptr)
{
		  ...
		  f_ptr_to_join = FenceOffInstanceHolder(instance_f_ptr_to_join);
			if (f_ptr_to_join != SESSION_GEOFENCE_CURRENT(sesn_ptr)) {
				...
				if (!IsStickyGeogroupForFenceSet (sesn_ptr, f_ptr_geo_current)) {
					...
					if ((eid = RemoveUserFromFence(instance_sesn_ptr, f_ptr_geo_current, FENCE_CALLFLAG_ROAMING_GEOFENCE)) != 0) {
...
//					}
				...
				return (_HandleGeoJoinForWanderer(instance_sesn_ptr));
			}
			//else it seems we are cycling back into a fence that we joined once (or still members of)
		}
		//fallback below
	} else return (_HandleGeoJoinForWanderer(instance_sesn_ptr));
 ...
 */
static UFSRVResult *
_HanldeRoamingModeJournalerGeoJoin (InstanceHolderForSession *instance_sesn_ptr)
{
	Fence				*f_ptr										= NULL;
	...
	res_ptr = IsUserAllowedGeoFenceInvite(instance_sesn_ptr, GetUfsrvSystemUser(), FENCE_CALLFLAG_ROAMING_GEOFENCE);
...
static UFSRVResult *
_ResolveRoamingModeAndJoin (InstanceHolderForSession *instance_sesn_ptr)
{
  ...
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (SESSION_USERPREF_ONOFF_GET(sesn_ptr, roaming_mode)) {
		if (SESSION_USERPREF_ONOFF_GET(sesn_ptr, roaming_mode_conquerer)) {
			return (_HanldeRoamingModeConquererGeoJoin(instance_sesn_ptr));
		} else	if (SESSION_USERPREF_ONOFF_GET(sesn_ptr, roaming_mode_wanderer)) {
			return (_HandleRoamingModeWandererGeoJoin (instance_sesn_ptr));
		} else 	if	(SESSION_USERPREF_ONOFF_GET(sesn_ptr, roaming_mode_journaler)) {
			return (_HanldeRoamingModeJournalerGeoJoin(instance_sesn_ptr));
		}
	}
 ...
 */
int
ProcessUserLocation (InstanceHolderForSession *instance_sesn_ptr, LocationDescription *loc_ptr, unsigned short origin)
{
  Session *sesn_ptr = SessionOffInstanceHolder(instance_sesn_ptr);
	...
	if (origin == 1) {//server
		if (u_ptr->user_details.user_location_by_server_initialised) {
			...
			//newly logged on user or user for which we have no prior history and drop user into respective base fence
			  ...
			  UFSRVResult	*res_ptr = _ResolveRoamingModeAndJoin(instance_sesn_ptr);
...
//	static const char *gecode_url_prefix = "http://maps.googleapis.com/maps/api/geocode/json?address=";
  ...
  int result = HttpRequestGetUrlInJson(http_ptr, APIURL_GEOCODING, address);
  if (result == 0) {
