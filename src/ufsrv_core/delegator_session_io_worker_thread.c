...
#include "include/sessionworkers_utils.h"
#include "msgqueue_backend/ufsrvmsgqueue.h"
#include "jobworkers/ufsrvworker_pool_descriptor_type.h"

static void *ThreadWorkerDelegator(void *session_delegator);
...
inline static void AddPipeConnectionToMonitoredEvents(SessionsDelegator *sd_ptr);
static Session *DestructDelegatorWorkerInterConnectionPipe(SessionsDelegator *sd_ptr) __attribute__((unused));
inline static void AddAllWorkerDelegatorPipeConnectionsToMonitoredEvents(SessionsDelegator *sd_ptr);

static SessionsDelegator sessions_delegator;
SessionsDelegator *const sessions_delegator_ptr = &sessions_delegator;

__thread ThreadContext ufsrv_thread_context; //todo: use _Thread_local. one global multiplexed as thread local storage for each thread
//todo all implementations should move over to this generic pointer implementation, which each protocol type defines
//for itself and with pre-allocated storage. That storage should be pointed at this thread-local pointer for each thread.
//__thread ThreadContextData *ufsrv_thread_context_data;

static const char *epoll_ctl_error(int error);
...
int
LaunchSessionsDelegatorThread(SessionsDelegator *sd_ptr)
{
	syslog(LOG_INFO, ">> %s: Creating Sessions Delegator thread...", __func__);
	pthread_create(&sd_ptr->session_delegator_thread, NULL, ThreadWorkerDelegator, sd_ptr);
...
SessionsDelegator *
InitialiseDelegator(void)
{
	sessions_delegator.events_container = malloc(sizeof(struct epoll_event) * sessions_delegator.setsize);
  ...
  pthread_barrier_init(&sessions_delegator.init_barrier, NULL, sessions_delegator.setsize + 1);//extra for delegator to reach the same barrier

	{//mutexes and cond vars
		//TOO: MIGRATE TO THE USE OF THE VARS defined below in ufsrv_thread_pool
		pthread_mutexattr_init(&sessions_delegator.work_queue_mutex_attr);
		pthread_mutexattr_settype(&sessions_delegator.work_queue_mutex_attr,  PTHREAD_MUTEX_ADAPTIVE_NP);//PTHREAD_MUTEX_ERRORCHECK);//Opportunistic "spinklock"
    int result = pthread_mutex_init (&sessions_delegator.work_queue_mutex, &sessions_delegator.work_queue_mutex_attr);

		result = pthread_cond_init(&sessions_delegator.queue_not_empty_cond, NULL);
		result = pthread_cond_init(&sessions_delegator.queue_not_full_cond, NULL);
		result = pthread_cond_init(&sessions_delegator.queue_empty_cond, NULL);

		if (result != 0) {
			char error_str[MEDIUMBUF];
			strerror_r(errno, error_str, MEDIUMBUF);
			syslog(LOG_ERR, "InitialiseWorkerDelegator: TERMINATING (errno: '%d'): COULD NOT INITIALISE mutex and cond vars: error: '%s'...", errno, error_str);
	...
	/*{//mutexes and cond vars for Events Work Queue
		...
		int result;
		pthread_mutexattr_init(&sessions_delegator.jobworkers.work_queue_mutex_attr);
		pthread_mutexattr_settype(&sessions_delegator.jobworkers.work_queue_mutex_attr,  PTHREAD_MUTEX_ADAPTIVE_NP);//PTHREAD_MUTEX_ERRORCHECK);
		result = pthread_mutex_init (&sessions_delegator.jobworkers.work_queue_mutex, &sessions_delegator.jobworkers.work_queue_mutex_attr);

		result = pthread_cond_init (&sessions_delegator.jobworkers.queue_not_empty_cond, NULL);
		result = pthread_cond_init (&sessions_delegator.jobworkers.queue_empty_cond, NULL);
		//result = pthread_cond_init (&sessions_delegator.queue_not_full_cond, NULL);
  ...
  LaunchSessionWorkerThreads(&sessions_delegator, sizeof(ThreadContext));
...
void
RegisterJobWorkersConfigurationDescriptor(WorkersConfigDescriptor *workers_config_descriptor)
{
  sessions_delegator.jobworkers_config_descriptor = workers_config_descriptor;
}

__pure WorkersConfigDescriptor *
GetJobWorkersConfigurationDescriptor()
{
...
AddWorkEvent(SessionsDelegator *sd_ptr, InstanceHolderForSession *instance_sesn_ptr)
{
  ...
  if (epoll_ctl(sd_ptr->epoll_handle, EPOLL_CTL_ADD, sesn_ptr->ssptr->sock, &epoll_event) == -1) {
		const char *error_str = epoll_ctl_error(errno);
	...
	if(try_flag)
	{
		...
		else
		{
			...
			syslog(LOG_DEBUG, "%s (pid:'%lu'): ERROR: COULD NOT ACQUIRE TRY-LOCK (errno='%d'): '%s'",
					__func__, pthread_self(), errno, err_str);
		}
	}
	else
	{
...
WorkQueueLock(SessionsDelegator *sd_ptr, int try_flag)
{
	...
	if (try_flag) {
		...
		if (lock_state == 0) {
			...
			char *err_str = thread_error(errno);
			syslog(LOG_DEBUG, "%s (pid:'%lu'): ERROR: COULD NOT ACQUIRE TRY-LOCK (errno='%d'): '%s'", __func__, pthread_self(), errno, err_str);
		}
	} else {
...
WorkQueueUnLock(SessionsDelegator *sd_ptr)
{
	...
	if (lock_state == 0) {
		...
		char *err_str = thread_error(errno);
		syslog(LOG_WARNING, "%s: (pid='%lu' lock:30:0 ): ERROR: COULD NOT RELEASE LOCK (errno='%d'): '%s'", __func__, pthread_self(),  errno, err_str);
	}
...
AddAllWorkerDelegatorPipeConnectionsToMonitoredEvents(SessionsDelegator *sd_ptr)
{
	...
	for (size_t i=0; i!=sd_ptr->setsize; i++) {
		instance_sesn_ptr_ipc = sd_ptr->worker_delegator_ipc[i];
...
NewConnectionsPipeDrain(Session *sesn_ptr_ipc, SessionsDelegator *sd_ptr)
{
	...
	if (++drain_threshold_keeper < CONFIG_NEW_CONNECTIONS_PIPE_DRAIN_THREASHOLD) {
    return 0;
  }
...
#endif
		if (amount_read > drain_threshold_keeper) {
...
static void * __attribute__ (( no_sanitize_thread, noreturn ))
ThreadWorkerDelegator(void *session_delegator)
{
	SessionsDelegator *sd_ptr = (SessionsDelegator *)session_delegator;

  SetThreadName("ufWorkDelegatr");
  sessions_delegator.pid = pthread_self();

	LocklessSpscQueue *new_connections_queue_ptr = &(sessions_delegator_ptr->new_connections.queue);

  unsigned 					connections_queue = 0;
  unsigned long 		stat_atomic = 0L;
  QueueClientData 	*client_data_ptr = NULL;
  ...
  syslog(LOG_INFO, "%s (pid:'%lu'): SUCCESS Reached delegator initialisation barrier: waiting for other worker threads...", __func__, pthread_self());
  pthread_barrier_wait(&sessions_delegator.init_barrier);

  //new connections queue processing block todo: use elsewhere defined _PollConnectionsListeners()
  dequeue:
