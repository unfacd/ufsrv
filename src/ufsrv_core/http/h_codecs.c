...
void onion_unquote_inplace(char *str){
	...
	while (*r){
		if (*r == '%'){
      //AA+ guard against malformed %xx values
      if ((sscanf(r + 1, "%c%c", &tmp[0], &tmp[1])) == 2) {
        r += 2;
        *w = strtol(tmp, (char **) NULL, 16);
      } else {
        *w=*r; //AA+ just pass through value as is
      }
		}
		else if (*r=='+'){
			*w=' ';
...
static inline int decode_hex (int ch)
{
    if ('0' <= ch && ch <= '9')
        return ch - '0';
    if ('A' <= ch && ch <= 'F')
...
/**
 * 	@dynamic_memory: returns result in dynamic buffer, which teh user must free.
 */
char *DecodeUrl(const char *s, size_t len)
{
    size_t i;
    //h2o_iovec_t ret;
    char *dst;
