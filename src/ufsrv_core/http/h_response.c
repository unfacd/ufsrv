...
static int HttpSendMessage(InstanceHolderForSession *, const char *msg, size_t msglen);
...
onion_response_free(InstanceHolderForSession *instance_sesn_ptr, onion_response *res)
{
  ...
  onion_response_network_flush(instance_sesn_ptr, res);
	...
	if (req) {
		...
		syslog(LOG_DEBUG, "%s: keep alive [req wants] %d && ([SENDFILE] %d || [skip] %d || [length ok] %d==%d || [chunked] %d)", __func__,
...
//AA+
void onion_response_set_content_type(onion_response *res, const char *content_type) {
  if (res->flags&OR_HEADER_SENT) {
    syslog(LOG_DEBUG, "Trying to set length after headers sent. Undefined onion behaviour.");
    return;
  }

  onion_response_set_header(res, "Content-Type", content_type);
...
/**
 * @short Helper that is called on each header, and writes the header in a http protocol format
 * @memberof onion_response_t
 * @ingroup response
 */
static void write_header(InstanceHolderForSession *instance_sesn_ptr, /*onion_response *res,*/ const char *key, const char *value, int flags) {
...
int onion_response_write_headers(InstanceHolderForSession *instance_sesn_ptr, onion_response *res) {
	...
	res->flags |= OR_HEADER_SENT; // I Set at the beginning, so I can do normal writing.
	...
	onion_dict_preorder(res->headers, write_header, instance_sesn_ptr);
...
//AA+
ssize_t
onion_response_write_with_known_content_length(InstanceHolderForSession *instance_sesn_ptr, onion_response *res, const char *data, size_t length, const char *content_type)
{
  onion_response_set_length(res, length);
  if (IS_PRESENT(content_type)) onion_response_set_content_type(res, content_type);

  return onion_response_write(instance_sesn_ptr, res, data, length);
...
onion_response_write(InstanceHolderForSession *instance_sesn_ptr, onion_response *res, const char *data, size_t length)
{
...
#ifdef __UF_TESTING
		syslog(LOG_DEBUG, "%s {pid:'%lu', content_sz:'%lu'}: BUFFER OVER CAPACITY: buffer position: '%lu'. AMOUNT TO BE COPIED: '%d' ", __func__, pthread_self(), length, res->buffer_pos, wb);
#endif

		memcpy(&res->buffer[res->buffer_pos], data, wb);//copy exactly to calculated capacity

		res->buffer_pos = sizeof(res->buffer);//
		if (onion_response_network_flush(instance_sesn_ptr, res) < 0)
			return w;//could be zero

		l -= wb;//if 0 , we have written the full lot, we have some left over due to have reached buffer capacity
		data += wb;
  ...
  int flushed = onion_response_network_flush(instance_sesn_ptr, res); //AA+
	return flushed? w : flushed;
}

/** onion_response_network_flush
 ...
 * 	+1 on success
 */
int onion_response_network_flush(InstanceHolderForSession *instance_sesn_ptr, onion_response *res)
{
	...
	if (!(res->flags&OR_HEADER_SENT)) { // Automatic header write
		...
		res->buffer_pos = 0; //note how buffer is rewind to the start to accommodate headers
	...
	if (res->flags&OR_CHUNKED) {
		...
		if ((w = HttpSendMessage(instance_sesn_ptr, tmp, strlen(tmp))) < 0) { //note '<0' as opposed to '<=0'
			syslog(LOG_DEBUG, "%s: Error writing chunk encoding length (%X). Aborting write.", __func__, (unsigned int)res->buffer_pos);
	...
	if ((w = HttpSendMessage(instance_sesn_ptr, &res->buffer[pos], res->buffer_pos)) < 0) { //note '<0' as opposed to '<=0'
		syslog(LOG_DEBUG, "%s: Error writing chunk encoding length (%X). Aborting write.", __func__, (unsigned int)res->buffer_pos);

		return OCS_CLOSE_CONNECTION;
	}

	//AA+ necessary to prevent duplicate output of the same buffer
	res->buffer_pos = 0;
...
bool onion_response_add_cookie(onion_response *res, const char *cookiename, const char *cookievalue, time_t validity_t, const char *path, const char *domain, int flags){
  ...
  if (pos>=sizeof(data)){
    syslog(LOG_DEBUG, "Cookie too long to be constructed. Not added to response.");
    return false;
  }
