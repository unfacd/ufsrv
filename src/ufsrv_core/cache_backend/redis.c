...
//		extern ufsrv *const masterptr;
		...
		per_ptr->backend_label            = cfg->backend_label;
		...
		syslog (LOG_INFO, "%s {pid:'%lu', th_ctx:'%p'}: Received PONG confirmation from RedisBackend (fd:%d, o:'%p', %s:%d): Issuing CLIENT SETNAME %s-%d-%lu...", __func__, pthread_self(), &ufsrv_thread_context, per_ptr->persistance_agent->fd, per_ptr,  cfg->con_tcp.host, cfg->con_tcp.port, cfg->backend_label, cfg->con_tcp.port, pthread_self());

		reply = redisCommand(per_ptr->persistance_agent, "CLIENT SETNAME %s-%d-%lu", cfg->backend_label, cfg->con_tcp.port, pthread_self());
