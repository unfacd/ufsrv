...
static onion_dict *onion_mime_dict=0;
...
static void onion_mime_fill(){
	...
	if (!fd){
		syslog(LOG_NOTICE, "%s: Could not read MIME types (etc/mime.types), returned mime types may be incorrect. Adding minimal set.", __func__);
	...
	//syslog(LOG_INFO, "I know %d mime types", onion_dict_count(onion_mime_dict));
}
