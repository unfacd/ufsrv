...
int
onion_handler_auth_pam_handler(Session *sesn_ptr, onion_handler_auth_pam_data *d, onion_request *request, onion_response *res)
{
  ...
  if (path && (strlen(path)<MBUF)) {
	...
	if (IS_PRESENT(ufsrvuid) && IS_PRESENT(passwd)) {
      ...
      SESSION_USERID_TEMP(sesn_ptr)		=	userid;
			...
			if (IsRateLimitExceeded(sesn_ptr, SESSION_USRMSG_CACHEBACKEND(sesn_ptr), userid, 0, RLNS_REQUESTS))	goto exit_ratelimit_exceeded;

			exit_invoke_next_handler:
			return onion_handler_handle(sesn_ptr, d->inside, request, res);//AA+ sesn_ptr
		}
	}
    ...
    syslog(LOG_DEBUG, "%s {pid:'%lu', basicauth:'%s', basicauth_decoded:'%s', cookie:'%s'}: ERROR: AUTHENTICATION FAILED", __func__, pthread_self(), basicauth_decoded?basicauth_decoded:"*", o?o:"*", cookie?cookie:"*");
		...
		onion_response_write(sesn_ptr, res, RESPONSE_UNAUTHORIZED, sizeof(RESPONSE_UNAUTHORIZED));
    ...
    syslog(LOG_DEBUG, "%s {pid:'%lu', basicauth:'%s', basicauth_decoded:'%s', cookie:'%s'}: ERROR: RATELIMIT EXCEEDED", __func__, pthread_self(), basicauth_decoded?basicauth_decoded:"*", o?o:"*", cookie?cookie:"*");
		...
		onion_response_write(sesn_ptr, res, RESPONSE_RATELIMIT_EXCEEDED, sizeof(RESPONSE_RATELIMIT_EXCEEDED));
...
InitBasicAuthDescriptorRecyclerTypePool ()
{
	...
	BasicAuthDescriptorPoolHandle = RecyclerInitTypePool ("BasicAuthDescriptor", sizeof(BasicAuthDescriptor) + sizeof(uintptr_t), _BasicAuthDescriptor_EXPANSION_THRESHOLD, &ops_basicauth_descriptor);
