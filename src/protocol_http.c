 ...
 */
UFSRVResult *
proto_http_init_session_callback (Session *sesn_ptr, unsigned callflags)
{
	HttpSession *http_ptr;

	if (callflags==0) {//brand new, heap based instance
#ifdef __UF_FULLDEBUG
		syslog(LOG_DEBUG, "%s (cid:'%lu): INITIALISING NEW HTTP SESSION UNDER HOST APP SESSION: '%lu'...", __func__, SESSION_ID(sesn_ptr), SESSION_ID(sesn_ptr));
#endif
...
#ifdef __UF_FULLDEBUG
		syslog(LOG_DEBUG, "%s (cid:'%lu): INITIALISING RECYCLER HTTP SESSION UNDER HOST APP SESSION: '%lu'...", __func__, SESSION_ID(sesn_ptr), SESSION_ID(sesn_ptr));
#endif
 ...
 */
UFSRVResult *
proto_http_reset_session_callback (Session *sesn_ptr, unsigned callflags)
{
	HttpSession *http_ptr;
	http_ptr=(HttpSession *)SESSION_PROTOCOLSESSION(sesn_ptr);
...
UFSRVResult *
proto_http_hanshake_callback (Session *sesn_ptr, SocketMessage *sm_ptr, unsigned callflags, int **comeback)
{
	__RETURN_RESULT(sesn_ptr->sservice, NULL, RESULT_TYPE_NOOP, RESCODE_PROG_NULL_POINTER)

}

UFSRVResult *proto_http_post_hanshake_callback (Session *ses_ptr)
{
	return 0;
 ...
 */
UFSRVResult *proto_http_msg_callback (Session *sesn_ptr, SocketMessage *sock_msg_ptr, unsigned frame_offset, size_t len)
{
	ssize_t amount_read;
	int rescode;
		...
		if (st != OCS_NEED_MORE_DATA) {
			...
			if (st == OCS_REQUEST_READY) {
				...
				if (st == OCS_CLOSE_CONNECTION) {
					...
					syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): SUCCESS: HANDLER RETURN VALEU: '%d': TERMINATING CONNECTION...", __func__, pthread_self(), SESSION_ID(sesn_ptr), st);
					...
					syslog(LOG_DEBUG, "%s (pid:'%lu' cid:'%lu'): ERROR: UNKNOWN HANDLER RETURN VALEU: '%d': TERMINATING CONNECTION...", __func__, pthread_self(), SESSION_ID(sesn_ptr), st);
					goto protocol_exit_terminal;
				}
 ...
 */
UFSRVResult *
proto_http_service_timeout_callback (Session *sesn_ptr, time_t now, unsigned long call_flags)
{
 ...
 */
UFSRVResult *
proto_http_error_callback (Session *sesn_ptr, unsigned call_flags)
{
...
UFSRVResult *
proto_http_recycler_error_callback (Session *sesn_ptr, unsigned call_flags)
{
	_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_SUCCESS, RESCODE_PROG_NULL_POINTER);
}

UFSRVResult *
proto_http_close_callback (Session *sesn_ptr)
{
...
_FenceCommandIntraMarshal (Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
  ...
  msgqueue_msg.has_ufsrvuid = 1;
	...
	StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																								&((IncomingMessageDescriptor){MSGCMD_FENCE, fencecmd_ptr->header->when, SESSION_USERID(sesn_ptr), fencecmd_ptr->fences[0]->fid, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																								0, command_buf);
...
_GetTargetId (MessageCommand *msgcmd_ptr)
{
  ...
  if (msgcmd_ptr->header->command == MESSAGE_COMMAND__COMMAND_TYPES__INTRO) return msgcmd_ptr->to[0]->cid;
...
_MessageCommandIntraMarshal(Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
	...
	if (msgcmd_ptr->header->command == MESSAGE_COMMAND__COMMAND_TYPES__SAY && (IS_EMPTY(msgcmd_ptr->fences) || IS_EMPTY(msgcmd_ptr->fences[0])) )
	{
		...
		_RETURN_RESULT_SESN(sesn_ptr, NULL, RESULT_TYPE_ERR, RESCODE_PROG_NULL_POINTER)
	} else if (msgcmd_ptr->header->command == MESSAGE_COMMAND__COMMAND_TYPES__INTRO && (IS_EMPTY(msgcmd_ptr->to) || IS_EMPTY(msgcmd_ptr->to[0])) ) {
      syslog(LOG_DEBUG,
    ...
    msgqueue_msg.has_ufsrvuid = 1;
		...
		unsigned char command_buf[packed_sz + MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_MESSAGE, msgcmd_ptr->header->when, SESSION_USERID(sesn_ptr), target_id, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
...
_SessionCommandIntraMarshal(Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
...

	{
		...
		unsigned char command_buf[packed_sz+MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_SESSION, sesncmd_ptr->header->when, SESSION_USERID(sesn_ptr), sesncmd_ptr->header->cid, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
...
_UserCommandIntraMarshal(Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
    ...
    msgqueue_msg.has_ufsrvuid = 1;
		...
		unsigned char command_buf[packed_sz+MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_USER, usercmd_ptr->header->when, SESSION_USERID(sesn_ptr), 0, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
...
_CallCommandIntraMarshal (Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
    ...
    msgqueue_msg.has_ufsrvuid = 1;
		...
		unsigned char command_buf[packed_sz+MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_CALL, callcmd_ptr->header->when, SESSION_USERID(sesn_ptr), 0, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
...
_ReceiptCommandIntraMarshal (Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
    ...
    msgqueue_msg.has_ufsrvuid = 1;
		...
		unsigned char command_buf[packed_sz+MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_RECEIPT, rectcmd_ptr->header->when, SESSION_USERID(sesn_ptr), 0, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
...
_SyncCommandIntraMarshal (Session *sesn_ptr, UfsrvInstanceDescriptor *ufsrv_ptr, UfsrvCommandWire *ufsrvcmd_ptr, WireProtocolData *data, const unsigned char *rawmsg_b64encoded)
{
    ...
    msgqueue_msg.has_ufsrvuid = 1;
		...
		unsigned char command_buf[packed_sz+MBUF];
		StoreStagedMessageCacheRecordForIntraCommand (sesn_ptr,
																									&((IncomingMessageDescriptor){MSGCMD_SYNC, synccmd_ptr->header->when, SESSION_USERID(sesn_ptr), 0, (const char *)packed_msg, packed_sz, ufsrv_ptr}),
																									0, command_buf);
