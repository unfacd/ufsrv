 ...
 */
int
GetSignedCertificate(ec_private_key *private_ky, ec_public_key *public_key, ServerCertificate *cert_server_ptr, Certificate *cert_key_ptr, uint32_t server_key_id)
{
	ec_private_key *ec_private_key  = private_ky;//&(MASTER_CONF_SERVER_PRIVATEKEY);
  ec_public_key *ec_public_key    = public_key;//&(MASTER_CONF_SERVER_PUBLICKEY_SERIALISED);
