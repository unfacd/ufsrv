  ...
  if (tmp == NULL) {
    ...
    switch (data->type) {
      ...
      case HOEL_COL_TYPE_TEXT:
        if (((struct _h_type_text *)data->t_data)->length==0) {
          return H_ERROR_PARAMS;
        }
...
//          ((struct _h_type_text *)tmp[cols].t_data)->value = malloc(strlen(((struct _h_type_text *)data->t_data)->value)+sizeof(char));
          ((struct _h_type_text *)tmp[cols].t_data)->value = calloc(1, ((struct _h_type_text *)data->t_data)->length+sizeof(char));//overallocate for an '\0'
...
//          strncpy(((struct _h_type_text *)tmp[cols].t_data)->value, ((struct _h_type_text *)data->t_data)->value, (strlen(((struct _h_type_text *)data->t_data)->value)+1));
          memcpy(((struct _h_type_text *)tmp[cols].t_data)->value, ((struct _h_type_text *)data->t_data)->value, (((struct _h_type_text *)data->t_data)->length));
...
default_finaliser
GetDefaultDbOpResultFinaliser()
{
  return   h_clean_result;
}
 ...
 */
struct _h_data * h_new_data_int(const u_int64_t value) {
 ...
 * return NULL on error
 */
struct _h_data * h_new_data_text(const char * value, const size_t length_provided) {
  struct _h_data * data = malloc(sizeof(struct _h_data));
  if (data != NULL) {
    ...
    size_t length_processed = length_provided>0?length_provided:strlen(value);
    ((struct _h_type_text *)data->t_data)->value = calloc(1, length_processed+sizeof(char)); //calloc to ensure '\0' terminated string automatically catered for for nonbinary strings
...
//      strncpy(((struct _h_type_text *)data->t_data)->value, value, (strlen(value)+sizeof(char)));
      memcpy(((struct _h_type_text *)data->t_data)->value, value, length_processed);
      ((struct _h_type_text *)data->t_data)->length = length_processed;
    }
  } else {
    //y_log_message(Y_LOG_LEVEL_ERROR, "Hoel - Error allocating memory for data");
  }
