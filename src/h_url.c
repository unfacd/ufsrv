...
int onion_url_handler(InstanceHolderForSession *instance_sesn_ptr, onion_url_data **dd, onion_request *request, onion_response *response)
{
	...
	while (next)	{
		...
		if (next->flags&OUD_STRCMP){
			if (strcasecmp(path, next->str)==0){
				//syslog(LOG_DEBUG, "%s: Ok, simple match: %s", __func__, next->str);
			...
			for (i=1; i<16; i++){
				...
				if (rm->rm_so!=-1){
					...
					//syslog(LOG_DEBUG, "%s: Add group %d: %s (%d-%d)", __func__, i, tmp, rm->rm_so, rm->rm_eo);
				}
				else
					break;
			}
...
int onion_url_add_handler(onion_url *url, const char *regexp, onion_handler *next){
	...
	syslog(LOG_DEBUG, "%s: Adding handler at %p", __func__, w);
	*w=malloc(sizeof(onion_url_data));
	...
	if (data->flags&OUD_REGEXP){
		...
		if (err){
			...
			syslog(LOG_DEBUG, "%s: Error analyzing regular expression '%s': %s.", __func__, regexp, buffer);
			free(data);
			*w=NULL;
			return 1;
		}
		...
		data->str=mystrdup(regexp);
	}
	data->next=NULL;
	data->inside=next;
#ifdef __DEBUG__
	data->orig=mystrdup(regexp);
#endif
